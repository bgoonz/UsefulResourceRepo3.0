...
($ => {
  ...
  $.fn.yiiactiveform = function (options) {
    return this.each(function () {
      const settings = $.extend({}, $.fn.yiiactiveform.defaults, options || {});
      const $form = $(this);
      const id = $form.attr("id");
      if (settings.validationUrl == undefined)
        settings.validationUrl = $form.attr("action");
      $.each(settings.attributes, (i, attribute) => {
      ...
      const validate = (attribute, forceValidate) => {
        ...
        settings.timer = setTimeout(() => {
          ...
          if (
            ...
            $.fn.yiiactiveform.validate($form, data => {
              let hasError = false;
              $.each(settings.attributes, function () {
      ...
      $.each(settings.attributes, (i, attribute) => {
        if (attribute.validateOnChange) {
          $("#" + attribute.inputID, $form)
            .change(() => {
              const inputType = $("#" + attribute.inputID).attr("type");
              validate(
                attribute,
            ...
            $.fn.yiiactiveform.validate($form, data => {
              ...
              if (
                ...
                if (!hasError) {
                  validated = true;
                  const $button =
                    $form.data("submitObject") || $form.find(":submit:first");
      ...
      $form.bind("reset", () => {
        ...
        setTimeout(() => {
          $.each(settings.attributes, (i, attribute) => {
            attribute.status = 0;
            const $error = $("#" + attribute.errorID, $form);
            const $container = $.fn.yiiactiveform.getInputContainer(
              attribute,
              $form
            );
   ...
   */
  $.fn.yiiactiveform.getInputContainer = (attribute, form) => {
    if (attribute.inputContainer == undefined)
      return $("#" + attribute.inputID, form).closest("div");
    else
      return $(attribute.inputContainer).filter(
        ':has("#' + attribute.inputID + '")'
      );
  };
  ...
  $.fn.yiiactiveform.updateInput = (attribute, messages, form) => {
    ...
    const $error = $("#" + attribute.errorID, form);
    const $container = $.fn.yiiactiveform.getInputContainer(attribute, form);
    $container
      .removeClass(attribute.validatingCssClass)
      .removeClass(attribute.errorCssClass)
      .removeClass(attribute.successCssClass);
  ...
  $.fn.yiiactiveform.updateSummary = (form, messages) => {
    ...
    $.each(settings.attributes, (i, attribute) => {
      if (messages && $.isArray(messages[attribute.id])) {
        $.each(messages[attribute.id], (j, message) => {
   ...
   */
  $.fn.yiiactiveform.validate = (form, successCallback, errorCallback) => {
    const $form = $(form);
    const settings = $form.data("settings");

    const messages = {};
    let needAjaxValidation = false;
    $.each(settings.attributes, function () {
      const msg = [];
      if (
        ...
        const value = $("#" + this.inputID, $form).val();
   ...
   */
  $.fn.yiiactiveform.getSettings = form => {
