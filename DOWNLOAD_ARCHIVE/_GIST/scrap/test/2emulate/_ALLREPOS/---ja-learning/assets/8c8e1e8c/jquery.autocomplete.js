...
($ => {
  $.fn.extend({
    legacyautocomplete(urlOrData, options) {
  ...
  $.Autocompleter = (input, options) => {
    ...
    function trimWords(value) {
      ...
      return $.map(value.split(options.multipleSeparator), word => {
    ...
    function request(term, success, failure) {
      ...
      if (data && data.length) {
        ...
        $.ajax({
          ...
          success(data) {
            const parsed = (options.parse && options.parse(data)) || parse(data);
  ...
  $.Autocompleter.Select = (options, input, select, config) => {
    ...
    let listItems, active = -1, data, term = "", needsInit = true, element, list;
    ...
    function fillList() {
      ...
      for (let i = 0; i < max; i++) {
        if (!data[i]) continue;
        const formatted = options.formatItem(
          data[i].data,
          i + 1,
          max,
          data[i].value,
  ...
  $.fn.selection = function (start, end) {
    ...
    if (field.createTextRange) {
      const range = document.selection.createRange(), orig = field.value, teststring = "<->", textLength = range.text.length;
