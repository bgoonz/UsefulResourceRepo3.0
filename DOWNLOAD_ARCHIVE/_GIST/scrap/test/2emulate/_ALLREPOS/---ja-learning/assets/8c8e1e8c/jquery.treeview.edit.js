($ => {
  const CLASSES = $.treeview.classes;
    ...
    return proxied
      ...
      .bind("add", function (event, branches) {
        ...
        $(branches)
          ...
          .applyClasses(settings, $(this).data("toggler"));
      })
      .bind("remove", function (event, branches) {
        const prev = $(branches).prev();
        const parent = $(branches).parent();
        $(branches).remove();
        prev
          .filter(":last-child")
          .addClass(CLASSES.last)
          .filter("." + CLASSES.expandable)
