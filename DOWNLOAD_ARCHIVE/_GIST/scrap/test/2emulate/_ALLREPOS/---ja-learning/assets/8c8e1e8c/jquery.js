...
((window, undefined) => {
  ...
  const document = window.document, navigator = window.navigator, location = window.location;
  const jQuery = (() => {
    ...
    const // Map over the $ in case of overwrite
    _$ = window.$;

    let // A central reference to the root jQuery(document)
    rootjQuery;

    const // A simple way to check for HTML strings or ID strings
    // (both of which we optimize for)
    quickExpr = /^(?:[^<]*(<[\w\W]+>)[^>]*$|#([\w\-]*)$)/;

    const // Check if a string has a non-whitespace character in it
    rnotwhite = /\S/;

    let // Used for trimming whitespace
    trimLeft = /^\s+/;

    let trimRight = /\s+$/;

    const // Check for digits
    rdigit = /\d/;

    const // Match a standalone tag
    rsingleTag = /^<(\w+)\s*\/?>(?:<\/\1>)?$/;
    ...
    const // Keep a UserAgent string for use with jQuery.browser
    userAgent = navigator.userAgent;

    let // For matching the engine and version of the browser
    browserMatch;

    let // The deferred used on DOM ready
    readyList;

    let // The ready event handler
    DOMContentLoaded;

    const // Save a reference to some core methods
    ...
    jQuery.extend({
      ...
      map(elems, callback, arg) {
        ...
        const // jquery objects are treated as arrays
        isArray =
  ...
  jQuery.extend({
    ...
    _Deferred() {
      ...
      let // stored [ context , args ]
      fired;

      let // to avoid firing when already doing so
      firing;

      let // flag to know if the deferred has been cancelled
      cancelled;

      const // the deferred itself
      deferred = {
        // done( f1, f2, ...)
        done() {
          if (!cancelled) {
    ...
    Deferred(func) {
      const deferred = jQuery._Deferred();
      const failDeferred = jQuery._Deferred();
      let promise;
  ...
  const rbrace = /^(?:\{.*\}|\[.*\])$/, rmultiDash = /([a-z])([A-Z])/g;
  ...
  jQuery.extend({
    ...
    data(elem, name, data, pvt /* Internal Use Only */) {
      ...
      const // We have to handle DOM nodes and JS objects differently because IE6-7
      // can't GC object references properly across the DOM-JS boundary
      isNode = elem.nodeType;

      const // Only DOM nodes need the global jQuery cache; JS object data is
    ...
    removeData(elem, name, pvt /* Internal Use Only */) {
      ...
      if (name) {
        const thisCache = pvt ? cache[id][internalKey] : cache[id];
      ...
      const internalCache = cache[id][internalKey];
  ...
  jQuery.fn.extend({
    data(key, value) {
      ...
      if (value === undefined) {
        ...
        return this.each(function () {
          const $this = jQuery(this), args = [parts[0], value];
  ...
  function dataAttr(elem, key, data) {
    ...
    if (data === undefined && elem.nodeType === 1) {
      const name = "data-" + key.replace(rmultiDash, "$1-$2").toLowerCase();
  ...
  function handleQueueMarkDefer(elem, type, src) {
    const deferDataKey = type + "defer", queueDataKey = type + "queue", markDataKey = type + "mark", defer = jQuery.data(elem, deferDataKey, undefined, true);
    if (
      defer &&
  ...
  jQuery.fn.extend({
    ...
    toggleClass(value, stateVal) {
      const type = typeof value, isBool = typeof stateVal === "boolean";
  ...
  jQuery.extend({
    ...
    attr(elem, name, value, pass) {
      ...
      let ret;
      let hooks;
      const notxml = nType !== 1 || !jQuery.isXMLDoc(elem);
  ...
  jQuery.event = {
    ...
    add(elem, types, handler, data) {
      ...
      let events = elemData.events, eventHandle = elemData.handle;
    ...
    trigger(event, data, elem, onlyHandlers) {
      ...
      let type = event.type || event, namespaces = [], exclusive;
      ...
      if (!elem) {
        ...
        jQuery.each(jQuery.cache, function () {
          ...
          const internalKey = jQuery.expando, internalCache = this[internalKey];
          if (
            internalCache &&
      ...
      const // IE doesn't like method names with a colon (#3533, #8272)
      ontype = type.indexOf(":") < 0 ? "on" + type : "";

      // Fire event on the current element, then bubble up the DOM tree
      do {
        const handle = jQuery._data(cur, "handle");
    ...
    fix(event) {
      ...
      if (event.pageX == null && event.clientX != null) {
        const eventDocument = event.target.ownerDocument || document, doc = eventDocument.documentElement, body = eventDocument.body;
      ...
      setup(data, namespaces) {
        if (!jQuery.nodeName(this, "form")) {
          jQuery.event.add(this, "click.specialSubmit", function (e) {
            const elem = e.target, type = elem.type;
  ...
  if (!jQuery.support.changeBubbles) {
    ...
    jQuery.event.special.change = {
      filters: {
        ...
        click(e) {
          const elem = e.target, type = jQuery.nodeName(elem, "input") ? elem.type : "";
  ...
  function trigger(type, elem, args) {
    ...
    const event = jQuery.extend({}, args[0]);
    event.type = type;
    event.originalEvent = {};
  ...
  if (!jQuery.support.focusinBubbles) {
    jQuery.each({ focus: "focusin", blur: "focusout" }, (orig, fix) => {
      ...
      function handler(donor) {
        ...
        const e = jQuery.event.fix(donor);
        e.type = fix;
        e.originalEvent = {};
  ...
  (() => {
    ...
    class Sizzle {
      ...
      static matches(expr, set) {
        return Sizzle(expr, null, null, set);
      }

      static matchesSelector(node, expr) {
        return Sizzle(expr, null, null, [node]).length > 0;
      }

      static find(expr, context, isXML) {
        let set;
      ...
      static filter(expr, set, inplace, not) {
        let match;
        let anyFound;
        let old = expr;
        let result = [];
    ...
    var Expr = (Sizzle.selectors = {
      ...
      relative: {
        "+": function (checkSet, part) {
          const isPartStr = typeof part === "string", isTag = isPartStr && !rNonWord.test(part), isPartStrNotTag = isPartStr && !isTag;
      ...
      find: {
        ...
        NAME(match, context) {
          if (typeof context.getElementsByName !== "undefined") {
            const ret = [], results = context.getElementsByName(match[1]);
      ...
      filters: {
        ...
        text(elem) {
          const attr = elem.getAttribute("type"), type = elem.type;
      ...
      filter: {
        PSEUDO(elem, match, i, array) {
          const name = match[1], filter = Expr.filters[name];
        ...
        CHILD(elem, match) {
          ...
          switch (type) {
            ...
            case "nth":
              const first = match[2], last = match[3];

              if (first === 1 && last === 0) {
                return true;
              }

              const doneName = match[0], parent = elem.parentNode;
              ...
              const diff = elem.nodeIndex - last;
        ...
        POS(elem, match, i, array) {
          const name = match[2], filter = Expr.setFilters[name];
      ...
      makeArray = (array, results) => {
        ...
        if (toString.call(array) === "[object Array]") {
          ...
          if (typeof array.length === "number") {
            for (const l = array.length; i < l; i++) {
              ret.push(array[i]);
            }
          } else {
            for (; array[i]; i++) {
    ...
    (() => {
      ...
      if (matches) {
        ...
        const disconnectedMatch = !matches.call(
    ...
    if (document.documentElement.contains) {} else if (document.documentElement.compareDocumentPosition) {} else {}
  ...
  jQuery.each(
    ...
    (name, fn) => {
      jQuery.fn[name] = function (until, selector) {
        ...
        const // The variable 'args' was introduced in
  ...
  function winnow(elements, qualifier, keep) {
    ...
    if (jQuery.isFunction(qualifier)) {
      ...
      const filtered = jQuery.grep(elements, elem => {
  ...
  jQuery.fn.extend({
    ...
    replaceWith(value) {
      if (this[0] && this[0].parentNode) {
        ...
        if (jQuery.isFunction(value)) {
          return this.each(function (i) {
            const self = jQuery(this), old = self.html();
        ...
        return this.each(function () {
          const next = this.nextSibling, parent = this.parentNode;
  ...
  function cloneCopyEvent(src, dest) {
    ...
    const internalKey = jQuery.expando;
    let oldData = jQuery.data(src);
    let curData = jQuery.data(dest, oldData);
  ...
  const // fixed for IE9, see #8346
  rupper = /([A-Z]|^ms)/g;

  const rnumpx = /^-?\d+(?:px)?$/i;
  const rnum = /^-?\d/;
  const rrelNum = /^[+\-]=/;
  const rrelNumFilter = /[^+\-\.\de]+/g;
  ...
  jQuery.fn.css = function (name, value) {
    ...
    return jQuery.access(this, name, value, true, (elem, name, value) => {
  ...
  if (!jQuery.support.opacity) {
    jQuery.cssHooks.opacity = {
      ...
      set(elem, value) {
        const style = elem.style, currentStyle = elem.currentStyle;
  ...
  if (document.documentElement.currentStyle) {
    currentStyle = (elem, name) => {
      ...
      const rsLeft = elem.runtimeStyle && elem.runtimeStyle[name];
      const style = elem.style;
  ...
  if (jQuery.expr && jQuery.expr.filters) {
    jQuery.expr.filters.hidden = elem => {
      const width = elem.offsetWidth, height = elem.offsetHeight;
  ...
  const // Keep a copy of the old load method
  ...
  let // Document location segments
  ajaxLocParts;
  ...
  jQuery.extend({
    ...
    ajaxSetup(target, settings) {
      ...
      for (const field in { context: 1, url: 1 }) {
    ...
    ajax(url, options) {
      ...
      const // Context for global events
      ...
      const completeDeferred = jQuery._Deferred();
  ...
  function ajaxConvert(s, response) {
    ...
    let // Current and previous dataTypes
    current = dataTypes[0];
    ...
    let // Conversion functions (transitive conversion)
    conv1;
  ...
  jQuery.ajaxPrefilter("json jsonp", (s, originalSettings, jqXHR) => {
    ...
    if (
      ...
      const jsonpCallback = (s.jsonpCallback = jQuery.isFunction(s.jsonpCallback)
        ? s.jsonpCallback()
        : s.jsonpCallback);
      ...
      if (gotoEnd || t >= options.duration + this.startTime) {
        ...
        if (done) {
          ...
          if (options.hide || options.show) {
            for (const p in options.animatedProperties) {
  ...
  function defaultDisplay(nodeName) {
    if (!elemdisplay[nodeName]) {
      let elem = jQuery("<" + nodeName + ">").appendTo("body"), display = elem.css("display");
  ...
  const rtable = /^t(?:able|d|h)$/i, rroot = /^(?:body|html)$/i;
  ...
  if ("getBoundingClientRect" in document.documentElement) {
    jQuery.fn.offset = function (options) {
      ...
      const doc = elem.ownerDocument, docElem = doc.documentElement;
    ...
    jQuery.fn.offset = function (options) {
      ...
      let prevComputedStyle = defaultView
  ...
  jQuery.offset = {
    ...
    bodyOffset(body) {
      let top = body.offsetTop, left = body.offsetLeft;
  ...
  jQuery.each(["Height", "Width"], (i, name) => {
    ...
    jQuery.fn[type] = function (size) {
      ...
      if (jQuery.isWindow(elem)) {
        ...
        const orig = jQuery.css(elem, type), ret = parseFloat(orig);
