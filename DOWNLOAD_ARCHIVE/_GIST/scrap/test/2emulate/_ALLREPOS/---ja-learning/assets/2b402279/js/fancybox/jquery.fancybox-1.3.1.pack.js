...
(b => {
  ...
  let E = null;
  let s = new Image();
  const G = /\.(jpg|gif|png|bmp|jpeg)(.*)?$/i;
  const S = /[^\.]\.(swf)\s*$/i;
  let H;
  let I = 1;
  let k;
  let l;
  let h = false;
  const y = b.extend(b("<div/>")[0], { prop: 0 });
  let v = 0;
  const O = !b.support.opacity && !window.XMLHttpRequest;
  ...
  const V = () => {
    let a = c.title;
    const d = l.width - c.padding * 2;
    const f = "fancybox-title-" + c.titlePosition;
  ...
  const M = a => {
    let d = Math.round(k.width + (l.width - k.width) * a);
    let f = Math.round(k.height + (l.height - k.height) * a);
    const o = Math.round(k.top + (l.top - k.top) * a);
    const t = Math.round(k.left + (l.left - k.left) * a);
    g.css({
      width: d + "px",
      height: f + "px",
      top: o + "px",
