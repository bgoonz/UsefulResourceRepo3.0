...
($ => {
  ...
  $.extend($.fn, {
    swapClass(c1, c2) {
      const c1Elements = this.filter("." + c1);
      this.filter("." + c2)
        .removeClass(c2)
        .addClass(c1);
    ...
    heightToggle(animated, callback) {
      animated
    ...
    applyClasses(settings, toggler) {
      ...
      if (!settings.prerendered) {
        ...
        let hitarea = this.find("div." + CLASSES.hitarea);
        if (!hitarea.length)
    ...
    treeview(settings) {
      ...
      if (settings.toggle) {
        const callback = settings.toggle;
      ...
      function deserialize() {
        const stored = $.cookie(settings.cookieId);
        if (stored) {
      ...
      switch (settings.persist) {
        case "cookie":
          const toggleCallback = settings.toggle;
          settings.toggle = function () {
            serialize();
            if (toggleCallback) {
          ...
          break;
        case "location":
          const current = this.find("a").filter(function () {
