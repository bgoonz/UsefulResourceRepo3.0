...
function tick() {
  ...
  var domhrtRelativeTime = Math.round(
    window.performance.webkitNow() - domhrtTimeAtStartOfPerformance
  );

  if ((msgIndex = sequenceLength)) return; // we've hit the end of the sequence.  This shouldn't be hit, except for an empty initial sequence.
  ...
  while (delay <= PREQUEUE) {
    ...
    msgIndex++;
    if (msgIndex == sequenceLength) return; // we've hit the end of the sequence.
    msg = sequence[msgIndex];
    delay = msg.timestamp - domhrtRelativeTime;
  }
