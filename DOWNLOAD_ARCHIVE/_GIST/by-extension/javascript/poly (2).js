...
if (typeof t0 === 'undefined') {
  ...
  t0 = timer.on(1 / st.beat, function (ti, b, off, swing) {
    ...
    var synth = function (t) {
      var x = stringer.ring(
        t,
        amod(1 / 2, 3 / 9, t, 24 * st.c),
        $.amod(Math.sqrt(st.pi), st.pi / 4, t, 8)
      );
...
delay = $.jdelay(
  Math.floor((((sampleRate * st.tempo) / 60) * 3) / 2 / 2 / 2 / 2),
  0.667,
  1
);

delay2 = $.jdelay(Math.floor((sampleRate * st.tempo) / 60 / 2), 0.887, 1 / 2);
...
return function (t, s, i) {
  ...
  var s =
    sn(
      ((((t * st.sn * 2) % 1) /
        $.amod(st.amod, (st.amod * 4) / 5, t, ((3 / 2) * bpm) / 60)) *
        bpm) /
        60
    ) / 16;
