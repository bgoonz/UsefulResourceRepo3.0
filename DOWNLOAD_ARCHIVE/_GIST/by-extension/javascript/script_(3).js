...
function createLights() {
  var globalLight = new THREE.HemisphereLight(
    Colors.white_d,
    Colors.white_d,
    0.8
  );
...
Tree.prototype.fly = function (callback) {
  ...
  TweenMax.to(this.mesh.scale, 1.5, { y: 1.1, x: 0.9, ease: Strong.easeInOut });
};
...
Trunc = function (complex) {
  ...
  // parametrables
  this.truncColor = complex
    ? parameters.truncColor
    : Colors.getRandomFrom(Colors.trunc);
  this.truncHeight = complex
    ? parameters.truncHeight
    : Math2.rangeRandom(70, 100);
  this.truncStartRadius = complex
    ? parameters.truncThickness
    : Math2.rangeRandom(2, 4);
  this.verticalSegments = complex
    ? Math2.rangeRandomInt(9, 12)
    : Math2.rangeRandomInt(3, 5);
  this.radiusSegments = complex
    ? Math2.rangeRandomInt(6, 10)
    : Math2.rangeRandomInt(4, 6);
  this.shapeAngleStart = Math2.rangeRandom(Math.PI / 4, Math.PI / 2);
  this.shapeAmplitude = Math2.rangeRandom(
    this.truncStartRadius / 4,
    this.truncStartRadius * 6
  );
  this.noise = complex
    ? parameters.truncNoise
    : Math2.rangeRandom(this.truncStartRadius / 8, this.truncStartRadius / 4);
  ...
  for (i = 0; i < this.verticalSegments; i++) {
    tx =
      Math.sin(this.shapeAngleStart + i * this.freq) * this.shapeAmplitude +
      this.truncStartRadius;
    tz = 0;
  ...
  this.mesh = new CustomMesh.Lathe(
    this.pointsTrunc,
    this.radiusSegments,
    this.truncColor
  );
  ...
  for (i = 0; i < this.attachsVerts.length; i++) {
    ...
    if (type == 'moss') {
      ...
      attach = new Tomatoe(
        s,
        Colors.getRandomFrom(Colors.pinks, complex),
        Colors.getRandomFrom(Colors.greens)
      ).mesh;
      attach.quaternion.setFromUnitVectors(
        new THREE.Vector3(0, 1, 0),
        this.verticesNormals[attDef.index]
      ...
      th = Math2.rangeRandom(
        ...
        (this.truncStartRadius * 60) / (1 + v.y)
      );
      attach = new ElbowBranch(
        r,
        th,
        col,
        colorFoliagePalette,
...
Trunc.prototype.grow = function () {
  ...
  for (var i = 0; i < l; i++) {
    ...
    TweenMax.from(v, s * 0.5, {
      x: 0,
      z: 0,
      delay: d * 2,
      ease: Strong.easeInOut,
      onUpdate: scope.replaceAttachs,
      onUpdateScope: scope,
    });
...
ElbowBranch = function (
  ...
  this.mesh = new CustomMesh.QuarterTorusMesh(
    radius,
    thickness,
    radSegs,
    tubSegs,
  ...
  this.mesh.geometry.applyMatrix(
    new THREE.Matrix4().makeTranslation(thickness, -thickness * 2, 0)
  );
  this.mesh.userData.hierarchy = hierarchy;
  this.mesh.userData.refClass = this;

  var folThick = complex
    ? parameters.foliageSize * (1 + Math.random() * 0.5)
    : Math2.rangeRandom(8, 24);
  this.attach = new Foliage(
    folThick,
    colorFoliagePalette,
    hierarchy + 1,
    complex
...
Tomatoe = function (scale, colorFruit, colorLeaves, complex) {
  ...
  for (var i = 0; i < 5; i++) {
    var leaf = new CustomMesh.DiamondMesh(
      scale,
      scale * 1.5,
      0.33,
      scale * 0.2,
    ...
    leaf.position.x = Math.cos(stepAngle * i) * 0.3;
    leaf.position.z = Math.sin(stepAngle * i) * 0.3;
...
Foliage = function (scale, colorPalette, hierarchy, complex) {
  ...
  var noise = complex
    ? parameters.foliageNoise * scale
    : Math2.rangeRandom(scale / 20, scale / 5);
  ...
  for (var i = 0; i < this.attachsVerts.length; i++) {
    var attDef = this.attachsVerts[i];
    var v = geom.vertices[attDef.index];
    var s = Math2.rangeRandom(scale * 0.05, scale * 0.2);
...
Foliage.prototype.launchParticle = function () {
  ...
  p.userData.speedY = Math2.rangeRandom(0.5, 1);
...
function grow(mesh) {
  ...
  for (var i = 0; i < mesh.children.length; i++) {
    ...
    if (child.userData.refClass && child.userData.refClass.type == 'foliage') {
      ...
      TweenMax.from(child.rotation, s, {
        x: 0,
        y: 0,
        z: 0,
        delay: d,
...
function loop() {
  ...
  if (Math.random() > 0.9) {
    var fol =
      foliagesComplex[Math.floor(Math.random() * foliagesComplex.length)];
    fol.launchParticle();
  }
  ...
  for (var i = 0; i < flyingParticles.length; i++) {
    ...
    p.rotation.x += 0.01 + Math.random() / 100;
    p.rotation.y += 0.01 + Math.random() / 10;
    p.rotation.z += 0.1 + Math.random() / 20;
