...
function accept(title) {
  return (
    ...
    (Boolean(
      /\b(design|architect|ui|ux|freelancer?|angular|react|vue|\.net|full[\s\-]stack|dev|programm(er|ing)|code|developer|software|js|javascript|mobile|web|engineer(ing)?|application|front[\s\-]?end|back[\s\-]?end|tech(nology|nical)?|computer|science|architect|consult(ant|ing))\b/i.test(
        title
      )
    ) &&
      !Boolean(
        /\b(upwork|java|ceo|cfo|cto|talent|acquisition|looking|student|trainee|recruit(er|ing)|hiring|staffing|opportunit(y|ies)|manager|analyst|business|financial|found(er|ing)|blockchain|crypto)\b/i.test(
          title
        )
      ))
  );
