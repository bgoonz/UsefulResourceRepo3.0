...
module.exports.lintText = (str, opts) => {
  ...
  if (opts.overrides && opts.overrides.length > 0) {
    ...
    const foundOverrides = optionsManager.findApplicableOverrides(
      filename,
      overrides
    );
    opts = optionsManager.mergeApplicableOverrides(
      opts,
      foundOverrides.applicable
    );
  }
  ...
  if (opts.filename) {
    ...
    if (
      multimatch(filename, opts.ignores).length > 0 ||
      globby.gitignore.sync({ cwd: opts.cwd, ignore: opts.ignores })(
        opts.filename
      )
    ) {
      return {
...
module.exports.lintFiles = (patterns, opts) => {
  ...
  return globby(isEmptyPatterns ? [defaultPattern] : arrify(patterns), {
    ...
    return mergeReports(
      grouped.map((data) => runEslint(data.paths, data.opts))
    );
  });
};
