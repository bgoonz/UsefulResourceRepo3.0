...
const quantityCounter = createReducer(0, {
  [types.CART_ADD_ITEM](state, action) {
    return 1;
  },
  [types.CART_REMOVE_ITEM](state, action) {
    return 0;
  },
  [types.CART_INC_ITEM_QUANTITY](state, action) {
    return state + 1;
  },
  [types.CART_DEC_ITEM_QUANTITY](state, action) {
...
export default function cartReducer(state = initialState, action) {
  ...
  return {
    ...state,
    items: itemsByIdReducer(state.items, {
      type: listActionType,
      id: action.item.id,
      innerAction: action,
    }),
