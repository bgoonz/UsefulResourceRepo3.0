...
class UnminifiedCSS extends ByteEfficiencyAudit {
  ...
  static computeWaste(stylesheet, networkRecord, pageUrl) {
    ...
    if (!url || url === pageUrl) {
      const contentPreview = UnusedCSS.determineContentPreview(
        stylesheet.content
      );
      url = contentPreview;
    }
  ...
  static audit_(artifacts, networkRecords) {
    ...
    for (const stylesheet of artifacts.CSSUsage.stylesheets) {
      const networkRecord = networkRecords.find(
        (record) => record.url === stylesheet.header.sourceURL
      );
      if (!stylesheet.content) continue;

      const result = UnminifiedCSS.computeWaste(
        stylesheet,
        networkRecord,
        pageUrl
      );
      ...
      if (
        result.wastedPercent < IGNORE_THRESHOLD_IN_PERCENT ||
        result.wastedBytes < IGNORE_THRESHOLD_IN_BYTES ||
        !Number.isFinite(result.wastedBytes)
      )
        continue;
      items.push(result);
    }
