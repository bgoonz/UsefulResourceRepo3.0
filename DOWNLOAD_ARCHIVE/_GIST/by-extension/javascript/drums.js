...
t0 = timer.beat(
  ...
  function (ti, b, off, swing) {
    ...
    var synth = function (t, s, i) {
      ...
      function amod_(c, r, t, f) {
        return (
          c + r * (Math.log((1.0001 + sin(f, t)) * 50) / Math.log(10) / 2 - 2)
        );
      }
      function wmod_(c, r, t, f) {
        return (
          c +
          Math.floor(
            r * (Math.log((1.0001 + sin(f, t)) * 50) / Math.log(10) / 2 - 2)
          )
        );
      }
      function amod(c, r, f) {
        return (
          c + r * (Math.log((1.0001 + sin(f)) * 50) / Math.log(10) / 2 - 2)
        );
      }
      function sin(x) {
