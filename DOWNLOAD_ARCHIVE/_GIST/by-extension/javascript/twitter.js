...
var twitter_config = JSON.parse(
  fs.readFileSync(path.resolve(__dirname, 'twitter-config.json'), 'utf-8')
);
...
function createAuthHeaders(_oauth, uri, method) {
  ...
  if (!oa.oauth_timestamp)
    oa.oauth_timestamp = Math.floor(new Date().getTime() / 1000).toString();
