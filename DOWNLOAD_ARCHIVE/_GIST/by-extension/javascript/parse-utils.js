...
function parseBundle(bundlePath) {
  ...
  if (walkState.locations) {
    modules = _.mapValues(walkState.locations, (loc) =>
...
function isModulesList(node) {
  return (
    isSimpleModulesList(node) || // Modules are contained in expression `Array([minimum ID]).concat([<module>, <module>, ...])`
...
function isModulesArray(node) {
  return (
    ...
    _.every(
      node.elements,
      (
        elem // Some of array items may be skipped because there is no module with such id
