...
const KEYWORDS_ATOM = array_to_hash(['false', 'null', 'true', 'undefined']);
...
const WHITESPACE_CHARS = array_to_hash(
  characters(' \u00a0\n\r\t\f\u000b\u200b')
);
...
const ATOMIC_START_TOKEN = array_to_hash([
  'atom',
  'num',
  'string',
  'regexp',
...
function parse($TEXT, exigent_mode, embed_tokens) {
  ...
  function maybe_embed_tokens(parser) {
    if (embed_tokens)
      return function (...args) {
        const start = S.token;
        const ast = parser.apply(this, args);
  ...
  const statement = maybe_embed_tokens(() => {
    if (is('operator', '/')) {
      S.peeked = null;
  ...
  function maybe_assign(no_in) {
    const left = maybe_conditional(no_in),
      val = S.token.value;
...
function member(name, array) {
  for (let i = array.length; --i >= 0; ) if (array[i] === name) return true;
  return false;
}
