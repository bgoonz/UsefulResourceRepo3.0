...
function doRuns() {
  ...
  var overheadpertask =
    (instrumentingtotal - notinstrumentingtotal) /
    (instrumentedTaskRuns * marksPerTask);
  var overheadMicro = Number((overheadpertask * 1000).toFixed(3));
  var baselineStr = `  Baseline work cost per run: ${notinstrumentingtotal.toFixed(
    2
  )}ms`;
  console.log(
    `Overhead of ${instrumentedTaskRuns.toLocaleString()} ${
      instrumentedRun.callname
    } calls:`,
    overheadMicro,
    'µs/call.',
    baselineStr
