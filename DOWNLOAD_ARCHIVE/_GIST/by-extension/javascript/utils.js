...
export async function addNpmPackage(pkgName, params, isDev) {
  if (!params) {
    throw new Error(
      `Did you forget to pass params to addNpmPackage('${pkgName}')?`
    );
  }

  if (!npmDependencies[pkgName]) {
    throw new Error(
      `Package "${pkgName}" is missing in the npmDependencies.json`
    );
  }
  ...
  array.forEach((line, index) => {
    ...
    if (isMatch) {
      ...
      if (indentLevel || opts.indentLevel) {
        newSrcData = indentCode(newSrcData, {
          indentLevel: indentLevel || opts.indentLevel,
        });
      }
...
export async function replaceCodeMemory(
  params,
  filepath,
  templateString,
  module,
