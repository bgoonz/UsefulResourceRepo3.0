...
function expectShowToBeCalled(
  ...
  if (foundation.getIsRich()) {
    ...
    if (foundation["isInteractive"]) {
      expect(mockAdapter.setAnchorAttribute).toHaveBeenCalledWith(
        "aria-expanded",
        "true"
      );
    }
  ...
  expect(mockAdapter.registerDocumentEventHandler).toHaveBeenCalledWith(
    "click",
    jasmine.any(Function)
  );
  expect(mockAdapter.registerDocumentEventHandler).toHaveBeenCalledWith(
  ...
  expect(mockAdapter.registerWindowEventHandler).toHaveBeenCalledWith(
    "scroll",
    jasmine.any(Function)
  );
  expect(mockAdapter.registerWindowEventHandler).toHaveBeenCalledWith(
...
function expectHideToBeCalled(
  ...
  expect(mockAdapter.deregisterDocumentEventHandler).toHaveBeenCalledWith(
    "click",
    jasmine.any(Function)
  );
  expect(mockAdapter.deregisterDocumentEventHandler).toHaveBeenCalledWith(
  ...
  expect(mockAdapter.deregisterWindowEventHandler).toHaveBeenCalledWith(
    "scroll",
    jasmine.any(Function)
  );
  expect(mockAdapter.deregisterWindowEventHandler).toHaveBeenCalledWith(
...
function expectHideNotToBeCalled(
  ...
  if (foundation.getIsRich()) {
    expect(mockAdapter.deregisterEventHandler).not.toHaveBeenCalledWith(
      "focusout",
      jasmine.any(Function)
    );
    if (foundation["isInteractive"]) {
      expect(mockAdapter.setAnchorAttribute).not.toHaveBeenCalledWith(
        "aria-expanded",
        "false"
      );
    }
  ...
  expect(mockAdapter.setAttribute).not.toHaveBeenCalledWith(
    "aria-hidden",
    "true"
  );
  ...
  expect(mockAdapter.deregisterDocumentEventHandler).not.toHaveBeenCalledWith(
    "click",
    jasmine.any(Function)
  );
  expect(mockAdapter.deregisterDocumentEventHandler).not.toHaveBeenCalledWith(
  ...
  expect(mockAdapter.deregisterWindowEventHandler).not.toHaveBeenCalledWith(
    "scroll",
    jasmine.any(Function)
  );
  expect(mockAdapter.deregisterWindowEventHandler).not.toHaveBeenCalledWith(
...
function setUpFoundationTestForRichTooltip(
  tooltipFoundation: typeof MDCTooltipFoundation,
  {
    isInteractive,
    isPersistent,
  }: { isInteractive?: boolean; isPersistent?: boolean } = {}
) {
  ...
  mockAdapter.getAttribute
    .withArgs(attributes.PERSISTENT)
    .and.returnValue(isPersistent ? "true" : "false");
  mockAdapter.getAnchorAttribute
    .withArgs(attributes.ARIA_EXPANDED)
    .and.returnValue(isInteractive ? "false" : null);
  mockAdapter.getAnchorAttribute
    .withArgs(attributes.ARIA_HASPOPUP)
...
describe("MDCTooltipFoundation", () => {
  ...
  it(`#handleAnchorMouseLeave hides the tooltip after a ${numbers.HIDE_DELAY_MS}ms delay`, () => {
    const { foundation, mockAdapter } =
      setUpFoundationTest(MDCTooltipFoundation);
    foundation.show();
    foundation.handleAnchorMouseLeave();
  ...
  it(`#handleAnchorMouseEnter shows the tooltip after a ${numbers.SHOW_DELAY_MS}ms delay`, () => {
    const { foundation, mockAdapter } =
      setUpFoundationTest(MDCTooltipFoundation);
  ...
  it(`#handleAnchorFocus shows the tooltip after a ${numbers.SHOW_DELAY_MS}ms delay`, () => {
    const { foundation, mockAdapter } =
      setUpFoundationTest(MDCTooltipFoundation);
  ...
  it(`#handleRichTooltipMouseLeave hides the tooltip after a ${numbers.HIDE_DELAY_MS}ms delay`, () => {
    const { foundation, mockAdapter } =
      setUpFoundationTestForRichTooltip(MDCTooltipFoundation);
  ...
  it("properly calculates tooltip position (START alignment)", () => {
    ...
    foundation.show();
    expect(mockAdapter.setStyleProperty).toHaveBeenCalledWith(
      "top",
      `${expectedTooltipHeight}px`
    );
  ...
  it("positions tooltip within viewport if threshold cannot be maintained (x-axis)", () => {
    ...
    expect(mockAdapter.setStyleProperty).toHaveBeenCalledWith("left", `400px`);
  });
  ...
  it("allows users to specify a position within viewport if threshold cannot be maintained (START alignment instead of CENTER)", () => {
    ...
    expect(mockAdapter.setStyleProperty).toHaveBeenCalledWith("left", `10px`);
  });
  ...
  it("recalculates position of tooltip if anchor position changes", () => {
    ...
    expect(mockAdapter.setStyleProperty).toHaveBeenCalledWith(
      "top",
      `${expectedTooltipTop + yPositionDiff}px`
    );
    expect(mockAdapter.setStyleProperty).toHaveBeenCalledWith(
      "left",
      `${expectedTooltipLeft + xPositionDiff}px`
    );
  });
});
