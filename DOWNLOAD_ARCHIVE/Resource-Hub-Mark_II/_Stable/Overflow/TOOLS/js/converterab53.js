...
function removeChainedTools(tool) {
  ...
  var widget = tool.sides.querySelector(
    ".side.output .side-widgets .widget-chain"
  );
  ...
  Site.GET("chain.json?tool=" + url, function (state) {
    ...
    if (state.readyState == 4 && state.status == 200) {
      try {
        ...
        console.log(e);
        // Unparsable data
        side.showNegativeBadge(
          "Server error",
          "Something has gone wrong while chaining the tool.",
          -1
        );
        widget.classList.remove("is-disabled");
      ...
      side.showNegativeBadge(
        "Server error",
        "We can't chain this tool due to a server error.",
        -1
      );
      widget.classList.remove("is-disabled");
      widget.textContent = "Retry chain?";
...
function createTool(name, extra) {
  var bridge =
    window.bridges[name] !== undefined ? window.bridges[name]() : false;
  var tool = null;
  ...
  function downloadBlob(src, callback) {
    ...
    xhttp.onreadystatechange = function () {
      ...
      // CORS Error, forward to backend
        ...
        downloadBlob(
          "/get-external-file?url=" + encodeURIComponent(src),
          callback
        );
      }
    };
...
function ImageTool(bridge, config, extra) {
  ...
  if (config.input) {
    ...
    if (config.input.image == true) {
      ...
      document.addEventListener(
        "paste",
        imagePasteListener.bind(tool, "image/")
      );

      var element = tool.input.element.querySelector(".side-box");
      element.addEventListener(
        "dragover",
        imageDragOverHandler.bind(tool),
        false
      );
      element.addEventListener(
        "dragend",
        imageDragEndHandler.bind(tool),
        false
      );
      element.addEventListener(
        "dragleave",
        imageDragLeaveHandler.bind(tool),
        false
      );
      element.addEventListener(
        "drop",
        imageDropHandler.bind(tool, "image/"),
        false
      );
    ...
    if (config.input.download) {
      tool.input.download = downloadCanvas.bind(
        tool,
        "input",
  ...
  if (config.output) {
    ...
    if (config.output.download) {
      tool.output.download = downloadCanvas.bind(
        tool,
        "output",
...
function Tool(config) {
      ...
      if (source) {
        ...
        for (var i = 0; i < inputs.length; i++) {
          ...
          if (typeof options[index] !== "undefined") {
            ...
            if (input.type == "textarea") autosize.update(input);
          }
  ...
  tool.init = function () {
      ...
      share.addEventListener("click", function (e) {
        if (!link.contains(e.target)) {
          if (share.classList.contains("active")) hide_share();
            ...
            var options = tool.options.get();
            var overridden = {};
            var overriddenData = tool.sides.getAttribute(
            ...
            if (tool.input.element) {
              ...
              if (inputFromURL) {
                ...
                query.push(
                  "input=" + encodeURIComponent(tool.input.getValue())
                );
              }
            }
            ...
            if (option_keys.length) {
              for (var i = 0; i < option_keys.length; i++) {
                ...
                if (overridden[option_key] === undefined) {
                  ...
                  query.push(
                    encodeURIComponent(option_key) +
                      "=" +
                      encodeURIComponent(option_val)
                  );
                }
              }
            }
    ...
    if (tool.output.element) {
      ...
      tool.addEventListener({
        ...
        handler: function (e) {
          if (e.name == "toggle-remove-chain") {
            ...
            var widget = sides.querySelector(
              ".output .toggle-remove-chain .remove-how-many"
            );
      ...
      var remove_chain_confirm = sides.querySelector(
        ".output .toggle-remove-chain .remove-chain-yes"
      );
      ...
      var chain_search = sides.querySelector(
        ".output .toggle-chain .chain-search"
      );
      ...
      var download_output_widget = sides.querySelector(
        ".output .widget-save-as"
      );
...
function chainSearch(sides) {
  ...
  var search = output.querySelector(
    ".side-widgets .toggle-chain .chain-search"
  );
...
function makeColorpickers() {
  Site.applyToAllElements(
    ".input-option.color.active.not-initialized",
    function (e) {
...
function populateChainCombinator(tool, output) {
  var panel = output.querySelector(
    ".side-widgets-toggle .toggle-chain .widget-chain-search"
  );
...
function updateExamples() {
  ...
  for (var i = 0; i < wrappers.length; i++) {
    ...
    if (!sample.querySelector(".file-sample")) {
      ...
      if (sample_width >= wrapper_width - threshold)
        sample.classList.add("big");
    }
  }
}
