...
(function () // wrap this in an ad-hoc function to avoid unecessary pollution of global namespace
{
  ...
  if (this_script.innerHTML.replace(/\s+/, "")) {
    try {
      // DEBUGGING: Comment out this line and the catch(e) below
      ...
      e // DEBUGGING: Comment out this line and the try below
    ) {
      if (typeof console != "undefined" && typeof console.log == "function") {
        ...
        warn(
          "Xinha: Check for common problems like a missing comma after a configuration section, or semicolons instead of commas after configuration sections."
        );
        warn(
          "Xinha: If you get really stuck, comment the try and catch lines around here and the native error might be more useful."
        );
...
if (typeof Xinha == "undefined") {
  ...
  Xinha.is_designMode =
    typeof document.designMode != "undefined" && !Xinha.is_ie; // IE has designMode, but we're not using it
  Xinha.isSupportedBrowser =
    Xinha.is_gecko ||
    (Xinha.is_opera && Xinha.opera_version >= 9.1) ||
  ...
  Xinha.loadPlugins = function (plugins, callbackIfNotReady) {
    ...
    Xinha.loadStyle(
      typeof _editor_css == "string" ? _editor_css : "Xinha.css",
      "XinhaCoreDesign"
    );
  ...
  : function () {
      ...
      function parse_plugins(xinha_plugins) {
        ...
        for (var i = 0; i < load_plugins.length; i++) {
          ...
          if (
            typeof load_plugins[i] == "object" &&
            typeof load_plugins[i].from == "string"
          ) {
            ...
            for (var ii = 0; ii < externs.length; ii++) {
              ...
              if (externs[ii].match(/\//)) {
                xinha_plugins.push({
                  url: load_plugins[i].from + "/" + externs[ii],
                  plugin: externs[ii].replace(/.*\/([^.]+)\..*$/, "$1"),
                });
              } else {
                xinha_plugins.push({
                  url:
          ...
          if (
            typeof load_plugins[i] == "object" &&
            typeof load_plugins[i].url == "string"
          ) {
          ...
          switch (load_plugins[i]) {
            case "loaded":
              {
                Array.prototype.push.apply(xinha_plugins, [
                  "CharacterMap",
                  "ContextMenu",
                  "FancySelects",
                  "SmartReplace",
            ...
            default:
              if (load_plugins[i].match(/^!/)) {
                Array.prototype.push.apply(
                  remove_plugins,
                  parse_plugins(load_plugins[i].replace(/^!/, ""))
                );
              } else {
                xinha_plugins.push(load_plugins[i]);
              }
        ...
        for (var i = 0; i < xinha_plugins.length; i++) {
          ...
          if (OK)
            for (var j = 0; j < remove_plugins.length; j++) {
              if (remove_plugins[j] == xinha_plugins[i]) {
                OK = false;
                break;
              }
            }
          ...
          if (OK)
            for (var j = 0; j < return_plugins.length; j++) {
              if (return_plugins[j] == xinha_plugins[i]) {
                OK = false;
                break;
              }
            }
      ...
      if (typeof xinha_plugins_specific == "function") {
        ...
        for (var i in xinha_editors) {
          ...
          var plugins_returned = xinha_plugins_specific(
            specific_plugins,
            xinha_editors[i]._textArea,
            xinha_editors[i]
          );
      ...
      if (typeof xinha_config_specific == "function") {
        for (var i in xinha_editors) {
          var returned_config = xinha_config_specific(
            xinha_editors[i].config,
            xinha_editors[i]._textArea,
            xinha_editors[i]
          );
