...
GetHtmlImplementation._pluginInfo = {
  ...
  developer_url:
    "$HeadURL: http://svn.xinha.org/trunk/modules/GetHtml/DOMwalk.js $".replace(
      /^[^:]*:\s*(.*)\s*\$$/,
      "$1"
    ),
  sponsor: "",
...
Xinha.emptyAttributes =
  " checked disabled ismap readonly nowrap compact declare selected defer multiple noresize noshade ";
...
Xinha.getHTMLWrapper = function (root, outputRoot, editor, indent) {
  ...
  switch (root.nodeType) {
    ...
    case 9: // Node.DOCUMENT_NODE
      ...
      if (
        (root_tag == "script" || root_tag == "noscript") &&
        editor.config.stripScripts
      ) {
        break;
      }
      if (outputRoot) {
        outputRoot = !(
          editor.config.htmlRemoveTags &&
      ...
      if (Xinha.is_ie && root_tag == "head") {
        ...
        var txt = root.innerHTML
          .replace(Xinha.RE_tagName, function (str, p1, p2) {
        ...
        for (i = attrs.length - 1; i >= 0; --i) {
          ...
          if (
            ...
            (a.nodeName.toLowerCase() == "height" ||
              a.nodeName.toLowerCase() == "width")
          ) {
          ...
          if (
            root.tagName.toLowerCase() == "img" &&
            a.nodeName.toLowerCase() == "complete"
          )
            continue;
          ...
          if (
            ...
            !(
              root.tagName.toLowerCase().match(/input|option/) &&
              a.nodeName == "value"
            ) &&
            !(
              root.tagName.toLowerCase().match(/area/) &&
              a.nodeName.match(/shape|coords/i)
            )
          ) {
            continue;
          }
          ...
          if (
            /_moz_editor_bogus_node/.test(name) ||
            (name == "class" && a.nodeValue == "webkit-block-placeholder")
          ) {
            html = "";
            break;
          }
          ...
          if (Xinha.emptyAttributes.indexOf(" " + name + " ") != -1) {
            ...
            // unless we get them using nodeValue()
              ...
              typeof root[a.nodeName] != "undefined" &&
              name != "href" &&
              name != "src" &&
              !/^on/.test(name)
            ) {
              ...
              if (
                editor.config.only7BitPrintablesInURLs &&
                (name == "href" || name == "src")
              ) {
            ...
            value = root.style.cssText.replace(/rgb\(.*?\)/gi, function (rgb) {
              return Xinha._colorToRgb(rgb);
            });
          } else if (!value) {
            // IE8 has style in attributes (see below), but it's empty!
            continue;
          }
...
*/
        ...
        if (
          Xinha.is_ie &&
          root.tagName.toLowerCase() == "option" &&
          root.selected
        ) {
      ...
      if (root_tag == "script" || root_tag == "noscript") {
        if (!editor.config.stripScripts) {
          if (Xinha.is_ie) {
            ...
            var innerText = root.hasChildNodes()
        ...
        if (outputRoot && !closed) {
          html +=
            ((Xinha.isBlockElement(root) && containsBlock) ||
            root_tag == "head" ||
            root_tag == "html"
              ? "\n" + indent
              : "") +
