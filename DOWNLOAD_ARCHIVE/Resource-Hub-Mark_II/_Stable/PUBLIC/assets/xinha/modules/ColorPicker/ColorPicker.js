...
Xinha.colorPicker = function (params) {
  ...
  td.appendChild(
    document.createTextNode(Xinha._lc("Click a color...", "ColorPicker"))
  );
  ...
  this.foreSample.appendChild(
    document.createTextNode(Xinha._lc("Sample", "ColorPicker"))
  );
  ...
  function rgbToWebsafe(color) {
    ...
    color.red = doubleHexDec(
      nearestPowerOf(parseInt(toHex(color.red).charAt(0), 16), 3)
    );
    color.blue = doubleHexDec(
      nearestPowerOf(parseInt(toHex(color.blue).charAt(0), 16), 3)
    );
    color.green = doubleHexDec(
      nearestPowerOf(parseInt(toHex(color.green).charAt(0), 16), 3)
    );
    return color;
  }
  ...
  this.open = function (anchorage, element, initcolor) {
    ...
    while (e.parentNode) {
      ...
      if (
        ...
        !e.tagName.toLowerCase().match(/body|html/)
      ) {
        if (
          (e.scrollTop || e.scrollLeft) &&
          !e.style.position.match(/fixed|absolute|relative/)
        ) {
          Xinha.debugMsg(
            "Warning, the Xinha editor is included in a " +
              e.tagName +
              " which has scroll (overflow) but is not positioned relative, absolute, or fixed.  The ColorPicker plugin may not display in the correct location.",
            "warn"
          );
        }
      }
    ...
    if (
      /top/.test(anchorage) ||
      top + this.table.offsetHeight > document.body.offsetHeight
    ) {
    ...
    if (
      /left/.test(anchorage) ||
      left + this.table.offsetWidth > document.body.offsetWidth
    ) {
      if (left - (this.table.offsetWidth - element.offsetWidth) > 0) {
        this.table.style.left =
          left - (this.table.offsetWidth - element.offsetWidth) + "px";
      } else {
  ...
  function pickCell(cell) {
    ...
    if (
      (cell.hue >= 195 && cell.saturation > 0.5) ||
  ...
  function pickValue(cell) {
    ...
    picker.backSample.style.backgroundColor =
      picker.saved_cells[pickrow][pickcol].colorCode;
    picker.foreSample.style.color =
      picker.saved_cells[pickrow][pickcol].colorCode;
  }

  function unpickCell(row, col) {
    picker.saved_cells[row][col].style.borderColor =
      picker.saved_cells[row][col].colorCode;
  }
    ...
    if (this.saved_cells === null) {
      ...
      for (var row = 0; row < this.side; row++) {
        ...
        for (var col = 0; col < this.side; col++) {
          var td = document.createElement("td");
          if (constrain) {
            td.colorCode = tupleToColor(
              rgbToWebsafe(hsvToRGB(huestep * row, saturstep * col, this.value))
            );
          } else {
            td.colorCode = tupleToColor(
              hsvToRGB(huestep * row, saturstep * col, this.value)
            );
          }
          ...
          td.onmousedown = function () {
            ...
            picker.saved_cells[pickrow][pickcol].style.borderColor =
              picker.saved_cells[pickrow][pickcol].colorCode;
            pickCell(this);
          };
          ...
          td.ondblclick = function () {
            Xinha.colorPicker.remember(this.colorCode, picker.savecolors);
        ...
        td.style.width = this.cellsize - 2 + "px";
        td.style.height = this.cellsize + "px";
        td.constrainedColorCode = tupleToColor(
          rgbToWebsafe(hsvToRGB(0, 0, valustep * row))
        );
        td.style.backgroundColor = td.colorCode = tupleToColor(
          hsvToRGB(0, 0, valustep * row)
        );
        td.style.borderWidth = "1px";
        td.style.borderStyle = "solid";
        ...
        td.onmousedown = function () {
          ...
          picker.saved_cells[valuerow][valuecol].style.borderColor =
            picker.saved_cells[valuerow][valuecol].colorCode;
      ...
      for (var col = 0; col < this.side; col++) {
        ...
        if (constrain) {
          td.colorCode = tupleToColor(
            rgbToWebsafe(hsvToRGB(0, 0, valustep * (this.side - col - 1)))
          );
        } else {
          td.colorCode = tupleToColor(
            hsvToRGB(0, 0, valustep * (this.side - col - 1))
          );
        }
        this.saved_cells[row][col] = td;
        td.style.height = this.cellsize + "px";
      ...
      if (this.websafe) {
        ...
        label.appendChild(
          document.createTextNode(Xinha._lc("Web Safe: ", "ColorPicker"))
        );
      ...
      label.style.fontSize = "x-small";
      label.appendChild(
        document.createTextNode(Xinha._lc("Color: ", "ColorPicker"))
      );
      ...
      but.style.cursor = "pointer";
      but.onclick = function () {
        Xinha.colorPicker.remember(picker.chosenColor.value, picker.savecolors);
      ...
      for (
        var savedCols = 0;
        savedCols < Xinha.colorPicker.savedColors.length;
        savedCols++
      ) {
      ...
      for (var row = 0; row < this.side; row++) {
        for (var col = 0; col < this.side; col++) {
          if (constrain) {
            this.saved_cells[row][col].colorCode = tupleToColor(
              rgbToWebsafe(hsvToRGB(huestep * row, saturstep * col, this.value))
            );
          } else {
            this.saved_cells[row][col].colorCode = tupleToColor(
          ...
          this.saved_cells[row][col].style.backgroundColor =
            this.saved_cells[row][col].colorCode;
          this.saved_cells[row][col].style.borderColor =
            this.saved_cells[row][col].colorCode;
        }
      }
      ...
      if (
        (pickcell.hue >= 195 && pickcell.saturation > 0.5) ||
        (pickcell.hue === 0 &&
          pickcell.saturation === 0 &&
          pickcell.value < 0.5) ||
...
Xinha.colorPicker.remember = function (color, savecolors) {
  ...
  // limit elements
  Xinha.colorPicker.savedColors = Xinha.colorPicker.savedColors.slice(
    0,
    savecolors
  );
  ...
  document.cookie =
    "XinhaColorPicker=" +
    escape(Xinha.colorPicker.savedColors.join("-")) +
    ";expires=" +
    expdate.toGMTString();
  return true;
};
...
Xinha.colorPicker.loadColors = function () {
  ...
  if (index != -1) {
    ...
    Xinha.colorPicker.savedColors = unescape(
...
Xinha.colorPicker.InputBinding = function (input, pickerConfig) {
  ...
  chooser.onmouseover = function () {
    chooser.className = "chooser buttonColor-hilite";
  };
  ...
  clearColor.onmouseover = function () {
    clearColor.className = "nocolor buttonColor-hilite";
    clearColor.style.color = "#f00";
  };
  clearColor.onmouseout = function () {
    clearColor.className = "nocolor";
    clearColor.style.color = "#000";
  };
  clearColor.onclick = function () {
    input.value = "";
    chooser.style.backgroundColor = "";
  };
  ...
  Xinha._addEvent(input, "change", function () {
    chooser.style.backgroundColor = this.value;
  });

  pickerConfig = pickerConfig
    ? Xinha.cloneObject(pickerConfig)
    : { cellsize: "5px" };
  pickerConfig.callback = pickerConfig.callback
    ? pickerConfig.callback
    : function (color) {
        chooser.style.backgroundColor = color;
        input.value = color;
        if (typeof input.oncolorpicked == "function") input.oncolorpicked();
      };
