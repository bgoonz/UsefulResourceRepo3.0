...
function PopupWin(editor, title, handler, initFunction) {
  ...
  var dlg = window.open(
    "",
    "__ha_dialog",
    "toolbar=no,menubar=no,personalbar=no,width=600,height=600,left=20,top=40,scrollbars=no,resizable=yes"
  );
  this.window = dlg;
  ...
  if (
    typeof _editor_url != "undefined" &&
    !/^\//.test(_editor_url) &&
    !/http:\/\//.test(_editor_url)
  ) {
...
PopupWin.prototype.addButtons = function () {
  ...
  for (var i = 0; i < arguments.length; ++i) {
    ...
    switch (btn.toLowerCase()) {
      case "ok":
        Xinha.addDom0Event(button, "click", function () {
          self.callHandler();
          self.close();
          return false;
        });
