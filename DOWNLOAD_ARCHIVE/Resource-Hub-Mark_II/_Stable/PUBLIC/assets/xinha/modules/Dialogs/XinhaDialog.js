...
Xinha.Dialog = function (editor, html, localizer, size, options) {
  ...
  */
    ...
    backG.onreadystatechange = function () {
      ...
      if (this.readyState == "complete" && doc && doc.body) {
        ...
        for (var i = 0; i < stylesheets.length; i++) {
          if (
            stylesheets[i].id.indexOf("Xinha") != -1 &&
  ...
  if (!this.modal) {
    Xinha._addEvent(rootElem, "mousedown", function () {
      Xinha.Dialog.activateModeless(dialog);
    });
  }
  ...
  Xinha._addEvent(captionBar, "mousedown", function (ev) {
    dialog.dragStart(ev);
  });
  ...
  captionBar.onselectstart = function () {
    return false;
  };
...
Xinha.Dialog.prototype.show = function (values) {
  ...
  if (Xinha._someEditorHasBeenActivated) {
    ...
    if (Xinha.is_ie && !modal) {
      dialog.saveSelection = function () {
        dialog._lastRange = dialog.editor.saveSelection();
      };
  ...
  if (Xinha.is_ff2 && modal) {
    this._restoreTo = [
      this.editor._textArea.style.display,
      this.editor._iframe.style.visibility,
  ...
  if (!this.attached) {
    ...
    if (this.size.top && this.size.left) {
      ...
      if (dialogHeight + rootElem.offsetTop > viewportHeight) {
        rootElemStyle.top =
          (rootElemStyle.position == "absolute" ? this.scrollPos.y : 0) + "px";
      }
...
Xinha.Dialog.prototype.getElementsByName = function (name) {
  ...
  var els = this.document.getElementsByName(
    this.id[name] ? this.id[name] : name
  );
...
Xinha.Dialog.prototype.dragStart = function (ev) {
  ...
  dialog.mouseMove = function (ev) {
    dialog.dragIt(ev);
  };
  Xinha._addEvent(document, "mousemove", dialog.mouseMove);
  if (Xinha.is_ie) {
    Xinha._addEvent(
      this.background.contentWindow.document,
      "mousemove",
      dialog.mouseMove
    );
  }

  dialog.mouseUp = function (ev) {
    dialog.dragEnd(ev);
  };
...
Xinha.Dialog.prototype.attachToPanel = function (side) {
  ...
  this.captionBar.ondblclick = function (ev) {
    dialog.detachFromPanel(Xinha.getEvent(ev));
  };
...
Xinha.Dialog.prototype.detachFromPanel = function () {
  ...
  dialog.captionBar.ondblclick = function () {
    dialog.attachToPanel(rootElem.side);
  };
...
Xinha.Dialog.calcFullBgSize = function () {
  ...
  var viewport = Xinha.viewportSize();
  return {
    width: (page.x > viewport.x ? page.x : viewport.x) + "px",
    height: (page.x > viewport.y ? page.y : viewport.y) + "px",
  };
};
...
Xinha.Dialog.prototype.getValues = function () {
  var values = [];
  var inputs = Xinha.collectionToArray(
    this.rootElem.getElementsByTagName("input")
  )
    .append(
      Xinha.collectionToArray(this.rootElem.getElementsByTagName("textarea"))
    )
    .append(
      Xinha.collectionToArray(this.rootElem.getElementsByTagName("select"))
    );
...
Xinha.Dialog.prototype.translateHtml = function (html, localizer) {
  ...
  html = html
    .replace(
      /((?:name)|(?:id))=(['"])\[([a-z0-9_]+)\]\2/gi,
      function (fullString, type, quote, id) {
        return type + "=" + quote + dialog.createId(id) + quote;
      }
    )
    .replace(/<l10n>(.*?)<\/l10n>/gi, function (fullString, translate) {
      return dialog._lc(translate);
    })
    .replace(/\="_\((.*?)\)"/g, function (fullString, translate) {
      return '="' + dialog._lc(translate) + '"';
    });
  return html;
};
...
Xinha.Dialog.prototype.fixupDOM = function (root, plugin) {
  ...
  for (var index = 0; index < images.length; ++index) {
    ...
    if (reference) {
      var fixedReference = reference.replace(
        /^\[(editor|plugin|images)\]/,
        linkReplace
      );
...
Xinha.Dialog.activateModeless = function (dialog) {
  ...
  if (Xinha.Dialog.activeModeless) {
    Xinha.Dialog.activeModeless.rootElem.style.zIndex =
      parseInt(Xinha.Dialog.activeModeless.rootElem.style.zIndex, 10) - 10;
  }
...
Xinha.Dialog.fadeIn = function (el, endOpacity, callback, delay, step) {
  ...
  if (op < endOpacity) {
    ...
    el.timeOut = setTimeout(function () {
      Xinha.Dialog.fadeIn(el, endOpacity, callback, delay, step);
    }, delay);
  } else {
...
Xinha.Dialog.fadeOut = function (el, delay, step) {
  ...
  if (op >= 0) {
    ...
    Xinha.Dialog.setOpacity(el, op);
    el.timeOut = setTimeout(function () {
      Xinha.Dialog.fadeOut(el, delay, step);
    }, delay);
  } else {
    Xinha.Dialog.setOpacity(el, 0);
