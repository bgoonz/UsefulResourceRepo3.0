...
WebKit._pluginInfo = {
  name: "WebKit",
  origin: "Xinha Core",
  version: "$LastChangedRevision: 1452 $".replace(/^[^:]*:\s*(.*)\s*\$$/, "$1"),
  developer: "The Xinha Core Developer Team",
  developer_url:
    "$HeadURL: http://svn.xinha.org/trunk/modules/WebKit/WebKit.js $".replace(
      /^[^:]*:\s*(.*)\s*\$$/,
      "$1"
    ),
  sponsor: "",
...
WebKit.prototype.onKeyPress = function (ev) {
  ...
  switch (editor.getKey(ev)) {
    ...
    case " ":
      ...
      if (
        editor.config.convertUrlsToLinks &&
        s &&
        s.isCollapsed &&
        s.anchorNode.nodeType == 3 &&
        s.anchorNode.data.length > 3 &&
        s.anchorNode.data.indexOf(".") >= 0
      ) {
        var midStart = s.anchorNode.data
          .substring(0, s.anchorOffset)
          .search(/\S{4,}$/);
        if (midStart == -1) {
          break;
        }
        ...
        var matchData = s.anchorNode.data
          .substring(0, s.anchorOffset)
          .replace(/^.*?(\S*)$/, "$1");
        ...
        if (mUrl) {
          ...
          autoWrap(midTextUrl, "a").href =
            (mUrl[1] ? mUrl[1] : "http://") + mUrl[2];
          return true;
        }
      }
      break;
        ...
        if (editor.config.tabSpanClass) {
          if (!ev.shiftKey) {
            ...
            editor.insertHTML(
              "<" +
                'span class="' +
                editor.config.tabSpanClass +
                '">' +
                editor.config.tabSpanContents +
                "</span>"
            );
            ...
            if (
              existingTab &&
              existingTab.className.match(editor.config.tabSpanClass)
            ) {
              var s = editor.getSelection();
              s.removeAllRanges();
    ...
    default:
      ...
      if (s.anchorNode && s.anchorNode.nodeType == 3) {
        ...
        if (!a._updateAnchTimeout) {
          if (
            s.anchorNode.data.match(Xinha.RE_email) &&
          ...
          if (
            ...
            a.href.match(
              new RegExp(
                "http(s)?://" +
                  Xinha.escapeStringForRegExp(s.anchorNode.data.trim())
              )
            )
          ) {
  ...
  switch (cmdID) {
    ...
    case "removeformat":
      ...
      function clean(el) {
        ...
        if (
          (el.tagName.toLowerCase() == "span" && !el.attributes.length) ||
      ...
      if (sel.isCollapsed) {
        ...
        for (i = 0; i < els.length; i++) {
          ...
          if (
            ...
            (els[i] == start && range.startOffset == 0)
          ) {
            clean(el);
          }
        }
...
WebKit.prototype.onMouseDown = function (ev) {
  ...
  if (
    ev.target.tagName.toLowerCase() == "hr" ||
...
Xinha.prototype.activeElement = function (sel) {
  ...
  if (!sel.isCollapsed) {
    if (
      sel.anchorNode.childNodes.length > sel.anchorOffset &&
...
Xinha.prototype.isKeyDownThatShouldGetButDoesNotGetAKeyPressEvent = function (
  ...
  if (typeof keyEvent.key != "undefined") {
    if (typeof Xinha.DOM3_WebKit_KeyDownKeyPress_RE == "undefined") {
      ...
      Xinha.DOM3_WebKit_KeyDownKeyPress_RE =
  ...
  // Old Safari and Chrome
    ...
    if (kI == 9 /* Tab */ || kI == 8 /* Backspace */ || kI == 127 /* Delete */)
      return true;
  }
  // Legacy
  else {
...
Xinha.prototype.getKey = function (keyEvent) {
  ...
  // Old DOM3 used by (Old?) Safari SOMETIMES (but not ALWAYS!) and old Chrome
    ...
    typeof keyEvent.keyIdentifier != "undefined" &&
    keyEvent.keyIdentifier.length > 0 &&
    keyEvent.keyIdentifier.match(/^U\+/)
  ) {
    var key = String.fromCharCode(
      parseInt(keyEvent.keyIdentifier.replace(/^U\+/, ""), 16)
    );
...
Xinha.prototype.setCC = function (target) {
  ...
  try {
    if (target == "textarea") {
      ...
      if (after.match(/^[^<]*>/)) {
        // make sure cursor is in an editable area (outside tags, script blocks, enities and inside the body)
        var tagEnd = after.indexOf(">") + 1;
        ta.value =
          before +
          after.substring(0, tagEnd) +
          cc +
          after.substring(tagEnd, after.length);
