...
InsertImage._pluginInfo = {
  ...
  developer_url:
    "$HeadURL: http://svn.xinha.org/trunk/modules/InsertImage/insert_image.js $".replace(
      /^[^:]*:\s*(.*)\s*\$$/,
      "$1"
    ),
  sponsor: "",
...
InsertImage.loadAssets = function () {
  ...
  self.loading = true;
  Xinha._getback(
    _editor_url + "modules/InsertImage/dialog.html",
    function (getback) {
      self.html = getback;
  ...
  Xinha._getback(
    _editor_url + "modules/InsertImage/pluginMethods.js",
    function (getback) {
      eval(getback);
...
InsertImage.prototype.prepareDialog = function () {
  ...
  this.dialog.onresize = function () {
    ...
    this.getElementById("ipreview").style.height =
      (newHeightForPreview > 0 ? newHeightForPreview : 0) + "px"; // no-go beyond 0
