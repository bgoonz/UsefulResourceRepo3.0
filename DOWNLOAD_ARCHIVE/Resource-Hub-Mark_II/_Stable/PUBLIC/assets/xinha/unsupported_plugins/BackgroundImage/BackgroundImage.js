...
BackgroundImage.prototype.onGenerateOnce = function (editor) {
  ...
  Xinha._getback(
    Xinha.getPluginDir("BackgroundImage") + "/dialog.html",
    function (getback) {
      // Replace the template line with one line per image.
      self.html = getback.replace(
        /<template>(.*?)<\/template>/gi,
        function (fullString, template) {
          var replacement = "";

          for (bg in backgrounds) {
            var thumbURL =
              Xinha.getPluginDir("BackgroundImage") +
...
BackgroundImage.prototype.prepareDialog = function (editor) {
  ...
  dialog.getElementById("set").onclick = function () {
    self.apply();
  };
  dialog.getElementById("delete").onclick = function () {
    self.deleteBg();
  };
...
BackgroundImage.prototype.show = function (editor) {
  ...
  if (
    editor._doc.body.background.split(".").pop() in
