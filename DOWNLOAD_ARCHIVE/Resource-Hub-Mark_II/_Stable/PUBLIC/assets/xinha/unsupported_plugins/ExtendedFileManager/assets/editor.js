...
function toggle(action) {
  if (current_action != action) {
    ...
    if (action == "save") {
      ...
      var format = document
        .getElementById("save_filename")
        .value.match(/\.(gif|png|jpe?g)/i)[1]
        .toLowerCase();
      switch (format) {
        case "png":
...
function toggleMarker() {
  ...
  if (marker != null && marker.src != null) {
    if (marker.src.indexOf("t_black.gif") >= 0) marker.src = "img/t_white.gif";
    else marker.src = "img/t_black.gif";
...
function winOnResize() {
  ...
  document.getElementById("contents").style.height =
    win.y -
    parseInt(document.getElementById("indicator").offsetHeight, 10) -
    5 +
    "px";
}
