...
function init() {
  ...
  if (manager_mode == "image" && param) {
    ...
    if (
      param.f_url.length > 0 &&
      !absoluteURL.test(param.f_url) &&
      typeof param.baseHref == "string"
    ) {
    ...
    var rd = _resized_dir
      ? _resized_dir.replace(Xinha.RE_Specials, "\\$1") + "/"
      : "";
    ...
    document.getElementById("f_backgroundColor").value =
    ...
    document.getElementById("f_preview").src =
      _backend_url + "__function=thumbs&img=" + param.f_url;
    ...
    startDir = document.cookie.match(
      new RegExp("EFMStartDir" + manager_mode + "=(.*?)(;|$)")
    );
    if (startDir) {
      startDir = startDir[1];
    }
  ...
  if (
    manager_mode == "image" &&
    typeof Xinha.colorPicker != "undefined" &&
    document.getElementById("f_backgroundColor") &&
    document.getElementById("f_backgroundColor").type == "text"
  ) {
    ...
    new Xinha.colorPicker.InputBinding(
      document.getElementById("f_backgroundColor"),
      pickerConfig
    );
    new Xinha.colorPicker.InputBinding(
      document.getElementById("f_borderColor"),
      pickerConfig
    );
  }
  setTimeout(function () {
...
function pasteButton(action) {
  ...
  if (!buttonHolder.firstChild) {
    ...
    img.src =
      window.opener.Xinha.getPluginDir("ExtendedFileManager") +
      "/img/edit_paste.gif";
    img.alt = i18n("Paste");
    a.appendChild(img);
  ...
  switch (action.action) {
    case "copyFile":
      buttonHolder.firstChild.title = i18n(
        'Copy "$file=' +
          action.file +
          '$" from "$dir=' +
          decodeURIComponent(action.dir) +
          '$" here'
      );
      break;
    case "copyDir":
...
function onOK() {
  if (manager_mode == "image") {
    ...
    if (origsize.w != param.f_width || origsize.h != param.f_height) {
      // Yup, need to resize
      var resized = Xinha._geturlcontent(
        window.opener.Xinha.getPluginDir("ExtendedFileManager") +
          "/" +
          _backend_url +
          "&__function=resizer&img=" +
          encodeURIComponent(document.getElementById("f_url").value) +
          "&width=" +
          param.f_width +
          "&height=" +
          param.f_height
...
function makeURL(pathA, pathB) {
  if (pathA.substring(pathA.length - 1) != "/") pathA += "/";
...
function goUpDir() {
  ...
  if (currentDir.length < 2) return false;
...
function changeDir(newDir) {
  if (typeof imgManager != "undefined") imgManager.changeDir(newDir);
}
...
function showMessage(newMessage) {
  ...
  if (message.firstChild) message.removeChild(message.firstChild);
...
function doUpload() {
  var uploadForm = document.getElementById("uploadForm");
  if (uploadForm) showMessage("Uploading");
}
...
function newFolder() {
  ...
  if (Xinha.ie_version > 6) {
    popupPrompt(
      i18n("Please enter name for new folder..."),
      i18n("Untitled"),
      createFolder,
      i18n("New Folder")
    );
...
function resize() {
  ...
  var messagesHeight =
    m && m.style.display != "none"
      ? parseInt(document.getElementById("messages").offsetHeight, 10)
      : 0;
  document.getElementById("imgManager").style.height =
    parseInt(
      win.y - 130 - document.getElementById("controls").offsetHeight,
      10
    ) -
    messagesHeight +
    "px";
