...
function ClientsideSpellcheck(editor) {
  ...
  cfg.registerButton(
    id,
    this._lc("Spell Check using ieSpell"),
    editor.imgURL("clientside-spellcheck.gif", "ClientsideSpellcheck"),
    false,
    function (editor, id) {
...
ClientsideSpellcheck.prototype.buttonPress = function (editor) {
  try {
    ...
    if (exception.number == -2146827859) {
      if (
        confirm(
          this.lc("ieSpell not detected.  Click Ok to go to download page.")
        )
      )
