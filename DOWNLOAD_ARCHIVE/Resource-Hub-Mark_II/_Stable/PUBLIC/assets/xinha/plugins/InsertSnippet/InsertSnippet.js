...
function InsertSnippet(editor) {
  ...
  if (cfg.InsertSnippet.backend_data != null) {
    for (var i in cfg.InsertSnippet.backend_data) {
      backend +=
        "&" + i + "=" + encodeURIComponent(cfg.InsertSnippet.backend_data[i]);
    }
  }
  Xinha._getback(backend, function (getback) {
    eval(getback);
...
InsertSnippet.prototype.onGenerate = function () {
  this.editor.addEditorStylesheet(
    Xinha.getPluginDir("InsertSnippet") + "/InsertSnippet.css"
  );
};
...
InsertSnippet.prototype.buttonPress = function (editor) {
  ...
  var self = this;
  editor._popupDialog(
    "plugin://InsertSnippet/insertsnippet",
    function (param) {
      if (!param) {
        return false;
      }
