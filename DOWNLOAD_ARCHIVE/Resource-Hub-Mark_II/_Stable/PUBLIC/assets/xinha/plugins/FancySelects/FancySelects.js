...
Xinha.loadLibrary("jQuery")
  ...
  .loadScriptIf(
    _editor_lang != "en",
    "select2-4.0.6-rc.1-dist/js/i18n/" + _editor_lang + ".js",
    "FancySelects"
  );
...
FancySelects.prototype.onGenerateOnce = function () {
  ...
  if (
    !(
      ...
      jQuery.fn &&
      jQuery.fn.select2 &&
      jQuery(editor._htmlArea).find(".toolbarElement select").length
    )
  ) {
    var self = this;
  ...
  try {
    jQuery(editor._htmlArea)
      .find(".toolbarElement select")
      .each(function (i, e) {
        var txt = e.name;
        var el = e;
    ...
    var failed = Xinha.getElementsByClassName(
      editor._htmlArea,
      "select2-container"
    );
