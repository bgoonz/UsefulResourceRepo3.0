...
Xinha.Config.prototype.stylistLoadStylesheet = function (
  url,
  altnames,
  skip_imports
) {
...
Xinha.Config.prototype.stylistLoadStyles = function (
  styles,
  altnames,
  skip_imports,
  imports_relative_to
...
Xinha.prototype._fillStylist = function () {
  ...
  for (var x in this.config.css_style) {
        ...
        if (this._getFirstAncestor(this._getSelection(), [tag]) != null) {
          ...
          if (
            ...
            if (!this._selectionEmpty(this._getSelection())) {
              ...
              apply_to = this._getFirstAncestor(sel, [
                "p",
                "h1",
                "h2",
                "h3",
    ...
    var applied =
      this._ancestorsWithClasses(sel, tag, className).length > 0 ? true : false;
    ...
    if (applicable) {
      ...
      anch.onclick = function () {
        if (this._stylist_applied == true) {
          editor._stylistRemoveClasses(
            this._stylist_className,
            this._stylist_appliedTo
          );
        } else {
          editor._stylistAddClasses(
            this._stylist_applyTo,
            this._stylist_applyTag,
            this._stylist_className
          );
        }
        return false;
  ...
  if (el == "new") {
    ...
    if (tag != null && el.tagName.toLowerCase() != tag) {
      ...
      if (typeof el._stylist_usedToBe != "undefined") {
        ...
        new_el._stylist_usedToBe = [
          { tagName: el.tagName, className: el.getAttribute("class") },
        ];
      }
...
Xinha.prototype._stylistRemoveClassesFull = function (el, classes) {
  if (el != null) {
    ...
    if (
      ...
      var last_classes = Xinha.arrayFilter(
        last_el.className.trim().split(" "),
        function (c) {
          if (c == null || c.trim() == "") {
            return false;
          }
...
Xinha.prototype._getAncestorsClassNames = function (sel) {
  ...
  if (prnt == null) {
    prnt = Xinha.is_ie
      ? this._createRange(sel).parentElement()
...
Xinha.ripStylesFromCSSString = function (
  css,
  skip_imports,
  imports_relative_to
) {
...
Stylist.prototype.onGenerateOnce = function () {
  ...
  if (
    typeof cfg.css_style != "undefined" &&
    Xinha.objectProperties(cfg.css_style).length != 0
  ) {
    this._prepareDialog();
  }
};
Stylist.prototype._prepareDialog = function () {
  ...
  main.style.height =
    this.editor._framework.ed_cell.offsetHeight - caption.offsetHeight + "px";
  ...
  editor.notifyOn("panel_change", function (e, args) {
    ...
    switch (args.action) {
      case "show":
        ...
        main.style.height =
          (newHeight > 0 ? main.offsetHeight - args.panel.offsetHeight : 0) +
          "px";
...
Stylist.prototype.resize = function () {
  ...
  for (var i = 0; i < panelContainer.childNodes.length; ++i) {
    if (
      panelContainer.childNodes[i] == rootElem ||
      !panelContainer.childNodes[i].offsetHeight
    ) {
      continue;
    }
  ...
  this.dialog.main.style.height =
    newSize - this.dialog.captionBar.offsetHeight - 5 + "px";
};
...
Stylist.prototype.onUpdateToolbar = function () {
  if (this.dialog) {
    ...
    this._timeoutID = window.setTimeout(function () {
      if (e.editorIsActivated()) e._fillStylist();
    }, 500);
  }
};
