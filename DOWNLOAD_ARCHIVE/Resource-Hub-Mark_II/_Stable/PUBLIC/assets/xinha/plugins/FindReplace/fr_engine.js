...
function execSearch(params) {
  ...
  if (buffer == null) buffer = ihtml;

  if (params["fr_pattern"] != tosearch) {
    if (tosearch != "") clearDoc();
    tosearch = params["fr_pattern"];
  }

  if (matches == 0) {
    er = params["fr_words"]
      ? "/(?!<[^>]*)(\\b" + params["fr_pattern"] + "\\b)(?![^<]*>)/g"
      : "/(?!<[^>]*)(" + params["fr_pattern"] + ")(?![^<]*>)/g";
    if (!params["fr_matchcase"]) er += "i";
    ...
    for (var i = 0; i < getallspans.length; i++)
      if (/^frmark/.test(getallspans[i].id)) fr_spans.push(getallspans[i]);
  }

  spanWalker(
    params["fr_pattern"],
    params["fr_replacement"],
    params["fr_replaceall"]
  );
}
    ...
    if (!/[0-9]$/.test(elm.id)) {
      ...
      if (/\w/.test(replacement)) {
        if (replaceall || confirm(_lc("Substitute this occurrence?"))) {
          elm.firstChild.replaceData(
            0,
            elm.firstChild.data.length,
            replacement
          );
          replaces++;
          disab("fr_undo", false);
