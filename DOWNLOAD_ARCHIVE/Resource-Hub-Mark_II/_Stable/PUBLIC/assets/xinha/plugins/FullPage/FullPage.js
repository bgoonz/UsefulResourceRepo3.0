...
function FullPage(editor) {
  ...
  cfg.registerButton(
    "FP-docprop",
    this._lc("Document properties"),
    editor.imgURL("docprop.gif", "FullPage"),
    false,
    function (editor, id) {
    ...
    case "FP-docprop":
      ...
      for (var i = links.length; --i >= 0; ) {
        ...
        if (/stylesheet/i.test(link.rel)) {
          if (/alternate/i.test(link.rel)) style2 = link.href;
          else style1 = link.href;
        }
      }
      ...
      for (var i = metas.length; --i >= 0; ) {
        ...
        if (/content-type/i.test(meta.httpEquiv)) {
          ...
          /description/i.test(meta.name) ||
          /description/i.test(meta.id)
        ) {
...
FullPage.prototype.setDocProp = function (params) {
  ...
  for (var i = links.length; --i >= 0; ) {
    var link = links[i];
    if (/stylesheet/i.test(link.rel)) {
      if (/alternate/i.test(link.rel)) style2 = link;
      else style1 = link;
    }
  }
  for (var i = metas.length; --i >= 0; ) {
  ...
  if (!style1 && params.f_base_style) style1 = createLink(false);
  if (params.f_base_style) style1.href = params.f_base_style;
  else if (style1) head.removeChild(style1);

  if (!style2 && params.f_alt_style) style2 = createLink(true);
  if (params.f_alt_style) style2.href = params.f_alt_style;
  else if (style2) head.removeChild(style2);
  ...
  if (!charset_meta && params.f_charset)
    charset_meta = createMeta(
      "Content-Type",
      "",
      "text/html; charset=" + params.f_charset
    );

  if (!keywords && params.f_keywords)
    keywords = createMeta("", "keywords", params.f_keywords);
  else if (params.f_keywords) keywords.content = params.f_keywords;
  else if (keywords) head.removeChild(keywords);

  if (!description && params.f_description)
    description = createMeta("", "description", params.f_description);
  else if (params.f_description) description.content = params.f_description;
  else if (description) head.removeChild(description);
  ...
  for (var i in params) {
    ...
    switch (i) {
      case "f_title":
        ...
        } else while ((node = title.lastChild)) title.removeChild(node);
        if (!Xinha.is_ie) title.appendChild(doc.createTextNode(val));
        else doc.title = val;
        break;
      case "f_doctype":
