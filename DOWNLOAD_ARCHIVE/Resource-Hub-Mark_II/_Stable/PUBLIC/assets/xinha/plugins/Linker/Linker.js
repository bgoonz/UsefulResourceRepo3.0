...
function Linker(editor, args) {
  ...
  if (!editor.config.btnList.createlink) {
    editor.config.registerButton(
      "createlink",
      "Insert/Modify Hyperlink",
      [_editor_url + "images/ed_buttons_main.gif", 6, 1],
      false,
...
Linker.loadAssets = function () {
  ...
  self.loading = true;
  Xinha._getback(
    Xinha.getPluginDir("Linker") + "/pluginMethods.js",
    function (getback) {
      eval(getback);
  ...
  Xinha._loadback(
    Xinha.getPluginDir("Linker") + "/dTree/dtree.js",
    function () {
      Linker.dTreeReady = true;
    }
  );
  Xinha._getback(
    Xinha.getPluginDir("Linker") + "/dialog.html",
    function (getback) {
      self.html = getback;
    }
  );
