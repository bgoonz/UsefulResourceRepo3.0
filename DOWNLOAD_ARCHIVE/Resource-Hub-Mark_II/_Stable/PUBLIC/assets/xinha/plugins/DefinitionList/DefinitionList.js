...
function DefinitionList(editor) {
  ...
  for (var i = 0; i < bl.length; ++i) {
    ...
    cfg.registerButton(
      id,
      this._lc(btn[1]),
      editor.imgURL("ed_" + btn[0] + ".gif", "DefinitionList"),
      false,
      function (editor, id) {
...
DefinitionList.prototype.onGenerate = function () {
  this.editor.addEditorStylesheet(
    Xinha.getPluginDir("DefinitionList") + "/definition-list.css"
  );
};
...
DefinitionList.prototype.buttonPress = function (editor, button_id) {
  ...
  if (button_id == "dl") {
  ...
  } else if (button_id == "dt" || button_id == "dd") {
    //definition term or description
    pe = editor.getParentElement();
    while (pe && pe.nodeType == 1 && pe.tagName.toLowerCase() != "body") {
      if (pe.tagName.toLowerCase() == "dl") {
        ...
        pe.tagName.toLowerCase() == "dt" ||
        pe.tagName.toLowerCase() == "dd"
      ) {
        dx = editor._doc.createElement(button_id);
        dx.innerHTML = "&nbsp;";
    ...
    if (pe.tagName.toLowerCase() == "body")
      alert(
        "You can insert a definition term or description only in a definition list!"
      );
  }
};
