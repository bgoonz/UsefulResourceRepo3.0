...
InsertSnippet2.prototype._prepareDialog = function () {
  ...
  if (!this.html) {
    ...
    Xinha._getback(
      Xinha.getPluginDir("InsertSnippet2") + "/dialog.html",
      function (getback) {
        self.html = getback;
        self._prepareDialog();
      }
...
InsertSnippet2.prototype.drawSnippetTable = function () {
  ...
  for (var i = 0, trowNo = 0; i < snippets.length; i++) {
    ...
    if (
      this.categoryFilter &&
      snippet_cat != this.categoryFilter &&
    ...
    newCell.onmouseover = function (event) {
      return self.preview(event || window.event);
    };
    ...
    btn.onclick = function (event) {
      self.doInsert(event || window.event);
      return false;
    };
...
InsertSnippet2.prototype.drawCatTabs = function () {
  ...
  tab.onclick = function () {
    self.categoryFilter = self.cat;
    self.drawCatTabs();
    self.drawSnippetTable();
    self.search();
  };
  ...
  for (var i = 0; i < this.categories.length; i++) {
    ...
    tabsdiv.appendChild(tab);
    if (Xinha.is_gecko)
      tabsdiv.appendChild(document.createTextNode(String.fromCharCode(8203)));
...
InsertSnippet2.prototype.search = function () {
  ...
  if (searchField.value) {
    ...
    try {
      var re = new RegExp(wordstart + val, "i");
    } catch (e) {
      var re = null;
    }
  } else var re = null;
...
InsertSnippet2.prototype.preparePreview = function () {
  var editor = this.editor;
  var self = this;
  var preview_iframe = (this.previewIframe =
    this.dialog.getElementById("preview_iframe"));
