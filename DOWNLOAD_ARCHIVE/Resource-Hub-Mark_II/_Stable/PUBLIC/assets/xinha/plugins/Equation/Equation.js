...
function Equation(editor) {
  ...
  if (typeof AMprocessNode != "function") {
    Xinha._loadback(
      Xinha.getPluginDir("Equation") + "/ASCIIMathML.js",
      function () {
        translate();
      }
    );
...
Equation.prototype.parse = function () {
  if (this.enabled) {
    ...
    for (var i = 0; i < spans.length; i++) {
      ...
      if (node.innerHTML.indexOf(this.editor.cc) != -1) {
        // avoid problems with source code position auxiliary character
      ...
      clone.parentNode.insertBefore(
        doc.createTextNode(String.fromCharCode(32)),
        clone
      );
      clone.parentNode.insertBefore(
        doc.createTextNode(String.fromCharCode(32)),
...
Equation.prototype.unParse = function () {
  ...
  for (var i = 0; i < spans.length; i++) {
    var node = spans[i];
    if (
      node.className.indexOf("AM") == -1 ||
      node.getElementsByTagName("math").length == 0
    )
      continue;
    var formula =
      "`" + node.getElementsByTagName("math")[0].getAttribute("title") + "`";
...
Equation.prototype.buttonPress = function () {
  ...
  Dialog(
    Xinha.getPluginDir("Equation") + "/popups/dialog.html",
    function (params) {
      self.insert(params);
    },
    args
...
Equation.prototype.insert = function (param) {
  ...
  if (typeof param["formula"] != "undefined") {
    var formula =
      param["formula"] != ""
        ? param["formula"].replace(/^`?(.*)`?$/m, "`$1`")
        : "";

    if (
      param["editedNode"] &&
      param["editedNode"].tagName.toLowerCase() == "span"
    ) {
