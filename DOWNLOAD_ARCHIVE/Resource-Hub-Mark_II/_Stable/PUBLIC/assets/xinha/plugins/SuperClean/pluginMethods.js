        ...
        switch (response.action) {
          ...
          case "alert":
            superclean._dialog.dialog.getElementById("buttons").style.display =
              "";
            superclean._dialog.dialog.getElementById("ok").style.display =
              "none";
            superclean._dialog.dialog.getElementById("waiting").style.display =
              "none";
            superclean._dialog.dialog.getElementById("alert").style.display =
              "";
            superclean._dialog.dialog.getElementById("alert").innerHTML =
              superclean._lc(response.value);
            break;
          default:
            // make the dialog go away if sth goes wrong, who knows...
            superclean._dialog.hide();
            break;
        }
      };
...
SuperClean.filterFunctions.remove_emphasis = function (html) {
  html = SuperClean.stripTags(html, [
    "b",
    "strong",
    "i",
    "em",
...
SuperClean.filterFunctions.remove_sup_sub = function (html) {
  return SuperClean.stripTags(html, ["sup", "sub"]).replace(
    /vertical-align:\s*(sub|super)[^;}"']+;?/gi
  );
};
...
SuperClean.filterFunctions.remove_alignment = function (html) {
  return html
    .replace(/ align=[^\s|>]*/gi, "")
    .replace(/(text-align|vertical-align|float):[^;}"']+;?/gi, "");
};
  ...
  for (var filter in this.SuperClean.editor.config.SuperClean.filters) {
    ...
    if (typeof filtDetail.label == "undefined") {
      ...
      htmlFilters +=
        ...
        filter +
        ']">' +
        this.SuperClean.editor.config.SuperClean.filters[filter] +
        "</label>\n";
    } else {
      htmlFilters +=
        '        <input type="checkbox" name="[' +
