...
function SmartReplace(editor) {
  ...
  cfg.registerButton({
    ...
    textMode: false,
    action: function (e, objname, obj) {
      self.buttonPress(null, obj);
    },
  });
...
SmartReplace.prototype.onGenerate = function () {
  ...
  var self = this;
  Xinha._addEvent(self.editor._doc, "keypress", function (event) {
  ...
  if (this.openingQuotes == "OpeningDoubleQuotes") {
    //If nothing else is defined, English style as default
  ...
  if (Xinha.is_ie) {
    ...
    if (r.toString().match(/[^\s\xA0]/)) {
      ...
      editor.insertNodeAtSelection(
        document.createTextNode(r.toString() + opening)
      );
    }
...
SmartReplace.prototype.smartReplace = function (
  ev,
  lookback,
  re,
  replace,
  ...
  if (Xinha.is_ie) {
    ...
    if (r.startOffset > 1)
      r.setStart(r.startContainer, r.startOffset - lookback);
...
SmartReplace.prototype._prepareDialog = function () {
  ...
  if (!this.html) {
    Xinha._getback(
      Xinha.getPluginDir("SmartReplace") + "/dialog.html",
      function (getback) {
        self.html = getback;
        self._prepareDialog();
      }
