...
function LangMarks(editor, args) {
  ...
  if (!cfg.LangMarks.languages) {
    Xinha.debugMsg(
      "Warning: Old style LangMarks configuration detected, please update your LangMarks configuration."
    );
    var newConfig = {
      languages: [],
  ...
  for (var i = 0; i < cfg.LangMarks.languages.length; i++) {
    options[this._lc(cfg.LangMarks.languages[i].name)] =
      cfg.LangMarks.languages[i].code;
  }
...
Xinha.Config.prototype.LangMarks = {
  languages: [
    // Below are so lc_parse_strings.php finds them
...
LangMarks.prototype.onGenerate = function () {
  this.editor.addEditorStylesheet(
    Xinha.getPluginDir("LangMarks") + "/lang-marks.css"
  );
};

LangMarks.prototype.onSelect = function (
  editor,
  obj,
  context,
  updatecontextclass
  ...
  var update_parent =
    ...
    updatecontextclass &&
    parent &&
    parent.tagName.toLowerCase() == context;
  ...
  if (is_span) {
    ...
    if (parent.childNodes.length == 1) {
      ...
      for (var i = 0; i < this.editor.config.LangMarks.attributes.length; i++) {
        parent.setAttribute(
          this.editor.config.LangMarks.attributes[i],
          language
        );
      }
  ...
  for (var i = 0; i < parents.length; i++) {
    for (var j = 0; j < editor.config.LangMarks.attributes.length; j++) {
      if (parents[i].getAttribute(this.editor.config.LangMarks.attributes[j])) {
        parent = parents[i];
        lang = parents[i].getAttribute(
          this.editor.config.LangMarks.attributes[j]
        );
      }
    }
  }
