...
MootoolsFileManager.prototype.OpenImageManager = function (image) {
  ...
  if (image) {
    outparam = {
      ...
      f_alt: image.alt,
      f_border: image.style.borderWidth
      ...
      f_hspace:
        image.hspace && image.hspace != "-1" ? parseInt(image.hspace) : null,
      f_vspace:
        image.vspace && image.vspace != "-1" ? parseInt(image.vspace) : null,
    };
  ...
  if (!this.ImageManagerWidget) {
    this.ImageManagerWidget = new FileManager({
      ...
      assetBasePath:
        Xinha.getPluginDir("MootoolsFileManager") +
        "/mootools-filemanager/Assets",
      ...
      propagateData: Object.merge(
        { __function: "image-manager" },
        this.editor.config.MootoolsFileManager.backend_data
      ),
      propagateType: "POST",

      uploadAuthData: Object.merge(
        { __function: "image-manager" },
        this.editor.config.MootoolsFileManager.backend_data
      ),

      onComplete: function (path, file, mgr) {
        self.ImageManagerReturn(path, file);
      },
      onHide: function () {
        if (this.swf && this.swf.box) this.swf.box.style.display = "none";
      },
      onShow: function () {
      ...
      onDetails: function (details) {
        ...
        if (this.info.getElement(".filemanager-preview dl+p")) {
          this.info.getElement(".filemanager-preview dl+p").style.display =
            "none";
        }
        return true;
      },
...
MootoolsFileManager.prototype.ImageManagerAttributes = function (details) {
  ...
  if (!this._ImageManagerAttributesTable) {
    this._ImageManagerAttributesTable = (function () {
...

      {
...

        {
          ...
          img.src =
            Xinha.getPluginDir("MootoolsFileManager") + "/img/locked.gif";
          img.width = 25;
          img.height = 32;
    ...
    f("f_constrain").onclick = function () {
      if (this.checked && f("f_width").value && f("f_height").value) {
        var new_dim = self.ScaleImage(self._LastImageDetails, {
          width: f("f_width").value,
          height: f("f_height").value,
        });
  ...
  if (this.current_attributes) {
    ...
    f("f_width").value = this.current_attributes.f_width
      ? this.current_attributes.f_width
      : "";
    f("f_height").value = this.current_attributes.f_height
      ? this.current_attributes.f_height
      : "";
  ...
  if (
    details.width &&
    !isNaN(parseInt(details.width)) &&
    parseInt(details.width) > 400
  ) {
    this._ImageManagerAttributesTable
      .getElement(".filemanager-img-size-notice")
      .set(
        "html",
        '<p class="tip" style="margin-top:4px;">This original image is ' +
          details.width +
          ' pixels wide, you may wish to reduce the size by putting a lesser number in the "Width" field above.  It is not usually desirable to exceed 400 pixels in width on most normal pages.</p>'
      );
  } else {
    this._ImageManagerAttributesTable
      .getElement(".filemanager-img-size-notice")
  ...
  if (
    ...
    (!details.url ||
      !this.current_image ||
      this.current_image.getAttribute("src") != details.url)
  ) {
    // Check if this is the same image
    var warn = function () {
      new FileManager.Dialog(
        "This image is a different size, would you like to use the new size?",
        {
          language: {
          ...
          onConfirm: function () {
            if (f("f_constrain").checked) {
              var new_size = self.ScaleImage(details, {
                width: f("f_width").value,
                height: f("f_height").value,
              });
    ...
    if (!details.path) warn();
      ...
      if (!src.match(/^(([a-z]+:)|\/)/i)) {
        ...
        if (
          src.match(/^[a-z]+:/i) &&
          !self.phpcfg.images_url.match(/^[a-z]:/i)
        ) {
          src = src.replace(/^[a-z]+:(\/\/?)[^/]*/i, "");
        }
      }
...
MootoolsFileManager.prototype.ImageManagerReturn = function (path, file) {
  ...
  if (!img) {
    if (0 && Xinha.is_ie) {
      // Maybe IE7 Needs this, 9 seems to be ok, I think 8 too
  ...
  for (field in param) {
    ...
    switch (field) {
      ...
      case "f_border":
        if (value.length) {
          img.style.borderWidth = /[^0-9]/.test(value)
            ? value
            : parseInt(value) + "px";
      ...
      case "f_padding":
        {
          if (value.length) {
            img.style.padding = /[^0-9]/.test(value)
              ? value
              : parseInt(value) + "px";
          } else {
            img.style.padding = "";
      ...
      case "f_margin":
        {
          if (value && value.length) {
            img.style.margin = /[^0-9]/.test(value)
              ? value
              : parseInt(value) + "px";
          } else {
            img.style.margin = "";
