...
Fx.ProgressBar = new Class({
  ...
  start: function (to, total) {
    return this.parent(
      this.now,
      arguments.length == 1 || !total ? to.limit(0, 100) : (to / total) * 100
    );
  },
  ...
  set: function (to) {
    this.now = to;
    var css = this.fill
      ? (this.fill / -2 + (to / 100) * (this.element.width || 1) || 0).round() +
        "px"
      : 100 - to + "%";

    this.element.setStyle("backgroundPosition", css + " 0px").title =
      Math.round(to) + "%";
