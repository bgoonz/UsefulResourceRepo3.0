...
FileManager.implement({
  ...
  hideUpload: function () {
    ...
    if (this.upload.uploadButton.label) {
      this.upload.uploadButton.label
        .fade(0)
        .get("tween")
        .chain(function () {
  ...
  startUpload: function () {
    ...
    this.upload = {
      ...
      uploadButton: this.addMenuButton("upload")
    ...
    if (this.upload.can_support_xhr) {
      ...
      this.upload.uploadButton.addEvent("click", function (e) {
        ...
        self.upload.form
          .getElement("input[type=file]")
          .addEvent("change", self.doUpload.bind(self));
    ...
    if (this.options.resizeImages) {
      ...
      check();
      this.upload.uploadButton.label = new Element("label", {
        class: "filemanager-resize",
      })
        .adopt(
          this.upload.resizer,
  ...
  doUpload: function () {
    if (this.upload.form.getElement("input[type=file]").files.length == 0)
      return;
      ...
      Object.merge(
        ...
        this.options.uploadAuthData || {},
        {
          ...
          resize: this.options.resizeImages
            ? this.upload.resizer.hasClass("checkboxChecked")
            : false,
    ...
    var doUploadNextFile = function () {
      if (i <= files.length - 1) {
        ...
        Object.each(
          Object.merge(tx_cfg.data, self.options.propagateData),
          function (v, k) {
            upload.append(k, v);
          }
        );
  ...
  doUploadFallback: function () {
    ...
    this.upload.dummyframe = new IFrame()
      .set({ src: "about:blank", name: "dummyframe" })
    ...
    this.upload.dummyframe.addEvent("load", function () {
      var iframe = this;
      self.diag.log(
        "NoFlash upload response: ",
        this,
        ", iframe: ",
        self.upload.dummyframe,
        ", ready:",
        1 * self.upload.dummyframe_active
      );

      // make sure we don't act on premature firing of the event in MSIE browsers:
      if (!self.upload.dummyframe_active) return;
      ...
      Function.attempt(
        ...
        function () {
          ...
          //debugger;
          response =
            '{status: 0, error: "noFlashUpload: document innerText grab FAIL: Can\'t find response."}';
        }
      );
    ...
    self.upload.form.getElements("input[type=hidden]").each(function (e) {
      e.destroy();
    });
...
FileManager.UploadListEntry = new Class({
  ...
  highlightAndClear: function () {
    ...
    self.ui.element
      ...
      .highlight(null, self.valid ? "#e6efc2" : "#f0c2c2")
      .get("tween")
      .chain(function () {
        self.ui.element.style.backgroundColor = self.valid
          ? "#e6efc2"
          : "#f0c2c2";
      });
  ...
  render: function () {
    ...
    this.ui.icon = new Asset.image(
      this.base.assetBasePath +
        "Images/Icons/" +
        this.file.name.replace(/.*\./, "").toLowerCase() +
        ".png",
      {
        class: "icon",
        onerror: function () {
          new Asset.image(
            self.base.assetBasePath + "Images/Icons/default.png"
          ).replaces(this);
        },
      }
    );
    ...
    this.ui.size = new Element("span", {
      class: "file-size",
      text: this.formatUnit(this.file.size, "b"),
    });

    this.ui.cancel = new Asset.image(
      this.base.assetBasePath + "Images/cancel.png",
      { class: "file-cancel", title: this.base.language.cancel }
    ...
    var progress = new Element("img", {
      class: "file-progress",
      src: this.base.assetBasePath + "Images/bar.gif",
    });
    ...
    this.ui.element
      ...
      .inject(this.base.upload.list)
      .highlight();

    this.ui.progress = progress;

    // Initialise the progress position to zero
    this.ui.progress.setStyle(
      "background-position-x",
      Math.floor(100 - ((0 / 100) * 20 + 40)) + "%"
    );
  },
