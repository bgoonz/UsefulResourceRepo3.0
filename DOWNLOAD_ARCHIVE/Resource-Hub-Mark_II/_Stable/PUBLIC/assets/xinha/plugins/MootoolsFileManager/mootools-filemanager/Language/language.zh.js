...
FileManager.Language.zh = {
  ...
  uploader: {
    unknown: "未知错误",
    sizeLimitMin:
      '添加 "<em>${name}</em>" (${size})失败, 文件不足<strong>${size_min}</strong>!',
    sizeLimitMax:
      '添加 "<em>${name}</em>" (${size})失败, 文件超过<strong>${size_max}</strong>!',
    mod_security:
      "上传器没有反应，很可能是因为“mod_security”是开启的同时它的其中一个规则取消了上传请求。如果您不能解除“mod_security”，请使用非flash上传器。",
  },
  ...
  flash: {
    ...
    flash:
      '上传前需安装好<a href="http://www.adobe.com/shockwave/download/download.cgi?P1_Prod_Version=ShockwaveFlash">Adobe Flash</a>.',
  },
