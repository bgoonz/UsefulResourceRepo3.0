function InsertAnchor(editor) {
  ...
  this.placeholderImg =
    '<img class="IA_placeholder" src="' +
    Xinha.getPluginDir("InsertAnchor") +
    '/img/insert-anchor.gif" />';
...
InsertAnchor._pluginInfo = {
  name: "InsertAnchor",
  origin:
    "version: 1.0, by Andre Rabold, MR Printware GmbH, http://www.mr-printware.de",
  version: "2.0",
...
InsertAnchor.prototype.onGenerate = function () {
  this.editor.addEditorStylesheet(
    Xinha.getPluginDir("InsertAnchor") + "/insert-anchor.css"
  );
};
...
InsertAnchor.prototype.inwardHtml = function (html) {
  html = html.replace(
    /(<a[^>]*class="anchor"[^>]*>)/g,
    "$1" + this.placeholderImg
  );
  return html;
};
InsertAnchor.prototype.outwardHtml = function (html) {
  html = html.replace(/(<img[^>]*class="?IA_placeholder"?[^>]*>)/gi, "");
  return html;
};
...
InsertAnchor.prototype._prepareDialog = function () {
  ...
  if (!this.html) {
    Xinha._getback(
      Xinha.getPluginDir("InsertAnchor") + "/dialog.html",
      function (getback) {
        self.html = getback;
        self._prepareDialog();
      }
  ...
  this.dialog = new Xinha.Dialog(editor, this.html, "InsertAnchor", {
    width: 400,
  });
