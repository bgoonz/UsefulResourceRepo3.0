...
function TableOperations(editor) {
    ...
    for (var i = 0; i < tb_order.length; i++) {
      if (tb_order[i] == null) {
        ...
        for (var j = 0; j < TableOperations.btnList.length; j++) {
          if (
            TableOperations.btnList[j] != null &&
            TableOperations.btnList[j][0] == tb_order[i]
          ) {
  ...
  for (var i = 0; i < bl.length; ++i) {
    ...
    if (!btn) {
      ...
      cfg.registerButton(
        id,
        Xinha._lc(btn[2], "TableOperations"),
        editor.imgURL(btn[0] + ".gif", "TableOperations"),
        false,
        function (editor, id) {
...
TableOperations.prototype.getClosestMatch = function (regExpTagName) {
  ...
  */
  ...
  var currentElement = editor.activeElement(sel)
    ? editor.activeElement(sel)
    : editor.getParentElement(sel);

  if (
    typeof currentElement.tagName != "undefined" &&
    currentElement.tagName.match(regExpTagName)
  ) {
  ...
  switch (button_id) {
    ...
    case "TO-row-insert-under":
      ...
      tr.parentNode.insertBefore(
        otr,
        /under/.test(button_id) ? tr.nextSibling : tr
      );
      editor.forceRedraw();
      editor.focusEditor();
      break;
...
TableOperations.prototype.dialogMerge = function (
  merge_func,
  cell_index,
  row_index
) {
  ...
  if (!this.dialogMergeCellsHtml) {
    Xinha._getback(
      Xinha.getPluginDir("TableOperations") + "/popups/dialogMergeCells.html",
      function (getback) {
        self.dialogMergeCellsHtml = getback;
        self.dialogMerge(merge_func, cell_index, row_index);
      }
    );
    return;
  }
  ...
  if (!this.dialogMergeCells) {
    this.dialogMergeCells = new Xinha.Dialog(
      editor,
      this.dialogMergeCellsHtml,
      "TableOperations",
      { width: 400 }
    );
    this.dialogMergeCells.getElementById("cancel").onclick = function () {
      self.dialogMergeCells.hide();
    };
  }
...
TableOperations.prototype.dialogTableProperties = function () {
  ...
  if (!this.dialogTablePropertiesHtml) {
    // retrieve the raw dialog contents
    Xinha._getback(
      Xinha.getPluginDir("TableOperations") + "/popups/dialogTable.html",
      function (getback) {
        self.dialogTablePropertiesHtml = getback;
        self.dialogTableProperties();
      }
    );
    return;
  ...
  if (!this.dialogTable) {
    ...
    this.dialogTable = new Xinha.Dialog(
      editor,
      this.dialogTablePropertiesHtml,
      "TableOperations",
      { width: 440 }
    );
    this.dialogTable.getElementById("cancel").onclick = function () {
      self.dialogTable.hide();
    };
  }
    ...
    if (editor.config.TableOperations.noFrameRulesOptions) {
      ...
      function findCells(inThis) {
        for (var i = 0; i < inThis.childNodes.length; i++) {
          if (
            inThis.childNodes[i].nodeType == 1 &&
            inThis.childNodes[i].tagName.toLowerCase().match(/tbody|thead|tr/)
          ) {
            findCells(inThis.childNodes[i]);
          } else if (
            inThis.childNodes[i].nodeType == 1 &&
            inThis.childNodes[i].tagName.toLowerCase().match(/td|th/)
          ) {
...
TableOperations.prototype.dialogRowCellProperties = function (cell) {
  ...
  if (!self.dialogRowCellPropertiesHtml) {
    // retrieve the raw dialog contents
    Xinha._getback(
      Xinha.getPluginDir("TableOperations") + "/popups/dialogRowCell.html",
      function (getback) {
        self.dialogRowCellPropertiesHtml = getback;
        self.dialogRowCellProperties(cell);
      }
    );
    return;
  }
  if (!this.dialogRowCell) {
    // Now we have everything we need, so we can build the dialog.
    this.dialogRowCell = new Xinha.Dialog(
      editor,
      self.dialogRowCellPropertiesHtml,
      "TableOperations",
      { width: 440 }
    );
    this.dialogRowCell.getElementById("cancel").onclick = function () {
      self.dialogRowCell.hide();
    };
  }

  var dialog = this.dialogRowCell;
  dialog.getElementById("title").innerHTML = cell
    ? Xinha._lc("Cell Properties", "TableOperations")
    : Xinha._lc("Row Properties", "TableOperations");
  ...
  typeSelect.options[0] = new Option(
    Xinha._lc("Do Not Change", "TableOperations")
  );
  typeSelect.options[1] = new Option(
    Xinha._lc("Normal (td)", "TableOperations"),
    "td"
  );
  typeSelect.options[2] = new Option(
    Xinha._lc("Header (th)", "TableOperations"),
    "th"
  );
  ...
  function apply() {
    ...
    if (typeSelect.selectedIndex > 0) {
      if (element.tagName.toLowerCase() == "tr") {
        // Change td into th
        var toChange = element.getElementsByTagName(
          typeSelect.options[typeSelect.selectedIndex].value == "td"
            ? "th"
            : "td"
        );
        for (var i = toChange.length - 1; i >= 0; i--) {
          if (element == toChange[i].parentNode) {
            var newNode = editor.convertNode(
              toChange[i],
        ...
        if (
          ...
          Styler.element = editor.convertNode(
            element,
            typeSelect.options[typeSelect.selectedIndex].value
          );
...
TableOperations.prototype.onKeyPress = function (ev) {
  ...
  for (var i = 0; i < all.length; i++) {
    ...
    if (all[i].parentNode.parentNode != currentcell.parentNode.parentNode)
      continue;

    if (
      all[i].tagName.toLowerCase() == "td" ||
      all[i].tagName.toLowerCase() == "th"
    ) {
