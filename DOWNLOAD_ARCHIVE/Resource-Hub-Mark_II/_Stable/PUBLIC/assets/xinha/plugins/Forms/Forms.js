    ...
    if (!btn) {
      ...
      if (i < 3)
        cfg.registerButton(
          id,
          this._lc(btn[1]),
          editor.imgURL("ed_" + btn[0] + ".gif", "Forms"),
          false,
          function (editor, id) {
...
Forms._pluginInfo = {
  name: "Forms",
  origin:
    "version: 1.0, by Nelson Bright, BrightWork, Inc., http://www.brightworkweb.com",
  version: "2.0",
...
Forms.prototype.buttonPress = function (editor, button_id, node) {
  ...
  if (button_id == "form") {
      ...
      function (param) {
        if (param) {
          if (frm) {
            ...
            if (param["f_enctype"] != "")
              frm += ' enctype="' + param["f_enctype"] + '"';
            if (param["f_target"] != "")
              frm += ' target="' + param["f_target"] + '"';
            frm += ">";
    ...
    // form element (checkbox, radio, text, password, textarea, select, button, submit, reset, image, hidden)
    var tagName = "";
    ...
    editor._popupDialog(
      "plugin://Forms/" + tagName + ".html",
      function (param) {
        if (param) {
          if (param["f_cols"])
            if (
              isNaN(parseInt(param["f_cols"], 10)) ||
              parseInt(param["f_cols"], 10) <= 0
            )
              param["f_cols"] = "";
          if (param["f_rows"])
            if (
              isNaN(parseInt(param["f_rows"], 10)) ||
              parseInt(param["f_rows"], 10) <= 0
            )
              param["f_rows"] = "";
          if (param["f_size"])
            if (
              isNaN(parseInt(param["f_size"], 10)) ||
              parseInt(param["f_size"], 10) <= 0
            )
              param["f_size"] = "";
          if (param["f_maxlength"])
            if (
              isNaN(parseInt(param["f_maxLength"], 10)) ||
              parseInt(param["f_maxLength"], 10) <= 0
            )
              param["f_maxLength"] = "";
          if (node) {
            ...
            for (field in param) {
              ...
              else node.removeAttribute(field.substring(2, 20));
            }
            if (type == "textarea") {
              ...
              //input
              if (param["f_checked"] != "") node.checked = true;
              else node.checked = false;
            } else {
              if (param["f_onclick"]) {
                node.onclick = "";
                if (param["f_onclick"] != "") node.onclick = param["f_onclick"];
              }
            }
          } else {
            //create Element
            ...
            if (type == "textarea") {
              ...
              text = "<fieldset" + text + ">";
              if (param["f_legend"] != "")
                text += "<legend>" + param["f_text"] + "</legend>";
              text += "</fieldset>";
            } else {
