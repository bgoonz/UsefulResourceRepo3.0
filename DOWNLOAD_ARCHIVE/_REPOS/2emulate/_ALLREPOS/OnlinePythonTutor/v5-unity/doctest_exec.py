...
def opt_run_doctest(doctest, example_number, student_code, probe_exprs):
    def my_finalizer(input_code, output_trace):
        ret = dict(
            doctest=encode_doctest(doctest),
        ...
        json_output = json.dumps(
            ret, indent=None
        )  # use indent=None, indent=2 for debugging
        print(json_output)
    ...
    module_name = doctest.name.split(".")[
        0
    ]  # grab the module part out of the test's name
    if example_number != "all":
        try:
            ...
            print(
                json.dumps(
                    "ERROR - example_number "
                    + str(example_number)
                    + " out of bounds for test "
                    + doctest.name
                )
        ...
        opt_doctest_exec_script_str(
            example_to_run.source, custom_modules={module_name: student_code}
        )
    else:
...
if __name__ == "__main__":
    ...
    try:
        ...
        with open(fullpath, "rU") as f:  # use 'U' to work on Windows
            ...
            if not found:
                print(
                    json.dumps("ERROR - test not found: " + test_name)
                )  # print JSON to stdout
                sys.exit(1)
        ...
        print(
            json.dumps("ERROR - invalid lab pickle file: " + fullpath)
        )  # print JSON to stdout
        traceback.print_exc()
