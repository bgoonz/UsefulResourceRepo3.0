...
class watchedlist(list):
    ...
    def __getitem__(self, i):
        print("fetching A[{}]; value is {};".format(self, i, list.__getitem__(self, i)))
        self.watchlocals()
...
def watch(object, watchedspec):
    ...
    if not watchedspec:
        return object
    kind, locals = (
        watchedspec if isinstance(watchedspec, (tuple, list)) else (watchedspec, ())
    )
    if isinstance(locals, str):
        locals = locals.split()
    watched = kind(object)
