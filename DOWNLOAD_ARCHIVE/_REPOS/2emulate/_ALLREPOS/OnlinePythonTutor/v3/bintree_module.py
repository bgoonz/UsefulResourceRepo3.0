...
class TNode:
    ...
    def graphviz_render(self, ios, compress=False):
        ...
        for level in nodes_by_level:
            ...
            if len(node_ids) > 1:
                ios.write(
                    ("{rank=same %s [style=invis]}" % "->".join(node_ids)) + separator
                )
...
if __name__ == "__main__":
    ...
    r = TNode(
        ...
        left=TNode(
            "b0",
            left=TNode("c0", right=TNode("d1")),
            right=TNode("c1", left=TNode("d3"), right=TNode("d4")),
        ),
        right=TNode("b1", left=TNode("c2", left=TNode("d2"))),
    )
