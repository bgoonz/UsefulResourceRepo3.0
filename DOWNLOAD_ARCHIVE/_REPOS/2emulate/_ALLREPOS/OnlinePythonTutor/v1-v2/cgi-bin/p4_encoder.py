...
def encode(dat, ignore_id=False):
    def append_attributes(ret, new_compound_obj_ids, dict):
        ...
        for attr in user_attrs:
            foo = [
                encode_helper(attr, new_compound_obj_ids),
    ...
    def encode_helper(dat, compound_obj_ids):
        ...
        # compound type
            ...
            if typ == list:
                ...
                repr(typ)[:6] == "<class" and obj_as_string.find("object") >= 0
            ):  # is it  an instance?
...
if __name__ == "__main__":
    ...
    def test(actual, expected=0):
        ...
        if expected == actual:
            ...
            msg = "Test on line %s failed. Expected '%s', but got '%s'." % (
                linenum,
                expected,
                actual,
            )
    ...
    test(
        ...
        [
            ...
            ["object"],
            ["p_attr1", 123],
            ["p_method", ["function", addr + 1, "p_method"]],
        ],
    )
