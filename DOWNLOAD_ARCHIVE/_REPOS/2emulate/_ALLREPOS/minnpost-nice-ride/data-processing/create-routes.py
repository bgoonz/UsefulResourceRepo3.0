    ...
    """ % (
        ...
        for c, v in combinations.items():
            ...
            for transport in transport_priority:
                ...
                if v["start"] == v["end"]:
                    pp("[Circle route]    ")
                    route_this = command_circle % {
                        "path": r_path,
                        "c_path": conf_path,
                        "lat1": v["start_lat"],
                        "lon1": v["start_lon"],
                        "lat2": popular_location_lat,
                        "lon2": popular_location_lon,
                        "lat3": v["end_lat"],
                        "lon3": v["end_lon"],
                        "transport": transport,
                        "type": route_type,
                ...
                if (
                    "Routed OK" in out[1] and check_route_output == True
                ) or check_route_output == False:
                    pp("[Route found]    ")
                    ...
                    if len(line_points) == 0:
                        ...
                        db.execute(
                            ...
                            (
                                ...
                                ppygis.Point(
                                    float(v["end_lon"]), float(v["end_lat"]), srid=srid
                                ),
                                route,
                            ),
                        )
