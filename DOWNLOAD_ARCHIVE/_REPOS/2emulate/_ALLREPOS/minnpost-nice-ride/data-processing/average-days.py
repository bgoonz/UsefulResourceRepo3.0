...
while count < times:
  ...
  """,
    ...
    # Save
    db.execute(
        "INSERT INTO average_day (start_time, end_time, total, average) VALUES (%s, %s, %s, %s)",
        (interval_start, interval_end, rides, float(float(rides) / float(season_days))),
    )
    committed = conn.commit()
    pp(".")
    count += 1
...
while current_date <= max_date:
    ...
    """,
        ...
        db.execute(
            "INSERT INTO average_all_days (day, start_time, end_time, total, difference) VALUES (%s, %s, %s, %s, %s)",
            (current_date, avg[1], avg[2], rides, interval_diff),
        )
        committed = conn.commit()
