...
for y in years:
    ...
    ##########################################
    # 2011
    if (input_year == None and y == "2011") or (
        input_year != None and input_year == "2011" and y == "2011"
    ):
        ...
        reader = csv.reader(
            open(locations_file, "rU"), delimiter=",", dialect=csv.excel_tab
        )
        row_count = 0
        for row in reader:
            if row_count > 0:
                if row[0]:
                    ...
                    db.execute(
                        "INSERT INTO stations_"
                        + y
                        + " (terminal_id, common_name, station, lat, lon, install_date, location_geom) VALUES (%s, %s, %s, %s, %s, %s, %s)",
                        (
                            ct(row[0]),
                            ct(row[1]),
                            ct(row[2]),
                            ...
                            float(row[4]),
                            dt(row[5], "date"),
                            ppygis.Point(float(row[4]), float(row[3]), srid=4326),
                        ),
                    )
                    committed = conn.commit()
    ...
    """
        ...
        locations_file = os.path.join(
            path, "../data/NRMN_2011_reports/NRMN_Rentals_2011_Season.csv"
        )
        reader = csv.reader(
            open(locations_file, "rU"), delimiter=",", dialect=csv.excel_tab
        )
        ...
        for row in reader:
            if row_count > 0:
                if row[0]:
                    ...
                    # birth_date date,
                    db.execute(
                        "INSERT INTO rentals_"
                        + y
                        + " (rental_id, account_number, subscriber_id, start_date, start_station, start_terminal, end_date, end_station, end_terminal, duration_seconds, bike_id, gender, zip, subscription_type_a, subscription_type_b, birth_date) VALUES (%s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s)",
                        (
                            ct(row[0]),
                            ct(row[1]),
                            ct(row[2]),
    ...
    ##########################################
    # 2012
    elif (input_year == None and y == "2012") or (
        input_year != None and input_year == "2012" and y == "2012"
    ):
        pp("\n[%s] Importing to stations." % y)
        db.execute("TRUNCATE TABLE stations_" + y)
        committed = conn.commit()
        ...
        locations_file = os.path.join(
            path, "../data/NRMN-2012-usage/2012-station_locations .csv"
        )
        reader = csv.reader(
            open(locations_file, "rU"), delimiter=",", dialect=csv.excel_tab
        )
            ...
            if row_count > 0:
                if row[0]:
                    ...
                    db.execute(
                        "INSERT INTO stations_"
                        + y
                        + " (terminal_id, station, lat, lon, bike_docks, location_geom) VALUES (%s, %s, %s, %s, %s, %s)",
                        (
                            ct(row[0]),
                            ct(row[1]),
                            float(row[2]),
                            float(row[3]),
                            int(row[4]),
                            ppygis.Point(float(row[3]), float(row[2]), srid=4326),
                        ),
                    )
                    committed = conn.commit()
        ...
        locations_file = os.path.join(
            path, "../data/NRMN-2012-usage/2012-all-rentals.csv"
        )
        reader = csv.reader(
            open(locations_file, "rU"), delimiter=",", dialect=csv.excel_tab
        )
            ...
            if row_count > 0:
                if row[0]:
                    ...
                    db.execute(
                        "INSERT INTO rentals_"
                        + y
                        + " (rental_id, subscriber_id, start_date, start_station, start_terminal, end_date, end_station, end_terminal, duration_seconds, bike_id, zip, subscription_type_a) VALUES (%s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s)",
                        (
                            ct(row[0]),
                            ct(row[1]),
                            dt(row[2], ""),
