...
export default produce((draft, action) => {
  ...
  if (hasDomainId() === true && hasDomainValue() === false)
    draft.value[action.domainId] = initialSubState();
  ...
  switch (action.type) {
    ...
    case SET_REFERRERS_VALUE:
      draft.value[action.domainId].value =
        action.payload || initialSubState().value;
      break;
    case SET_REFERRERS_FETCHING:
      draft.value[action.domainId].fetching =
        action.payload || initialSubState().fetching;
      break;
    case SET_REFERRERS_ERROR:
      draft.value[action.domainId].error =
        action.payload || initialSubState().error;
      break;
