...
class FrontendHighlighter(PygmentsHighlighter):
    ...
    def __init__(self, frontend, lexer=None):
        ...
        self._classic_prompt_re = re.compile(
            r"^(%s)?([ \t]*>>> |^[ \t]*\.\.\. )"
            % re.escape(frontend.other_output_prefix)
        )
...
class FrontendWidget(HistoryConsoleWidget, BaseFrontendMixin):
    ...
    def copy(self):
        ...
        if self._page_control is not None and self._page_control.hasFocus():
            ...
            if text:
                ...
                cursor.setPosition(
                    cursor.block().position(), QtGui.QTextCursor.KeepAnchor
                )
                ...
                if len(remaining_lines) > 0 and remaining_lines[-1]:
                    ...
                    cursor.setPosition(
                        start_pos + length - 1, QtGui.QTextCursor.KeepAnchor
                    )
                    last_line_full = cursor.selection().toPlainText()
                    prompt_len = len(last_line_full) - len(
                        remove_prompts(last_line_full)
                    )
    ...
    def _context_menu_make(self, pos):
        ...
        for before_action in menu.actions():
            if (
                before_action.shortcut().matches(QtGui.QKeySequence.Paste)
        ...
        if self.kernel_manager:
            ...
            if self.confirm_restart:
                ...
                result = QtWidgets.QMessageBox.question(
                    self, "Restart kernel?", message, buttons
                )
