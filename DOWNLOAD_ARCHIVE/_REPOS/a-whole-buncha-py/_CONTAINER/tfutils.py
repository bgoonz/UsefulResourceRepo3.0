...
def pool_sequence_embedding(
    ...
    """
    if pool_mode == "mean":
        seq_token_embeddings_masked = sequence_token_embeddings * tf.expand_dims(
            sequence_token_masks, axis=-1
        )  # B x T x D
        seq_token_embeddings_sum = tf.reduce_sum(
            seq_token_embeddings_masked, axis=1
        )  # B x D
        sequence_lengths = tf.expand_dims(
            tf.cast(sequence_lengths, dtype=tf.float32), axis=-1
        )  # B x 1
        ...
        token_weights = tf.layers.dense(
            sequence_token_embeddings, units=1, activation=tf.sigmoid, use_bias=False
        )  # B x T x 1
