...
class PythonParser(LanguageParser):
            ...
            for n in import_from_statement.children:
                if n.type == "from":
                    ...
                    if mode == "from":
                        ...
                        if library:
                            context[
                                match_from_span(n, blob).strip().strip(",")
                            ] = library
            return context
    ...
    def get_function_metadata(function_node, blob: str) -> Dict[str, str]:
        metadata = {"identifier": "", "parameters": "", "return_statement": ""}
        is_header = False
    ...
    def is_function_empty(function_node) -> bool:
        ...
        for child in function_node.children:
            if seen_header_end and (
                child.type == "pass_statement" or child.type == "raise_statement"
            ):
                return True
    ...
    def __process_functions(
        functions: Iterable, blob: str, func_identifier_scope: Optional[str] = None
        ...
        for function_node in functions:
            ...
            if func_identifier_scope is not None:
                function_metadata["identifier"] = "{}.{}".format(
                    func_identifier_scope, function_metadata["identifier"]
                )
                if function_metadata["identifier"].startswith(
                    "__"
                ) and function_metadata["identifier"].endswith("__"):
                    continue  # Blacklist built-in functions
            docstring_node = PythonParser.__get_docstring_node(function_node)
            function_metadata["docstring"] = PythonParser.get_docstring(
                docstring_node, blob
            )
            function_metadata["docstring_summary"] = get_docstring_summary(
                function_metadata["docstring"]
            )
            function_metadata["function"] = match_from_span(function_node, blob)
            function_metadata["function_tokens"] = tokenize_code(
                function_node, blob, {docstring_node}
            )
    ...
    def get_definition(tree, blob: str) -> List[Dict[str, Any]]:
        ...
        for _class in classes:
            ...
            class_metadata["docstring"] = PythonParser.get_docstring(
                docstring_node, blob
            )
            class_metadata["docstring_summary"] = get_docstring_summary(
            ...
            definitions.extend(
                PythonParser.__process_functions(
                    functions, blob, class_metadata["identifier"]
                )
            )
