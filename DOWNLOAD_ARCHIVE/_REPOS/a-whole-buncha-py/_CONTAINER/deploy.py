...
class Context(object):
    ...
    def remote(self, cmd, *args, **kwargs):
        ...
        start = time.time()
        status = remote(
            self.env["host"], cmd, output=False, *args, **remote_kwargs
        ).values()[0]
        end = time.time()
    ...
    def local(self, cmd, *args, **kwargs):
        ...
        try:
            ...
            self._output(
                cmd_status(
                    end - start, "localhost", cmd, status, state="failed", color="red"
                ),
                logging.warning,
            )
            raise
...
def hosts(hosts, remote_limit=25, remote_kwargs=None):
    ...
    def wrapper(f):
        ...
        def inner_wrapper(*args, **kwargs):
            ...
            logging.info(
                "Finished %s (%0.3fs)" % (getattr(f, "__name__", repr(f)), end - start)
            )
