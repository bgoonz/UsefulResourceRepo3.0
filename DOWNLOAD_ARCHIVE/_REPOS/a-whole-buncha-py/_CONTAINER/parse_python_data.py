...
def parse_raw_data_into_function_list(blob, require_docstring: bool = True):
    ...
    try:
        ...
        for function_def in function_defs:
            ...
            should_use_function = not (
                re.search(r"(__.+__)|(.*test.*)|(.*Test.*)", function_name)
                or re.search(  # skip __*__ methods and test code
                    r"NotImplementedException|@abstractmethod", function_code
                )
                or len(function_code.split("\n")) <= 2
                or (  # should have more than 1 line of code (the declaration is one line)
                    len(first_docstring_line.split()) <= 2
                )
                and require_docstring
            )  # docstring should have at least 3 words.
...
def run(args):
    ...
    with Pool() as pool:
        function_data = pool.map(
            parse_raw_data_into_function_list, raw_code_data_df.content.tolist()
        )
    assert (
        len(function_data) == raw_code_data_df.shape[0]
    ...
    raw_code_data_df = (
        raw_code_data_df.set_index(["repo", "path"])["function_data"]
        .apply(pd.Series)
        .stack()
    )
    ...
    assert len(raw_code_data_df) == len(
        function_data_df
    ...
    with Pool() as pool:
        ...
        function_data_df["docstring_tokens"] = pool.map(
            tokenize_docstring_from_string, function_data_df["docstring"].tolist()
        )
    function_data_df.dropna(
        subset=["code_tokens", "comment_tokens", "docstring_tokens"], inplace=True
    )
    ...
    chunked_save_df_to_jsonl(
        df=function_data_df[cols_to_keep], output_folder=output_folder, parallel=True
    )
