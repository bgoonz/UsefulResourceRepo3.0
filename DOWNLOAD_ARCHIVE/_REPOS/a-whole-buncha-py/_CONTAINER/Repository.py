...
class Repository(github.GithubObject.CompletableGithubObject):
    ...
    def get_collaborator_permission(self, collaborator):
        ...
        if isinstance(collaborator, github.NamedUser.NamedUser):
            collaborator = collaborator._identity
        headers, data = self._requester.requestJsonAndCheck(
            "GET", f"{self.url}/collaborators/{collaborator}/permission"
        )
    ...
    def create_git_blob(self, content, encoding):
        ...
        post_parameters = {"content": content, "encoding": encoding}
        headers, data = self._requester.requestJsonAndCheck(
    ...
    def create_git_ref(self, ref, sha):
        ...
        post_parameters = {"ref": ref, "sha": sha}
        headers, data = self._requester.requestJsonAndCheck(
    ...
    def create_secret(self, secret_name, unencrypted_value):
        ...
        put_parameters = {"key_id": public_key.key_id, "encrypted_value": payload}
        status, headers, data = self._requester.requestJson(
    ...
    def create_fork(self, organization=github.GithubObject.NotSet):
        ...
        headers, data = self._requester.requestJsonAndCheck(
            "POST", f"{self.url}/forks", input=post_parameters
        )
    ...
    def get_source_import(self):
        ...
        import_header = {"Accept": Consts.mediaTypeImportPreview}
        headers, data = self._requester.requestJsonAndCheck(
            "GET", f"{self.url}/import", headers=import_header
        )
        if not data:
            return None
        else:
    ...
    def create_check_suite(self, head_sha):
        ...
        headers, data = self._requester.requestJsonAndCheck(
            "POST", f"{self.url}/check-suites", input={"head_sha": head_sha}
        )
    ...
    def get_check_suite(self, check_suite_id):
        ...
        headers, data = self._requester.requestJsonAndCheck(
            "GET", f"{self.url}/check-suites/{check_suite_id}", headers=requestHeaders
        )
        return github.CheckSuite.CheckSuite(
            self._requester, headers, data, completed=True
        )
    ...
    def create_check_run(
        ...
        post_parameters = {"name": name, "head_sha": head_sha}
        ...
        headers, data = self._requester.requestJsonAndCheck(
            "POST", f"{self.url}/check-runs", input=post_parameters
        )
