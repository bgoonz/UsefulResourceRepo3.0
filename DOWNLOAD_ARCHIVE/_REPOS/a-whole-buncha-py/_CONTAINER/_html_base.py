...
class HTMLTranslator(nodes.NodeVisitor):
    ...
    def visit_citation_reference(self, node):
        ...
        self.body.append(
            self.starttag(node, "a", "[", CLASS="citation-reference", href=href)
        )
    ...
    def visit_definition_list_item(self, node):
        ...
        node.children[0]["classes"] = node.get("classes", []) + node.children[0].get(
            "classes", []
        )
        node.children[0]["ids"] = node.get("ids", []) + node.children[0].get("ids", [])
        node.children[0]["names"] = node.get("names", []) + node.children[0].get(
            "names", []
        )
    ...
    def visit_field_name(self, node):
        self.body.append(
            self.starttag(node, "dt", "", CLASS="".join(node.parent["classes"]))
        )
    ...
    def visit_field_body(self, node):
        self.body.append(
            self.starttag(node, "dd", "", CLASS="".join(node.parent["classes"]))
        )
    ...
    def depart_label(self, node):
        ...
        if self.settings.footnote_backlinks and len(backrefs) > 1:
            backlinks = [
                '<a href="#%s">%s</a>' % (ref, i) for (i, ref) in enumerate(backrefs, 1)
            ]
            self.body.append(
