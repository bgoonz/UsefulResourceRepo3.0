...
class ConsoleWidget(
    MetaQObjectHasTraits(
        "NewBase", (LoggingConfigurable, superQ(QtWidgets.QWidget)), {}
    )
):
    ...
    execute_on_complete_input = Bool(
        True,
        config=True,
    ...
    def eventFilter(self, obj, event):
        ...
        # Make middle-click paste safe.
            ...
            getattr(event, "button", False)
            and etype == QtCore.QEvent.MouseButtonRelease
    ...
    def sizeHint(self):
        ...
        margin = (
            self._control.frameWidth() + self._control.document().documentMargin()
        ) * 2
        style = self.style()
    ...
    def print_(self, printer=None):
        ...
        if not printer:
            ...
            if (
                QtPrintSupport.QPrintDialog(printer).exec_()
    ...
    def _append_html_fetching_plain_text(self, html, before_prompt=False):
        ...
        """
        return self._append_custom(
            self._insert_html_fetching_plain_text, html, before_prompt
        )
    ...
    def _complete_with_items(self, cursor, items):
        ...
        if len(items) == 1:
            cursor.setPosition(
                self._control.textCursor().position(), QtGui.QTextCursor.KeepAnchor
            )
    ...
    def _event_filter_console_keypress(self, event):
        ...
        cmd_down = sys.platform == "darwin" and self._control_key_down(
            event.modifiers(), include_command=True
        )
        if cmd_down:
            ...
            if key == QtCore.Qt.Key_G:
                ...
                if self._in_buffer(position):
                    ...
                    if not cursor.hasSelection():
                        ...
                        cursor.movePosition(
                            QtGui.QTextCursor.NextBlock, QtGui.QTextCursor.KeepAnchor
                        )
                        cursor.movePosition(
                ...
                if self._in_buffer(position):
                    ...
                    cursor.movePosition(
                        QtGui.QTextCursor.Right, QtGui.QTextCursor.KeepAnchor, offset
                    )
                    self._kill_ring.kill_cursor(cursor)
                    self._set_cursor(cursor)
                intercepted = True
        ...
        # ------ No modifiers ---------------------------------------------------
            ...
            if key == QtCore.Qt.Key_Escape:
                ...
                if c.position() > start_pos + spaces or c.columnNumber() == len(
                    self._continuation_prompt
                ):
                ...
                # Regular forwards deletion:
                    ...
                    anchor = cursor.anchor()
                    intercepted = not self._in_buffer(anchor) or not self._in_buffer(
                        position
                    )
    ...
    def _event_filter_page_keypress(self, event):
        ...
        if ctrl_down:
            ...
            new_event = QtGui.QKeyEvent(
                QtCore.QEvent.KeyPress, QtCore.Qt.Key_PageUp, QtCore.Qt.NoModifier
            )
            QtWidgets.QApplication.instance().sendEvent(self._page_control, new_event)
            return True
    ...
    def _get_last_lines_from_list(self, text_list, num_lines):
        ...
        for text in reversed(text_list):
            text, lines_added = self._get_last_lines(
                text, lines_pending, return_count=True
            )
            ret.append(text)
    ...
    def _get_word_start_cursor(self, position):
        ...
        if position == self._prompt_pos:
            ...
            position = cursor.position()
            while position >= self._prompt_pos and is_whitespace(
            ...
            while (
                ...
                and position >= line_start_pos
                and not is_letter_or_number(document.characterAt(position))
            ):
                position -= 1
            ...
            while (
                position >= self._prompt_pos
                and position >= line_start_pos
                and is_letter_or_number(document.characterAt(position))
            ):
                position -= 1
        ...
        if position == end_pos:
            ...
            if is_whitespace(document.characterAt(position)):
                ...
                if is_indentation_whitespace:
                    ...
                    while (
                        position < end_pos
                        and position < line_end_pos
                        and is_whitespace(document.characterAt(position))
                    ):
                        position += 1
                    cursor.setPosition(position)
                    return cursor
        ...
        for _ in range(num_newlines + 1):
            ...
            if dedent:
                ...
                cur.movePosition(
                    QtGui.QTextCursor.Right,
                    QtGui.QTextCursor.KeepAnchor,
                    min(spaces, safe_step if safe_step != 0 else self._tab_width),
                )
    ...
    def _scroll_to_end(self):
        ...
        end_scroll = (
            self._control.verticalScrollBar().maximum()
    ...
    def _insert_plain_text(self, cursor, text, flush=False):
        ...
        if (
            self._executing
            and not flush
            and self._pending_text_flush_interval.isActive()
            and cursor.position() == self._get_end_pos()
        ):
            ...
            for substring in self._ansi_processor.split_string(text):
                ...
                if len(selection) == 0:
                    ...
                    if len(substring) >= len(selection):
                        ...
                        cursor.movePosition(
                            cursor.PreviousCharacter, cursor.KeepAnchor, len(old_text)
                        )
        else:
    ...
    def _set_paging(self, paging):
        ...
        if self._splitter is None:
            raise NotImplementedError(
                """can only switch if --paging=hsplit or
        ...
        if paging == "hsplit":
            ...
            raise NotImplementedError(
                """switching to 'inside' paging not
                    supported yet."""
            )
        else:
    ...
    def _readline(self, prompt="", callback=None, password=False):
        ...
        if not callback and not self.isVisible():
            ...
            raise RuntimeError(
                "Cannot synchronously read a line if the widget " "is not visible!"
            )

        self._reading = True
        if password:
            self._show_prompt(
                "Warning: QtConsole does not support password mode, "
        ...
        if callback is None:
            ...
            self._reading_callback = lambda: callback(
                self._get_input_buffer(force=True).rstrip("\n")
            )
