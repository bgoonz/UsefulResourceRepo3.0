...
class EmbedHTMLExporter(HTMLExporter):
    ...
    def replfunc(self, node):
        ...
        if url.startswith("http"):
            ...
            if b64_data is None:
                raise ValueError(
                    """Could not find attachment for image '%s'
                                    in notebook"""
                    % imgname
                )
        else:
    ...
    def from_notebook_node(self, nb, resources=None, **kw):
        ...
        output, resources = super(EmbedHTMLExporter, self).from_notebook_node(
            nb, resources
        )
        ...
        embedded_output = et.tostring(
            root.getroottree(), method="html", encoding="unicode"
        )
