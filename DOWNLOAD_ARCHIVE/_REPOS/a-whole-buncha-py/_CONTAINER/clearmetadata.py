...
class ClearMetadataPreprocessor(Preprocessor):
    ...
    clear_cell_metadata = Bool(
        True,
        help=(
            "Flag to choose if cell metadata is to be cleared "
            "in addition to notebook metadata."
        ),
    ).tag(config=True)
    clear_notebook_metadata = Bool(
        True,
        help=(
            "Flag to choose if notebook metadata is to be cleared "
            "in addition to cell metadata."
        ),
    ).tag(config=True)
