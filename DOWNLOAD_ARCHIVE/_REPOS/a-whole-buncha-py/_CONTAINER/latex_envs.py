...
def figcaption(text, label=" "):
    display(
        HTML(
            "<div class=caption><b> Caption: </b> %s</div>" % text.replace("\n", "<br>")
        )
    )
    text = text.replace("<b>", r"\textbf{").replace(
        "</b>", "}"
    )  # some replacement of HTML formatting
...
class LenvsLatexPreprocessor(Preprocessor):
    ...
    def replacement(self, match):
        ...
        if match.group(2) is not None:
            opt_parameter = match.group(2).replace("[", "!lb!").replace("]", "!rb!")
        out = (
            "!sl!begin!op!"
            + theenv
            + "!cl@"
        ...
        if theenv in self.environmentMap:
            ...
            return (
                match.group(0).replace("\\begin", "/begin").replace("\\end", "/end")
            )  # out
    ...
    def preprocess_cell(self, cell, resources, index):
        ...
        if cell.cell_type == "markdown":
            ...
            data = re.sub(
                r"\\\[([\s\S]*?)\\\]", r"$$\1$$", data
            )  # $$.$$ is converted into \[.\] #noqa
            data = re.sub(
                r"\\\(([\s\S]*?)\\\)", self.inline_math_strip_space, data
            )  # $.$ converted into \(.\) #noqa
            # data = data.replace('\n', '!nl!\n')
            ...
            for output in cell.outputs:
                if "data" in output:
                    if (
                        ...
                        and "img src" in output.data["text/html"]
                    ):
                        ...
                        "application/json" in output.data
                        and "caption" in output.data["application/json"]
                    ):  # found a json field with caption
                        json_metadata.append(output.data["application/json"])

                    if (
                        any(x in output.data for x in mime_types)
                        and len(json_metadata) > 0
                    ):
                        output.metadata.update(
                            json_metadata.pop()
                        )  # write caption to data field metadata
...
class LenvsHTMLPreprocessor(Preprocessor):
    ...
    def preprocess_cell(self, cell, resources, index):
        ...
        if cell.cell_type == "markdown":
            ...
            cell.source = re.sub(
                r"\\begin{(\w+)}([\s\S]*?)\\end{\1}", self.replacement, cell.source
            )
        return cell, resources
...
def include_userenvs_cfg():
    ...
    user_envs_file = os.path.join(
        datadir, "nbextensions", "latex_envs", "user_envs.json"
    )
    text = ""
    try:
...
class LenvsSlidesExporter(SlidesExporter):
    ...
    def from_notebook_node(self, nb, resources=None, **kw):
        ...
        self._init_preprocessors()
        nb, resources = lenvshtmlpreprocessor(nb, resources)
        output, resources = super(LenvsSlidesExporter, self).from_notebook_node(
            nb, resources, **kw
        )
...
class LenvsLatexExporter(LatexExporter):
    ...
    removeHeaders = Bool(False, help="Remove headers and footers").tag(
        config=True, alias="rh"
    )
    figcaptionProcess = Bool(True, help="Process figcaptions").tag(
        config=True, alias="fc"
    )
    tocrefRemove = Bool(True, help="Remove tocs and ref sections, + some cleaning").tag(
    ...
    def tocrefrm(self, text):
        ...
        newtext = re.sub(
            r"(?:\\[sub]?section|\\chapter){Table of Contents}([\s\S]*?)(?=(?:\\[sub]?section|\\chapter))",
            "",
            text,
            flags=re.M,
        )  # noqa
        ...
        newtext = re.sub(
            r"\\section{References}[\S\s]*?(?=(?:\\[sub]*section|\\chapter|\\end{document}|\Z))",
            "",
            newtext,
            flags=re.M,
        )  # noqa
        ...
        newtext = re.sub(
            "\\\\begin{center}\\\\rule{[\S\s]*?\\\\end{center}[\s]*?\S*\href{toc.ipynb}{Index}[\S\s ]*?.ipynb}{Next}",
            "",
            newtext,
            flags=re.M,
        )  # noqa
    ...
    def postprocess(self, nb_text):
        ...
        if self.removeHeaders:
            tex_text = re.search(
                "begin{document}([\s\S]*?)\\\\end{document}", nb_text, flags=re.M
            )  # noqa
    ...
    def from_notebook_node(self, nb, resources=None, **kw):
        ...
        output, resources = super(LenvsLatexExporter, self).from_notebook_node(
            nb, resources, **kw
        )  # noqa
