...
if datastore_pbs._CLOUD_DATASTORE_ENABLED:
    from googlecloudsdk.third_party.appengine.datastore.datastore_pbs import (
        googledatastore,
    )
...
def ValidateInteger(
  ...
  """
    ...
    if not isinstance(value, six_subset.integer_types):
        raise exception(
            "%s should be an integer; received %s (a %s)."
            % (name, value, typename(value))
        )
...
class Key(object):
  ...
  """
    ...
    """
        ...
        if parent is not None:
            ...
            if not parent.has_id_or_name():
                raise datastore_errors.BadKeyError("The parent Key is incomplete.")
    ...
    """
        ...
        if self.__reference.has_name_space():
            args.append("namespace=%r" % self.__reference.name_space().decode("utf-8"))
...
class Category(six_subset.text_type):
  ...
  """
    ...
    def ToXml(self):
        return u'<category term="%s" label=%s />' % (
            Category.TERM,
...
class Link(six_subset.text_type):
  ...
  """
    ...
    def __init__(self, link):
        ...
        if (
            not scheme
            or (scheme != "file" and not domain)
            or (scheme == "file" and not path)
        ):
...
class GeoPt(object):
  ...
  """
    ...
    def ToXml(self):
        return u"<georss:point>%s %s</georss:point>" % (
            six_subset.text_type(self.lat),
            six_subset.text_type(self.lon),
        )
  ...
  """
    ...
    def __init__(self, rating):
        ...
        if isinstance(rating, float) or isinstance(rating, complex):
            ...
            raise datastore_errors.BadValueError(
                "Expected int or long; received %s (a %s)." % (rating, typename(rating))
            )

        try:
            if (
                _PREFERRED_NUM_TYPE(rating) < Rating.MIN
    ...
    """
        ...
        if isinstance(arg, six_subset.text_type):
            if encoding is not None:
                raise TypeError(
                    "Text() with a unicode argument " "should not specify an encoding"
                )
        ...
        raise TypeError(
            "Text() argument should be str or unicode, not %s" % type(arg).__name__
        )
...
class _BaseByteType(str):
  ...
  """
    ...
    """
        ...
        raise TypeError(
            "%s() argument should be str instance, not %s"
...
def DatetimeToTimestamp(value):
  ...
  """
    ...
    return (
        _PREFERRED_NUM_TYPE(calendar.timegm(value.timetuple()) * 1000000)
        + value.microsecond
    )
...
def PropertyValueToKeyValue(prop_value):
  ...
  """
    if not isinstance(prop_value, entity_pb.PropertyValue):
        raise datastore_errors.BadArgumentError(
            "prop_value arg expected to be entity_pb.PropertyValue (%r)" % (prop_value,)
        )
    ...
    if prop_value.has_pointvalue():
        return (
            entity_pb.PropertyValue.kPointValueGroup,
            prop_value.pointvalue().x(),
            prop_value.pointvalue().y(),
        )
    ...
    if prop_value.has_uservalue():
        ...
        if uservalue.has_auth_domain():
            result.append(
                (entity_pb.PropertyValue.kUserValueauth_domain, uservalue.auth_domain())
            )
        if uservalue.has_nickname():
            result.append(
                (entity_pb.PropertyValue.kUserValuenickname, uservalue.nickname())
            )
        if uservalue.has_gaiaid():
            result.append(
                (entity_pb.PropertyValue.kUserValuegaiaid, uservalue.gaiaid())
            )
        if uservalue.has_obfuscated_gaiaid():
            result.append(
                (
                    entity_pb.PropertyValue.kUserValueobfuscated_gaiaid,
        ...
        if uservalue.has_federated_identity():
            result.append(
                (
                    entity_pb.PropertyValue.kUserValuefederated_identity,
        ...
        if uservalue.has_federated_provider():
            result.append(
                (
                    entity_pb.PropertyValue.kUserValuefederated_provider,
