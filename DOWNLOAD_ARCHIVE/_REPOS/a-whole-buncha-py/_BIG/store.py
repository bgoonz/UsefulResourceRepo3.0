...
GOOGLE_OAUTH2_PROVIDER_REVOKE_URI = "https://accounts.google.com/o/oauth2/revoke"
...
class PrintTokenAuthenticationException(Error):
    ...
    def __init__(self, message):
        super(PrintTokenAuthenticationException, self).__init__(
            textwrap.dedent(
                """\
        {message}
        Please run:
    ...
    def __init__(self, active_config_path=None):
        if active_config_path:
            if not os.path.exists(active_config_path):
                log.warning(
                    "Could not open the configuration file: [%s].", active_config_path
                )
...
class TokenRefreshError(AuthenticationException):
    ...
    def __init__(self, error, for_adc=False, should_relogin=True):
        message = "There was a problem refreshing your current auth tokens: {0}".format(
            error
        )
...
class TokenRefreshReauthError(ReauthenticationException):
    ...
    def __init__(self, error, for_adc=False):
        message = (
            "There was a problem reauthenticating while refreshing your "
            "current auth tokens: {0}"
        ).format(error)
...
class WebLoginRequiredReauthError(Error):
  ...
  """
    ...
    def __init__(self, for_adc=False):
        ...
        to complete reauthentication.""".format(
...
class InvalidCredentialFileException(Error):
    ...
    def __init__(self, f, e):
        super(InvalidCredentialFileException, self).__init__(
            "Failed to load credential file: [{f}].  {message}".format(
                f=f, message=six.text_type(e)
            )
        )
...
def _TokenExpiresWithinWindow(
    expiry_window, token_expiry_time, max_window_seconds=3600
):
  ...
  """
    try:
        ...
        if min_expiry.total_seconds > max_window_seconds:
            raise ValueError(
                "Invalid expiry window duration [{}]: "
                "Must be between 0s and 1h".format(expiry_window)
            )
    except times.Error as e:
        message = six.text_type(e).rstrip(".")
        raise ValueError(
            "Error Parsing expiry window duration "
            "[{}]: {}".format(expiry_window, message)
        )

    token_expiry_time = times.LocalizeDateTime(
        token_expiry_time, tzinfo=dateutil.tz.tzutc()
    )
...
def GetAccessTokenIfEnabled(
    account=None, scopes=None, allow_account_impersonation=True
):
    """Returns the access token of the given account or the active account.
...
def GetFreshAccessToken(
  ...
  """
    creds = LoadFreshCredential(
        account, scopes, min_expiry_duration, allow_account_impersonation, True
    )
    return _GetAccessTokenFromCreds(creds)


def GetFreshAccessTokenIfEnabled(
    account=None,
    scopes=None,
    min_expiry_duration="1h",
    allow_account_impersonation=True,
  ...
  """
    ...
    return GetFreshAccessToken(
        account, scopes, min_expiry_duration, allow_account_impersonation
    )


def LoadFreshCredential(
    account=None,
    scopes=None,
    min_expiry_duration="1h",
    allow_account_impersonation=True,
    ...
    if allow_account_impersonation and impersonate_service_account:
        ...
        if not IMPERSONATION_TOKEN_PROVIDER:
            raise AccountImpersonationError(
                ...
                "impersonation support is not available.".format(target_principal)
            )
        log.warning(
        ...
        if use_google_auth:
            ...
            cred = IMPERSONATION_TOKEN_PROVIDER.GetElevationAccessTokenGoogleAuth(
                google_auth_source_creds,
                target_principal,
                delegates,
                scopes or config.CLOUDSDK_SCOPES,
            )
        else:
...
def _LoadFromFileOverride(cred_file_override, scopes, use_google_auth):
    ...
    if not use_google_auth:
        ...
        if token_uri_override:
            ...
            if cred_type in (
                c_creds.CredentialType.SERVICE_ACCOUNT,
        ...
        from google.auth import (
            credentials as google_auth_creds,
        ...
        if (
            isinstance(cred, google_auth_external_account.Credentials)
            and not cred.service_account_email
        ):
...
def _Refresh(
    ...
    try:
        ...
        if is_impersonated_credential:
            ...
            if not IMPERSONATION_TOKEN_PROVIDER.IsImpersonationCredential(credentials):
                raise AccountImpersonationError(
...
def Revoke(account=None):
  ...
  """
    ...
    credentials = Load(account, prevent_refresh=True, use_google_auth=True)
    if not credentials:
        raise NoCredentialsForAccountException(account)
    ...
    try:
        ...
        if not account.endswith(".gserviceaccount.com") and not isinstance(
            credentials, google_auth_external_account.Credentials
        ):
...
class _LegacyGenerator(object):
  ...
  """
    ...
    def Clean(self):
        ...
        paths = [self._bq_path, self._gsutil_path, self._p12_key_path, self._adc_path]
        for p in paths:
            try:
                os.remove(p)
            except OSError:
    ...
    def WriteTemplate(self):
        ...
        if (
            self._cred_type == c_creds.EXTERNAL_ACCOUNT_CREDS_NAME
            or self._cred_type == c_creds.EXTERNAL_ACCOUNT_USER_CREDS_NAME
        ):
            return
        ...
        if self._cred_type == c_creds.P12_SERVICE_ACCOUNT_CREDS_NAME:
            ...
            self._WriteFileContents(
                ...
                "\n".join(
                    ...
                    account=self.credentials.service_account_email,
        ...
        if self._cred_type == c_creds.USER_ACCOUNT_CREDS_NAME:
            ...
            self._WriteFileContents(
                self._gsutil_path,
                "\n".join(["[Credentials]", "gs_service_key_file = {key_file}"]).format(
                    key_file=self._adc_path
                ),
            )
