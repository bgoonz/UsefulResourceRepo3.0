    ...
    def __init__(self, code=None, malformed=False, extra_repo=None):
        ...
        if extra_repo:
            msg += (
                ...
                "  $ gcloud components repositories remove {0}".format(extra_repo)
            )
  ...
  """
            ...
            if url:
                ...
                for c in sdk_def.components:
                    if not c.data or not c.data.source:
                        continue
                    c.data.source = ComponentSnapshot._GetAbsoluteURL(
                        url, c.data.source
                    )
    ...
    def __init__(self, sdk_definition):
        ...
        for comp, dep_ids in six.iteritems(deps):
            self.__dependencies[comp] = set(
                dep_id for dep_id in dep_ids if dep_id in deps
            )
    ...
    """
        return set(
            c_id
            for c_id, component in six.iteritems(self.components)
    ...
    """
        return ComponentSnapshotDiff(
            self, latest_snapshot, platform_filter=platform_filter
        )

    def CreateComponentInfos(self, platform_filter=None):
        all_components = self.AllComponentIdsMatching(platform_filter)
        infos = [
            ComponentInfo(component_id, self, platform_filter=platform_filter)
    ...
    """
        ...
        if component_id:
            component_dict = [
                c for c in sdk_def_dict["components"] if c["id"] == component_id
            ]
        ...
        files.WriteFileContents(
            path,
            json.dumps(sdk_def_dict, indent=2, sort_keys=True, separators=(",", ": ")),
        )
    ...
    """
        ...
        for c_id in component_ids:
            if (
                c_id in self.components
                and not self.components[c_id].platform.architectures
                and not self.components[c_id].platform.operating_systems
                and self.components[c_id].platform_required
            ):
...
class ComponentSnapshotDiff(object):
  ...
  """
    ...
    """
        ...
        self.__all_components = current.AllComponentIdsMatching(
            platform_filter
        ...
        if self.__enable_fallback:
            ...
            self.__all_darwin_x86_64_components = current.AllComponentIdsMatching(
                self.DARWIN_X86_64
            ...
            self.__diffs = [
                ComponentDiff(
                    component_id, current, latest, platform_filter=platform_filter
                )
        ...
        self.__removed_components = set(
            diff.id for diff in self.__diffs if diff.state is ComponentState.REMOVED
        )
        self.__new_components = set(
            diff.id for diff in self.__diffs if diff.state is ComponentState.NEW
        )
    ...
    """
        ...
        if missing_platform:
            log.warning(
                "The platform specific binary does not exist for components [{}].".format(
    ...
    def FilterDuplicatesArm(self, component_ids):
        ...
        return set(
            ...
            if not (
                "darwin-x86_64" in i and i.replace("x86_64", "arm") in component_ids
            )
        )
    ...
    """
        ...
        if self._EnableFallback():
            ...
            if x86_removal_seed:
                log.warning(
                    "The ARM versions of the following components are "
                    "available, replacing installed x86_64 versions: [{}].".format(
                        ", ".join(x86_removal_seed)
                    )
                )
            ...
            # arm versions
            return (
                self.__removed_components | self.__updated_components | x86_removal_seed
            ) & removal_candidates
        else:
            connected = self.current.ConnectedComponents(
                update_seed, platform_filter=self.__platform_filter
            ...
            # updated
            return (
                self.__removed_components | self.__updated_components
            ) & removal_candidates
    ...
    """
        ...
        if self._EnableFallback():
            ...
            if darwin_x86_64:
                log.warning(
                    ...
                    "available yet, using x86_64 versions instead: [{}].".format(
                        ", ".join(
                            [c_id for c_id in darwin_x86_64 if "darwin" not in c_id]
                        )
                    )
                )
            ...
            remote_connected = self.latest.ConnectedComponents(
                local_connected | valid_seed, platform_filter=self.__platform_filter
            )
            remote_connected |= self.latest.ConnectedComponents(
                local_connected_darwin_x86_64 | valid_seed,
                platform_filter=self.__platform_filter,
        ...
        # don't have it
        return set(
            c for c in all_required if c in different or c not in installed_components
        )
    ...
    """
        return sorted(
            self.current.ComponentsFromIds(component_ids),
    ...
    """
        return sorted(
            self.latest.ComponentsFromIds(component_ids),
            key=lambda c: c.details.display_name,
        )
...
class ComponentInfo(object):
  ...
  """
    ...
    def __str__(self):
        return "{name} ({id})\t[{current_version}]".format(
            name=self.name, id=self.id, current_version=self.current_version_string
        )
...
class ComponentDiff(object):
  ...
  """
    ...
    def __init__(
        self, component_id, current_snapshot, latest_snapshot, platform_filter=None
    ):
        """Create a new diff.
    ...
    """
        ...
        self.current_version_string = (
            self.__current.version.version_string if self.__current else None
        )
        self.latest_version_string = (
            self.__latest.version.version_string if self.__latest else None
        )
    ...
    def _ComputeState(self):
        ...
        # same.
            ...
            # Both have data, check to see if they are the same.
                ...
                self.__latest.data.contents_checksum
    ...
    def __str__(self):
        return "[ {status} ]\t{name} ({id})\t[{current_version}]\t[{latest_version}]".format(
