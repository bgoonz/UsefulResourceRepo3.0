...
_UUID_FILE_PATH = os.path.expanduser(os.path.join("~", ".gsutil/analytics-uuid"))
...
class MetricsCollector(object):
  ...
  """
    ...
    """
        ...
        for section, small_int_category in (
            ...
            GetAndValidateConfigValue(
                ...
                and int(val) < small_int_threshold,
            )
        ...
        GetAndValidateConfigValue(
            ...
            validation_fn=lambda val: val
            in ("if_fast_else_fail", "if_fast_else_skip", "always", "never"),
        )
        ...
        return ",".join(
            sorted(["{0}:{1}".format(config[0], config[1]) for config in config_values])
        )
    ...
    @classmethod
    def StartTestCollector(
        cls, endpoint="https://example.com", user_agent="user-agent-007", ga_params=None
    ):
    ...
    def _CollectCommandAndErrorMetrics(self):
        ...
        if command_name:
            self.CollectGAMetric(
                ...
                action=command_name,
                **{
                    _GA_LABEL_MAP["Retryable Errors"]: sum(
                        self.retryable_errors.values()
                    )
                }
            )
    ...
    def _CollectPerformanceSummaryMetric(self):
        ...
        for attr_name, label in (
            ...
            custom_params[_GA_LABEL_MAP[label]] = getattr(
                self.perf_sum_params, attr_name
            )
        ...
        if system_util.IS_LINUX:
            ...
            custom_params[_GA_LABEL_MAP["Disk I/O Time"]] = sum(
                [stat[4] + stat[5] for stat in disk_end.values()]
        ...
        if total_time:
            custom_params[_GA_LABEL_MAP["Thread Idle Time Percent"]] = float(
                self.perf_sum_params.thread_idle_time
            ) / float(total_time)
        ...
        transfer_types = {
            "CloudToCloud": self.perf_sum_params.has_cloud_src
            and self.perf_sum_params.has_cloud_dst,
            "CloudToFile": self.perf_sum_params.has_cloud_src
            and self.perf_sum_params.has_file_dst,
            "DaisyChain": self.perf_sum_params.is_daisy_chain,
            "FileToCloud": self.perf_sum_params.has_file_src
            and self.perf_sum_params.has_cloud_dst,
            "FileToFile": self.perf_sum_params.has_file_src
            and self.perf_sum_params.has_file_dst,
        }
        action = ",".join(
            sorted(
    ...
    """
        ...
        try:
            ...
            # to be True.
            p = subprocess.Popen(
                execution_args, env=sm_env, shell=(six.PY3 and system_util.IS_WINDOWS)
            )
  ...
  """
    ...
    def Wrapper(*args, **kwds):
        try:
            ...
            logger.debug(
                "Exception captured in %s during metrics collection: %s",
                func.__name__,
                e,
            )
...
def CheckAndMaybePromptForAnalyticsEnabling():
  ...
  """
    ...
    if (
        not os.path.exists(_UUID_FILE_PATH)
        and not disable_prompt
        and not system_util.InvokedViaCloudSdk()
    ):
