...
class Document(object):
    ...
    def get_word_before_cursor(self, WORD=False):
        ...
        if self.text_before_cursor[-1:].isspace():
            ...
            return self.text_before_cursor[
                self.find_start_of_previous_word(WORD=WORD) :
            ]
    ...
    def find_next_word_ending(
        self, include_current_position=False, count=1, WORD=False
    ):
        """
    ...
    def selection_range_at_line(self, row):
        ...
        if self.selection:
            ...
            row_end = self.translate_row_col_to_index(
                row, max(0, len(self.lines[row]) - 1)
            )
