...
def _IsUserAccountCredentialsGoogleAuth(creds):
    ...
    if c_devshell.IsDevshellEnvironment():
        return (
            CredentialTypeGoogleAuth.FromCredentials(creds)
            == CredentialTypeGoogleAuth.GCE
        )
    else:
        return False
...
def IsOauth2clientP12AccountCredentials(creds):
    return CredentialType.FromCredentials(creds) == CredentialType.P12_SERVICE_ACCOUNT
...
class SqliteCredentialStore(CredentialStore):
    ...
    def __init__(self, store_file):
        ...
        self._Execute(
            ...
            "(account_id TEXT PRIMARY KEY, value BLOB)".format(_CREDENTIAL_TABLE_NAME)
        )
...
class AccessTokenCache(object):
    ...
    def __init__(self, store_file):
        ...
        try:
            ...
            self._Execute(
                'ALTER TABLE "{}" ADD COLUMN id_token TEXT'.format(_ACCESS_TOKEN_TABLE)
            )
...
class AccessTokenStoreGoogleAuth(object):
  ...
  """
    ...
    def Put(self):
        ...
        self._access_token_cache.Store(
            self._account_id, self._credentials.token, expiry, rapt_token, id_token
        )
  ...
  """
    ...
    if not account_id:
        account_id = hashlib.sha256(
            six.ensure_binary(credentials.refresh_token)
        ).hexdigest()
...
def MaybeAttachAccessTokenCacheStoreGoogleAuth(credentials, access_token_file=None):
  ...
  """
    ...
    if not account_id and isinstance(
  ...
  """
    ...
    """
        ...
        if IsOauth2ClientCredentials(credentials):
            ...
            store = AccessTokenStoreGoogleAuth(
                self._access_token_cache, account_id, credentials
            )
            credentials = store.Get()
...
class CredentialTypeGoogleAuth(enum.Enum):
    ...
    def FromCredentials(creds):
        ...
        if (
            isinstance(creds, google_auth_external_account.Credentials)
            and not creds.is_user
        ):
...
def ToJson(credentials):
    ...
    if creds_type == CredentialType.USER_ACCOUNT:
        ...
        creds_dict = {
            ...
            # text object.
            "private_key": (
                base64.b64encode(credentials._private_key_pkcs12).decode("ascii")
            ),
...
def ToJsonGoogleAuth(credentials):
    ...
    if creds_type == CredentialTypeGoogleAuth.SERVICE_ACCOUNT:
        ...
        creds_type == CredentialTypeGoogleAuth.EXTERNAL_ACCOUNT
        or creds_type == CredentialTypeGoogleAuth.EXTERNAL_ACCOUNT_USER
    ):
...
def ToDictGoogleAuth(credentials):
  ...
  """
    ...
    filtered_list = [
        ...
        and attr not in ["signer", "_abc_negative_cache_version"]
    ]
    ...
    for attr in attr_list:
        if hasattr(credentials, attr):
            ...
            # Allow only primitive types and list/dict/tuple for json dump input.
                ...
                and val_type not in (int, float, bool, str, list, dict, tuple)
            ):
                continue
            creds_dict[attr] = val
...
def FromJsonGoogleAuth(json_value):
  ...
  """
    ...
    if cred_type == CredentialTypeGoogleAuth.USER_ACCOUNT:
        ...
        # global once google-auth is ready to replace oauth2client.
        # pylint: disable=g-import-not-at-top
        from googlecloudsdk.core.credentials import (
            google_auth_credentials as c_google_auth,
        )
...
class ADC(object):
    ...
    def is_user(self):
        return (
            IsUserAccountCredentials(self._credentials)
            and self._impersonated_service_account is None
        )

    @property
    def adc(self):
        """Json representation of the credentials for ADC."""
        return _ConvertCredentialsToADC(
            self._credentials, self._impersonated_service_account, self._delegates
        )
...
def _ConvertOauth2ClientCredentialsToADC(credentials):
    ...
    if creds_type not in (CredentialType.USER_ACCOUNT, CredentialType.SERVICE_ACCOUNT):
        raise ADCError(
            "Cannot convert credentials of type {} to application "
...
def _ConvertCredentialsToADC(credentials, impersonated_service_account, delegates):
    ...
    impersonated_creds_dict = {
        "source_credentials": creds_dict,
        "service_account_impersonation_url": IMPERSONATION_TOKEN_URL.format(
