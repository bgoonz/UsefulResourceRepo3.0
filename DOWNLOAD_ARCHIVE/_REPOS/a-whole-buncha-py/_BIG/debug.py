...
def LogQueryV2String(breakpoint, separator=" "):
  ...
  """
    query = (
        ...
        service=breakpoint.service,
        version=breakpoint.version,
        logLevel=breakpoint.logLevel or "INFO",
        sep=separator,
    )
...
class DebugObject(object):
    ...
    def __init__(
        ...
        self._debug_client = debug_client or apis.GetClientInstance(
            "clouddebugger", "v2"
        )
...
class Debugger(DebugObject):
    ...
    """
        ...
        if not debuggees:
            ...
            debuggees = [d for d in latest_debuggees if match_re.search(d.name)] + [
                d
                for d in latest_debuggees
...
class Debuggee(DebugObject):
    ...
    def __eq__(self, other):
        return isinstance(other, self.__class__) and self.target_id == other.target_id
    ...
    """
        request = self._debug_messages.ClouddebuggerDebuggerDebuggeesBreakpointsGetRequest(
    ...
    Args:
      breakpoint_id: A breakpoint ID.
    """
        request = self._debug_messages.ClouddebuggerDebuggerDebuggeesBreakpointsDeleteRequest(
            breakpointId=breakpoint_id,
            debuggeeId=self.target_id,
            clientVersion=self.CLIENT_VERSION,
        )
    ...
    """
        ...
        request = self._debug_messages.ClouddebuggerDebuggerDebuggeesBreakpointsListRequest(
            debuggeeId=self.target_id,
        ...
        if not patterns and not ids:
            return self._FilteredDictListWithInfo(
                response.breakpoints, restrict_to_type
            )
        ...
        if full_details:
            ...
            def IsCompletedSnapshot(bp):
                return (
                    (
                        not bp.action
                        or bp.action == self.BreakpointAction(self.SNAPSHOT_TYPE)
                    )
                    and bp.isFinalState
                    and not (bp.status and bp.status.isError)
                )
    ...
    def CreateSnapshot(
        self, location, condition=None, expressions=None, user_email=None, labels=None
    ):
    ...
    """
        ...
        if not expressions:
            expressions = []
        request = self._debug_messages.ClouddebuggerDebuggerDebuggeesBreakpointsSetRequest(
            debuggeeId=self.target_id,
            breakpoint=self._debug_messages.Breakpoint(
                ...
                action=(self._debug_messages.Breakpoint.ActionValueValuesEnum.CAPTURE),
            ),
            clientVersion=self.CLIENT_VERSION,
        )
        try:
    ...
    """
        ...
        location = self._LocationFromString(location)
        if log_level:
            log_level = self._debug_messages.Breakpoint.LogLevelValueValuesEnum(
                log_level.upper()
            )
        ...
        request = self._debug_messages.ClouddebuggerDebuggerDebuggeesBreakpointsSetRequest(
            ...
            breakpoint=self._debug_messages.Breakpoint(
                ...
                labels=labels_value,
                userEmail=user_email,
                action=(self._debug_messages.Breakpoint.ActionValueValuesEnum.LOG),
            ),
            clientVersion=self.CLIENT_VERSION,
        )
        try:
    ...
    def WaitForBreakpointSet(
        self, breakpoint_id, original_location, timeout=None, retry_ms=500
    ):
    ...
    """
        ...
        def MovedOrFinal(r):
            return r.breakpoint.isFinalState or (
                original_location
                and original_location != _FormatLocation(r.breakpoint.location)
            )
    ...
    def WaitForBreakpoint(
        self, breakpoint_id, timeout=None, retry_ms=500, completion_test=None
    ):
    ...
    """
        ...
        try:
            result = retryer.RetryOnResult(
                self._CallGet, [request], should_retry_if=retry_if, sleep_ms=retry_ms
            )
    ...
    """
        ...
        if (
            message.action == self._debug_messages.Breakpoint.ActionValueValuesEnum.LOG
            and not message.logLevel
        ):
            result[
                "logLevel"
        ...
        if message.logMessageFormat:
            result["logMessageFormat"] = MergeLogExpressions(
                message.logMessageFormat, message.expressions
            )
    ...
    """
        ...
        try:
            return self._debug_messages.SourceLocation(
                path=components[0], line=int(components[1])
            )
        except ValueError:
            raise errors.InvalidLocationException(
  ...
  """
    ...
    def _WaitForOne(self, debuggee, breakpoint_id):
        try:
            breakpoint = debuggee.WaitForBreakpoint(
                breakpoint_id, timeout=self._timeout, completion_test=self._IsComplete
            )
            if not breakpoint:
    ...
    def AddTarget(self, debuggee, breakpoint_id):
        self._threads.append(
            threading.Thread(target=self._WaitForOne, args=(debuggee, breakpoint_id))
        )
  ...
  """
    ...
    def __getattr__(self, attr):
        ...
        raise AttributeError(
            'Type "{0}" does not have attribute "{1}"'.format(self._orig_type, attr)
        )
