    ...
    [
        Metric(
            ...
            "{0}&cm2=3&ea=cmd1+action1&ec={1}&el={2}&ev=0".format(
                GLOBAL_DIMENSIONS_URL_PARAMS, metrics._GA_COMMANDS_CATEGORY, VERSION
            ),
            "user-agent-007",
        ),
        Metric(
            ...
            "{0}&cm2=2&ea=Exception&ec={1}&el={2}&ev=0".format(
                GLOBAL_DIMENSIONS_URL_PARAMS, metrics._GA_ERRORRETRY_CATEGORY, VERSION
            ),
            "user-agent-007",
        ),
        Metric(
        ...
        Metric(
            ...
            "{0}&ea=CommandException&ec={1}&el={2}&ev=0".format(
                GLOBAL_DIMENSIONS_URL_PARAMS, metrics._GA_ERRORFATAL_CATEGORY, VERSION
            ),
            "user-agent-007",
        ),
    ]
...
class TestMetricsUnitTests(testcase.GsUtilUnitTestCase):
    ...
    def setUp(self):
        ...
        MetricsCollector.StartTestCollector(
            "https://example.com", "user-agent-007", {"a": "b", "c": "d"}
        )
    ...
    def testDisabling(self):
        ...
        with mock.patch.dict(
            os.environ, values={"CLOUDSDK_WRAPPER": "1", "GA_CID": "555"}
        ):
        ...
        # there.
        with mock.patch.dict(
            os.environ, values={"CLOUDSDK_WRAPPER": "1", "GA_CID": ""}
        ):
            MetricsCollector._CheckAndSetDisabledCache()
    ...
    def testPerformanceSummaryEventCollection(self):
        ...
        self.assertEqual(
            self.collector.perf_sum_params.thread_throughputs[(1, 1)].GetThroughput(),
            10,
        )
        ...
        for label, exp_value in label_and_value_pairs:
            self.assertIn(
                "{0}={1}".format(metrics._GA_LABEL_MAP[label], exp_value), metric_body
            )
    ...
    def testCommandCollection(self):
        ...
        self.assertEqual(
            "ls", self.collector.ga_params.get(metrics._GA_LABEL_MAP["Command Name"])
        )
        self.assertEqual(
            "list", self.collector.ga_params.get(metrics._GA_LABEL_MAP["Command Alias"])
        )
    ...
    def testExceptionCatchingDecorator(self):
        ...
        self.assertIn(
            "Exception captured in mock_exc_fn during metrics collection",
            debug_messages[0],
        )
        ...
        debug_messages = self.log_handler.messages["debug"]
        self.assertIn(
            "Exception captured in mock_err_fn during metrics collection",
            debug_messages[0],
        )
        self.log_handler.reset()
...
class _JSONForceHTTPErrorCopyCallbackHandler(object):
    ...
    def call(self, total_bytes_transferred, unused_total_size):
        ...
        if (
            total_bytes_transferred >= self._startover_at_byte
...
class _ResumableUploadRetryHandler(object):
    ...
    def __init__(self, retry_at_byte, exception_to_raise, exc_args, num_retries=1):
    ...
    def call(self, total_bytes_transferred, unused_total_size):
        ...
        if (
            total_bytes_transferred >= self._retry_at_byte
...
class TestMetricsIntegrationTests(testcase.GsUtilIntegrationTestCase):
    ...
    def testMetricsReporting(self):
        ...
        # PYTHONPATH.
        missing_paths = set(sys.path) - set(
            args[1]["env"]["PYTHONPATH"].split(os.pathsep)
        )
    ...
    def testMetricsPosting(self):
        ...
        def CollectMetricAndSetLogLevel(log_level, log_file_path):
            ...
            self.collector.ReportMetrics(
                wait_for_report=True, log_level=log_level, log_file_path=log_file_path
            )
            self.assertEqual([], self.collector._metrics)
    ...
    def testCommandCollection(self):
        ...
        self._CheckParameterValue(
            "Event Category", metrics._GA_COMMANDS_CATEGORY, metrics_list
        )
    ...
    def testRetryableErrorMetadataCollection(self):
        ...
        with mock.patch.object(
            http_wrapper,
            "_MakeRequestNoRetry",
            side_effect=apitools_exceptions.HttpError("unused", "unused", "unused"),
        ):
            _TryExceptAndPass(
                gsutil_api.DeleteObject, bucket_uri.bucket_name, object_uri.object_name
            )
    ...
    def testRetryableErrorMediaCollection(self):
        ...
        with SetBotoConfigForTest(boto_config_for_test):
            ...
            self._CheckParameterValue(
                "Event Action", "BadStatusCodeError", metrics_list
            )
    ...
    def testFatalErrorCollection(self):
        ...
        def CheckForCommandException(log_output):
            self._CheckParameterValue(
                "Event Category", metrics._GA_ERRORFATAL_CATEGORY, log_output
            )
    ...
    """
        ...
        def _ExtractNumberMetric(param_name):
            extracted_match = re.search(
                metrics._GA_LABEL_MAP[param_name] + r"=(\d+\.?\d*)&", metrics_to_search
            )
    ...
    def testPerformanceSummaryFileToFile(self):
        ...
        with SetBotoConfigForTest(
            ...
            self._CheckParameterValue(
                "Event Category", metrics._GA_PERFSUM_CATEGORY, metrics_list
            )
            ...
            self._CheckParameterValue(
                "Size of Files/Objects Transferred", file_size, metrics_list
            )
            self._CheckParameterValue(
                "Number of Files/Objects Transferred", 1, metrics_list
            )
    ...
    def testPerformanceSummaryFileToCloud(self):
        ...
        with SetBotoConfigForTest(
            ...
            self._CheckParameterValue(
                "Provider Types", "file%2C" + bucket_uri.scheme, metrics_list
            )
            self._CheckParameterValue(
                "Size of Files/Objects Transferred", 2 * file_size, metrics_list
            )
            self._CheckParameterValue(
                "Number of Files/Objects Transferred", 2, metrics_list
            )
            (_, _, io_time) = self._GetAndCheckAllNumberMetrics(metrics_list)
            if IS_LINUX:  # io_time will be None on other platforms.
    ...
    def testPerformanceSummaryCloudToCloud(self):
        ...
        (slowest_throughput, fastest_throughput, _) = self._GetAndCheckAllNumberMetrics(
        ...
        self._CheckParameterValue(
            "Event Action", "CloudToCloud%2CDaisyChain", metrics_list
        )
