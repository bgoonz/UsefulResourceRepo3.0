    ...
    try:
        ...
        if filename.__class__ != str:
            raise AssertionError(
                "Paths passed to _NormPaths must be str. Found: %s (%s)"
        ...
        if (
            os_path_abspath is None or os_path_isabs is None or rPath is None
...
def setup_client_server_paths(paths):
    ...
    for i, (path0, path1) in enumerate(paths_from_eclipse_to_python[:]):
        ...
        paths_from_eclipse_to_python[i] = (
            _normcase_from_client(path0),
            normcase(path1),
        )
        ...
        try:
            ...
            for eclipse_prefix, server_prefix in paths_from_eclipse_to_python:
                if translated_normalized.startswith(eclipse_prefix):
                    ...
                    if DEBUG_CLIENT_SERVER_TRANSLATION:
                        pydev_log.critical(
                            "pydev debugger: sent to server: %s", translated
                        )
                    break
            else:
        ...
        try:
            ...
            for i, (eclipse_prefix, python_prefix) in enumerate(
                ...
                if translated.startswith(python_prefix):
                    if DEBUG_CLIENT_SERVER_TRANSLATION:
                        pydev_log.critical(
                            "pydev debugger: replacing to client: %s", translated
                        )
