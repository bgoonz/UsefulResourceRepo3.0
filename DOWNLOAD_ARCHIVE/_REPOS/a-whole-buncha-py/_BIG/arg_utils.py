...
class InvalidFieldPathError(Error):
    ...
    def __init__(self, field_path, message, reason):
        super(InvalidFieldPathError, self).__init__(
            "Invalid field path [{}] for message [{}]. Details: [{}]".format(
                field_path, message.__class__.__name__, reason
            )
        )
...
class ArgumentGenerationError(Error):
    ...
    def __init__(self, field_name, reason):
        super(ArgumentGenerationError, self).__init__(
            "Failed to generate argument for field [{}]: {}".format(field_name, reason)
        )
    ...
    for i, f in enumerate(fields):
        ...
        try:
            ...
            raise InvalidFieldPathError(
                field_path, root_message, UnknownFieldError(f, message)
            )
        if index_found:
        ...
        if not message and i < len(fields) - 1:
            if isinstance(field, messages.MessageField):
                ...
                raise InvalidFieldPathError(
                    field_path,
                    root_message,
                    "[{}] is not a valid field on field [{}]".format(
...
def Limit(method, namespace):
    ...
    if (
        hasattr(namespace, "limit")
        and method.IsPageableList()
        and method.ListItemField()
    ):
...
def PageSize(method, namespace):
    ...
    if (
        hasattr(namespace, "page_size")
        and method.IsPageableList()
        and method.ListItemField()
        and method.BatchPageSizeField()
    ):
...
def ParseResourceIntoMessage(
  ...
  """
    ...
    if (
        request_id_field
        and method.resource_argument_collection.detailed_params
        ...
        ref = ref.Parent(parent_collection=method.request_collection.full_name)
...
def ParseStaticFieldsIntoMessage(message, static_fields=None):
  ...
  """
    ...
    for field_path, value in six.iteritems(static_fields):
        ...
        SetFieldInMessage(message, field_path, ConvertValue(field, value))
...
def CheckValidEnumNames(api_names, choices_values):
    ...
    if api_names:
        bad_choices = [
            name
            for name in choices_values
            if not (
                name in api_names or ChoiceToEnumName(six.text_type(name)) in api_names
            )
        ]
    else:
        bad_choices = []
...
def ChoiceToEnum(choice, enum_type, item_type="choice", valid_choices=None):
    ...
    valid_choices = valid_choices or [EnumNameToChoice(n) for n in enum_type.names()]
    try:
...
def FieldHelpDocs(message, section="Fields"):
  ...
  """
    ...
    match = re.search(
        r"^\s+{}:.*$".format(section), message.__doc__ or "", re.MULTILINE
    )
    if not match:
    ...
    for field in message.all_fields():
        ...
        if field.variant == messages.Variant.MESSAGE:
            ...
            if field.variant == messages.Variant.ENUM:
                ...
                field_data["choices"] = {
                    n: enum_help.get(n) for n in field.type.names()
                }
...
def IsOutputField(help_text):
    ...
    return help_text and (
        help_text.startswith("[Output Only]") or help_text.endswith("@OutputOnly")
    )
...
class ChoiceEnumMapper(object):
  ...
  """
    ...
    _CUSTOM_MAPPING_ERROR = (
        "custom_mappings must be a dict of enum string "
    ...
    """
        ...
        if include_filter is not None and not callable(include_filter):
            raise TypeError(
                "include_filter must be callable received [{}]".format(include_filter)
            )
    ...
    """
        if self._custom_mappings:  # Process Custom Mappings
            ...
            if diff:
                raise ValueError(
                    "custom_mappings [{}] may only contain mappings"
                    " for enum values. invalid values:[{}]".format(
                        ", ".join(self._custom_mappings.keys()), ", ".join(diff)
                    )
                )
            try:
            ...
            self._choice_to_enum = {
                EnumNameToChoice(x.name): x for x in self._filtered_enum
            }
            self._enum_to_choice = {
                y.name: x for x, y in six.iteritems(self._choice_to_enum)
            }
