...
class Namespace(argparse.Namespace):
  ...
  """
    ...
    """
        ...
        return (
            self._GetCommand()
            ._cli_generator.Generate()
            .Execute(command, call_arg_complete=call_arg_complete)
        )
...
class ArgumentParser(argparse.ArgumentParser):
  ...
  """
    ...
    def _Suggest(self, unknown_args):
        ...
        self._Error(
            parser_errors.UnrecognizedArgumentsError(
    ...
    """
        ...
        namespace, unknown_args = parser.parse_known_args(args, namespace) or (
            namespace,
            [],
        )
    ...
    def validate_specified_args(
        self, ai, specified_args, namespace, is_required=True, top=True
    ):
    ...
    """
        ...
        if need_required:
            ...
            if have_optional or have_required:
                have_args = parser_arguments.Argument(
                    arguments=have_optional + have_required,
                    is_group=True,
        ...
        count = len(self.GetDestinations(have_required)) + len(
            self.GetDestinations(have_optional)
        )
            ...
            if is_required and ai.is_required:
                if count != 1:
                    if count:
                        ...
                        argument = None
                    self._Error(
                        parser_errors.RequiredMutexError(
                            parser=self, argument=argument, conflict=conflict
                        )
                    )
            elif count > 1:
                argument = usage_text.GetArgUsage(
                    sorted(have_required + have_optional, key=usage_text.GetArgSortKey)[
                        0
                    ],
                    value=False,
                    hidden=True,
    ...
    def parse_args(self, args=None, namespace=None, raise_error=False):
        ...
        if deepest_parser._remainder_action:
            ...
            # pylint:disable=protected-access
            try:
                namespace, unknown_args = deepest_parser._remainder_action.ParseRemainingArgs(
    ...
    """
        ...
        if missing_components:
            msg = (
                "You do not currently have this command group installed.  Using "
        ...
        self._Error(
            parser_errors.UnknownCommandError(
                message,
    ...
    """
        return [
            " ".join(alternate.GetPath())
            for alternate in self._CommandAlternativeReleaseTracks(value=value)
        ]

    def _ReportErrorMetricsHelper(
        self, dotted_command_path, error, error_extra_info=None
    ):
    ...
    """
        ...
        if isinstance(error, parser_errors.ArgumentError):
            if error.extra_path_arg:
                dotted_command_path = ".".join(
                    [dotted_command_path, error.extra_path_arg]
                )
            self._ReportErrorMetricsHelper(
                dotted_command_path, error.__class__, error.error_extra_info
            )
            return
        ...
        if "too few arguments" in message:
            self._ReportErrorMetricsHelper(
                dotted_command_path, parser_errors.TooFewArgumentsError
            )
            return

        # Catchall for any error we didn't explicitly detect.
        self._ReportErrorMetricsHelper(
            dotted_command_path, parser_errors.OtherParsingError
        )
    ...
    """
        if reproduce and context:
            ...
            if (
                ...
                and not isinstance(error, parser_errors.DetailedArgumentError)
                and (
                    self._probe_error
                    or "Invalid choice" in message
                    or "unknown parser" in message
                )
            ):
        ...
        if "_ARGCOMPLETE" not in os.environ:
            if re.search("too few arguments", message):
                return
            if (
                re.search("arguments? .* required", message)
                and not re.search("in dict arg but not provided", message)
                and not re.search(r"\[.*\brequired\b.*\]", message)
            ):
                return
...
# pylint:disable=protected-access
class CloudSDKSubParsersAction(
    six.with_metaclass(abc.ABCMeta, argparse._SubParsersAction)
):
    """A custom subclass for arg parsing behavior.
...
class DynamicPositionalAction(
    six.with_metaclass(abc.ABCMeta, CloudSDKSubParsersAction)
):
  ...
  """
    ...
    def _AddParser(self, choice):
        ...
        return self.add_parser(
            choice,
            add_help=False,
            prog=self._parent_ai.parser.prog,
    ...
    def __call__(self, parser, namespace, values, option_string=None):
        ...
        ai = parser_arguments.ArgumentInterceptor(
            ...
            data=self._parent_ai.data,
        )

        for flag in itertools.chain(
            self._parent_ai.flag_args, self._parent_ai.ancestor_flag_args
        ):
