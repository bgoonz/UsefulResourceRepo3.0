...
class RegionalBackendServicesCompleter(compute_completers.ListCommandCompleter):
    def __init__(self, **kwargs):
        super(RegionalBackendServicesCompleter, self).__init__(
            ...
            list_command=("compute backend-services list " "--filter=region:* --uri"),
            **kwargs
        )
...
class BackendServicesCompleter(completers.MultiResourceCompleter):
    def __init__(self, **kwargs):
        super(BackendServicesCompleter, self).__init__(
            completers=[
                RegionalBackendServicesCompleter,
...
def BackendServiceArgumentForUrlMap(
    required=True, include_l7_internal_load_balancing=False
):
    return compute_flags.ResourceArgument(
        resource_name="backend service",
...
def AddLoadBalancingScheme(
    parser, include_l7_ilb=False, include_gfe3=False, include_l7_rxlb=False
):
    parser.add_argument(
...
def HealthCheckArgument(required=False, support_regional_health_check=False):
    return compute_flags.ResourceArgument(
        ...
        if support_regional_health_check
        else None,
        short_help="""\
        ...
        if health_check_refs:
            ...
            health_check_refs.extend(
                resource_resolver.HEALTH_CHECK_ARG.ResolveAsResource(
                    args, resource_parser, default_scope=compute_scope.ScopeEnum.GLOBAL
                )
            )
...
def AddSessionAffinity(
    parser, target_pools=False, hidden=False, support_client_only=False
):
...
def AddProtocol(
  ...
  """
    ilb_protocols = (
        "TCP, UDP, UNSPECIFIED" if support_unspecified_protocol else "TCP, UDP"
    )
    td_protocols = (
        "HTTP, HTTPS, HTTP2, GRPC" if support_grpc_protocol else "HTTP, HTTPS, HTTP2"
    )
    netlb_protocols = (
        "TCP, UDP, UNSPECIFIED" if support_unspecified_protocol else "TCP, UDP"
    )
    parser.add_argument(
      ...
      HTTPS, HTTP2.
      """.format(
            ilb_protocols, td_protocols, netlb_protocols
        ),
    )
...
def AddInstanceGroupAndNetworkEndpointGroupArgs(
    parser, verb, support_global_neg=False, support_region_neg=False
):
    ...
    neg_group_arg = GetNetworkEndpointGroupArg(
        support_global_neg=support_global_neg, support_region_neg=support_region_neg
    )
