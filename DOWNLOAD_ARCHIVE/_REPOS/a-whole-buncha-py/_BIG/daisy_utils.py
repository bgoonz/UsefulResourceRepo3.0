...
_DEFAULT_BUILDER_DOCKER_PATTERN = (
    "gcr.io/compute-image-tools/{executable}:{docker_image_tag}"
)
...
def _CheckIamPermissions(
  ...
  """
        ...
        if not services_api.IsServiceEnabled(project.projectId, service_name):
            ...
            if enable_service:
                ...
                log.warning(
                    ...
                    "https://cloud.google.com/service-usage/docs/enable-disable.".format(
                        service_name
                    )
                )
    ...
    compute_account = "serviceAccount:{0}-compute@developer.gserviceaccount.com".format(
        project.projectNumber
    )
    ...
    if ROLE_EDITOR not in current_compute_account_roles:
        _VerifyRolesAndPromptIfMissing(
            project_id,
            compute_account,
            current_compute_account_roles,
            compute_service_account_roles,
        )


def _VerifyRolesAndPromptIfMissing(project_id, account, applied_roles, required_roles):
  ...
  """
    ...
    for role in sorted(required_roles):
        ...
        role_permissions = set(
            apis.GetClientInstance("iam", "v1").roles.Get(request).includedPermissions
        )
...
def _CreateCloudBuild(build_config, client, messages):
  ...
  """
    ...
    build_ref = resources.REGISTRY.Create(
        collection="cloudbuild.projects.builds", projectId=build.projectId, id=build.id
    )
...
def CreateDaisyBucketInProject(bucket_location, storage_client):
  ...
  """
    ...
    try:
        storage_client.CreateBucketIfNotExists(bucket_name, location=bucket_location)
        ...
        if not bucket_in_project_created_or_found:
            ...
            raise DaisyBucketCreationException(
                "Unable to create a temporary bucket `{0}` needed for the operation to proceed as it exists in another project.".format(
                    bucket_name
                )
            )
    return bucket_name
...
def RunImageImport(
  ...
  """
    ...
    builder = _GetBuilder(
        _IMAGE_IMPORT_BUILDER_EXECUTABLE, docker_image_tag, builder_region
    )
    return RunImageCloudBuild(
        args,
        builder,
        import_args,
        tags,
...
def _GetBuilder(executable, docker_image_tag, builder_region):
  ...
  """
    if builder_region:
        regionalized_builder = _GetRegionalizedBuilder(
            executable, builder_region, docker_image_tag
        )
...
def _GetRegionalizedBuilder(executable, region, docker_image_tag):
  ...
  """
    ...
    regionalized_builder = _REGIONALIZED_BUILDER_DOCKER_PATTERN.format(
        executable=executable, region=region, docker_image_tag=docker_image_tag
    )
...
def RunOnestepImageImport(
  ...
  """
    ...
    builder_region = _GetBuilderRegion(_GetImageImportRegion, args)
    builder = _GetBuilder(
        _IMAGE_ONESTEP_IMPORT_BUILDER_EXECUTABLE, docker_image_tag, builder_region
    )
    return RunImageCloudBuild(
        args,
        builder,
...
def RunImageExport(
  ...
  """
    ...
    builder = _GetBuilder(
        _IMAGE_EXPORT_BUILDER_EXECUTABLE, docker_image_tag, builder_region
    )
    return RunImageCloudBuild(
        args,
        builder,
...
def _RunCloudBuild(
  ...
  """
    ...
    build_config = messages.Build(
        steps=[messages.BuildStep(name=builder, args=sorted(build_args))],
        tags=build_tags,
    ...
    if log_location:
        ...
        if hasattr(gcs_log_ref, "object"):
            build_config.logsBucket = "gs://{0}/{1}".format(
                gcs_log_ref.bucket, gcs_log_ref.object
            )
        else:
    ...
    if build_region and build_region in CLOUD_BUILD_REGIONS:
        build, build_ref = _CreateRegionalCloudBuild(
            build_config, client, messages, build_region
        )
    else:
...
def RunInstanceOVFImportBuild(
  ...
  """
    ...
    builder = _GetBuilder(
        _OVF_IMPORT_BUILDER_EXECUTABLE, args.docker_image_tag, builder_region
    )
    return _RunCloudBuild(
        args,
        builder,
...
def RunMachineImageOVFImportBuild(args, output_filter, release_track):
  ...
  """
    ...
    AppendArg(
        ovf_importer_args, "machine-image-storage-location", args.storage_location
    )
    AppendArg(ovf_importer_args, "client-id", "gcloud")
    AppendArg(ovf_importer_args, "ovf-gcs-path", args.source_uri)
    AppendBoolArg(ovf_importer_args, "no-guest-environment", not args.guest_environment)
    ...
    AppendBoolArg(
        ovf_importer_args, "no-restart-on-failure", not args.restart_on_failure
    )
    ...
    if "compute_service_account" in args:
        AppendArg(
            ovf_importer_args, "compute-service-account", args.compute_service_account
        )
    ...
    try:
        bucket = storage_api.StorageClient().GetBucket(
            storage_util.ObjectReference.FromUrl(
                MakeGcsUri(gcs_path), allow_empty_object=True
            ).bucket
        )
...
def RunOsUpgradeBuild(args, output_filter, instance_uri, release_track):
  ...
  """
    ...
    AppendBoolArg(
        os_upgrade_args, "use-staging-install-media", args.use_staging_install_media
    )
    ...
    builder = _GetBuilder(
        _OS_UPGRADE_BUILDER_EXECUTABLE, args.docker_image_tag, builder_region
    )
    return _RunCloudBuild(
        args,
        builder,
...
def AddGuestEnvironmentArg(parser, resource="instance"):
    ...
    parser.add_argument(
        ...
        action="store_true",
        default=True,
        help="The guest environment will be installed on the {}.".format(resource),
    )
...
def AddAWSImageImportSourceArgs(aws_group):
    ...
    begin_from_export = step_to_begin.add_group(
        help="""\
      If importing an AMI,  specify the following two flags:"""
    )
    begin_from_export.add_argument(
        "--aws-ami-id", required=True, help="AWS AMI ID of the image to import."
    )
    begin_from_export.add_argument(
    ...
    begin_from_file = step_to_begin.add_group(
        help="""\
...
def _AppendNodeAffinityLabelArgs(ovf_importer_args, args, compute_client_messages):
...
def _BuildOvfImporterNodeAffinityFlagValue(node_affinity):
    node_affinity_flag = node_affinity.key + "," + six.text_type(node_affinity.operator)
...
def ValidateZone(args, compute_client):
  ...
  """
    ...
    requests = [
        (
            compute_client.apitools_client.zones,
            "Get",
...
def IsLocalFile(file_name):
    return not (
        file_name.lower().startswith("gs://")
        or file_name.lower().startswith("https://")
    )
