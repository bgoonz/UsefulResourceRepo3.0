...
class _StorageStreamResponseHandler(requests.ResponseHandler):
    ...
    def update_destination_info(
        self, stream, digesters=None, processed_bytes=0, progress_callback=None
    ):
        ...
        while True:
            ...
            if data:
                ...
                if (
                    ...
                    and bytes_since_last_progress_callback
...
class GcsApi(cloud_api.CloudApi):
    ...
    def create_bucket(self, bucket_resource, fields_scope=cloud_api.FieldsScope.NO_ACL):
        """See super class."""
    ...
    @_catch_http_error_raise_gcs_api_error()
    def patch_bucket(
        self, bucket_resource, request_config, fields_scope=cloud_api.FieldsScope.NO_ACL
    ):
        """See super class."""
        projection = self._get_projection(
        ...
        metadata = (
            ...
            or gcs_metadata_util.get_metadata_from_bucket_resource(bucket_resource)
        )
        ...
        if request_config.predefined_default_acl_string:
            ...
            predefined_default_acl = getattr(
                self.messages.StorageBucketsPatchRequest.PredefinedDefaultObjectAclValueValuesEnum,
    ...
    def list_buckets(self, fields_scope=cloud_api.FieldsScope.NO_ACL):
        ...
        request = self.messages.StorageBucketsListRequest(
            project=properties.VALUES.core.project.GetOrFail(), projection=projection
        )
        ...
        try:
            ...
            core_exceptions.reraise(cloud_errors.translate_error(e, _ERROR_TRANSLATION))
    ...
    def copy_object(
        ...
        if request_config.predefined_acl_string:
            predefined_acl = getattr(
                self.messages.StorageObjectsRewriteRequest.DestinationPredefinedAclValueValuesEnum,
                request_config.predefined_acl_string,
            )
        else:
            predefined_acl = None
    ...
    def _download_object_resumable(
        self,
        cloud_resource,
        download_stream,
        apitools_download,
        ...
        def _should_retry_resumable_download(exc_type, exc_value, exc_traceback, state):
            ...
            if start_byte > progress_state["last_byte_processed"]:
                ...
                state.retrial = 0
            log.debug(
                "Retrying download from byte {} after exception: {}."
    ...
    def upload_object(
        ...
        if upload_strategy == cloud_api.UploadStrategy.SIMPLE:
            upload = gcs_upload.SimpleUpload(
                self,
        ...
        try:
            ...
            raise cloud_errors.ResumableUploadAbortError(
                ...
                "deleting the tracker files present in {}".format(
                    str(error), properties.VALUES.storage.tracker_files_directory.Get()
                )
            )
    ...
    def compose_objects(self, source_resources, destination_resource, request_config):
        ...
        if request_config.predefined_acl_string is not None:
            compose_request.destinationPredefinedAcl = getattr(
                self.messages.StorageObjectsComposeRequest.DestinationPredefinedAclValueValuesEnum,
                request_config.predefined_acl_string,
            )
