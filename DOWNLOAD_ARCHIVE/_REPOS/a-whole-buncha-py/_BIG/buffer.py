...
class AcceptAction(object):
    ...
    def run_in_terminal(cls, handler, render_cli_done=False):
        ...
        def _handler(cli, buffer):
            cli.run_in_terminal(
                lambda: handler(cli, buffer), render_cli_done=render_cli_done
            )
...
class CompletionState(object):
    ...
    def __init__(
        self, original_document, current_completions=None, complete_index=None
    ):
    ...
    def go_to_index(self, index):
        ...
        return CompletionState(
            self.original_document, self.current_completions, complete_index=index
        )
...
class Buffer(object):
    ...
    def __init__(
        ...
        assert on_cursor_position_changed is None or callable(
    ...
    def set_completions(self, completions, go_to_first=True, go_to_last=False):
            ...
            if self.completer:
                completions = list(
                    self.completer.get_completions(
                        self.document, CompleteEvent(completion_requested=True)
                    )
                )
            else:
                completions = []
        ...
        if completions:
            self.complete_state = CompletionState(
                original_document=self.document, current_completions=completions
            )
            if go_to_first:
