...
""".format(
        composite_namespace=copy_helper.PARALLEL_UPLOAD_TEMP_NAMESPACE
    )
)
...
class NotificationCommand(Command):
    ...
    def _WatchBucket(self):
        ...
        if not bucket_url.IsBucket():
            raise CommandException(
                "URL must name a bucket for the %s command." % self.command_name
            )

        self.logger.info(
            "Watching bucket %s with application URL %s ...", bucket_url, watch_url
        )
    ...
    def _StopChannel(self):
        ...
        self.logger.info(
            "Removing channel %s with resource identifier %s ...",
            channel_id,
            resource_id,
        )
    ...
    def _ListChannels(self, bucket_arg):
        ...
        channels = self.gsutil_api.ListChannels(
            bucket_url.bucket_name, provider="gs"
        ).items
        self.logger.info(
    ...
    def _Create(self):
        ...
        if should_setup_topic:
            ...
            service_account = self.gsutil_api.GetProjectServiceAccount(
                bucket_project_number, provider=bucket_url.scheme
            ).email_address
            self.logger.debug(
                "Service account for project %d: %s",
    ...
    """
        ...
        if binding not in policy.bindings:
            ...
            self.logger.debug(
                "GCS already has publish permission to topic %s.", pubsub_topic
            )
            return False
            ...
            if match:
                ...
                if not found:
                    raise NotFoundException(
                        "Could not find notification %s" % list_entry
                    )
            else:
