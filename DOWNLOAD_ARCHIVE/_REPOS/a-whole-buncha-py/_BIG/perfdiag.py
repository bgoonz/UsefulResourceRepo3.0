...
FanDownloadTuple = namedtuple(
    "FanDownloadTuple", "need_to_slice object_name file_name serialization_data"
)
...
SliceDownloadTuple = namedtuple(
    "SliceDownloadTuple", "object_name file_name serialization_data start_byte end_byte"
)
...
FanUploadTuple = namedtuple(
    "FanUploadTuple", "need_to_slice file_name object_name use_file gzip_encoded"
)
...
def _DownloadObject(cls, args, thread_state=None):
  ...
  """
    ...
    if args.need_to_slice:
        cls.PerformSlicedDownload(
            args.object_name, args.file_name, args.serialization_data
        )
    else:
        cls.Download(
            args.object_name, gsutil_api, args.file_name, args.serialization_data
        )
...
def _GenerateFileData(
    fp, file_size=0, random_ratio=100, max_unique_random_bytes=5242883
):
...
class PerfDiagCommand(Command):
    ...
    """
        ...
        if raise_on_error and p.returncode:
            raise CommandException(
                "Received non-zero return code (%d) from "
    ...
    class _PerfdiagFilter(logging.Filter):
        def filter(self, record):
            ...
            return not (
                ...
                or ("Computing CRC" in msg)
                or ("gsutil -m perfdiag" in msg)
            )
    ...
    def PerformFannedDownload(
        self, need_to_slice, object_names, file_names, serialization_data
    ):
    ...
    """
        ...
        for i in range(len(object_names)):
            ...
            args.append(
                FanDownloadTuple(
                    need_to_slice, object_names[i], file_name, serialization_data[i]
                )
            )
        self.Apply(
            _DownloadObject,
    ...
    def PerformFannedUpload(
        self, need_to_slice, file_names, object_names, use_file, gzip_encoded=False
    ):
    ...
    """
        ...
        for i in range(self.num_slices):
            ...
            component_size = min(
                component_size, temp_file_dict[file_name].size - component_start
            )
            args.append(
                SliceUploadTuple(
                    file_name,
    ...
    def _RunReadThruTests(self, use_file=False):
        ...
        self.logger.info(
            ...
            % (file_io_string, self.num_objects, MakeHumanReadable(self.thru_filesize))
        )
        self._WarnIfLargeData()
        ...
        if use_file:
            ...
            serialization_data = [
                GetDownloadSerializationData(obj_metadata)
            ] * self.num_objects
        ...
        if self.processes == 1 and self.threads == 1:
            for i in range(self.num_objects):
                ...
                self.Download(
                    object_names[i], self.gsutil_api, file_name, serialization_data[i]
                )
        else:
            if self.parallel_strategy in (self.FAN, self.BOTH):
                need_to_slice = self.parallel_strategy == self.BOTH
                self.PerformFannedDownload(
                    need_to_slice, object_names, file_names, serialization_data
                )
    ...
    def _RunListTests(self):
        ...
        self.Apply(
            _UploadObject, args, _PerfdiagExceptionHandler, arg_checker=DummyArgChecker
        )
        ...
        self.Apply(
            _DeleteWrapper, args, _PerfdiagExceptionHandler, arg_checker=DummyArgChecker
        )
    ...
    """
        ...
        try:
            if use_file:
                ...
                data = temp_file_dict[file_name].data[
    ...
    """
        ...
        try:
            netstat_output = self._Exec(
                ["netstat", "-s"], return_output=True, raise_on_error=False
            )
        except OSError:
            self.logger.warning(
                "netstat not found on your system; some measurement "
        ...
        for line in netstat_output.split("\n"):
            ...
            if (
                found_tcp
                and tcp_retransmit is None
                and (
                    "segments retransmited" in line
                    or "retransmit timeouts" in line
                    or "segments retransmitted" in line
                )
            ):
            ...
            if (
                found_tcp
                and tcp_received is None
                and ("segments received" in line or "packets received" in line)
            ):
            ...
            if (
                found_tcp
                and tcp_sent is None
                and (
                    "segments send out" in line
                    or "packets sent" in line
                    or "segments sent" in line
                )
            ):
    ...
    def _CollectSysInfo(self):
        ...
        sysinfo["gmt_timestamp"] = time.strftime(
            ...
            if sysinfo.get("on_gce", False):
                ...
                try:
                    ...
                    sysinfo["gce_instance_info"] = self._Exec(
                        cmd, return_output=True, mute_stderr=mute_stderr
                    )
                except (CommandException, OSError):
        ...
        for attr in dir(config):
            ...
            if attr.isupper() and not (
                isinstance(attr_value, six.string_types) and "\n" in attr_value
            ):
    ...
    def _DisplayResults(self):
        ...
        if "write_throughput" in self.results:
            ...
            if "parallelism" in write_thru:  # Compatibility with old versions.
                text_util.print_to_fd(
                    "Parallelism strategy: %s" % write_thru["parallelism"]
                )
        ...
        if "write_throughput_file" in self.results:
            ...
            if "parallelism" in write_thru_file:  # Compatibility with old versions.
                text_util.print_to_fd(
                    "Parallelism strategy: %s" % write_thru_file["parallelism"]
                )
        ...
        if "read_throughput" in self.results:
            ...
            if "parallelism" in read_thru:  # Compatibility with old versions.
                text_util.print_to_fd(
                    "Parallelism strategy: %s" % read_thru["parallelism"]
                )
        ...
        if "read_throughput_file" in self.results:
            ...
            if "parallelism" in read_thru_file:  # Compatibility with old versions.
                text_util.print_to_fd(
                    "Parallelism strategy: %s" % read_thru_file["parallelism"]
                )
        ...
        if "listing" in self.results:
            ...
            text_util.print_to_fd(
                "  Number of listing calls made: %s" % insert["num_listing_calls"]
            )
            text_util.print_to_fd(
                (
            ...
            text_util.print_to_fd(
                "  Number of listing calls made: %s" % delete["num_listing_calls"]
            )
            text_util.print_to_fd(
                (
                    "  Individual listing call latencies: [%s]"
        ...
        if "request_errors" in self.results and "total_requests" in self.results:
            ...
            availability = (
                ((total - numerrors) / float(total)) * 100 if total > 0 else 100
            )
    ...
    def _ParseArgs(self):
            ...
            for o, a in self.sub_opts:
                ...
                if o == "-t":
                    ...
                    for test_name in a.strip().split(","):
                        if test_name.lower() not in self.ALL_DIAG_TESTS:
                            raise CommandException(
                                "List of test names (-t) contains invalid "
                ...
                if o == "-i":
                    ...
                    try:
                        ...
                        raise CommandException(
                            "Could not decode input file (-i): '%s'." % a
                        )
                    return
                if o == "-j":
                    ...
                    if (
                        self.gzip_compression_ratio < 0
                        or self.gzip_compression_ratio > 100
                    ):
                        raise CommandException(
        ...
        if not self.bucket_url.IsCloudUrl() and self.bucket_url.IsBucket():
            raise CommandException(
                "The perfdiag command requires a URL that "
                'specifies a bucket.\n"%s" is not '
                "valid." % self.args[0]
            )
        ...
        slice_not_available = self.provider == "s3" and self.diag_tests.intersection(
            self.WTHRU, self.WTHRU_FILE
        )
        ...
        self.gsutil_api.GetBucket(
            self.bucket_url.bucket_name, provider=self.bucket_url.scheme, fields=["id"]
        )
        self.exceptions = [
...
def StorageUrlToUploadObjectMetadata(storage_url):
    if storage_url.IsCloudUrl() and storage_url.IsObject():
        ...
        raise CommandException(
            "Non-cloud URL upload target %s was created in "
