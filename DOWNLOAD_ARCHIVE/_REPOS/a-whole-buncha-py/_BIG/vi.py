...
class TextObject(object):
    ...
    def cut(self, buffer):
        ...
        to -= (
            1
        )  # SelectionState does not include the end position, `operator_range` does.
        ...
        def decorator(text_object_func):
            ...
            if not no_move_handler:
                ...
                @registry.add_binding(
                    *keys,
                    filter=~operator_given & filter & navigation_mode,
                    eager=eager
                )
                def _(event):
            ...
            if not no_selection_handler:
                ...
                @registry.add_binding(
                    *keys, filter=~operator_given & filter & selection_mode, eager=eager
                )
                def _(event):
...
def create_operator_decorator(registry):
    ...
    def operator_decorator(*keys, **kw):
        ...
        def decorator(operator_func):
            @registry.add_binding(
                *keys, filter=~operator_given & filter & navigation_mode, eager=eager
            )
            def _(event):
                """
...
def load_vi_bindings(get_search_state=None):
    ...
    vi_transform_functions = [
        ...
        (
            ("~",),
            Condition(lambda cli: cli.vi_state.tilde_operator),
    ...
    @handle(
        Keys.ControlJ, filter=navigation_mode
    )  # XXX: only if the selected buffer has a return handler.
    def _(event):
        """
    ...
    def _(event):
        ...
        cursor_position = buff.get_search_position(
            get_search_state(event.cli), include_current_position=False, count=event.arg
        )
    ...
    @handle(
        Keys.Any, filter=insert_multiple_mode, save_before=(lambda e: not e.is_repeat)
    )
    def _(event):
        """
...
def load_vi_search_bindings(get_search_state=None, search_buffer_name=SEARCH_BUFFER):
    ...
    @handle(
        "/", filter=(navigation_mode | selection_mode) & ~reverse_vi_search_direction
    )
    @handle(
        "?", filter=(navigation_mode | selection_mode) & reverse_vi_search_direction
    )
    ...
    def _(event):
        incremental_search(
            event.cli, IncrementalSearchDirection.BACKWARD, count=event.arg
        )

    @handle(Keys.ControlS, filter=has_focus)
    def _(event):
        incremental_search(
            event.cli, IncrementalSearchDirection.FORWARD, count=event.arg
        )
