...
class TestRewrite(testcase.GsUtilIntegrationTestCase):
    ...
    def test_rewrite_missing_decryption_key(self):
        ...
        with SetBotoConfigForTest(boto_config_for_test):
            ...
            self.assertIn(
                "No decryption key matches object %s" % suri(object_uri), stderr
            )
    ...
    def test_rewrite_unintentional_key_rotation_fails(self):
        ...
        with SetBotoConfigForTest(boto_config_for_test):
            ...
            stderr = self.RunGsUtil(
                ["rewrite", "-s", "dra", suri(encrypted_obj_uri)],
                return_stderr=True,
                expected_status=1,
            )
    ...
    def test_rewrite_key_rotation_bucket_subdir(self):
        ...
        with SetBotoConfigForTest(boto_config_for_test):
            stderr = self.RunGsUtil(
                ["rewrite", "-r", "-k", rotate_subdir], return_stderr=True
            )
    ...
    def test_rewrite_to_same_storage_class_is_skipped(self):
        ...
        object_uri = self.CreateObject(contents=b"bar")
        stderr = self.RunGsUtil(
            ["rewrite", "-s", "standard", suri(object_uri)], return_stderr=True
        )
    ...
    def test_rewrite_resume(self):
        self._test_rewrite_resume_or_restart(TEST_ENCRYPTION_KEY1, TEST_ENCRYPTION_KEY2)

    def test_rewrite_resume_restart_source_encryption_changed(self):
        self._test_rewrite_resume_or_restart(
            TEST_ENCRYPTION_KEY1, TEST_ENCRYPTION_KEY2, new_dec_key=TEST_ENCRYPTION_KEY3
        )

    def test_rewrite_resume_restart_dest_encryption_changed(self):
        self._test_rewrite_resume_or_restart(
            TEST_ENCRYPTION_KEY1, TEST_ENCRYPTION_KEY2, new_enc_key=TEST_ENCRYPTION_KEY3
        )
    ...
    def _test_rewrite_resume_or_restart(
        self, initial_dec_key, initial_enc_key, new_dec_key=None, new_enc_key=None
    ):
        ...
        try:
            ...
            with SetBotoConfigForTest(
                ...
                original_md5 = gsutil_api.GetObjectMetadata(
                    src_obj_metadata.bucket,
                    src_obj_metadata.name,
                    fields=["customerEncryption", "md5Hash"],
                ).md5Hash
