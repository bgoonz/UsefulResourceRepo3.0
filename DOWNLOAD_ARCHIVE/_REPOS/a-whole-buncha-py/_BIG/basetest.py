  ...
  """
    ...
    def SetTearDownAttr(cls):
        ...
        def tearDown(self):
            ...
            if (
                leaf.__tests_to_run is not None
...
class TestCase(unittest.TestCase):
    ...
    """
        ...
        try:
            ...
            self.fail(
                "For whole: len(%s) is not supported, it appears to be type: "
                "%s" % (whole, type(whole))
            )
        ...
        try:
            ...
            self.fail(
                msg or "prefix: %s not found at start of whole: %s." % (prefix, whole)
            )
    ...
    """
        ...
        if not re.search(regex, actual_str, re.MULTILINE):
            self.fail(
                message
                or (
                    'String "%s" does not contain any of these '
    ...
    def assertRaisesWithPredicateMatch(
        self, expected_exception, predicate, callable_obj, *args, **kwargs
    ):
    ...
    def assertRaisesWithRegexpMatch(
        self, expected_exception, expected_regexp, callable_obj, *args, **kwargs
    ):
    ...
    """
        ...
        self.assertRaisesRegexp(
            expected_exception, expected_regexp, callable_obj, *args, **kwargs
        )
    ...
    """
        ...
        def CheckOrder(small, big):
            ...
            self.assertFalse(
                big <= small, "%r unexpectedly less than or equal to %r" % (big, small)
            )
            self.assertGreater(big, small)
            self.assertFalse(
                small > big, "%r unexpectedly greater than %r" % (small, big)
            )
...
class CapturedStream(object):
    ...
    def __init__(self, stream, filename):
        ...
        cap_fd = os.open(self._filename, os.O_CREAT | os.O_TRUNC | os.O_WRONLY, 0o600)
    ...
    def RestartCapture(self):
        ...
        cap_fd = os.open(self._filename, os.O_CREAT | os.O_APPEND | os.O_WRONLY, 0o600)
...
def _RunInApp(function, args, kwargs):
  ...
  """
    if _IsInAppMain():
        ...
        # undone.
        saved_flags = dict(
            (f.name, SavedFlag(f)) for f in FLAGS.FlagDict().itervalues()
        )
