...
def AddPublishCrlFlag(parser, use_update_help_text=False):
    help_text = (
        PUBLISH_CRL_UPDATE_HELP if use_update_help_text else PUBLISH_CRL_CREATE_HELP
    )
    base.Argument(
        "--publish-crl",
        help=help_text,
...
def AddValidityFlag(
    ...
    base.Argument(
        "--validity",
        help="The validity of this {}, as an ISO8601 duration. Defaults to {}.".format(
            resource_name, default_value_text
        ),
  ...
  """
    ...
    group.AddArgument(
        base.Argument(
            "--key-usages",
            metavar="KEY_USAGES",
            help="The list of key usages for this {}. This can only be provided if `--use-preset-profile` is not provided.".format(
                resource_name
            ),
            type=arg_parsers.ArgList(element_type=_StripVal, choices=_VALID_KEY_USAGES),
        )
    )
    group.AddArgument(
        base.Argument(
            ...
            help="The list of extended key usages for this {}. This can only be provided if `--use-preset-profile` is not provided.".format(
                resource_name
            ),
...
def _StrToKnownExtension(arg_name, val):
    ...
    if trimmed_val in known_extensions:
        ...
        raise exceptions.UnknownArgumentException(
            arg_name, "expected one of [{}]".format(",".join(known_extensions.keys()))
        )
...
def ParsePredefinedValues(args):
    ...
    # fix.
        ...
        raise exceptions.InvalidArgumentException(
            "--predefined-values-file", "Unrecognized field in the X509Parameters file."
        )
...
def ParseIssuancePolicy(args):
    ...
    try:
        return messages_util.DictToMessageWithErrorCheck(
            args.issuance_policy, privateca_base.GetMessagesModule("v1").IssuancePolicy
        )
    # TODO(b/77547931): Catch `AttributeError` until upstream library takes the
    # fix.
    except (messages_util.DecodeError, AttributeError):
...
def ParsePublishingOptions(args):
    ...
    if tier == messages.CaPool.TierValueValuesEnum.DEVOPS:
        if args.IsSpecified("publish_crl") and publish_crl:
            raise exceptions.InvalidArgumentException(
                "--publish-crl", "CRL publication is not supported in the DevOps tier."
            )
...
_TIER_MAPPING = {"ENTERPRISE": "enterprise", "DEVOPS": "devops"}

_TIER_MAPPER = arg_utils.ChoiceEnumMapper(
    arg_name="--tier",
    default="enterprise",
    help_str="The tier for the Certificate Authority.",
    message_enum=privateca_base.GetMessagesModule("v1").CaPool.TierValueValuesEnum,
