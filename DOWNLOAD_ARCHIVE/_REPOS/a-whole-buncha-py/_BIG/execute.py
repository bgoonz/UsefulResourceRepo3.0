...
def _AddCommonInstanceFilterFlags(mutually_exclusive_group):
    ...
    individual_filters_group = mutually_exclusive_group.add_group(
        help="""\
...
def _AddTopLevelArguments(parser):
    ...
    instance_filter_group = parser.add_mutually_exclusive_group(
        required=True, help="Filters for selecting which instances to patch:"
    )
        ...
        base.ChoiceArgument(
            ...
            choices={
              ...
              sorting the regions or zones in alphabetical order.""",
                "concurrent-zones": "Patches are applied to VMs in all zones at the same time.",
            },
        )
    )
...
def _CreateWindowsUpdateSettings(args, messages):
    ...
    if not any(
        [
            args.windows_classifications,
            args.windows_excludes,
    ...
    classifications = (
        ...
        if args.windows_classifications
        else []
    )
...
def _CreateYumSettings(args, messages):
    ...
    return messages.YumSettings(
        ...
        if args.yum_exclusive_packages
        else [],
    )
...
def _CreateZypperSettings(args, messages):
    ...
    if not any(
        [
            args.zypper_categories,
            args.zypper_severities,
            args.zypper_with_optional,
...
def _CreateExecStepConfig(messages, arg_name, path, allowed_success_codes, is_windows):
    ...
    interpreter = (
        messages.ExecStepConfig.InterpreterValueValuesEnum.INTERPRETER_UNSPECIFIED
    )
    ...
    if gcs_params:
        ...
        return messages.ExecStepConfig(
            ...
            allowedSuccessCodes=allowed_success_codes if allowed_success_codes else [],
...
def _ValidatePrePostPatchStepArgs(
    executable_arg_name, executable_arg, success_codes_arg_name, success_codes_arg
):
...
def _CreateProgressTracker(patch_job_name):
    ...
    stages = [
        progress_tracker.Stage("Generating instance details...", key="pre-summary"),
        progress_tracker.Stage("Reporting instance details...", key="with-summary"),
    ]
...
def _CreateExecutionUpdateMessage(percent_complete, instance_details_json):
    ...
    for key, state in osconfig_command_utils.INSTANCE_DETAILS_KEY_MAP.items():
        num_instances = (
            int(instance_details_json[key]) if key in instance_details_json else 0
        )
...
def _UpdateProgressTracker(tracker, patch_job, unused_status):
  ...
  """
    ...
    if not details_json or details_json == "{}":
        ...
        details_str = _CreateExecutionUpdateMessage(
            patch_job.percentComplete, details_json
        )
...
def _CreatePatchConfig(args, messages):
    ...
    reboot_config = (
        getattr(
            messages.PatchConfig.RebootConfigValueValuesEnum, args.reboot_config.upper()
        )
        if args.reboot_config
        else None
    )
    ...
    return messages.PatchConfig(
        ...
        preStep=_CreatePrePostPatchStepSettings(args, messages, is_pre_patch_step=True),
...
def _CreatePatchRollout(args, messages):
    ...
    if not any(
        [
            args.rollout_mode,
            args.rollout_disruption_budget,
            args.rollout_disruption_budget_percent,
        ]
    ):
        return None

    if args.rollout_mode and not (
        args.rollout_disruption_budget or args.rollout_disruption_budget_percent
    ):
        raise exceptions.InvalidArgumentException(
            "rollout-mode",
    ...
    return messages.PatchRollout(
        ...
        disruptionBudget=messages.FixedOrPercent(
            ...
            if args.rollout_disruption_budget
            else None,
            percent=int(args.rollout_disruption_budget_percent)
            if args.rollout_disruption_budget_percent
            else None,
        ),
    )


def _CreateExecuteRequest(
    messages,
    project,
    description,
    dry_run,
...
def _CreateExecuteRequestAlpha(
    messages,
    project,
    description,
    dry_run,
...
def _CreateExecuteResponse(client, messages, request, is_async, command_prefix):
    ...
    if is_async:
        ...
        log.status.Print(
            "Run the [{} describe] command to check the status of this execution.".format(
                command_prefix
            )
        )
...
class Execute(base.Command):
    ...
    def Run(self, args):
        ...
        return _CreateExecuteResponse(
            client, messages, request, args.async_, self._command_prefix
        )
