  ...
  """
    ...
    """
        if self._flag_is_registered(flag_obj):
            return
        for flags_by_module_dict in (
            self.flags_by_module_dict(),
    ...
    """
        for flag_name, flag in six.iteritems(
            flag_values._flags()
    ...
    def __setitem__(self, name, flag):
        ...
        if name in fl and not flag.allow_override and not fl[name].allow_override:
            ...
            if self.find_module_defining_flag(name) == module_name and id(
                module
            ) != self.find_module_id_defining_flag(name):
