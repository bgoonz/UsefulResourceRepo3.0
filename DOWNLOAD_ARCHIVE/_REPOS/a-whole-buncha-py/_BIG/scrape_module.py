...
class DocstringSigParser(object):
    ...
    def _get_first_function_call(self, expr: str, name: str):
        ...
        if (
            not tokens[tokenLength - 1].isidentifier()
            and name not in tokens[tokenLength - 1]
        ):
...
VALUE_REPR_FIX = {float("inf"): "float('inf')", float("-inf"): "float('-inf')"}
...
MODULE_MEMBER_SUBSTITUTE = {"__spec__": None, "__loader__": None}
...
class ScrapeState(object):
    ...
    def _collect_members(self, mod, members, substitutes, outer_member):
        ...
        if not "__getattr__" in existing_names:
            ...
            members.append(
                MemberInfo("__getattr__", value, scope=None, module=self.module_name)
            )
