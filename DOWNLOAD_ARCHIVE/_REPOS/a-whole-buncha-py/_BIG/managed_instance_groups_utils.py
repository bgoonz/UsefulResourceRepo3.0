...
from googlecloudsdk.command_lib.compute.managed_instance_groups import (
    auto_healing_utils,
)
...
_ALLOWED_UTILIZATION_TARGET_TYPES = ["DELTA_PER_MINUTE", "DELTA_PER_SECOND", "GAUGE"]
...
""".format(
        ", ".join(_ALLOWED_UTILIZATION_TARGET_TYPES)
    )
    parser.add_argument(
...
def _ValidateRequiredByUpdateStackdriverMetric(
    args
...
def _ValidateSingleInstanceAssignmentVsUtilizationTarget(
    args
):  # pylint:disable=missing-docstring
  ...
  """
    ...
    if errors or len(igm_details) != 1:
        utils.RaiseException(
            errors, ResourceNotFoundException, error_message="Could not fetch resource:"
        )
...
def _BuildCustomMetricUtilizationsFromCustomMetricUtilizationFlag(flag, messages):
  ...
  """
    ...
    return messages.AutoscalingPolicy.ScalingSchedulesValue(
        additionalProperties=[
            scaling_schedule_wrapper(
                key=policy_name,
                value=messages.AutoscalingPolicyScalingSchedule(**scaling_schedule),
            )
        ]
    )
...
def ModifyAutohealingPolicies(current_policies, messages, args, health_check_url):
...
def ValidateAutohealingPolicies(auto_healing_policies):
  ...
  """
    ...
    if not policy.healthCheck and policy.initialDelaySec:
        message = (
            "WARNING: Health check should be provided when specifying " "initial delay."
        )
...
def CreateStandbyPolicy(messages, initial_delay_sec):
    ...
    if initial_delay_sec is None:
        return None
    return messages.InstanceGroupManagerStandbyPolicy(initialDelaySec=initial_delay_sec)
        ...
        for instance in instance_names:
            instance_refs.append(
                holder.resources.Parse(
                    instance,
                    params={"project": igm_ref.project, "zone": igm_ref.zone},
...
def GetDeviceNamesFromStatefulPolicy(stateful_policy):
    ...
    if (
        ...
        return []
    return [
        disk.key for disk in stateful_policy.preservedState.disks.additionalProperties
    ]
...
def _GetInterfaceNamesFromStatefulPolicyForIPs(stateful_policy, ips_field_lambda):
...
def IsStateful(client, igm_info, igm_ref):
    ...
    has_policy = (
        hasattr(igm_info, "statefulPolicy") and igm_info.statefulPolicy is not None
    )
...
def ValidateIgmReadyForStatefulness(igm_resource, client):
    ...
    replacement_method_is_substitute = igm_resource.updatePolicy.replacementMethod == (
