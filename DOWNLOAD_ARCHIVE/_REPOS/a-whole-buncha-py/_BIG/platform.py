...
_release_version = re.compile(
    r"([^0-9]+)" r"(?: release )?" r"([\d.]+)" r"[^(]*(?:\((.+)\))?", re.ASCII
)
...
def dist(distname="", version="", id="", supported_dists=_supported_dists):
    ...
    return _linux_distribution(
        distname, version, id, supported_dists=supported_dists, full_distribution_name=0
    )
...
def win32_ver(release="", version="", csd="", ptype=""):
    ...
    release = (
        _WIN32_CLIENT_RELEASES.get((maj, min))
        or _WIN32_CLIENT_RELEASES.get((maj, None))
        or release
    )
    ...
    if getattr(winver, "product_type", None) == 3:
        release = (
            _WIN32_SERVER_RELEASES.get((maj, min))
            or _WIN32_SERVER_RELEASES.get((maj, None))
            or release
        )
...
def _syscmd_file(target, default=""):
    ...
    try:
        proc = subprocess.Popen(
            ["file", target], stdout=subprocess.PIPE, stderr=subprocess.STDOUT
        )
    ...
    if system == "Windows":
        ...
        with warnings.catch_warnings():
            ...
            warnings.filterwarnings(
                "ignore",
                r"dist\(\) and linux_distribution\(\) " "functions are deprecated .*",
                DeprecationWarning,
            )
