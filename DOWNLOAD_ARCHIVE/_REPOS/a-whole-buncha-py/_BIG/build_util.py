...
_DEFAULT_TAGS = ["gcp-cloud-build-deploy", "gcp-cloud-build-deploy-gcloud"]
...
/gke-deploy apply \\
  ...
  --timeout=24h
""".format(
    cluster=_GKE_CLUSTER_SUB_VAR, location=_GKE_LOCATION_SUB_VAR
)
...
def SuggestedConfigsPath(gcs_config_staging_path, build_id):
  ...
  """
    return _SUGGESTED_CONFIGS_PATH.format(gcs_config_staging_path, build_id)
...
def ExpandedConfigsPath(gcs_config_staging_path, build_id):
  ...
  """
    return _EXPANDED_CONFIGS_PATH.format(gcs_config_staging_path, build_id)
...
def SaveConfigsBuildStepIsSuccessful(messages, build):
  ...
  """
    save_configs_build_step = next(
        (x for x in build.steps if x.id == _SAVE_CONFIGS_BUILD_STEP_ID), None
    )
...
def CreateBuild(
  ...
  """
    ...
    substitutions = _BaseBuildSubstitutionsDict(
        dockerfile_path,
        app_name,
        config_path,
        expose_port,
    ...
    build.substitutions = cloudbuild_util.EncodeSubstitutions(substitutions, messages)
    ...
    build.options.substitutionOption = (
        messages.BuildOptions.SubstitutionOptionValueValuesEnum.ALLOW_LOOSE
    )
...
def CreatePRPreviewBuildTrigger(
  ...
  """
    ...
    build = messages.Build(
        ...
        substitutions=cloudbuild_util.EncodeSubstitutions(substitutions, messages),
        options=messages.BuildOptions(
            substitutionOption=messages.BuildOptions.SubstitutionOptionValueValuesEnum.ALLOW_LOOSE
        ),
        images=[image],
    ...
    if comment_control:
        github_config.pullRequest.commentControl = (
            messages.PullRequestFilter.CommentControlValueValuesEnum.COMMENTS_ENABLED
        )
...
def CreateCleanPreviewBuildTrigger(
    messages,
    name,
    description,
    github_repo_owner,
  ...
  """
    ...
    substitutions = {_GKE_CLUSTER_SUB_VAR: cluster, _GKE_LOCATION_SUB_VAR: location}
    ...
    build_trigger = messages.BuildTrigger(
        ...
        github=messages.GitHubEventsConfig(
            owner=github_repo_owner,
            name=github_repo_name,
            push=messages.PushFilter(branch="$manual-only^"),
        ),
        build=messages.Build(
            steps=[
                messages.BuildStep(
...
def AddAnnotationToPrepareDeployStep(build_trigger, key, value):
  ...
  """
    ...
    annotations_substitution = next(
        (
            x
            for x in build_trigger.substitutions.additionalProperties
    ...
    annotations_substitution = next(
        (
            x
            for x in build_trigger.build.substitutions.additionalProperties
            if x.key == _K8S_ANNOTATIONS_SUB_VAR
        ),
        None,
    )
...
def _BaseBuildSubstitutionsDict(
    dockerfile_path,
    app_name,
    config_path,
    expose_port,
