...
def source_from_cache(path):
    ...
    if pycache != _PYCACHE:
        raise ValueError(
            "{} not bottom-level directory in " "{!r}".format(_PYCACHE, path)
        )
    dot_count = pycache_filename.count(".")
    if dot_count not in {2, 3}:
        raise ValueError(
            "expected only 2 or 3 dots in " "{!r}".format(pycache_filename)
        )
    elif dot_count == 3:
...
def _validate_timestamp_pyc(data, source_mtime, source_size, name, exc_details):
    ...
    *exc_details* is a dictionary passed to ImportError if it raised for
    ...
    if _r_long(data[8:12]) != (source_mtime & 0xFFFFFFFF):
        ...
        _bootstrap._verbose_message("{}", message)
        raise ImportError(message, **exc_details)
    if source_size is not None and _r_long(data[12:16]) != (source_size & 0xFFFFFFFF):
