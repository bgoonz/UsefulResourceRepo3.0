...
class Backend(object):
    ...
    def _build_openssl_digest_name(self, algorithm):
        if algorithm.name == "blake2b" or algorithm.name == "blake2s":
            alg = "{0}{1}".format(algorithm.name, algorithm.digest_size * 8).encode(
                "ascii"
            )
        else:
    ...
    def register_cipher_adapter(self, cipher_cls, mode_cls, adapter):
        if (cipher_cls, mode_cls) in self._cipher_registry:
            raise ValueError(
                "Duplicate registration for: {0} {1}.".format(cipher_cls, mode_cls)
            )
    ...
    def _bytes_to_bio(self, data):
        ...
        bio = self._lib.BIO_new_mem_buf(data_char_p, len(data))
    ...
    def create_x509_csr(self, builder, private_key, algorithm):
        ...
        if isinstance(algorithm, hashes.MD5) and not isinstance(
            private_key, rsa.RSAPrivateKey
        ):
            raise ValueError("MD5 is not a supported hash algorithm for EC/DSA CSRs")
        ...
        sk_extension = self._ffi.gc(sk_extension, self._lib.sk_X509_EXTENSION_free)
    ...
    def _raise_time_set_error(self):
        ...
        self.openssl_assert(
            errors[0]._lib_reason_match(
                self._lib.ERR_LIB_ASN1, self._lib.ASN1_R_ERROR_GETTING_TIME
            )
        )
        raise ValueError(
    ...
    def create_x509_crl(self, builder, private_key, algorithm):
        ...
        if isinstance(algorithm, hashes.MD5) and not isinstance(
            ...
            raise ValueError("MD5 is not a supported hash algorithm for EC/DSA CRLs")
    ...
    def create_x509_revoked_certificate(self, builder):
        ...
        rev_date = self._lib.ASN1_TIME_set(
            self._ffi.NULL, calendar.timegm(builder._revocation_date.timetuple())
        )
    ...
    def _handle_key_loading_error(self):
        ...
        if not errors:
            ...
            error._lib_reason_match(
                self._lib.ERR_LIB_EVP, self._lib.EVP_R_UNSUPPORTED_PRIVATE_KEY_ALGORITHM
            )
            for error in errors
        ):
    ...
    def derive_elliptic_curve_private_key(self, private_value, curve):
        ...
        self.openssl_assert(res == 1)
        res = self._lib.EC_KEY_set_private_key(ec_cdata, self._int_to_bn(private_value))
        self.openssl_assert(res == 1)
    ...
    def _private_key_bytes(
        self, encoding, format, encryption_algorithm, evp_pkey, cdata
    ):
        ...
        res = write_bio(
            bio, key, evp_cipher, password, passlen, self._ffi.NULL, self._ffi.NULL
        )
        self.openssl_assert(res == 1)
    ...
    def _public_key_bytes(self, encoding, format, key, evp_pkey, cdata):
        ...
        if (
            format is serialization.PublicFormat.OpenSSH
            or encoding is serialization.Encoding.OpenSSH
        ):
            if (
                format is not serialization.PublicFormat.OpenSSH
                or encoding is not serialization.Encoding.OpenSSH
            ):
