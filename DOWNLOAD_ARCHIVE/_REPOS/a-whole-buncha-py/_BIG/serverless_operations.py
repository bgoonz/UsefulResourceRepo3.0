...
def Connect(conn_context):
  ...
  """
    ...
    op_client = apis.GetClientInstance(conn_context.api_name, conn_context.api_version)
...
class JobConditionPoller(ConditionPoller):
    ...
    def _PotentiallyUpdateInstanceCompletions(self, job_obj, conditions):
        ...
        self._tracker.UpdateStage(
            terminal_condition,
            "{} / {} complete".format(
                job_obj.status.succeeded or 0, job_obj.completions
            ),
        )
...
class ServerlessOperations(object):
    ...
    def _GetRoute(self, service_ref):
        ...
        route_name = self._registry.Parse(
            ...
            params={"namespacesId": service_ref.namespacesId},
    ...
    def GetConfiguration(self, service_or_configuration_ref):
        ...
        if hasattr(service_or_configuration_ref, "servicesId"):
            name = self._registry.Parse(
                ...
                params={"namespacesId": service_or_configuration_ref.namespacesId},
    ...
    def GetRoute(self, service_or_route_ref):
        ...
        if hasattr(service_or_route_ref, "servicesId"):
            name = self._registry.Parse(
                ...
                params={"namespacesId": service_or_route_ref.namespacesId},
                collection="run.namespaces.routes",
            ).RelativeName()
        else:
    ...
    def _UpdateOrCreateService(self, service_ref, config_changes, with_code, serv):
    ...
    def CreateDomainMapping(
        self, domain_mapping_ref, service_name, config_changes, force_override=False
    ):
    ...
    def CreateJob(
        self, job_ref, config_changes, wait_for_completion, tracker=None, asyn=False
    ):
    ...
    def GetJob(self, job_ref):
        """Return the relevant Service from the server, or None if 404."""
        messages = self.messages_module
        get_request = messages.RunNamespacesJobsGetRequest(name=job_ref.RelativeName())
    ...
    """
        messages = self.messages_module
        request = messages.RunNamespacesJobsDeleteRequest(name=job_ref.RelativeName())
        try:
