...
_GCS_SOURCE_ERROR_MESSAGE = (
    "Invalid Cloud Storage URL. Must match the following format: " "gs://bucket/object"
)
...
def _IsHttpTriggered(args):
    return args.IsSpecified("trigger_http") or not (
        args.trigger_topic
        or args.trigger_bucket
        or args.trigger_event_filters
        or args.trigger_event
    )
...
def _UploadToGeneratedUrl(zip_file_path, url):
  ...
  """
    ...
    try:
        ...
        upload.stream.close()
        response = http_wrapper.MakeRequest(transports.GetApitoolsTransport(), request)
    finally:
        upload.stream.close()
...
def _GetSourceLocal(
    client, messages, region, function_name, source, stage_bucket_arg, ignore_file_arg
):
...
def _GetServiceConfig(args, messages, existing_function):
  ...
  """
    ...
    if (
        ...
        for (
            additional_property
        ) in existing_function.serviceConfig.environmentVariables.additionalProperties:
    ...
    vpc_connector, vpc_egress_settings, vpc_updated_fields = _GetVpcAndVpcEgressSettings(
...
def _GetEventTrigger(args, messages):
  ...
  """
    ...
    if args.trigger_event or args.trigger_resource:
        return (
            _GetEventTriggerForEventType(args, messages),
  ...
  """
    ...
    if trigger_event == "google.cloud.pubsub.topic.v1.messagePublished":
        ...
        return messages.EventTrigger(
            ...
            eventFilters=[messages.EventFilter(attribute="bucket", value=bucket_name)],
            serviceAccountEmail=service_account_email,
            triggerRegion=args.trigger_location,
        )
...
def _GetBuildConfig(
    args, client, messages, region, function_name, event_trigger, existing_function
):
  ...
  """
    ...
    if (
        ...
        for (
            additional_property
        ) in existing_function.buildConfig.environmentVariables.additionalProperties:
    ...
    signature_type, signature_updated_fields = _GetSignatureType(args, event_trigger)

    if (
        "GOOGLE_FUNCTION_SIGNATURE_TYPE" in build_env_vars
        and "GOOGLE_FUNCTION_SIGNATURE_TYPE" not in old_build_env_vars
    ):
        raise exceptions.FunctionsError(_SIGNATURE_TYPE_ENV_VAR_COLLISION_ERROR_MESSAGE)
    ...
    build_updated_fields = frozenset.union(
        signature_updated_fields, source_updated_fields, updated_fields
    )
    return (
  ...
  """
    ...
    if args.clear_vpc_connector:
        ...
        if args.egress_settings:
            ...
            return args.vpc_connector, None, frozenset(["service_config.vpc_connector"])
...
def _UpdateAndWait(client, messages, function_ref, function, updated_fields_set):
    """Update a function.
  ...
  """
    if updated_fields_set:
        ...
        update_request = messages.CloudfunctionsProjectsLocationsFunctionsPatchRequest(
            name=function_ref.RelativeName(), updateMask=update_mask, function=function
        )
...
def Run(args, release_track):
    ...
    labels_value, labels_updated_fields = _GetLabels(args, messages, existing_function)
