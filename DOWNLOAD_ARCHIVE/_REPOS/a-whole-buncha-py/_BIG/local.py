...
class ApplicationDefaultCredentialSetting(DataObject):
    ...
    def __new__(cls, **kwargs):
        if not cls._instance:
            cls._instance = super(ApplicationDefaultCredentialSetting, cls).__new__(
                cls, **kwargs
            )
...
class Settings(DataObject):
  ...
  """
    ...
    """
        ...
        replacements = {"service_name": knative_service.metadata.name}
        ...
        for label in knative_service.annotations:
            if label == container_resource.SECRETS_ANNOTATION:
                raise exceptions.Error(
                    "Referencing secrets from other projects is "
        ...
        for var in container.env:
            if var.valueFrom:
                if var.valueFrom.configMapKeyRef:
                    raise exceptions.Error(
                        "env_vars from config_maps are not supported"
                    )
        ...
        replacements.update(
            _MergedEnvVars(
                self.env_vars, self.env_vars_secrets, new_env_vars, new_env_vars_secrets
            )
        )
        ...
        for vol in knative_service.spec.template.spec.volumes:
            if vol.secret:
                ...
                raise exceptions.Error(
                    'Could not process volume "{}". Only volumes '
        ...
        for vol in container.volumeMounts:
            if vol.name not in all_vols:
                raise exceptions.Error(
                    'Container referenced volume "{}" which was not '
    ...
    def WithArgs(self, args):
        ...
        replacements.update(
            _MergedEnvVars(
                self.env_vars, self.env_vars_secrets, new_envs, new_env_secrets
            )
        )
...
def _MergedEnvVars(env_vars, env_vars_secrets, new_env_vars, new_env_vars_secrets):
    ...
    if yaml_file:
        try:
            ...
            if service_config_may_be_app_yaml:
                ...
                raise exceptions.Error(
                    "%r is unreadable as a service.yaml file." % yaml_file
                )
...
def _BuilderFromArg(builder_arg):
    ...
    return BuildpackBuilder(
        builder=builder_arg, trust=is_gcp_base_builder, devmode=is_gcp_base_builder
    )
...
def _AddEnvironmentVariables(container, env_vars):
  ...
  """
    ...
    if invalid_keys:
        raise ValueError(
            "Environment variable name must be a C_IDENTIFIER. "
...
def CreateDevelopmentServiceAccount(service_account_email):
  ...
  """
    ...
    if _IsReservedServiceAccountName(service_account_email):
        if not exists:
            raise ValueError(
                "%s cannot be created because it is a service "
    ...
    if not exists:
        ...
        _CreateAccount(
            "Serverless Local Development Service Account", account_id, project_id
        )

        permission_msg = (
            "The project editor role allows the service account "
        ...
        if console_io.PromptContinue(
            ...
            _AddBinding(
                project_id, "serviceAccount:" + service_account_email, "roles/editor"
            )
...
def _GetServiceAccountProject(service_account_email):
  ...
  """
    ...
    raise ValueError(service_account_email + " is not a valid service account address")
...
def _GetServiceAccountId(service_account_email):
    ...
    if not matcher:
        raise ValueError(
            service_account_email + " is not a valid service account address"
        )
...
def _IsReservedServiceAccountName(service_account_email):
    return _APPENGINE_SERVICE_ACCOUNT.match(
        service_account_email
    ) or _COMPUTE_SERVICE_ACCOUNT.match(service_account_email)
  ...
  """
    ...
    if not iam_util.BindingInPolicy(policy, account, role):
        iam_util.AddBindingToIamPolicy(
            CRM_MESSAGE_MODULE.Binding, policy, account, role
        )
        req = CRM_MESSAGE_MODULE.CloudresourcemanagerProjectsSetIamPolicyRequest(
            resource=project,
            setIamPolicyRequest=CRM_MESSAGE_MODULE.SetIamPolicyRequest(policy=policy),
        )
...
class CloudSqlProxyGenerator(KubeConfigGenerator):
    ...
    def ModifyDeployment(self, deployment):
        ...
        containers.append(
            _CreateCloudSqlProxyContainer(self._instance_names, self._secret_info.path)
        )
    ...
    """
        ...
        volume_mounts.append(
            {"name": "cloudsql", "mountPath": "/cloudsql", "readOnly": True}
        )
...
class SecretsGenerator(KubeConfigGenerator):
    ...
    def CreateConfigs(self):
        ...
        if self.allow_secret_manager is None:
            secrets_msg = (
                "This config references secrets stored in secret manager."
        ...
        return secrets.BuildSecrets(self.project_name, self.secret_map, self.namespace)
    ...
    def ModifyDeployment(self, deployment):
        ...
        for volume in self.secret_volumes:
            _AddSecretVolumeByName(
                volumes, volume.secret_name, volume.name, volume.items
            )
...
def CreateServiceAccountKey(service_account_name):
  ...
  """
    default_credential_path = os.path.join(
        config.Paths().global_config_dir, _Utf8ToBase64(service_account_name) + ".json"
    )
    credential_file_path = encoding.GetEncodedValue(
        os.environ, "LOCAL_CREDENTIAL_PATH", default_credential_path
    )
    ...
    warning_msg = (
        ...
        "service accounts from a production project."
    ).format(
        service_account_name=service_account_name, local_key_path=credential_file_path
    )
    ...
    create_key_request = message_module.IamProjectsServiceAccountsKeysCreateRequest(
        name=service_account_name,
        createServiceAccountKeyRequest=message_module.CreateServiceAccountKeyRequest(
            privateKeyType=message_module.CreateServiceAccountKeyRequest.PrivateKeyTypeValueValuesEnum.TYPE_GOOGLE_CREDENTIALS_FILE
        ),
    )
...
def LocalDevelopmentSecretSpec(key):
  ...
  """
    ...
    yaml_config["data"] = {"local_development_service_account.json": _Utf8ToBase64(key)}
    return yaml_config


def _AddSecretVolume(volumes, secret_name):
    _AddSecretVolumeByName(volumes, secret_name=secret_name, volume_name=secret_name)
