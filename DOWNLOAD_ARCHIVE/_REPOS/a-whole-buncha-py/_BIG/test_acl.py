...
class TestAcl(TestAclBase):
    ...
    def setUp(self):
        ...
        self._project_number = self.json_api.GetBucket(
            self.CreateBucket().bucket_name, fields=["projectNumber"]
        ).projectNumber
    ...
    def test_set_invalid_acl_object(self):
        ...
        stderr = self.RunGsUtil(
            self._set_acl_prefix + [inpath, obj_uri],
            return_stderr=True,
            expected_status=1,
        )
    ...
    def test_set_invalid_acl_bucket(self):
        ...
        inpath = self.CreateTempFile(contents=b"badAcl")
        stderr = self.RunGsUtil(
            self._set_acl_prefix + [inpath, bucket_uri],
            return_stderr=True,
            expected_status=1,
        )
        self.assertIn("ArgumentException", stderr)
    ...
    def test_set_valid_acl_bucket(self):
        ...
        bucket_uri = suri(self.CreateBucket())
        acl_string = self.RunGsUtil(
            self._get_acl_prefix + [bucket_uri], return_stdout=True
        )
        inpath = self.CreateTempFile(contents=acl_string.encode(UTF8))
    ...
    def test_set_valid_def_acl_bucket(self):
        ...
        acl_string = self.RunGsUtil(
            self._get_acl_prefix + [obj_uri1], return_stdout=True
        )
        ...
        def _Check1():
            ...
            acl_string2 = self.RunGsUtil(
                self._get_acl_prefix + [obj_uri2], return_stdout=True
            )
            self.assertNotEqual(acl_string, acl_string2)
        ...
        def _Check2():
            ...
            acl_string3 = self.RunGsUtil(
                self._get_acl_prefix + [obj_uri3], return_stdout=True
            )
            self.assertEqual(acl_string, acl_string3)
    ...
    def test_acl_set_version_specific_uri(self):
        ...
        for uri_str in (v0_uri_str, v1_uri_str):
            ...
            self.assertNotIn(
                PUBLIC_READ_JSON_ACL_TEXT, self._strip_json_whitespace(acl)
            )
    ...
    def testAclChangeWithUserId(self):
        change = acl_helper.AclChange(
            self.USER_TEST_ID + ":r", scope_type=acl_helper.ChangeType.USER
        )
    ...
    def testAclChangeWithGroupId(self):
        change = acl_helper.AclChange(
            self.GROUP_TEST_ID + ":r", scope_type=acl_helper.ChangeType.GROUP
        )
        acl = list(AclTranslation.BotoBucketAclToMessage(self.sample_uri.get_acl()))
        change.Execute(self.sample_url, acl, "acl", self.logger)
    ...
    def testAclChangeWithUserEmail(self):
        change = acl_helper.AclChange(
            self.USER_TEST_ADDRESS + ":r", scope_type=acl_helper.ChangeType.USER
        )
        acl = list(AclTranslation.BotoBucketAclToMessage(self.sample_uri.get_acl()))
        change.Execute(self.sample_url, acl, "acl", self.logger)
    ...
    def testAclChangeWithGroupEmail(self):
        change = acl_helper.AclChange(
            self.GROUP_TEST_ADDRESS + ":fc", scope_type=acl_helper.ChangeType.GROUP
        )
        acl = list(AclTranslation.BotoBucketAclToMessage(self.sample_uri.get_acl()))
        change.Execute(self.sample_url, acl, "acl", self.logger)
    ...
    def testAclChangeWithDomain(self):
        change = acl_helper.AclChange(
            self.DOMAIN_TEST + ":READ", scope_type=acl_helper.ChangeType.GROUP
        )
        acl = list(AclTranslation.BotoBucketAclToMessage(self.sample_uri.get_acl()))
        change.Execute(self.sample_url, acl, "acl", self.logger)
    ...
    def testAclChangeWithProjectOwners(self):
        change = acl_helper.AclChange(
            self._project_test_acl + ":READ", scope_type=acl_helper.ChangeType.PROJECT
        )
        acl = list(AclTranslation.BotoBucketAclToMessage(self.sample_uri.get_acl()))
        change.Execute(self.sample_url, acl, "acl", self.logger)
    ...
    def testAclChangeWithAllUsers(self):
        change = acl_helper.AclChange(
            "AllUsers:WRITE", scope_type=acl_helper.ChangeType.GROUP
        )
        acl = list(AclTranslation.BotoBucketAclToMessage(self.sample_uri.get_acl()))
        change.Execute(self.sample_url, acl, "acl", self.logger)
    ...
    def testAclChangeWithAllAuthUsers(self):
        change = acl_helper.AclChange(
            "AllAuthenticatedUsers:READ", scope_type=acl_helper.ChangeType.GROUP
        )
        acl = list(AclTranslation.BotoBucketAclToMessage(self.sample_uri.get_acl()))
        change.Execute(self.sample_url, acl, "acl", self.logger)
    ...
    def testAclDelWithUser(self):
        add = acl_helper.AclChange(
            self.USER_TEST_ADDRESS + ":READ", scope_type=acl_helper.ChangeType.USER
        )
        acl = list(AclTranslation.BotoBucketAclToMessage(self.sample_uri.get_acl()))
    ...
    def testAclDelWithGroup(self):
        add = acl_helper.AclChange(
            self.USER_TEST_ADDRESS + ":READ", scope_type=acl_helper.ChangeType.GROUP
        )
        acl = list(AclTranslation.BotoBucketAclToMessage(self.sample_uri.get_acl()))
        add.Execute(self.sample_url, acl, "acl", self.logger)
    ...
    def _AssertHas(self, current_acl, perm, scope, value=None):
        matches = list(self._YieldMatchingEntriesJson(current_acl, perm, scope, value))
    ...
    """
        for entry in current_acl:
            if (
                ...
                scope in ["UserByEmail", "GroupByEmail"]
                and entry.email
                and value == entry.email
                and entry.role == perm
            ):
                yield entry
            elif (
                scope == "GroupByDomain"
                and entry.domain
                and value == entry.domain
                and entry.role == perm
            ):
                yield entry
            elif scope == "Project" and entry.role == perm and value == entry.entityId:
                yield entry
            elif (
                scope in ["AllUsers", "AllAuthenticatedUsers"]
                and entry.entity.lower() == scope.lower()
                and entry.role == perm
            ):
                yield entry
    ...
    def testBucketAclChange(self):
        ...
        self.RunGsUtil(
            self._ch_acl_prefix + ["-d", self.USER_TEST_ADDRESS, suri(self.sample_uri)]
        )
    ...
    def testProjectAclChangesOnBucket(self):
        ...
        if self.test_api == ApiSelector.XML:
            ...
            self.assertIn(
                (
                    "CommandException: XML API does not support project"
                    " scopes, cannot translate ACL."
                ),
                stderr,
            )
        else:
    ...
    def testObjectAclChange(self):
        ...
        self.RunGsUtil(
            self._ch_acl_prefix + ["-g", self.GROUP_TEST_ADDRESS + ":READ", suri(obj)]
        )
        json_text = self.RunGsUtil(
            self._get_acl_prefix + [suri(obj)], return_stdout=True
        )
        ...
        self.RunGsUtil(
            self._ch_acl_prefix + ["-g", self.GROUP_TEST_ADDRESS + ":OWNER", suri(obj)]
        )
        json_text2 = self.RunGsUtil(
            self._get_acl_prefix + [suri(obj)], return_stdout=True
        )
    ...
    def testSeekAheadAcl(self):
        ...
        current_acl = self.RunGsUtil(
            ["acl", "get", suri(object_uri)], return_stdout=True
        )
        ...
        with SetBotoConfigForTest(
            ...
            stderr = self.RunGsUtil(
                ["-m", "acl", "ch", "-u", "AllUsers:R", suri(object_uri)],
                return_stderr=True,
            )
            self.assertIn("Estimated work for this command: objects: 1\n", stderr)

            stderr = self.RunGsUtil(
                ["-m", "acl", "set", current_acl_file, suri(object_uri)],
                return_stderr=True,
            )
            self.assertIn("Estimated work for this command: objects: 1\n", stderr)
        ...
        with SetBotoConfigForTest(
            [
                ("GSUtil", "task_estimation_threshold", "0"),
                ("GSUtil", "task_estimation_force", "True"),
            ]
        ):
            stderr = self.RunGsUtil(
    ...
    def testMultithreadedAclChange(self, count=10):
        ...
        for obj in objects:
            json_texts.append(
                self.RunGsUtil(self._get_acl_prefix + [suri(obj)], return_stdout=True)
            )
    ...
    def testRecursiveChangeAcl(self):
        ...
        obj = self.CreateObject(
            bucket_uri=self.sample_uri, object_name="foo/bar", contents=b"something"
        )
        self.AssertNObjectsInBucket(self.sample_uri, 1)
    ...
    def testMultiVersionSupport(self):
        ...
        obj1_uri = self.CreateObject(
            bucket_uri=bucket, object_name=object_name, contents=b"One thing"
        )
        ...
        self.RunGsUtil(
            self._ch_acl_prefix + ["-g", self.GROUP_TEST_ADDRESS + ":READ", obj_v1]
        )
        json_text = self.RunGsUtil(self._get_acl_prefix + [obj_v1], return_stdout=True)
        self.assertRegex(json_text, test_regex)
    ...
    def testAclGetWithoutFullControl(self):
        ...
        with self.SetAnonymousBotoCreds():
            stderr = self.RunGsUtil(
                self._get_acl_prefix + [suri(object_uri)],
                return_stderr=True,
                expected_status=1,
            )
    ...
    def testTooFewArgumentsFails(self):
        ...
        stderr = self.RunGsUtil(
            self._get_acl_prefix, return_stderr=True, expected_status=1
        )
        self.assertIn("command requires at least", stderr)

        # No arguments for set, but valid subcommand.
        stderr = self.RunGsUtil(
            self._set_acl_prefix, return_stderr=True, expected_status=1
        )
        self.assertIn("command requires at least", stderr)

        # No arguments for ch, but valid subcommand.
        stderr = self.RunGsUtil(
            self._ch_acl_prefix, return_stderr=True, expected_status=1
        )
        self.assertIn("command requires at least", stderr)
...
class TestS3CompatibleAcl(TestAclBase):
    ...
    def testAclObjectGetSet(self):
        ...
        stdout = self.RunGsUtil(
            self._get_acl_prefix + [suri(obj_uri)], return_stdout=True
        )
    ...
    def testAclBucketGetSet(self):
        bucket_uri = self.CreateBucket()
        stdout = self.RunGsUtil(
            self._get_acl_prefix + [suri(bucket_uri)], return_stdout=True
        )
        set_contents = self.CreateTempFile(contents=stdout.encode(UTF8))
