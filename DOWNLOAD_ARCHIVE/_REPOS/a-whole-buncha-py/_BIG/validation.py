...
ItemDumper.add_representer(SortedDict, ItemDumper.represent_dict)
...
class ValidatedBase(object):
    ...
    """
        raise NotImplementedError(
            "Subclasses of ValidatedBase must " "override GetValidator."
        )
    ...
    """
        raise NotImplementedError(
            "Subclasses of ValidatedBase must " "override ToDict."
        )
    ...
    """
        raise NotImplementedError(
            "Subclasses of ValidatedBase must " "override GetWarnings"
        )
...
class Validated(ValidatedBase):
  ...
  """
    ...
    """
        ...
        if not isinstance(self.ATTRIBUTES, dict):
            raise AttributeDefinitionError(
                "The class %s does not define an ATTRIBUTE variable." % self.__class__
            )
    ...
    """
        if key not in cls.ATTRIBUTES:  # pylint: disable=unsupported-membership-test
            raise ValidationError(
                "Unexpected attribute '%s' for object of type %s." % (key, cls.__name__)
            )
    ...
    def GetWarnings(self):
        ...
        for key in self.ATTRIBUTES.keys():
            ret.extend(
                self.GetValidator(key).GetWarnings(self.GetUnnormalized(key), key, self)
            )
        return ret
  ...
  """
    ...
    def Validate(self, value, key="???"):
        if not isinstance(value, six_subset.string_types):
            raise ValidationError(
                "Value %r for %s is not a valid text string." % (value, key)
            )
        return value
  ...
  """
    ...
    """
      ...
      """
            if not isinstance(alias, str):
                raise AttributeDefinitionError("All option values must be of type str.")
        ...
        for option in options:
            if isinstance(option, str):
                ...
                if len(option) != 2:
                    raise AttributeDefinitionError(
                        "Alias is defined as a list or tuple "
                ...
                if not isinstance(aliases, (list, tuple)):
                    raise AttributeDefinitionError(
                        "Alias lists must be a list or tuple"
                    )
                ...
                raise AttributeDefinitionError(
                    "All options must be of type str " "or of the form (str, [str...])."
                )
    ...
    """
        ...
        if value not in self.options:
            raise ValidationError(
                "Value '%s' for %s not in %s." % (value, key, self.options)
            )
...
class Regex(Validator):
  ...
  """
    ...
    """
        ...
        if not issubclass(
            string_type, six_subset.string_types
        ) or six_subset.is_basestring(string_type):
            raise AttributeDefinitionError(
    ...
    """
        ...
        if self.re.match(cast_value) is None:
            raise ValidationError(
                "Value '%s' for %s does not match expression "
  ...
  """
    ...
    """
        ...
        try:
            ...
            raise ValidationError(
                "Value '%s' for %s does not compile: %s" % (regex, self.__key, e), e
            )
  ...
  """
    ...
    """
        ...
        if self.maximum is None and cast_value < self.minimum:
            raise ValidationError(
                "Value '%s' for %s less than %s" % (value, key, self.minimum)
            )
        elif self.minimum is None and cast_value > self.maximum:
            raise ValidationError(
                "Value '%s' for %s greater than %s" % (value, key, self.maximum)
            )
  ...
  """
    ...
    """
        if not isinstance(value, list):
            raise ValidationError(
                "Value '%s' for %s should be a sequence but " "is not." % (value, key)
            )
        ...
        for idx, item in enumerate(value):
            if isinstance(self.constructor, Validator):
                ...
                raise ValidationError(
                    "Value element '%s' for %s must be type %s."
...
class TimeValue(Validator):
    ...
    _EXPECTED_SYNTAX = (
        "must be a non-negative number followed by a time unit, " "such as 1h or 3.5d"
    )
    ...
    """
        if not isinstance(value, six_subset.string_types):
            raise ValidationError(
                "Value '%s' for %s is not a string (%s)"
                % (value, key, TimeValue._EXPECTED_SYNTAX)
            )
        if not value:
            raise ValidationError(
                "Value for %s is empty (%s)" % (key, TimeValue._EXPECTED_SYNTAX)
            )
        if value[-1] not in "smhd":
            raise ValidationError(
                "Value '%s' for %s must end with a time unit, "
        ...
        try:
            ...
            raise ValidationError(
                "Value '%s' for %s is not a valid time value (%s)"
                % (value, key, TimeValue._EXPECTED_SYNTAX)
            )
        if t < 0:
            raise ValidationError(
                "Value '%s' for %s is negative (%s)"
                % (value, key, TimeValue._EXPECTED_SYNTAX)
            )
        return value
  ...
  """
    ...
    def CheckFieldInitialized(self, value, key, obj):
        ...
        if value is not None and deprecated_value is not None:
            raise ValidationError(
                "Only one of the two fields [{}] (preferred)"
                " and [{}] (deprecated) may be set.".format(key, self.deprecated)
            )
...
class Deprecated(Normalized):
  ...
  """
    ...
    def GetWarnings(self, value, key, obj):
        ...
        if value is not None:
            return [
                (key, "Field %s is deprecated; use %s instead." % (key, self.preferred))
            ]
        return []
