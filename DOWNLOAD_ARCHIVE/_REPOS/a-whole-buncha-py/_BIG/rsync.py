...
def _DiffToApplyArgChecker(command_instance, diff_to_apply):
    ...
    if (
        diff_to_apply.diff_action == DiffAction.REMOVE
...
def _ComputeNeededFileChecksums(
    logger,
    src_url_str,
    src_size,
    src_crc32c,
...
def _ListUrlRootFunc(cls, args_tuple, thread_state=None):
  ...
  """
    ...
    try:
        _BatchSort(
            _FieldedListingIterator(cls, gsutil_api, base_url_str, desc), out_file
        )
    except Exception as e:  # pylint: disable=broad-except
        ...
        cls.logger.error(
            "Caught non-retryable exception while listing %s: %s" % (base_url_str, e)
        )
    ...
    try:
        while True:
            ...
            output_chunk = io.open(
                "%s-%06i" % (out_file.name, len(chunk_files)),
                mode="w+",
...
class _DiffIterator(object):
    ...
    def __init__(self, command_obj, base_src_url, base_dst_url):
        ...
        if (
            base_src_url.IsCloudUrl()
            and base_dst_url.IsFileUrl()
            and self.preserve_posix
        ):
    ...
    """
        ...
        for src_url in self.sorted_src_urls_it:
            src_url_str, _, _, _, _, src_mode, src_uid, src_gid, _, _ = self._ParseTmpFileLine(
                src_url
            )
            valid, err = ValidateFilePermissionAccess(
                src_url_str, uid=src_uid, gid=src_gid, mode=src_mode
            )
            if not valid:
                errors.append(err)
        if errors:
            ...
            raise CommandException(
                "This sync will orphan file(s), please fix their "
    ...
    """
        ...
        use_hashes = self.compute_file_checksums or (
            StorageUrlFromString(src_url_str).IsCloudUrl()
            and StorageUrlFromString(dst_url_str).IsCloudUrl()
        )
        ...
        if not use_hashes and has_src_mtime and has_dst_mtime:
            return (
                src_mtime != dst_mtime or src_size != dst_size,
                has_src_mtime,
                has_dst_mtime,
            )
        ...
        while True:
            ...
            if dst_url_str is None:
                if not self.sorted_dst_urls_it.IsEmpty():
                    ...
                    (
                        ...
                        dst_crc32c,
                        dst_md5,
                    ) = self._ParseTmpFileLine(next(self.sorted_dst_urls_it))
                    # Skip past base URL and normalize slashes so we can compare across
                    # clouds/file systems (including Windows).
            ...
            if dst_url_str is None or src_url_str_to_check < dst_url_str_to_check:
                ...
                if self.delete_extras:
                    yield RsyncDiffToApply(
                        None, dst_url_str, POSIXAttributes(), DiffAction.REMOVE, None
                    )
                dst_url_str = None
            else:
                ...
                if (
                    StorageUrlFromString(src_url_str).IsCloudUrl()
                    and StorageUrlFromString(dst_url_str).IsFileUrl()
                    and src_mtime == NA_TIME
                ):
                ...
                if should_replace:
                    yield RsyncDiffToApply(
                        src_url_str, dst_url_str, posix_attrs, DiffAction.COPY, src_size
                    )
...
class _SeekAheadDiffIterator(object):
    ...
    def __iter__(self):
        for diff_to_apply in self.cloned_diff_iterator:
            ...
            if (
                diff_to_apply.diff_action == DiffAction.MTIME_SRC_TO_DST
                or diff_to_apply.diff_action == DiffAction.POSIX_SRC_TO_DST
            ):
...
def _RsyncFunc(cls, diff_to_apply, thread_state=None):
    ...
    if diff_to_apply.diff_action == DiffAction.REMOVE:
        ...
        if cls.dryrun:
            ...
            try:
                ...
                if src_url.IsCloudUrl():
                    src_generation = GenerationFromUrlAndString(
                        src_url, src_url.generation
                    )
                ...
                cls.logger.info(
                    "Skipping item %s with unsupported object type %s",
                    src_url,
                    e.unsupported_type,
                )
        ...
        if cls.dryrun:
            ...
            cls.logger.info(
                "Copying POSIX attributes from src to dst for %s", dst_url.url_string
            )
            obj_metadata = apitools_messages.Object()
            ...
            SerializeFileAttributesToObjectMetadata(
                posix_attrs, obj_metadata.metadata, preserve_posix=True
            )
            if dst_url.IsCloudUrl():
                dst_generation = GenerationFromUrlAndString(dst_url, dst_url.generation)
        ...
        raise CommandException(
            "Got unexpected DiffAction (%d)" % diff_to_apply.diff_action
        )
...
class RsyncCommand(Command):
    ...
    def RunCommand(self):
        ...
        LogPerformanceSummaryParams(
            has_file_src=src_url.IsFileUrl(),
