...
from googlecloudsdk.command_lib.compute.instance_templates import (
    flags as instance_templates_flags,
)
from googlecloudsdk.command_lib.compute.instance_templates import mesh_util
from googlecloudsdk.command_lib.compute.instance_templates import service_proxy_aux_data
from googlecloudsdk.command_lib.compute.instances import flags as instances_flags
from googlecloudsdk.command_lib.compute.resource_policies import (
    flags as maintenance_flags,
)
...
def _CommonArgs(
    ...
    maintenance_flags.AddResourcePoliciesArgs(parser, "added to", "instance-template")
    ...
    flags.AddRegionFlag(parser, resource_type="subnetwork", operation_type="attach")
    ...
    Create.InstanceTemplateArg = instance_templates_flags.MakeInstanceTemplateArg()
...
def _AddSourceInstanceToTemplate(
    ...
    if args.configure_disk:
        ...
        for disk in args.configure_disk:
            ...
            disk_config.instantiateFrom = messages.DiskInstantiationConfig.InstantiateFromValueValuesEnum(
    ...
    if getattr(args, "service_proxy", False):
        ...
        if getattr(args, "service_proxy_agent_location", False):
            service_proxy_agent_recipe["installSteps"] = [
                {
                    "scriptRun": {
                        "script": service_proxy_aux_data.startup_script_with_location_template
...
def _RunCreate(
  ...
  """
    ...
    if hasattr(args, "network_interface") and args.network_interface:
        network_interfaces = (instance_template_utils.CreateNetworkInterfaceMessages)(
    ...
    if (
        support_provisioning_model
        and hasattr(args, "provisioning_model")
    ...
    create_boot_disk = not (
        instance_utils.UseExistingBootDisk((args.disk or []) + (args.create_disk or []))
    )
    if create_boot_disk:
    ...
    persistent_disks = instance_template_utils.CreatePersistentAttachedDiskMessages(
        client.messages, args.disk or []
    )

    persistent_create_disks = instance_template_utils.CreatePersistentCreateDiskMessages(
        client,
    ...
    local_nvdimms = create_utils.CreateLocalNvdimmMessages(
        args, compute_api.resources, client.messages
    )
    ...
    if args.private_ipv6_google_access_type is not None:
        instance_template.properties.privateIpv6GoogleAccess = instances_flags.GetPrivateIpv6GoogleAccessTypeFlagMapperForTemplate(
            client.messages
        ).GetEnumForChoice(
    ...
    if (
        ...
        instance_template.properties.advancedMachineFeatures = instance_utils.CreateAdvancedMachineFeaturesMessage(
            client.messages,
...
DETAILED_HELP = {
    ...
    "DESCRIPTION": "*{command}* facilitates the creation of Compute Engine "
