...
def _RetryOperation(
    exc_info, func, args, retry_test_function=lambda func, exc_info: True
):
...
def _HandleRemoveError(func, failed_path, exc_info):
  ...
  Args:
    ...
    exc_info: sys.exc_info(), The current exception state.
  """
    logging.debug("Handling file system error: %s, %s, %s", func, failed_path, exc_info)
    ...
    if (
        WindowsError
        and issubclass(exc_info[0], WindowsError)
...
class FileLock(object):
  ...
  """
    ...
    """
        ...
        try:
            r.RetryOnException(
                self._impl.TryLock, args=[self._file.fileno()], sleep_ms=100
            )
...
def WriteFileAtomically(file_name, contents):
  ...
  """
    if not file_name or contents is None:
        raise ValueError(
            "Empty file_name [{}] or contents [{}].".format(file_name, contents)
        )
...
def WriteFileContents(
    path, contents, overwrite=True, private=False, create_path=True, newline=None
):
  ...
  """
    try:
        ...
        with FileWriter(
            path, private=private, create_path=create_path, newline=newline
        ) as f:
...
def WriteBinaryFileContents(
    path, contents, overwrite=True, private=False, create_path=True
):
...
def _CheckOverwrite(path, overwrite):
    if not overwrite and os.path.exists(path):
        raise Error("File [{0}] already exists and cannot be overwritten".format(path))
...
def FileWriter(path, private=False, append=False, create_path=False, newline=None):
...
def BinaryFileWriter(
    path, private=False, mode=BinaryFileWriterMode.TRUNCATE, create_path=False
):
...
def _FileOpener(
    path, mode, verb, encoding=None, private=False, create_path=False, newline=None
):
