...
class Download(_Transfer):
    ...
    def __init__(self, stream, progress_callback=None, finish_callback=None, **kwds):
    ...
    def FromFile(cls, filename, overwrite=False, auto_transfer=True, **kwds):
        ...
        return cls(
            open(path, "wb"), close_stream=True, auto_transfer=auto_transfer, **kwds
        )
    ...
    @classmethod
    def FromData(
        cls, stream, json_data, http=None, auto_transfer=None, client=None, **kwds
    ):
        ...
        download._Initialize(http, url)  # pylint: disable=protected-access
        return download
    ...
    def GetRange(self, start, end=None, additional_headers=None, use_chunks=True):
        ...
        * start < 0, end = None: Fetch the last -start bytes of the file.
        ...
        while not progress_end_normalized or end_byte is None or progress <= end_byte:
            end_byte = self.__ComputeEndByte(
                progress, end=end_byte, use_chunks=use_chunks
            )
            response = self.__GetChunk(
                progress, end_byte, additional_headers=additional_headers
            )
    ...
    def StreamMedia(
        ...
        while True:
            if self.__initial_response is not None:
                ...
                response = self.__GetChunk(
                    self.progress, end_byte, additional_headers=additional_headers
                )
            if self.total_size is None:
                self.__SetTotal(response.info)
            response = self.__ProcessResponse(response)
            ...
            if (
                ...
                or self.progress >= self.total_size
            ):
                break
...
class Upload(_Transfer):
    ...
    @classmethod
    def FromFile(
        cls, filename, mime_type=None, auto_transfer=True, gzip_encoded=False, **kwds
    ):
    ...
    def FromData(
        ...
        **kwds
        ...
        upload._Initialize(http, url)  # pylint: disable=protected-access
    ...
    def __SetDefaultUploadStrategy(self, upload_config, http_request):
        ...
        if (
            ...
            and self.total_size > _RESUMABLE_UPLOAD_THRESHOLD
        ):
    ...
    def ConfigureRequest(self, upload_config, http_request, url_builder):
        ...
        if self.strategy == SIMPLE_UPLOAD:
            ...
            if self.__gzip_encoded:
                ...
                # bytes container.
                http_request.body = compression.CompressStream(
                    six.BytesIO(http_request.body)
                )[0].read()
        else:
    ...
    def InitializeUpload(self, http_request, http=None, client=None):
        ...
        http_response = http_wrapper.MakeRequest(
            http, http_request, retries=self.num_retries
        )
    ...
    def __IsRetryable(self, response):
        return (
            ...
            or response.status_code == http_wrapper.TOO_MANY_REQUESTS
            or response.retry_after
        )
    ...
    def __StreamMedia(
        ...
        while not self.complete:
            ...
            if response.status_code not in (
                ...
                # recover from.
                if self.strategy != RESUMABLE_UPLOAD or not self.__IsRetryable(
                    response
                ):
                    raise exceptions.HttpError.FromResponse(response)
    ...
    def __SendMediaBody(self, start, additional_headers=None):
        ...
        request = http_wrapper.Request(
            url=self.url, http_method="PUT", body=body_stream
        )
