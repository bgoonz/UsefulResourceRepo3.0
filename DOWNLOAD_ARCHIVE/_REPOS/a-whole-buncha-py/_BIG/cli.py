        ...
        for arg, obj in six.iteritems(member.value):
            ...
            if arg == flag:
                ...
                for path in file_list:
                    locations = _ArgLocations(
                        arg, flags_file, line_col, parent_locations
                    )
...
class CLILoader(object):
    ...
    """
        ...
        for track, (module_dir, component) in six.iteritems(self.__release_tracks):
            impl_path = self.__ValidateCommandOrGroupInfo(
                module_dir, allow_non_existing_modules=self.__allow_non_existing_modules
            )
            if impl_path:
                ...
                for track, track_root_group in six.iteritems(loaded_release_tracks):
                    ...
                    if not parent_group:
                        ...
                        exception_if_present = command_loading.LayoutException(
                            "Root [{root}] for command group [{group}] does not exist.".format(
    ...
    def __ValidateCommandOrGroupInfo(
        self, impl_path, allow_non_existing_modules=False, exception_if_present=None
    ):
...
class CLI(object):
    ...
    def __init__(
        self, name, top_element, pre_run_hooks, post_run_hooks, known_error_handler
    ):
        # pylint: disable=protected-access
        self.__name = name
            ...
            if isinstance(resources, types.GeneratorType):
                ...
                def _Yield():
                    ...
                    try:
                        ...
                        self._HandleAllErrors(
                            exc, command_path_string, specified_arg_names
                        )
