...
class RequiredFileMissingError(Error):
    ...
    def __init__(self, filename):
        super(RequiredFileMissingError, self).__init__(
            ...
            "https://cloud.google.com/sdk/gcloud/reference/topic/gcloudignore)".format(
                filename
            )
        )
...
def GetFlexImageBuildOption(default_strategy=FlexImageBuildOptions.ON_CLIENT):
    ...
    trigger_build_server_side = properties.VALUES.app.trigger_build_server_side.GetBool(
        required=False
    )
  ...
  """
        ...
        if image:
            ...
            if flex_image_build_option == FlexImageBuildOptions.ON_SERVER:
                ...
                if timeout:
                    ...
                    cloud_build_options["cloudBuildTimeout"] = (
                        six.text_type(build_timeout) + "s"
                    )
    ...
    """
        if self.deploy_options.promote:
            ...
            log.info(
                "Not stopping previous version because new version was " "not promoted."
            )
    ...
    """
        ...
        if not image and (
            ...
            limit = None
            if (
                service_info.env == env.STANDARD
                and service_info.runtime in _RUNTIMES_WITH_FILE_SIZE_LIMITS
            ):
    ...
    * Stop the previous version (if new version promoted and
    ...
    """
        ...
        if (
            service.service_info.env == env.MANAGED_VMS
...
def ArgsDeploy(parser):
  ...
  """
    ...
    parser.add_argument(
        "--promote",
        action=actions.StoreBooleanProperty(properties.VALUES.app.promote_by_default),
...
def RunDeploy(
  ...
  """
    ...
    with files.TemporaryDirectory() as staging_area:
        stager = _MakeStager(
            args.skip_staging, use_beta_stager, args.staging_command, staging_area
        )
        ...
        ac_client = appengine_client.AppengineClient(args.server, args.ignore_bad_certs)
        ...
        for service in services:
            ...
            log.status.Print(
                "Deployed service [{0}] to [{1}]".format(
...
def RunDeployCloudScheduler(config):
  ...
  """
    # TODO(b/169069379): Upgrade to use GA once the relevant code is promoted
    scheduler_api = scheduler.GetApiAdapter(base.ReleaseTrack.BETA, legacy_cron=True)
    ...
    try:
        ...
        raise core_api_exceptions.HttpException(
            (
                ...
                "(roles/appengine.deployer) role.".format(
                    api_client._FormatApp(), active_account
                )
            )
        )
