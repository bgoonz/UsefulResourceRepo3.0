...
class Message(object):
    ...
    def use_edns(
        self, edns=0, ednsflags=0, payload=1280, request_payload=None, options=None
    ):
...
class _WireReader(object):
    ...
    def _get_question(self, qcount):
        ...
        for i in xrange(0, qcount):
            ...
            (rdtype, rdclass) = struct.unpack(
                "!HH", self.wire[self.current : self.current + 4]
            )
    ...
    def _get_section(self, section, count):
        ...
        for i in xrange(0, count):
            ...
            self.current = self.current + used
            (rdtype, rdclass, ttl, rdlen) = struct.unpack(
                "!HHIH", self.wire[self.current : self.current + 10]
            )
            ...
            if rdtype == dns.rdatatype.OPT:
                ...
                optslen = rdlen
                while optslen > 0:
                    (otype, olen) = struct.unpack(
                        "!HH", self.wire[current : current + 4]
                    )
                    current = current + 4
                ...
                (
                    self.message.keyalgorithm,
                    self.message.mac,
                ) = dns.tsig.get_algorithm_and_mac(self.wire, self.current, rdlen)
                ...
                if self.updating and (
                    rdclass == dns.rdataclass.ANY or rdclass == dns.rdataclass.NONE
                ):
                    deleting = rdclass
                ...
                if deleting == dns.rdataclass.ANY or (
                    deleting == dns.rdataclass.NONE and section is self.message.answer
                ):
                    covers = dns.rdatatype.NONE
                    rd = None
                else:
                ...
                rrset = self.message.find_rrset(
                    section, name, rdclass, rdtype, covers, deleting, True, force_unique
                )
                if rd is not None:
                    rrset.add(rd, ttl)
...
class _TextReader(object):
        ...
        if what == "id":
            ...
            while True:
                ...
                if not token.is_identifier():
                    ...
                    break
                self.message.flags = self.message.flags | dns.flags.from_text(
                    token.value
                )
            if dns.opcode.is_update(self.message.flags):
                self.updating = True
            ...
            while True:
                ...
                if not token.is_identifier():
                    self.tok.unget(token)
                    break
                self.message.ednsflags = (
                    self.message.ednsflags | dns.flags.edns_from_text(token.value)
                )
        elif what == "payload":
            ...
            self.message.flags = self.message.flags | dns.opcode.to_flags(
                dns.opcode.from_text(text)
            )
        elif what == "rcode":
            text = self.tok.get_string()
    ...
    def _question_line(self, section):
        ...
        self.message.find_rrset(
            self.message.question, name, rdclass, rdtype, create=True, force_unique=True
        )
        if self.updating:
            self.zone_rdclass = rdclass
        self.tok.get_eol()
    ...
    def _rr_line(self, section):
        ...
        if not token.is_eol_or_eof():
            ...
            covers = dns.rdatatype.NONE
        rrset = self.message.find_rrset(
            section, name, rdclass, rdtype, covers, deleting, True, self.updating
        )
        if rd is not None:
            rrset.add(rd, ttl)
...
def make_response(query, recursion_available=False, our_payload=8192, fudge=300):
    ...
    *fudge*, an ``int``, the TSIG time fudge.
    ...
    if query.had_tsig:
        response.use_tsig(
            query.keyring, query.keyname, fudge, None, 0, b"", query.keyalgorithm
        )
