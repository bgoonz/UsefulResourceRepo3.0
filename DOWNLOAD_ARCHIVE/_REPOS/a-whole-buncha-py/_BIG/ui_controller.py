...
_ZERO_TASKS_TO_DO_ARGUMENT = parallelism_framework_util.ZERO_TASKS_TO_DO_ARGUMENT
...
class StatusMessageManager(object):
  ...
  """
    ...
    def UpdateSpinner(self):
        ...
        self.current_spinner_index = (self.current_spinner_index + 1) % len(
            self.spinner_char_list
        )
    ...
    """
        estimate_message = (
            "Estimated work for this command: objects: %s" % status_message.num_objects
        )
        if status_message.size:
            estimate_message += ", total size: %s" % MakeHumanReadable(
                status_message.size
            )
    ...
    """
        return (
            cur_time - self.refresh_spinner_time > self.update_spinner_period
            and self.total_size
        )
    ...
    """
        while (
            len(self.old_progress) > 1
            and cur_time - self.old_progress[0].time > self.sliding_throughput_period
        ):
    ...
    Args:
      stream: Stream to print messages. Usually sys.stderr, but customizable
              for testing.
    """
        string_to_print = "Operation completed over %s objects" % DecimalShort(
            self.num_objects
        )
        if self.total_size:
...
class MetadataManager(StatusMessageManager):
  ...
  """
    ...
    """
        ...
        if (
            self.objects_finished == self.num_objects
            and self.num_objects_source == EstimationSource.PRODUCER_THREAD_FINAL
        ):
    ...
    """
        ...
        if self.num_objects_source <= EstimationSource.SEEK_AHEAD_THREAD:
            ...
            objects_completed = (
                "["
                + DecimalShort(self.objects_finished)
                + "/"
            ...
            if self.num_objects == self.objects_finished:
                ...
                percentage = "%3d" % min(
                    99, int(100 * float(self.objects_finished) / self.num_objects)
                )
        ...
        format_str = (
            "{char_to_print} {objects_completed} {percentage_completed}"
  ...
  """
    ...
    """
        ...
        return (
            ...
            or message_type == FileMessage.FILE_UPLOAD
            or message_type == FileMessage.FILE_CLOUD_COPY
            or message_type == FileMessage.FILE_DAISY_COPY
            or message_type == FileMessage.FILE_LOCAL_COPY
            or message_type == FileMessage.FILE_REWRITE
            or message_type == FileMessage.FILE_HASH
        )
    ...
    """
        if (
            status_message.message_type == FileMessage.EXISTING_COMPONENT
            and not status_message.finished
        ):
            ...
            status_message.message_type == FileMessage.COMPONENT_TO_UPLOAD
            or status_message.message_type == FileMessage.COMPONENT_TO_DOWNLOAD
        ):
            if not status_message.finished:
                # Component started.
                ...
                file_progress.dict[key] = (
                    status_message.size - last_update[1],
                    last_update[1],
                )
    ...
    """
        ...
        bytes_progress = "[%s/%s]" % (
            BytesToFixedWidthString(self.total_progress),
        ...
        if self.total_size_source <= EstimationSource.SEEK_AHEAD_THREAD:
            if self.num_objects == self.objects_finished:
                ...
                percentage = "%3d" % min(
                    99, int(100 * float(self.total_progress) / self.total_size)
                )
            percentage_completed = percentage + "% Done"
        else:
            percentage_completed = ""
        ...
        format_str = (
            "{char_to_print} {objects_completed}{bytes_progress}"
...
class UIController(object):
  ...
  """
    ...
    """
        ...
        if (
            (isinstance(status_message, FinalMessage) or self.manager.final_message)
    ...
    """
        ...
        if not self.manager:
            if isinstance(status_message, SeekAheadMessage) or isinstance(
                status_message, ProducerThreadMessage
            ):
        ...
        if not self.manager.CanHandleMessage(status_message):
            if isinstance(status_message, FileMessage) or isinstance(
