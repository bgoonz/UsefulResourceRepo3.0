...
def ProcessResults(
    resources, field_selector, sort_key_fn=None, reverse_sort=False, limit=None
):
...
def _GetResources(
    ...
    requests = FormatListRequests(service, project, scopes, scope_name, filter_expr)
...
def GetZonalResources(
    service, project, requested_zones, filter_expr, http, batch_url, errors
):
...
def GetRegionalResources(
    service, project, requested_regions, filter_expr, http, batch_url, errors
):
...
def GetGlobalResources(service, project, filter_expr, http, batch_url, errors):
...
def GetGlobalResourcesDicts(service, project, filter_expr, http, batch_url, errors):
...
class AllScopes(object):
    ...
    def __eq__(self, other):
        ...
        return (
            ...
            and self.zonal == other.zonal
            and self.regional == other.regional
        )
...
def _GetBaseListerFrontendPrototype(args, message=None):
  ...
  """
    ...
    if getattr(args, "regexp", None):
        filter_args.append(
            '(name ~ "^{}$")'.format(
                resource_expr_rewrite.BackendBase().Quote(args.regexp)
            )
        )
    if getattr(args, "names", None):
        name_regexp = " ".join(
            [
                resource_expr_rewrite.BackendBase().Quote(name)
...
class RegionalLister(object):
  ...
  """
    ...
    def __eq__(self, other):
        ...
        return (
            isinstance(other, RegionalLister)
    ...
    def __repr__(self):
        return "RegionalLister({}, {})".format(repr(self.client), repr(self.service))
...
class MultiScopeLister(object):
  ...
  """
    ...
    def __eq__(self, other):
        ...
        return (
            isinstance(other, MultiScopeLister)
            and self.client == other.client
            and self.zonal_service == other.zonal_service
            and self.regional_service == other.regional_service
            and self.global_service == other.global_service
            and self.aggregation_service == other.aggregation_service
            and self.allow_partial_server_failure == other.allow_partial_server_failure
    ...
    def __call__(self, frontend):
        ...
        if isinstance(scope_set, ZoneSet):
            ...
            # scopeSet is AllScopes
            # generate AggregatedList
            request_message = self.aggregation_service.GetRequestType("AggregatedList")
            for project_ref in sorted(list(scope_set.projects)):
                input_params = {}
  ...
  """
    ...
    def __call__(self, frontend):
        ...
        service_list_data = _Frontend(
            filter_expr=filter_expr, maxResults=frontend.max_results, scopeSet=zones
        )
