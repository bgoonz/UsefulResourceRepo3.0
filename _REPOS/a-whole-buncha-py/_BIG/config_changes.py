    ...
    def Adjust(self, resource):
        if self._launch_stage == base.ReleaseTrack.GA:
            ...
            resource.annotations[
                k8s_object.LAUNCH_STAGE_ANNOTATION
...
class SetClientNameAndVersionAnnotationChange(ConfigChanger):
    ...
    def Adjust(self, resource):
        if self._client_name is not None:
            resource.annotations[k8s_object.CLIENT_NAME_ANNOTATION] = self._client_name
            resource.template.annotations[
                k8s_object.CLIENT_NAME_ANNOTATION
            ] = self._client_name
        if self._client_version is not None:
            resource.annotations[
                k8s_object.CLIENT_VERSION_ANNOTATION
...
class SandboxChange(ConfigChanger):
    ...
    def Adjust(self, resource):
        resource.template.annotations[
            container_resource.SANDBOX_ANNOTATION
        ] = self._sandbox
        return resource
...
class VpcConnectorChange(ConfigChanger):
    ...
    def Adjust(self, resource):
        resource.template.annotations[
            container_resource.VPC_ACCESS_ANNOTATION
...
class ImageChange(ConfigChanger):
    ...
    def Adjust(self, resource):
        resource.annotations[container_resource.USER_IMAGE_ANNOTATION] = self.image
...
class RevisionNameChanges(ConfigChanger):
    ...
    def Adjust(self, resource):
        ...
        max_prefix_length = _MAX_RESOURCE_NAME_LENGTH - len(self._revision_suffix) - 1
...
class NoTrafficChange(ConfigChanger):
    ...
    def Adjust(self, resource):
        ...
        resource.spec_traffic.ZeroLatestTraffic(resource.status.latestReadyRevisionName)
        return resource
            ...
            if self._by_tag:
                ...
                for tag in self._new_percentages:
                    try:
                        ...
                        raise exceptions.ConfigurationError(
                            "There is no revision tagged with [{}] in the traffic allocation for [{}].".format(
                                tag, resource.name
                            )
                        )
            else:
...
class CpuThrottlingChange(ConfigChanger):
    ...
    def Adjust(self, resource):
        resource.template.annotations[container_resource.CPU_THROTTLE_ANNOTATION] = str(
            self._throttling
        )
        return resource
...
class ConfidentialChange(ConfigChanger):
    ...
    def Adjust(self, resource):
        resource.template.annotations[container_resource.CONFIDENTIAL_ANNOTATION] = str(
            self._confidential
        )
        return resource
