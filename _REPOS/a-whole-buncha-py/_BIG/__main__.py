    ...
    if boto.config.get("Boto", "proxy", None) or boto_port:
        for proxy_env_var in ["http_proxy", "https_proxy", "HTTPS_PROXY"]:
            if proxy_env_var in os.environ and os.environ[proxy_env_var]:
                ...
                if (
                    proxy_info.proxy_port or boto_port
...
def _CheckAndHandleCredentialException(e, args):
    ...
    if not boto_util.HasConfiguredCredentials() and not boto.config.get_value(
        ...
        e.reason
        and (
            e.reason == "AccountProblem"
            or e.reason == "Account disabled."
            or "account for the specified project has been disabled" in e.reason
        )
...
def _RunNamedCommandAndHandleExceptions(
    ...
    try:
        ...
        _OutputAndExit(
            "HttpError: %s, %s" % (getattr(e.response, "status", ""), e.content or ""),
            exception=e,
        )
        ...
        if system_util.InvokedViaCloudSdk():
            _OutputAndExit(
                "You must re-authenticate with your SAML IdP. "
                "Please run\n$ gcloud auth login"
            )
        else:
            _OutputAndExit(
