...
encoding.AddCustomJsonFieldMapping(
    privateca_message.PrivatecaProjectsLocationsCertificateTemplatesGetIamPolicyRequest,
    "options_requestedPolicyVersion",
    "options.requestedPolicyVersion",
)

MANAGED_BY = msgs.IamProjectsServiceAccountsKeysListRequest.KeyTypesValueValuesEnum
CREATE_KEY_TYPES = msgs.CreateServiceAccountKeyRequest.PrivateKeyTypeValueValuesEnum
...
SERVICE_ACCOUNT_FORMAT = 'table(displayName:label="DISPLAY NAME", email, ' "disabled)"
...
def _ConditionArgDict():
    condition_spec = {"expression": str, "title": str, "description": str, "None": None}
...
def ValidateConditionArgument(condition, exception):
    if "None" in condition:
        if (
            ...
            or "description" in condition
            or "title" in condition
        ):
            raise exception
    else:
...
def ValidateMutexConditionAndPrimitiveRoles(condition, role):
    ...
    if (
        _ConditionIsSpecified(condition)
        and not _IsNoneCondition(condition)
        and role in primitive_roles
    ):
...
def AddArgsForAddIamPolicyBinding(
    parser, role_completer=None, add_condition=False, hide_special_member_types=False
):
...
def AddBindingToIamPolicyWithCondition(
    binding_message_type, condition_message_type, policy, member, role, condition
):
  ...
  """
    ...
    if (
        not _PolicyContainsCondition(policy)
    ...
    _AddBindingToIamPolicyWithCondition(
        binding_message_type, condition_message_type, policy, member, role, condition
    )
...
def _ConditionsInPolicy(policy, member=None, role=None):
  ...
  """
    conditions = {}
    for binding in policy.bindings:
        if (member is None or member in binding.members) and (
            role is None or role == binding.role
        ):
    ...
    if "None" in conditions:
        ...
        del conditions["None"]
    conditions = [
        (condition_str, condition) for condition_str, condition in conditions.items()
    ]
...
def _ConditionToString(condition):
    ...
    for key in keys:
        if getattr(condition, key) is not None:
            key_values.append(
                "{key}={value}".format(key=key.upper(), value=getattr(condition, key))
            )
...
def _EqualConditions(binding_condition, input_condition):
    ...
    if binding_condition is None or input_condition is None:
        return False
    return (
        binding_condition.expression == input_condition.get("expression")
        and binding_condition.title == input_condition.get("title")
        and binding_condition.description == input_condition.get("description")
    )


def _AddBindingToIamPolicyWithCondition(
    binding_message_type, condition_message_type, policy, member, role, condition
):
    """Given an IAM policy, add a new role/member binding with condition."""
    for binding in policy.bindings:
    ...
    policy.bindings.append(
        binding_message_type(
            members=[member], role="{}".format(role), condition=condition_message
        )
    )


def RemoveBindingFromIamPolicyWithCondition(
    policy, member, role, condition, all_conditions=False
):
  ...
  """
    if (
        not all_conditions
        and _PolicyContainsCondition(policy)
        and not _ConditionIsSpecified(condition)
    ):
        if not console_io.CanPrompt():
            message = (
...
def _RemoveBindingFromIamPolicyAllConditions(policy, member, role):
    ...
    if not conditions_removed:
        raise IamPolicyBindingNotFound(
            "Policy bindings with the specified member " "and role not found!"
        )
...
def _RemoveBindingFromIamPolicyWithCondition(policy, member, role, condition):
    ...
    for binding in policy.bindings:
        ...
        raise IamPolicyBindingNotFound(
            "Policy binding with the specified member, "
...
def ParsePolicyFile(policy_file_path, policy_message_type):
  ...
  """
    policy, unused_mask = ParseYamlOrJsonPolicyFile(
...
def ParsePolicyFileWithUpdateMask(policy_file_path, policy_message_type):
  ...
  """
    policy, update_mask = ParseYamlOrJsonPolicyFile(
        policy_file_path, policy_message_type
    )
...
def ParseYamlOrJsonCondition(
    condition_file_content, file_format_exception=CONDITION_FILE_FORMAT_EXCEPTION
):
...
def GetDetailedHelpForSetIamPolicy(
    collection, example_id="", example_see_more="", additional_flags="", use_an=False
):
...
def GetDetailedHelpForAddIamPolicyBinding(
    collection, example_id, role="roles/editor", use_an=False, condition=False
):
  ...
  """
    ...
    detailed_help = {
        ...
        "DESCRIPTION": "{description}",
        "EXAMPLES": """To add an IAM policy binding for the role of '{role}' for the user
...
authenticated users on {collection} '{example_id}', run:
  ...
  $ {{command}} {example_id} --member='allAuthenticatedUsers' --role='{role}'
  """.format(
            collection=collection, example_id=example_id, role=role
        ),
    }
    if condition:
        detailed_help["EXAMPLES"] = (
...
def GetHintForServiceAccountResource(action="act on"):
  ...
  """
    ...
    return (
        "When managing IAM roles, you can treat a service account either as "
...
def KeyTypeToString(key_type):
  ...
  """
    if (
        key_type == KEY_TYPES.TYPE_PKCS12_FILE
        or key_type == CREATE_KEY_TYPES.TYPE_PKCS12_FILE
    ):
        return "p12"
    elif (
        key_type == KEY_TYPES.TYPE_GOOGLE_CREDENTIALS_FILE
        or key_type == CREATE_KEY_TYPES.TYPE_GOOGLE_CREDENTIALS_FILE
    ):
        return "json"
    else:
        return "unspecified"
  ...
  """
    ...
    if lower_stage_str not in stage_dict:
        raise gcloud_exceptions.InvalidArgumentException(
            "stage", "The stage should be one of " + ",".join(sorted(stage_dict)) + "."
        )
...
def GetRoleName(
    organization, project, role, attribute="custom roles", parameter_name="ROLE_ID"
):
    ...
    if role.startswith("projects/") or role.startswith("organizations/"):
        raise gcloud_exceptions.InvalidArgumentException(
            parameter_name,
            "The role id should not include any 'projects/' or "
...
def AddServiceAccountNameArg(parser, action="to act on"):
  ...
  """
    ...
    parser.add_argument(
        ...
        help=(
            "The service account {}. The account should be "
...
def ApiDisabledPermissionsWarning(permissions):
  ...
  """
    if permissions:
        msg = (
            "API is not enabled for permissions: ["
            + ", ".join(permissions)
