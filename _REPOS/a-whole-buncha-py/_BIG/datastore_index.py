    ...
    if index.properties is not None:
        for prop in index.properties:
            props.append(
                PropertySpec(
                    name=prop.name,
                    direction=(ASCENDING if prop.IsAscending() else DESCENDING),
                )
            )
...
class PropertySpec(object):
    ...
    def __repr__(self):
        ...
        if self._direction is not None:
            builder.append(
                "direction=%s"
                % entity_pb.Index_Property.Direction_Name(self._direction)
            )
...
assert entity_pb.Index_Property.DESCENDING == datastore_pb.Query_Order.DESCENDING
...
def RemoveNativelySupportedComponents(filters, orders, exists):
  ...
  """
    ...
    if not has_key_desc_order:
        for f in filters:
            if (
                f.op() in INEQUALITY_OPERATORS
                and f.property(0).name() != datastore_types.KEY_SPECIAL_PROPERTY
            ):
                break
        else:
            filters = [
...
def CompositeIndexForQuery(query):
  ...
  """
    ...
    assert (len(eq_filters) + len(ineq_filters) + len(exists_filters)) == len(
        filters
    ...
    if kind and not ancestor and property_count <= 1:
        ...
        if postfix_ordered:
            ...
            if (
                prop.name == datastore_types.KEY_SPECIAL_PROPERTY
                and prop.direction == DESCENDING
            ):
                required = True
  ...
  """
    ...
    for sub_list in itertools.chain((prefix,), postfix):
        if isinstance(sub_list, (frozenset, set)):
            ...
            result.extend(
                [
                    (p if isinstance(p, PropertySpec) else PropertySpec(name=p))
                    for p in sorted(sub_list)
                ]
            )
        else:
            result.extend(
                [
                    (
                        PropertySpec(name=p.name, direction=ASCENDING)
  ...
  """
    ...
    for prop in props:
        if prop.mode is GEOSPATIAL:
            ...
            qual = ' direction="%s"' % (
                "desc" if prop.direction == DESCENDING else "asc"
            )
