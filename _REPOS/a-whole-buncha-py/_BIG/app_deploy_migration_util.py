...
def _DoesAttributeNeedToBeUpdated(cur_queue_state, attribute, new_value):
  ...
  """
    ...
    if hasattr(cur_queue_state, "rateLimits") and hasattr(
        cur_queue_state.rateLimits, proto_attribute_name
    ):
    ...
    if (
        old_value is None
        and attribute != "max_concurrent_dispatches"
        and attribute in constants.PUSH_QUEUES_APP_DEPLOY_DEFAULT_VALUES
        and new_value == constants.PUSH_QUEUES_APP_DEPLOY_DEFAULT_VALUES[attribute]
    ):
        return False
  ...
  """
    ...
    for old_arg, new_arg in constants.APP_TO_TASKS_ATTRIBUTES_MAPPING.items():
        ...
        if value or (value is not None and new_arg in ("max_attempts",)):
            ...
            if (
                not cur_queue_state
                or new_arg in ("name", "type", "min_backoff", "max_backoff")
                or _DoesAttributeNeedToBeUpdated(cur_queue_state, new_arg, value)
            ):
            ...
            if new_arg in default_values:
                ...
                if (
                    not cur_queue_state
                    or new_arg in ("min_backoff", "max_backoff")
                    or _DoesAttributeNeedToBeUpdated(cur_queue_state, new_arg, value)
                ):
                    _SetSpecifiedArg(cloud_task_args, new_arg, value)
...
def _RaiseHTTPException(msg_body):
  ...
  """
    ...
    error = urllib.error.HTTPError(
        None, six.moves.http_client.BAD_REQUEST, exc_msg, None, None
    )
...
def _ValidateTaskRetryLimit(queue):
  ...
  """
    if (
        queue.retry_parameters.task_retry_limit
        and queue.retry_parameters.task_retry_limit < 0
    ):
        _RaiseHTTPException(
    ...
    for job in cron_yaml.cron:
        ...
        if job.retry_parameters:
            ...
            if (
                ...
                and job.retry_parameters.job_retry_limit > 5
            ):
                _RaiseHTTPException(
            ...
            # Job Age Limit
            if (
                job.retry_parameters.job_age_limit
                and int(
            ...
            if (
                job.retry_parameters.min_backoff_seconds is not None
                and job.retry_parameters.max_backoff_seconds is not None
            ):
    ...
    for queue in queue_yaml.queue:
        ...
        if not queue.mode or queue.mode == constants.PUSH_QUEUE:
            ...
            if queue.retry_parameters:
                ...
                # Task Age Limit
                if (
                    queue.retry_parameters.task_age_limit
                    and int(
                        convertors.CheckAndConvertStringToFloatIfApplicable(
                            queue.retry_parameters.task_age_limit
                        )
                ...
                # Min backoff
                if (
                    queue.retry_parameters.min_backoff_seconds
                    and queue.retry_parameters.min_backoff_seconds < 0
                ):
                    _RaiseHTTPException(
                ...
                # Max backoff
                if (
                    queue.retry_parameters.max_backoff_seconds
                    and queue.retry_parameters.max_backoff_seconds < 0
                ):
                    _RaiseHTTPException(
                ...
                # Max Doublings
                if (
                    queue.retry_parameters.max_doublings
                    and queue.retry_parameters.max_doublings < 0
                ):
                    _RaiseHTTPException(
                ...
                # Min & Max backoff comparison
                if (
                    queue.retry_parameters.min_backoff_seconds is not None
                    and queue.retry_parameters.max_backoff_seconds is not None
                ):
...
def DeployQueuesYamlFile(
    tasks_api, config, all_queues_in_db_dict, ct_api_version=base.ReleaseTrack.BETA
):
  ...
  """
    ...
    for queue in queue_yaml.queue:
        ...
        cloud_task_args = _PopulateCloudTasksArgs(
            queue, cur_queue_object, expected_args
        )
        rate_to_set = cloud_task_args.GetValue("max_dispatches_per_second")

        if (
            resume_paused_queues
            and cur_queue_object
            and (rate_to_set or queue.mode == constants.PULL_QUEUE)
            and cur_queue_object.state
            in (cur_queue_object.state.DISABLED, cur_queue_object.state.PAUSED)
        ):
        ...
        app_engine_routing_override = (
            ...
            if queue_config.appEngineHttpQueue is not None
            else None
        )
...
def _CreateUniqueJobKeyForExistingJob(job, project):
  ...
  """
    return (
        ...
        job.description,
        convertors.CheckAndConvertStringToFloatIfApplicable(
            job.retryConfig.minBackoffDuration
        )
        if job.retryConfig
        else None,
        convertors.CheckAndConvertStringToFloatIfApplicable(
            job.retryConfig.maxBackoffDuration
        )
        if job.retryConfig
        else None,
        job.retryConfig.maxDoublings if job.retryConfig else None,
        convertors.CheckAndConvertStringToFloatIfApplicable(
            job.retryConfig.maxRetryDuration
        )
        if job.retryConfig
        else None,
...
def _ReplaceDefaultRetryParamsForYamlJob(job):
  ...
  """
    ...
    if retry_data:
        ...
        if (
            retry_data.min_backoff_seconds is None
            and retry_data.max_backoff_seconds is None
        ):
            ...
            retry_data.min_backoff_seconds is None
            or retry_data.max_backoff_seconds is None
        ):
            ...
            if retry_data.max_backoff_seconds:
                retry_data.min_backoff_seconds = min(
                    retry_data.min_backoff_seconds, retry_data.max_backoff_seconds
                )
            if retry_data.max_backoff_seconds is None:
                retry_data.max_backoff_seconds = defaults["max_backoff"]
            retry_data.max_backoff_seconds = max(
                retry_data.min_backoff_seconds, retry_data.max_backoff_seconds
            )
...
def _CreateUniqueJobKeyForYamlJob(job):
  ...
  """
    ...
    return (
        ...
        convertors.CheckAndConvertStringToFloatIfApplicable(retry_params.job_age_limit)
        if retry_params
        else None,
