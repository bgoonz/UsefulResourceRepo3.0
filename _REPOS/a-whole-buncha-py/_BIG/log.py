...
STRUCTURED_RECORD_FIELDS = [
    ("version", "version", "Semantic version of the message format. E.g. v0.0.1"),
    (
        "verbosity",
        "levelname",
...
REQUIRED_STRUCTURED_RECORD_FIELDS = OrderedDict(
    (x[:2] for x in STRUCTURED_RECORD_FIELDS)
)
...
class _ConsoleFormatter(logging.Formatter):
    ...
    def __init__(self, out_stream):
        ...
        self._formats = (
            _ConsoleFormatter.COLOR_FORMATS if use_color else _ConsoleFormatter.FORMATS
        )
    ...
    def GetErrorDict(self, log_record):
        ...
        if log_record.exc_info:
            ...
            if issubclass(type(log_record.msg), BaseException):
                ...
                error_dict["stacktrace"] = getattr(
                    log_record.msg, "__traceback__", None
                )
...
class _ConsoleLoggingFormatterMuxer(logging.Formatter):
    ...
    def ShowStructuredOutput(self):
        ...
        if any(
            [
                show_messages == "terminal" and self.terminal,
...
class _LogManager(object):
  ...
  """
    ...
    def __init__(self):
        ...
        self.stdout_writer = _ConsoleWriter(
            self.file_only_logger, self._user_output_filter, self.stdout_stream_wrapper
        )
        self.stderr_writer = _ConsoleWriter(
            self.file_only_logger,
    ...
    def Reset(self, stdout, stderr):
        ...
        console_formatter = _ConsoleLoggingFormatterMuxer(
            json_formatter, self.stderr_writer, default_formatter=std_console_formatter
        )
    ...
    """
        ...
        try:
            ...
            warning(
                "Could not setup log file in {0}, ({1}: {2}.\n"
                "The configuration directory may not be writable. "
                "To learn more, see "
                "https://cloud.google.com/sdk/docs/configurations#"
                "creating_a_configuration".format(logs_dir, type(exp).__name__, exp)
            )
            return
...
def WriteToFileOrStdout(
    path, content, overwrite=True, binary=False, private=False, create_path=False
):
  ...
  """
    if path == "-":
        ...
        files.WriteBinaryFileContents(
            path, content, overwrite=overwrite, private=private, create_path=create_path
        )
    else:
...
def GetLogFileVerbosity():
  ...
  """
    return _GetEffectiveVerbosity(_log_manager.file_only_logger.getEffectiveLevel())
...
def _PrintResourceChange(
    operation, resource, kind, is_async, details, failed, operation_past_tense=None
):
...
def DeletedResource(resource, kind=None, is_async=False, details=None, failed=None):
...
def DetachedResource(resource, kind=None, is_async=False, details=None, failed=None):
...
def RestoredResource(resource, kind=None, is_async=False, details=None, failed=None):
...
def UpdatedResource(resource, kind=None, is_async=False, details=None, failed=None):
...
def ResetResource(resource, kind=None, is_async=False, details=None, failed=None):
  ...
  """
    _PrintResourceChange(
        "reset", resource, kind, is_async, details, failed, operation_past_tense="reset"
    )


def ExportResource(resource, kind=None, is_async=False, details=None, failed=None):
