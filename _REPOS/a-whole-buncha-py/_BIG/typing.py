...
def _type_check(arg, msg, is_argument=True):
    ...
    if (
        isinstance(arg, _SpecialForm)
        and arg not in (Any, NoReturn)
...
class _GenericAlias(_Final, _root=True):
    ...
    def __repr__(self):
        if (
            self._name != "Callable"
            or len(self.__args__) == 2
            and self.__args__[0] is Ellipsis
        ):
            if self._name:
    ...
    def __mro_entries__(self, bases):
        if self._name:  # generic version of an ABC or built-in class
            ...
            i = bases.index(self)
            if not any(
                isinstance(b, _GenericAlias) or issubclass(b, Generic)
                for b in bases[i + 1 :]
            ):
                res.append(Generic)
            return tuple(res)
    ...
    def __getitem__(self, params):
        ...
        if args is Ellipsis:
            ...
            if not isinstance(args, list):
                raise TypeError(
                    f"Callable[args, result]: args must be a list." f" Got {args}"
                )
    ...
    def __new__(*args, **kwargs):
        ...
        if args:
            try:
                ...
                raise TypeError(
                    f"NamedTuple.__new__() takes from 2 to 3 "
