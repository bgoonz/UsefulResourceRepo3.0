...
class ConfigureGKEDeploy(base.Command):
  ...
  """
    ...
    """
        ...
        parser.add_argument(
            "--repo-name", help="Name of the repository.", required=True
        )
        parser.add_argument("--repo-owner", help="Owner of the repository.")
        parser.add_argument(
            "--dockerfile",
            help="""
        ...
        parser.add_argument(
            "--cluster", help="Name of the target cluster to deploy to.", required=True
        )
        parser.add_argument(
            "--location",
        ...
        if args.repo_type == "github":
            if not args.repo_owner:
                raise c_exceptions.RequiredArgumentException(
                    "--repo-owner", "Repo owner is required for --repo-type=github."
                )
    ...
    def _VerifyCSRRepoExists(self, csr_repo_name):
        try:
            ...
            if csr_repo.mirrorConfig:
                log.error(
                    "CSR repo '{}' has mirrorConfig.url of {}",
                    csr_repo_name,
            ...
            raise c_exceptions.InvalidArgumentException(
                "--repo-name", "Repo '{}' is not found on CSR.".format(csr_repo_name)
            )

    def _VerifyBitbucketCSRRepoExists(
        self, csr_repo_name, bitbucket_repo_owner, bitbucket_repo_name
    ):
        try:
            csr_repo_ref = sourcerepo.ParseRepo(csr_repo_name)
            csr_repo = sourcerepo.Source().GetRepo(csr_repo_ref)
            ...
            if csr_repo.mirrorConfig and not csr_repo.mirrorConfig.url.startswith(
                "https://bitbucket.org/"
            ):
                log.error(
                    "CSR repo '{}' has mirrorConfig.url of {}",
                ...
                raise c_exceptions.InvalidArgumentException(
                    ...
                    "--repo-type.".format(
                        bitbucket_repo_owner, bitbucket_repo_name, csr_repo_name
                    ),
                )
        except HttpNotFoundError:
    ...
    def _VerifyGitHubCSRRepoExists(
        self, csr_repo_name, github_repo_owner, github_repo_name
    ):
        try:
            csr_repo_ref = sourcerepo.ParseRepo(csr_repo_name)
            csr_repo = sourcerepo.Source().GetRepo(csr_repo_ref)
            ...
            if csr_repo.mirrorConfig and not csr_repo.mirrorConfig.url.startswith(
                ...
                raise c_exceptions.InvalidArgumentException(
                    ...
                    "--repo-type.".format(
                        github_repo_owner, github_repo_name, csr_repo_name
                    ),
                )
        except HttpNotFoundError:
            raise SourceRepoNotConnectedException(
    ...
    def _VerifyClusterExists(self, cluster, location):
        ...
        if cluster_res.status != messages.Cluster.StatusValueValuesEnum.RUNNING:
            raise core_exceptions.Error(
                "Cluster was found but status is not RUNNING. Status is {}.".format(
                    cluster_res.status
                )
            )
        ...
        try:
            return client.projects_triggers.Get(
                messages.CloudbuildProjectsTriggersGetRequest(
                    projectId=project, triggerId=trigger_name
                )
            )  # Undocumented, but this field can be ID
            # or name.
        except HttpNotFoundError:
            return None
    ...
    """
        ...
        if existing:
            ...
            upserted_build_trigger = client.projects_triggers.Patch(
                messages.CloudbuildProjectsTriggersPatchRequest(
                    buildTrigger=build_trigger, projectId=project, triggerId=trigger_id
                )
            )
            log.debug(
                "updated existing BuildTrigger: "
            ...
            log.debug("created BuildTrigger: " + six.text_type(upserted_build_trigger))
        ...
        build_trigger_ref = resources.REGISTRY.Parse(
            ...
            api_version="v1",
            params={"projectId": project, "triggerId": trigger_id},
        )
    ...
    def _ConfigureGitPushBuildTrigger(
        ...
        name = self._FixBuildTriggerName(
    ...
    * A CloudScheduler Job that executes the BuildTrigger every day. This is
    ...
    """
        ...
        log.status.Print(
            ...
            "this job if you don't want to it to run anymore.".format(
    ...
    def _UpdateBuildTriggerWithSchedulerJobTags(
        ...
        build_trigger_tags = [
            x for x in build_trigger.tags if not x.startswith("cloudscheduler-job-")
        ]
        ...
        build_tags = [
            x
            for x in build_trigger.build.tags
    ...
    """
        ...
        if branch_pattern:
            ...
            return "{}{}b-{}".format(function_code, repo_type_code, full_repo_name)
    ...
    def _ConfigurePRPreviewBuildTrigger(
        ...
        description = 'Build and deploy on PR create/update against "{}"'.format(
            pull_request_pattern
        )
        ...
        log.status.Print(
            "Upserting Cloud Build trigger to build and deploy your "
    ...
    def _ConfigureCleanPreviewBuildTrigger(
        self, repo_name, repo_owner, pull_request_pattern, cluster, location, app_name
    ):
        ...
        description = 'Clean expired preview deployments for PRs against "{}"'.format(
            pull_request_pattern
        )
        ...
        log.status.Print(
            "Upserting Cloud Build trigger to clean expired preview "
    ...
    def _ConfigureCleanPreviewSchedulerJob(
        ...
        job = messages.Job(
            ...
            httpTarget=messages.HttpTarget(
                uri="https://cloudbuild.googleapis.com/v1/projects/{}/triggers/{}:run".format(
                    project, clean_preview_trigger_id
                ),
                ...
                body=bytes(
                    ...
                    '{{"projectId":"{}","repoName":"{}","branchName":"master"}}'.format(
                        project, repo_name
                    ).encode("utf-8")
                ),
        ...
        try:
            ...
            log.debug(
                "updated existing CloudScheduler job: " + six.text_type(upserted_job)
            )
