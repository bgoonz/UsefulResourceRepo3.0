  ...
  """
    if expiration is None:
        ...
        entry = "{user}:{public_key} google-ssh {jsondict}".format(
            ...
            jsondict=json.dumps(
                collections.OrderedDict([("userName", user), ("expireOn", expire_on)])
            ).replace(" ", ""),
        )
...
class BaseSSHHelper(object):
  ...
  """
    ...
    """
        ...
        new_metadata = _AddSSHKeyToMetadataMessage(
            client.messages, user, public_key, existing_metadata, expiration=expiration
        )
...
def AddVerifyInternalIpArg(parser):
    parser.add_argument(
        "--verify-internal-ip",
        action=actions.StoreBooleanProperty(properties.VALUES.ssh.verify_internal_ip),
        hidden=True,
...
class BaseSSHCLIHelper(BaseSSHHelper):
    ...
    def PreliminarilyVerifyInstance(self, instance_id, remote, identity_file, options):
    ...
    """
        ...
        remote_command = [
            '[ `curl "{}" -H "Metadata-Flavor: Google" -q` = {} ] || exit 23'.format(
                metadata_id_url, instance_id
            )
        ]
        cmd = ssh.SSHCommand(
            remote,
