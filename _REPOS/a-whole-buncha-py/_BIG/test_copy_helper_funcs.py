...
_CalculateB64EncodedMd5FromContents = hashing_helper.CalculateB64EncodedMd5FromContents
...
class TestCpFuncs(GsUtilUnitTestCase):
    ...
    def testFilterExistingComponentsNonVersioned(self):
        ...
        object_wrong_contents_url = StorageUrlFromString(
            "%s://%s/%s" % (self.default_provider, bucket_name, fpath_wrong_contents)
        )
        ...
        mock_api.MockCreateObjectWithMetadata(
            apitools_messages.Object(bucket=bucket_name, name="foo6", md5Hash=file_md5),
            contents=b"6",
        )
        ...
        bucket_url = StorageUrlFromString(
            "%s://%s" % (self.default_provider, bucket_name)
        )
    ...
    def testFilterExistingComponentsVersioned(self):
        ...
        existing_components = [
            ObjectFromTracker(
                fpath_uploaded_correctly, object_uploaded_correctly_url.generation
            ),
            ObjectFromTracker(
                fpath_duplicate, duplicate_uploaded_correctly_url.generation
            ),
        ...
        expected_to_delete = [
            (
                args_wrong_contents.dst_url.object_name,
                args_wrong_contents.dst_url.generation,
            ),
            (args_duplicate.dst_url.object_name, args_duplicate.dst_url.generation),
        ]
    ...
    @mock.patch(
        "time.time",
        new=mock.MagicMock(return_value=posix_util.ConvertDatetimeToPOSIX(_PI_DAY)),
    )
    def testWarnIfMvEarlyDeletionChargeApplies(self):
        ...
        for object_time_created in (
            self._PI_DAY,
            self._PI_DAY - datetime.timedelta(days=29, hours=23),
        ):
            ...
            with mock.patch.object(test_logger, "warn") as mocked_warn:
                WarnIfMvEarlyDeletionChargeApplies(
                    src_url, recent_nearline_obj, test_logger
                )
        ...
        for object_time_created in (
            self._PI_DAY,
            self._PI_DAY - datetime.timedelta(days=89, hours=23),
        ):
            recent_nearline_obj = apitools_messages.Object(
        ...
        for object_time_created in (
            self._PI_DAY,
            self._PI_DAY - datetime.timedelta(days=364, hours=23),
        ):
            ...
            with mock.patch.object(test_logger, "warn") as mocked_warn:
                WarnIfMvEarlyDeletionChargeApplies(
                    src_url, recent_archive_obj, test_logger
                )
                mocked_warn.assert_called_with(
                    "Warning: moving %s object %s may incur an early deletion "
                    "charge, because the original object is less than %s days old "
        ...
        with mock.patch.object(test_logger, "warn") as mocked_warn:
            ...
            WarnIfMvEarlyDeletionChargeApplies(
                src_url, not_old_enough_nearline_obj, test_logger
            )
            mocked_warn.assert_not_called()
...
class TestExpandUrlToSingleBlr(GsUtilUnitTestCase):
    ...
    def testContainsWildcardMatchesNotObject(
        self, mock_CreateWildcardIterator, mock_gsutil_api
    ):
    ...
    @mock.patch("gslib.cloud_api.CloudApi")
    @mock.patch("gslib.utils.copy_helper.CreateWildcardIterator")
    def testContainsWildcardMatchesObject(
        self, mock_CreateWildcardIterator, mock_gsutil_api
    ):
        storage_url = StorageUrlFromString("gs://test/helloworld")
        mock_CreateWildcardIterator.return_value = iter(
    ...
    @mock.patch("gslib.cloud_api.CloudApi")
    @mock.patch("gslib.utils.copy_helper.CreateWildcardIterator")
    def testContainsWildcardNoMatches(
        self, mock_CreateWildcardIterator, mock_gsutil_api
    ):
        mock_CreateWildcardIterator.return_value = iter([])
        with self.assertRaises(CommandException):
    ...
    def testNoSlashPrefixExactMatch(self, mock_gsutil_api):
        mock_gsutil_api.ListObjects.return_value = iter(
            [CloudApi.CsObjectOrPrefix("folder/", CloudApi.CsObjectOrPrefixType.PREFIX)]
        )
        (exp_url, have_existing_dst_container) = ExpandUrlToSingleBlr(
            "gs://test/folder",
            mock_gsutil_api,
        ...
        mock_gsutil_api.ListObjects.return_value = iter(
            [
                CloudApi.CsObjectOrPrefix(
                    "folderone/", CloudApi.CsObjectOrPrefixType.PREFIX
                )
            ]
        )
        (exp_url, have_existing_dst_container) = ExpandUrlToSingleBlr(
