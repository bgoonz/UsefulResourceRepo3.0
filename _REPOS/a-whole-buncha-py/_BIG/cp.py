  ...
  """
    ...
    def CopyFunc(self, copy_object_info, thread_state=None, preserve_posix=False):
        ...
        if copy_helper_opts.perform_mv:
            if copy_object_info.names_container:
                ...
                if ContainsWildcard(src_url.url_string):
                    raise CommandException(
                        "The mv command disallows naming source "
                        "directories using wildcards"
                    )

        if (
            copy_object_info.exp_dst_url.IsFileUrl()
        ...
        if dst_url.IsCloudUrl() and dst_url.HasGeneration():
            raise CommandException(
                "%s: a version-specific URL\n(%s)\ncannot be "
                "the destination for gsutil cp - abort." % (cmd_name, dst_url)
            )

        if not dst_url.IsCloudUrl() and copy_helper_opts.dest_storage_class:
            raise CommandException(
                "Cannot specify storage class for a non-cloud "
        ...
        if src_obj_metadata and dst_url.IsFileUrl():
            ...
            valid, err = ValidateFilePermissionAccess(
                src_url.url_string, uid=posix_attrs.uid, gid=posix_attrs.gid, mode=mode
            )
        ...
        try:
            ...
            if copy_helper_opts.no_clobber and copy_helper.IsNoClobberServerException(
                e
            ):
    ...
    def _ConstructNameExpansionIteratorDstTupleIterator(
        self, src_url_strs_iter, dst_url_strs
    ):
        copy_helper_opts = copy_helper.GetCopyHelperOpts()
        for src_url_str, dst_url_str in zip(src_url_strs_iter, dst_url_strs):
            ...
            exp_dst_url, have_existing_dst_container = copy_helper.ExpandUrlToSingleBlr(
            ...
            name_expansion_iterator_dst_tuple = NameExpansionIteratorDestinationTuple(
                NameExpansionIterator(
                    ...
                    continue_on_error=(
                        self.continue_on_error or self.parallel_operations
                    ),
            ...
            self.combined_src_urls = itertools.chain(
                self.combined_src_urls, src_url_str
            )
    ...
    def RunCommand(self):
        ...
        if dst_url.IsFileUrl() and (dst_url.object_name == "-" or dst_url.IsFifo()):
            if self.preserve_posix_attrs:
                raise CommandException(
                    "Cannot preserve POSIX attributes with a " "stream or a named pipe."
                )
            cat_out_fd = (
                GetStreamFromFileUrl(dst_url, mode="wb") if dst_url.IsFifo() else None
            )
        ...
        if self.op_failure_count:
            ...
            raise CommandException(
                ...
                "not be transferred.".format(count=self.op_failure_count, pl=plural_str)
            )
