...
IGNORED_FIELDS = {"project": "project", "projectId": "project", "projectsId": "project"}
    ...
    def __init__(
        ...
        if re.search(r"[A-Z]", name) and re.search("r[a-z]", name):
            raise ValueError(
                ...
                "snake case (foo_bar) so they can be transformed to flag names.".format(
                    name
                )
            )
        self.name = name
...
class Attribute(_Attribute):
  ...
  """
    ...
    def __init__(
        self, name, completion_request_params=None, completion_id_field=None, **kwargs
    ):
        """Initializes."""
    ...
    def __eq__(self, other):
        ...
        return (
            ...
            and self.completer == other.completer
            and self.completion_request_params == other.completion_request_params
...
class ResourceSpec(ConceptSpec):
  ...
  """
    ...
    def __init__(
        ...
        api_version=None,
        disable_auto_completers=disable_auto_complete,
        plural_name=None,
        **kwargs
    ):
    ...
    """
        ...
        for i, param_name in enumerate(collection_params):
            if i == len(collection_params) - 1:
                anchor = True
            attribute_config = kwargs.pop(
                param_name, ResourceParameterAttributeConfig()
            )
            attribute_name = self._AttributeName(
                param_name, attribute_config, anchor=anchor
            )
        ...
        if not self._attributes:
            raise ResourceConfigurationError(
                "Resource [{}] has no parameters; no "
                "arguments will be generated".format(self._name)
            )
        if kwargs:
            raise ResourceConfigurationError(
                "Resource [{}] was given an attribute "
                "config for unknown attribute(s): "
                "Expected [{}], Found [{}]".format(
                    self._name, ", ".join(collection_params), ", ".join(orig_kwargs)
                )
            )
    ...
    """
        ...
        self._resources.RegisterApiByName(
    ...
    """
        ...
        results = self._ParseFromPluralValue(
            attribute_to_args_map, base_fallthroughs_map, self.anchor, parsed_args
        )
        if results:
            return results
    ...
    """
        ...
        for attribute in self.attributes:
            fallthroughs_map[
                attribute.name
            ] = self.GetArgAndBaseFallthroughsForAttribute(
                attribute_to_args_map, base_fallthroughs_map, attribute, plural=plural
            )
    ...
    def GetArgAndBaseFallthroughsForAttribute(
        self, attribute_to_args_map, base_fallthroughs_map, attribute, plural=False
    ):
    ...
    def __eq__(self, other):
        return (
            super(ResourceSpec, self).__eq__(other)
...
class ResourceParameterAttributeConfig(object):
    ...
    """
        ...
        if completer and (completion_request_params or completion_id_field):
            raise ValueError(
                "Custom completer and auto-completer should not be "
...
DEFAULT_PROJECT_ATTRIBUTE_CONFIG = ResourceParameterAttributeConfig(
    name="project",
    help_text="Project ID of the Google Cloud Platform project for " "the {resource}.",
    fallthroughs=[
...
DEFAULT_RESOURCE_ATTRIBUTE_CONFIGS = {"project": DEFAULT_PROJECT_ATTRIBUTE_CONFIG}
