...
class OperationCancelledError(Error):
    ...
    def __init__(self, message=None):
        super(OperationCancelledError, self).__init__(message or self.DEFAULT_MESSAGE)
...
def CanPrompt():
  ...
  """
    return (
        IsInteractive(error=True)
        and not properties.VALUES.core.disable_prompts.GetBool()
    )
...
def _NormalPromptContinuePromptGenerator(
    message, prompt_string, default, throw_if_unattended, cancel_on_no, cancel_string
):
...
def PromptChoice(
  ...
  """
    ...
    if maximum > PROMPT_OPTIONS_OVERFLOW:
        ...
        write(
            'Too many options [{maximum}]. Enter "list" at prompt to print '
    ...
    while True:
        ...
        if allow_freeform and freeform_suggester:
            ...
            if suggestion is not None:
                write(
                    "[{answer}] not in list. Did you mean [{suggestion}]?".format(
                        answer=answer, suggestion=suggestion
                    )
                )
                write("\n")
        ...
        if allow_freeform:
            write(
                "Please enter a value between 1 and {maximum}, or a value present "
                "in the list:  ".format(maximum=maximum)
            )
        else:
            write(
                "Please enter a value between 1 and {maximum}:  ".format(
                    maximum=maximum
                )
            )


def PromptWithValidator(validator, error_message, prompt_string, message=None):
...
def ProgressBar(
    label, stream=log.status, total_ticks=60, first=True, last=True, screen_reader=False
):
...
class _NormalProgressBar(object):
  ...
  """
    ...
    """
        ...
        self._redraw = (
            self._box.d_dr != self._box.d_vr or self._box.d_dl != self._box.d_vl
        )
...
def JsonUXStub(ux_type, **kwargs):
    ...
    if extra_args:
        raise ValueError(
            "Extraneous args for Ux Element {}: {}".format(ux_type.name, extra_args)
        )
