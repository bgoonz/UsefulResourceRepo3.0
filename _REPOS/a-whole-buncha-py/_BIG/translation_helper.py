...
GOOG_METAGENERATION_MATCH_REGEX = re.compile(r"^x-goog-if-metageneration-match", re.I)
CUSTOM_GOOG_METADATA_REGEX = re.compile(r"^x-goog-meta-(?P<header_key>.*)", re.I)
    ...
    for header, value in headers.items():
        if CACHE_CONTROL_REGEX.match(header):
            ...
            if custom_goog_metadata_match:
                ...
                header_key = S3_HEADER_PREFIX + custom_amz_header_match.group(
                    "header_key"
                )
            if header_key:
        ...
        for additional_property in dst_obj_metadata.metadata.additionalProperties:
            ...
            if provider == "gs":
                ...
                if additional_property.key.startswith(S3_HEADER_PREFIX):
                    header_name = (
                        "x-amz-" + additional_property.key[len(S3_HEADER_PREFIX) :]
                    )
                else:
...
def PreconditionsFromHeaders(headers):
  ...
  """
    ...
    try:
        ...
        raise ArgumentException(
            "Invalid precondition header specified. "
...
def CreateBucketNotFoundException(code, provider, bucket_name):
    return BucketNotFoundException(
        "%s://%s bucket does not exist." % (provider, bucket_name),
        bucket_name,
        status=code,
    )


def CreateObjectNotFoundException(
    code, provider, bucket_name, object_name, generation=None
):
...
def CheckForXmlConfigurationAndRaise(config_type_string, json_txt):
    ...
    try:
        ...
        pass
    raise ArgumentException(
        "JSON %s data could not be loaded " "from: %s" % (config_type_string, json_txt)
    )
            ...
            for rule_message in lifecycle_message.rule:
                ...
                if rule_message.condition:
                    ...
                    if rule_message.condition.matchesStorageClass:
                        boto_rule.conditions[
                            boto.gs.lifecycle.MATCHES_STORAGE_CLASS
                        ] = [
        ...
        if boto_lifecycle:
            ...
            for boto_rule in boto_lifecycle:
                ...
                lifecycle_rule.condition = (
                    apitools_messages.Bucket.LifecycleValue.RuleValueListEntry.ConditionValue()
                )
                if boto_rule.action:
                    if boto_rule.action == boto.gs.lifecycle.DELETE:
                        lifecycle_rule.action = apitools_messages.Bucket.LifecycleValue.RuleValueListEntry.ActionValue(
                            type="Delete"
                        )
                    elif boto_rule.action == boto.gs.lifecycle.SET_STORAGE_CLASS:
                        lifecycle_rule.action = apitools_messages.Bucket.LifecycleValue.RuleValueListEntry.ActionValue(
                            type="SetStorageClass", storageClass=boto_rule.action_text
                        )
                ...
                if boto.gs.lifecycle.CREATED_BEFORE in boto_rule.conditions:
                    lifecycle_rule.condition.createdBefore = LifecycleTranslation.TranslateBotoLifecycleTimestamp(
                        boto_rule.conditions[boto.gs.lifecycle.CREATED_BEFORE]
                    )
                if boto.gs.lifecycle.IS_LIVE in boto_rule.conditions:
                    boto_is_live_str = boto_rule.conditions[
                        boto.gs.lifecycle.IS_LIVE
                    ].lower()
                ...
                if boto.gs.lifecycle.MATCHES_STORAGE_CLASS in boto_rule.conditions:
                    for storage_class in boto_rule.conditions[
                        boto.gs.lifecycle.MATCHES_STORAGE_CLASS
                    ]:
                        lifecycle_rule.condition.matchesStorageClass.append(
                            storage_class
                        )
                    ...
                    for element_tuple in cors_collection:
                        ...
                        if element_tuple[0] == boto.gs.cors.HEADERS:
                            for header_tuple in element_tuple[1]:
                                collection_message.responseHeader.append(
                                    header_tuple[1]
                                )
...
def S3MarkerAclFromObjectMetadata(object_metadata):
  ...
  """
    if (
        object_metadata
        and object_metadata.metadata
        and object_metadata.metadata.additionalProperties
    ):
...
class LabelTranslation(object):
  ...
  """
    ...
    def JsonFromMessage(cls, message, pretty_print=False):
        ...
        if pretty_print:
            return json.dumps(
                json.loads(json_str), sort_keys=True, indent=2, separators=(",", ": ")
            )
        return json_str

    @classmethod
    def DictToMessage(cls, label_dict):
        return encoding.DictToMessage(label_dict, apitools_messages.Bucket.LabelsValue)
  ...
  """
    ...
    def BotoObjectAclToMessage(cls, acl):
        for entry in cls.BotoAclToJson(acl):
            message = encoding.DictToMessage(
                entry, apitools_messages.ObjectAccessControl
            )
    ...
    def BotoBucketAclToMessage(cls, acl):
        for entry in cls.BotoAclToJson(acl):
            message = encoding.DictToMessage(
                entry, apitools_messages.BucketAccessControl
            )
    ...
    def BotoEntryFromJson(cls, entry_json):
        ...
        if entity.lower() == ALL_USERS.lower():
            ...
            raise CommandException(
                "XML API does not support project scopes, " "cannot translate ACL."
            )
            ...
            return Entry(
                type=scope_type, id=entry_json["entityId"], permission=permission
            )
        elif "domain" in entry_json:
            if entity.startswith("domain"):
                scope_type = GROUP_BY_DOMAIN
            return Entry(
                type=scope_type, domain=entry_json["domain"], permission=permission
            )
    ...
    def BotoEntryToJson(cls, entry):
        ...
        if scope_type_lower == ALL_USERS.lower():
            ...
            raise ArgumentException(
                "ACL contains invalid scope type: %s" % scope_type_lower
            )
    ...
    """
        ...
        return json.dumps(
            serializable_acl, sort_keys=True, indent=2, separators=(",", ": ")
        )
