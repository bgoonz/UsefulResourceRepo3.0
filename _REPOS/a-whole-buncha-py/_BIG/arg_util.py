...
def GetSetOfAllTestArgs(type_rules, shared_rules):
  ...
  """
    all_test_args_list = (
        ...
        + list(shared_rules["defaults"].keys())
    )
    for type_dict in type_rules.values():
        all_test_args_list += (
            type_dict["required"]
            + type_dict["optional"]
            + list(type_dict["defaults"].keys())
        )
  ...
  """
    for arg in lower_pri_args:
        if getattr(args, arg, None) is None:
            ...
            log.warning(
                'Command-line argument "--{0} {1}" overrides file argument "{2}: {3}"'.format(
                    ext_name,
                    _FormatArgValue(getattr(args, arg)),
                    ext_name,
