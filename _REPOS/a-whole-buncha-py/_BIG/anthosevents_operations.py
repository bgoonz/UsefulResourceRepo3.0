...
def Connect(conn_info):
  ...
  """
    ...
    v1beta1_client = apis_internal._GetClientInstance(
        ...
        if conn_info.supports_one_platform
        else None,
...
class TimeLockedUnfailingConditionPoller(serverless_operations.ConditionPoller):
  ...
  """
    ...
    def __init__(
        self, getter, tracker, dependencies=None, grace_period=_POLLING_GRACE_PERIOD
    ):
        super(TimeLockedUnfailingConditionPoller, self).__init__(
    ...
    def _PossiblyFailStage(self, condition, message):
        ...
        if self._HasGracePeriodPassed():
            # pylint:disable=protected-access
            return super(TimeLockedUnfailingConditionPoller, self)._PossiblyFailStage(
                condition, message
            )
...
class AnthosEventsOperations(object):
    ...
    def __init__(self, api_version, region, v1_client, v1alpha1_client, v1beta1_client):
    ...
    def GetNamespace(self, namespace_ref):
        ...
        request = self._core_client.MESSAGES_MODULE.AnthoseventsApiV1NamespacesGetRequest(
    ...
    def CreateNamespace(self, namespace_ref):
        ...
        request = self._core_client.MESSAGES_MODULE.AnthoseventsApiV1NamespacesCreateRequest(
    ...
    def ListNamespaces(self):
        ...
        request = (
            self._core_client.MESSAGES_MODULE.AnthoseventsApiV1NamespacesListRequest()
        )
    ...
    """
        ...
        if source_obj is not None:
            ...
            trigger_obj.filter_attributes[
                trigger.SOURCE_TRIGGER_LINK_FIELD
        ...
        request = messages.AnthoseventsNamespacesTriggersCreateRequest(
            trigger=trigger_obj.Message(), parent=trigger_ref.Parent().RelativeName()
        )
        try:
    ...
    def PollTrigger(self, trigger_ref, tracker):
        ...
        poller = TriggerConditionPoller(
            trigger_getter, tracker, stages.TriggerSourceDependencies()
        )
    ...
    def PollSource(self, source_obj, event_type_obj, tracker):
        ...
        source_ref = util.GetSourceRef(
            source_obj.name, source_obj.namespace, event_type_obj.crd, True
        )
        source_getter = functools.partial(
            self.GetSource, source_ref, event_type_obj.crd
        )
    ...
    def GetCustomResourceDefinition(self, crd_name):
        ...
        return custom_resource_definition.CustomResourceDefinition(response, messages)
    ...
    def ListSourceCustomResourceDefinitions(self):
        ...
        with metrics.RecordDuration(metric_names.LIST_SOURCE_CRDS):
            response = self._crd_client.customresourcedefinitions.List(request)
        source_crds = [
            custom_resource_definition.SourceCustomResourceDefinition(item, messages)
            for item in response.items
        ]
    ...
    Returns:
      None
    """
        control_plane_namespace = events_constants.ControlPlaneNamespaceFromProductType(
            product_type
        )
        ...
        secret_obj = secret.Secret.New(self._core_client, secret_ref.Parent().Name())
        ...
        try:
            ...
            request = messages.AnthoseventsApiV1NamespacesSecretsCreateRequest(
                secret=secret_obj.Message(), parent=secret_ref.Parent().RelativeName()
            )
    ...
    def CreateOrReplaceServiceAccountSecret(self, secret_ref, service_account_ref):
    ...
    def GetServiceAccount(self, k8s_service_account_ref):
        ...
        try:
            ...
            response = self._core_client.api_v1_namespaces_serviceaccounts.Get(request)
        except api_exceptions.HttpNotFoundError:
            return None
    ...
    def ReplaceServiceAccount(self, k8s_service_account_ref, service_account_obj):
        ...
        try:
            request = messages.AnthoseventsApiV1NamespacesServiceaccountsReplaceServiceAccountRequest(
                name=k8s_service_account_ref.RelativeName(),
                serviceAccount=service_account_obj.Message(),
            )
            response = self._core_client.api_v1_namespaces_serviceaccounts.ReplaceServiceAccount(
                request
            )
        except api_exceptions.HttpNotFoundError:
    ...
    def AnnotateServiceAccount(
        self, k8s_service_account_ref, annotation_key, annotation_value
    ):
        ...
        if service_account_obj.metadata.annotations:
            for (
                additional_property
            ) in service_account_obj.metadata.annotations.additionalProperties:
                annotations_map[additional_property.key] = additional_property.value

        annotations_map[annotation_key] = annotation_value

        arg_utils.SetFieldInMessage(
            service_account_obj.Message(), "metadata.annotations", annotations_map
        )

        return self.ReplaceServiceAccount(k8s_service_account_ref, service_account_obj)
    ...
    def IsClusterInitialized(self, product_type):
        ...
        if configmap_obj is None:
            return False
        return configmap_obj.annotations.get(_CLUSTER_INITIALIZED_ANNOTATION) == "true"
    ...
    """
        ...
        if configmap_obj is None:
            configmap_obj = configmap.ConfigMap.New(
                self._core_client, control_plane_namespace
            )
    ...
    def _PopulateDefaultAuthConfig(self, configmap_obj, cluster_defaults):
        ...
        auth_config = yaml.load(configmap_obj.data.get("default-auth-config", "{}"))
    ...
    def _CreateConfigMap(self, configmap_obj):
        ...
        request = messages.AnthoseventsApiV1NamespacesConfigmapsCreateRequest(
            configMap=configmap_obj.Message(), parent=ref.Parent().RelativeName()
        )
    ...
    def _ReplaceConfigMap(self, configmap_obj):
        ...
        request = messages.AnthoseventsApiV1NamespacesConfigmapsReplaceConfigMapRequest(
            configMap=configmap_obj.Message(), name=ref.RelativeName()
        )
    ...
    def GetKubeRun(self):
        ...
        messages = self._operator_client.MESSAGES_MODULE
        request = messages.AnthoseventsKuberunsGetRequest(name=_KUBE_RUN_RELATIVE_NAME)
        try:
    ...
    def UpdateCloudRunWithEventingEnabled(self):
        ...
        arg_utils.SetFieldInMessage(cloud_run_message, "spec.eventing.enabled", True)
    ...
    def PollCloudRunResource(self, tracker):
        ...
        util.WaitForCondition(
            poller, exceptions.EventingInstallError, util.OPERATOR_MAX_WAIT_MS
        )
