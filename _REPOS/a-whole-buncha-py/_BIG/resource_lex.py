...
class _TransformCall(object):
  ...
  """
    ...
    def __str__(self):
        args = [
            "<projecton>"
            if isinstance(arg, resource_projection_spec.ProjectionSpec)
            else arg
  ...
  """
    ...
    """
        if len(self._transforms) != 1 or (
            self._transforms[0].name != resource_projection_spec.GLOBAL_RESTRICTION_NAME
        ):
            return None
    ...
    def Evaluate(self, obj):
        ...
        for transform in self._transforms:
            if transform.map_transform and resource_property.IsListLike(obj):
                ...
                for item in items:
                    obj.append(
                        transform.func(item, *transform.args, **transform.kwargs)
                    )
        ...
        while not self.EndOfInput(i):
            ...
            if c == self._ESCAPE and not self.EndOfInput(i + 1):
                ...
                if (
                    c != self._ESCAPE
                    and c != quote
                    and (quote or c not in self._QUOTES)
                ):
    ...
    """
        ...
        while True:
            transform = self._ParseTransform(
                func_name, active=active, map_transform=map_transform
            )
            ...
            if not self.IsCharacter("("):
                raise resource_exceptions.ExpressionSyntaxError(
                    "Transform function expected [{0}].".format(self.Annotate(here))
                )
            if len(call) != 1:
                raise resource_exceptions.UnknownTransformError(
                    "Unknown transform function {0} [{1}].".format(
