...
class Table(object):
    ...
    _PROJECTION_TYPE_TO_INDEX = dict(
        ...
        local_indexes=dict(ALL=AllIndex, KEYS_ONLY=KeysOnlyIndex, INCLUDE=IncludeIndex),
    )
    ...
    def delete_global_secondary_index(self, global_index_name):
        ...
        if global_index_name:
            ...
            self.connection.update_table(
                self.table_name, global_secondary_index_updates=gsi_data
            )
    ...
    def flush(self):
        ...
        for put in self._to_put:
            ...
            batch_data[self.table.table_name].append(
                {"PutRequest": {"Item": item.prepare_full()}}
            )

        for delete in self._to_delete:
            batch_data[self.table.table_name].append(
                {"DeleteRequest": {"Key": self.table._encode_keys(delete)}}
            )
