...
class KubeconfigProcessor(object):
    ...
    """
        ...
        if self.gke_uri or self.gke_cluster:
            ...
            return (
                _GetGKEKubeconfig(cluster_project, location, name, temp_kubeconfig_dir),
                None,
            )
...
class KubernetesClient(object):
    ...
    def GetNamespaceUID(self, namespace):
        out, err = self._RunKubectl(
            ["get", "namespace", namespace, "-o", "jsonpath='{.metadata.uid}'"], None
        )
        if err:
            raise exceptions.Error(
    ...
    """
        ...
        out, err = self._RunKubectl(
            ["get", "namespaces", "--selector", label, "-o", "jsonpath={.items}"], None
        )
        if err:
            raise exceptions.Error(
    ...
    def GetMembershipOwnerID(self):
        ...
        out, err = self._RunKubectl(
            ["get", "membership", "membership", "-o", "jsonpath={.spec.owner.id}"], None
        )
        if err:
            if "NotFound" in err:
                return None
    ...
    """
        ...
        try:
            ...
            raise exceptions.Error(
                "Failed to get OpenID Provider Configuration "
                "from {}: {}".format(url, e)
            )
