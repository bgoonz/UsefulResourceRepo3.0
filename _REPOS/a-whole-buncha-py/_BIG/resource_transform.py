...
def TransformDate(
    r, format="%Y-%m-%dT%H:%M:%S", unit=1, undefined="", tz=None, tz_default=None
):
...
def TransformDuration(
    r, start="", end="", parts=3, precision=3, calendar=True, unit=1, undefined=""
):
...
def TransformFilter(r, expression):
  ...
  """
    ...
    # import loop
    from googlecloudsdk.core.resource import (
        resource_filter,
...
def TransformFormat(r, projection, fmt, *args):
  ...
  """
    if args:
        columns = projection.compiler(
            "({0})".format(",".join(args)), by_columns=True, defaults=projection
        ).Evaluate(r)
  ...
  """
    if isinstance(r, dict):
        if show == "keys":
            ...
            return separator.join(
                ["{k}={v}".format(k=k, v=v) for k, v in sorted(six.iteritems(r))]
            )
    if isinstance(r, list):
        return separator.join(map(six.text_type, r))
    return r or undefined
...
_API_TO_TRANSFORMS = {
    "cloudbuild": ("googlecloudsdk.api_lib.cloudbuild.transforms", "GetTransforms"),
    "compute": ("googlecloudsdk.api_lib.compute.transforms", "GetTransforms"),
    "container": ("googlecloudsdk.api_lib.container.transforms", "GetTransforms"),
    "debug": ("googlecloudsdk.api_lib.debug.transforms", "GetTransforms"),
    "functions": ("googlecloudsdk.api_lib.functions.transforms", "GetTransforms"),
    "runtimeconfig": (
        "googlecloudsdk.api_lib.runtime_config.transforms",
        "GetTransforms",
    ),
