...
class LoggingInterceptor(
  ...
  """
    ...
    """
        ...
        log.status.Print("-- body end --")
        log.status.Print(
            "total round trip time (request+response): {0:.3f} secs".format(time_taken)
        )
...
def MakeTransport(client_class, credentials, address_override_func, mtls_enabled=False):
...
def MakeAsyncTransport(
    client_class, credentials, address_override_func, mtls_enabled=False
):
    """Instantiates a grpc transport."""
