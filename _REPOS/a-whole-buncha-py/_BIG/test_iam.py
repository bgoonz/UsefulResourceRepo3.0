...
class TestIamHelpers(testcase.GsUtilUnitTestCase):
    ...
    def test_patch_bindings_grant(self):
        ...
        diff = [bvle(role="d", members=["user:foo@bar.com"])]
        expected = base + diff
    ...
    def test_patch_bindings_remove(self):
        ...
        diff = [bvle(members=["user:foo@bar.com"], role="a")]
        expected = [
            bvle(members=["user:foo@bar.com"], role="b"),
            bvle(members=["user:foo@bar.com"], role="c"),
        ]
    ...
    def test_patch_bindings_remove_all(self):
        ...
        diff = [bvle(members=["user:foo@bar.com"], role="")]
        res = PatchBindings(base, BindingsTuple(False, diff))
    ...
    def test_patch_bindings_multiple_users(self):
        ...
        expected = [bvle(members=["user:fii@bar.com"], role="b")]
        base = [
            bvle(members=["user:foo@bar.com"], role="a"),
    ...
    def test_patch_bindings_grant_all_users(self):
        ...
        diff = [bvle(role="a", members=["allUsers"])]
        expected = [
    ...
    def test_patch_bindings_public_member_overwrite(self):
        ...
        base = [bvle(role="a", members=["allUsers"])]
        diff = [bvle(role="a", members=["allAuthenticatedUsers"])]
    ...
    def test_valid_public_member_single_role(self):
        ...
        self.assertIn(bvle(members=["allUsers"], role="roles/storage.admin"), bindings)
    ...
    def test_valid_member(self):
        ...
        self.assertEquals(len(bindings), 1)
        self.assertIn(
            bvle(members=["user:foo@bar.com"], role="roles/storage.admin"), bindings
        )
    ...
    def test_valid_deleted_member(self):
        ...
        self.assertEquals(len(bindings), 1)
        self.assertIn(
            bvle(members=["deleted:user:foo@bar.com?uid=123"], role=""), bindings
        )
        (_, bindings) = bstt(True, "deleted:User:foo@bar.com?uid=123:admin")
        self.assertEquals(len(bindings), 1)
        self.assertIn(
            bvle(
                members=["deleted:user:foo@bar.com?uid=123"], role="roles/storage.admin"
            ),
            bindings,
        )
        # These emails can actually have multiple query params
        (_, bindings) = bstt(
            True, "deleted:user:foo@bar.com?query=param,uid=123?uid=456:admin,admin2"
        )
        self.assertEquals(len(bindings), 2)
        self.assertIn(
            bvle(
...
class TestIamCh(TestIamIntegration):
    ...
    def test_patch_single_grant_single_bucket(self):
        ...
        self.RunGsUtil(
            [
                "iam",
                "ch",
                "%s:%s" % (self.user, IAM_BUCKET_READ_ROLE_ABBREV),
                self.bucket.uri,
            ]
        )
    ...
    def test_patch_public_grant_single_bucket(self):
        ...
        self.RunGsUtil(
            ["iam", "ch", "allUsers:%s" % IAM_BUCKET_READ_ROLE_ABBREV, self.bucket.uri]
        )
    ...
    def test_patch_multithreaded_single_object(self):
        ...
        self.assertHasNo(self.object_iam_string, self.user, IAM_OBJECT_READ_ROLE)
        self.RunGsUtil(
            ["-m", "iam", "ch", "%s:legacyObjectReader" % self.user, self.object.uri]
        )
    ...
    def test_patch_multiple_objects(self):
        ...
        self.RunGsUtil(
            ["iam", "ch", "-r", "%s:legacyObjectReader" % self.user, self.bucket.uri]
        )
    ...
    """
        ...
        raise AssertionError(
            "Member '%s' does not have permission '%s' in "
...
class TestIamSet(TestIamIntegration):
    ...
    def test_set_invalid_iam_bucket(self):
        ...
        def _Check1():
            ...
            stderr = self.RunGsUtil(
                ["iam", "set", inpath, self.bucket.uri],
                return_stderr=True,
                expected_status=1,
            )
    ...
    def test_get_invalid_bucket(self):
        ...
        stderr = self.RunGsUtil(
            ["iam", "get", self.nonexistent_bucket_name],
            return_stderr=True,
            expected_status=1,
        )
        self.assertIn("CommandException", stderr)
        ...
        def _Check():  # pylint: disable=invalid-name
            ...
            stderr = self.RunGsUtil(
                ["iam", "get", "gs://*"], return_stderr=True, expected_status=1
            )
            self.assertIn("CommandException", stderr)
    ...
    def test_set_valid_iam_bucket(self):
        ...
        self.assertIn(
            self.public_bucket_read_binding[0], json.loads(set_iam_string)["bindings"]
        )
    ...
    def test_set_and_get_valid_bucket_policy_with_conditions(self):
        ...
        self.assertIn(
            TEST_CONDITION_EXPR_RESOURCE_IS_OBJECT, get_iam_string.replace("\\", "")
        )
    ...
    def test_set_valid_iam_multiple_objects(self):
        ...
        self.assertIn(
            self.public_object_read_binding[0], json.loads(set_iam_string)["bindings"]
        )

        # Check that old versions are not affected by the set IAM call.
        iam_string_old = self.RunGsUtil(
            ["iam", "get", old_gsutil_object.version_specific_uri], return_stdout=True
        )
        iam_string_old2 = self.RunGsUtil(
            ["iam", "get", old_gsutil_object2.version_specific_uri], return_stdout=True
        )
