...
NON_EXISTENT_OBJECT_REGEX = re.compile(r".*non-\s*existent\s*object", flags=re.DOTALL)
...
class BotoTranslation(CloudApi):
  ...
  """
    ...
    """
        ...
        self.api_version = boto.config.get_value("GSUtil", "default_api_version", "1")
    ...
    def GetBucket(self, bucket_name, provider=None, fields=None):
        ...
        try:
            return self._BotoBucketToBucket(
                bucket_uri.get_bucket(validate=True, headers=headers), fields=fields
            )
    ...
    def PatchBucket(
        ...
        try:
            ...
            if canned_def_acl:
                ...
                bucket_uri.set_def_acl(
                    canned_def_acl, bucket_uri.object_name, headers=headers
                )
            if metadata.cors:
            ...
            if metadata.versioning:
                bucket_uri.configure_versioning(
                    metadata.versioning.enabled, headers=headers
                )
            if metadata.website:
                main_page_suffix = metadata.website.mainPageSuffix
                error_page = metadata.website.notFoundPage
                bucket_uri.set_website_config(
                    main_page_suffix, error_page, headers=headers
                )
    ...
    def CreateBucket(
        self, bucket_name, project_id=None, metadata=None, provider=None, fields=None
    ):
        """See CloudApi class for function doc strings."""
        _ = provider
        bucket_uri = self._StorageUriForBucket(bucket_name)
        ...
        if bucket_uri.scheme == "gs":
            ...
            if (
                metadata
                and metadata.retentionPolicy
                and metadata.retentionPolicy.retentionPeriod
            ):
            ...
            try:
                bucket_uri.create_bucket(
                    headers=headers, location=location, storage_class=storage_class
                )
            except TRANSLATABLE_BOTO_EXCEPTIONS as e:
                self._TranslateExceptionAndRaise(e, bucket_name=bucket_name)
        else:
    ...
    def DeleteBucket(self, bucket_name, preconditions=None, provider=None):
        ...
        try:
            ...
            if translated_exception and "BucketNotEmpty" in translated_exception.reason:
                try:
                    if bucket_uri.get_versioning_config():
                        ...
                        raise NotEmptyException(
                            "BucketNotEmpty (%s)" % bucket_name, status=e.status
                        )
    ...
    def ListObjects(
        ...
        try:
            objects_iter = bucket_uri.list_bucket(
                prefix=prefix or "",
        ...
        try:
            for key in objects_iter:
                if yield_prefixes and isinstance(key, Prefix):
                    yield CloudApi.CsObjectOrPrefix(
                        key.name, CloudApi.CsObjectOrPrefixType.PREFIX
                    )
                elif yield_objects:
                    key_to_convert = key
                    ...
                    if not get_fields or (
                        get_fields and not get_fields.issubset(key_http_fields)
                    ):
                        ...
                        key_to_convert = self._GetBotoKey(
                            bucket_name, key.name, generation=generation
                        )
                    return_object = self._BotoKeyToObject(
                        key_to_convert, fields=get_fields
                    )

                    yield CloudApi.CsObjectOrPrefix(
                        return_object, CloudApi.CsObjectOrPrefixType.OBJECT
                    )
        except TRANSLATABLE_BOTO_EXCEPTIONS as e:
            self._TranslateExceptionAndRaise(e, bucket_name=bucket_name)

    def GetObjectMetadata(
        self, bucket_name, object_name, generation=None, provider=None, fields=None
    ):
        """See CloudApi class for function doc strings."""
        _ = provider
        try:
            return self._BotoKeyToObject(
                self._GetBotoKey(bucket_name, object_name, generation=generation),
                fields=fields,
            )
        except TRANSLATABLE_BOTO_EXCEPTIONS as e:
    ...
    def GetObjectMedia(
        ...
        headers = self._CreateBaseHeaders()
        AddAcceptEncodingGzipIfNeeded(headers, compressed_encoding=compressed_encoding)
        ...
        try:
            if download_strategy is CloudApi.DownloadStrategy.RESUMABLE:
                ...
                raise ArgumentException(
                    "Unsupported DownloadStrategy: %s" % download_strategy
                )
        except TRANSLATABLE_BOTO_EXCEPTIONS as e:
            self._TranslateExceptionAndRaise(
                e,
          ...
          """
                ...
                for alg_name in digesters:
                    if (
                        ...
                        (
                            getattr(key, "local_hashes", None)
                            and alg_name in key.local_hashes
                        )
                    ):
    ...
    """
        ...
        while True:  # Retry as long as we're making progress.
            ...
            sleep_time_secs = min(
                random.random() * (2 ** progress_less_iterations), GetMaxRetryDelay()
            )
            if debug >= 1:
                self.logger.info(
    ...
    def PatchObjectMetadata(
        ...
        """See CloudApi class for function doc strings."""
        _ = provider
        object_uri = self._StorageUriForObject(
        ...
        if metadata_changed:
            try:
                object_uri.set_metadata(
                    metadata_plus, metadata_minus, False, headers=headers
                )
            except TRANSLATABLE_BOTO_EXCEPTIONS as e:
                self._TranslateExceptionAndRaise(
                    e,
        ...
        if metadata.acl:
            boto_acl = AclTranslation.BotoAclFromMessage(metadata.acl)
            try:
                object_uri.set_xml_acl(
                    boto_acl.to_xml(), key_name=object_name, headers=headers
                )
            except TRANSLATABLE_BOTO_EXCEPTIONS as e:
                self._TranslateExceptionAndRaise(
                    e,
    ...
    def _PerformSimpleUpload(
        ...
        dst_uri.set_contents_from_file(
            upload_stream,
            md5=md5,
            policy=canned_acl,
            cb=progress_callback,
    ...
    def _PerformStreamingUpload(
        ...
        if dst_uri.get_provider().supports_chunked_transfer():
            dst_uri.set_contents_from_stream(
                upload_stream, policy=canned_acl, cb=progress_callback, headers=headers
            )
        else:
            ...
            try:
                ...
                with open(temp_path, "rb") as in_fp:
                    dst_uri.set_contents_from_file(
                        in_fp, policy=canned_acl, headers=headers
                    )
            finally:
                os.close(temp_fh)
                os.unlink(temp_path)
    ...
    """
        ...
        dst_uri = self._StorageUriForObject(
    ...
    def UploadObjectResumable(
        ...
        """See CloudApi class for function doc strings."""
        if gzip_encoded:
            raise NotImplementedError("XML API does not support gzip-encoded uploads.")
        if self.provider == "s3":
        ...
        headers, dst_uri = self._UploadSetup(
            object_metadata, preconditions=preconditions
        )
        if not tracker_callback:
            raise ArgumentException(
                "No tracker callback function set for "
                "resumable upload of %s" % dst_uri
            )
        try:
            self._PerformResumableUpload(
                dst_uri.new_key(headers=headers),
                upload_stream,
                size,
                tracker_callback,
                canned_acl=canned_acl,
            ...
            return self._HandleSuccessfulUpload(dst_uri, object_metadata, fields=fields)
        except TRANSLATABLE_BOTO_EXCEPTIONS as e:
    ...
    def DeleteObject(
        ...
        uri = self._StorageUriForObject(bucket_name, object_name, generation=generation)
        try:
    ...
    def ComposeObject(
        ...
        for src_obj in src_objs_metadata:
            src_uri = self._StorageUriForObject(
                dst_bucket_name, src_obj.name, generation=src_obj.generation
            )
        ...
        try:
            ...
            return self.GetObjectMetadata(dst_bucket_name, dst_obj_name, fields=fields)
        except TRANSLATABLE_BOTO_EXCEPTIONS as e:
            self._TranslateExceptionAndRaise(
                e, dst_obj_metadata.bucket, dst_obj_metadata.name
            )
    ...
    """
        ...
        if self.provider == "gs":
            ...
            if not fields or "acl" in fields:
                try:
                    ...
                    if translated_exception and isinstance(
                        translated_exception, AccessDeniedException
                    ):
            ...
            if not fields or "lifecycle" in fields:
                try:
                    ...
                    cloud_api_bucket.lifecycle = LifecycleTranslation.BotoLifecycleToMessage(
                        boto_lifecycle
                    )
                except TRANSLATABLE_BOTO_EXCEPTIONS as e:
                    self._TranslateExceptionAndRaise(e, bucket_name=bucket.name)
                ...
                try:
                    ...
                    if boto_logging and "Logging" in boto_logging:
                        ...
                        if log_object_prefix_present or log_bucket_present:
                            cloud_api_bucket.logging = (
                                apitools_messages.Bucket.LoggingValue()
                            )
                            ...
                            if log_bucket_present:
                                cloud_api_bucket.logging.logBucket = logging_config[
                                    "LogBucket"
                                ]
                except TRANSLATABLE_BOTO_EXCEPTIONS as e:
                    self._TranslateExceptionAndRaise(e, bucket_name=bucket.name)
                ...
                try:
                    ...
                    if boto_website and "WebsiteConfiguration" in boto_website:
                        ...
                        if main_page_suffix_present or not_found_page_present:
                            cloud_api_bucket.website = (
                                apitools_messages.Bucket.WebsiteValue()
                            )
                            if main_page_suffix_present:
                                cloud_api_bucket.website.mainPageSuffix = website_config[
                                    "MainPageSuffix"
                                ]
                            if not_found_page_present:
                                cloud_api_bucket.website.notFoundPage = website_config[
                                    "NotFoundPage"
                                ]
                except TRANSLATABLE_BOTO_EXCEPTIONS as e:
                    self._TranslateExceptionAndRaise(e, bucket_name=bucket.name)
        ...
        if not fields or "labels" in fields:
            try:
                ...
                try:
                    ...
                    cloud_api_bucket.labels = LabelTranslation.BotoTagsToMessage(
                        boto_tags
                    )
        ...
        if not fields or "versioning" in fields:
            ...
            if versioning:
                if (
                    self.provider == "s3"
                    and "Versioning" in versioning
                    and versioning["Versioning"] == "Enabled"
                ):
    ...
    """
        ...
        if (
            ...
            or len([field for field in fields if field.startswith("metadata/")]) >= 1
        ):
    ...
    def _TranslateBotoKeyAcl(self, key, cloud_api_object, generation=None):
        ...
        storage_uri_for_key = self._StorageUriForObject(
            key.bucket.name, key.name, generation=generation
        )
        headers = self._CreateBaseHeaders()
        try:
            if self.provider == "gs":
    ...
    def _TranslateExceptionAndRaise(
        self,
        e,
        bucket_name=None,
        object_name=None,
    ...
    """
        if isinstance(e, boto.exception.StorageResponseError):
            if e.status == 400:
                ...
                if not_found_exception:
                    ...
                    return CreateBucketNotFoundException(
                        e.status, self.provider, bucket_name
                    )
                ...
                return NotEmptyException(
                    "BucketNotEmpty (%s)" % bucket_name, status=e.status, body=e.body
                )
            elif e.status == 410:
        ...
        if isinstance(e, boto.exception.StorageCreateError):
            return ServiceException(
                "Bucket already exists.", status=e.status, body=e.body
            )
        ...
        if isinstance(e, boto.exception.ResumableUploadException):
            if e.disposition == boto.exception.ResumableTransferDisposition.ABORT:
                ...
                e.disposition == boto.exception.ResumableTransferDisposition.START_OVER
            ):
                return ResumableUploadStartOverException(e.message)
            else:
    ...
    def XmlPassThroughSetAcl(
        self, acl_text, storage_url, canned=True, def_obj_acl=False
    ):
        """See CloudApiDelegator class for function doc strings."""
        headers = self._CreateBaseHeaders()
        try:
    ...
    def XmlPassThroughSetCors(self, cors_text, storage_url):
        ...
        try:
            ...
            raise CommandException(
                ...
                "column %s" % (e.getMessage(), e.getLineNumber(), e.getColumnNumber())
            )
    ...
    def XmlPassThroughGetLogging(self, storage_url):
        ...
        try:
            ...
            logging_config_xml = UnaryDictToXml(uri.get_logging_config(headers=headers))
        except TRANSLATABLE_BOTO_EXCEPTIONS as e:
            self._TranslateExceptionAndRaise(e)
