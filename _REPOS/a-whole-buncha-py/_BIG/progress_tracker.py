...
def ProgressTracker(
  ...
  """
    ...
    if style == properties.VALUES.core.InteractiveUXStyles.OFF.name:
        ...
        tracker_cls = (
            _NormalProgressTracker if is_tty else _NonInteractiveProgressTracker
        )
        screen_reader = (
            screen_reader or properties.VALUES.accessibility.screen_reader.GetBool()
        )
...
def CompletionProgressTracker(
    ofile=None, timeout=4.0, tick_delay=0.1, background_ttl=60.0, autotick=True
):
  ...
  """
    ...
    if (
        ...
        or style == properties.VALUES.core.InteractiveUXStyles.TESTING.name
    ):
...
class _NormalCompletionProgressTracker(object):
  ...
  """
    ...
    def _Spin(self, unused_sig=None, unused_frame=None):
        ...
        self._ticks += 1
        self._WriteMark(
            self._symbols.spin_marks[self._ticks % len(self._symbols.spin_marks)]
        )
  ...
  """
    ...
    """
        ...
        if not allow_complete and stage.status not in {
            StageCompletionStatus.NOT_STARTED,
            StageCompletionStatus.RUNNING,
        }:
        ...
        with self._lock:
            if not self._done:
                ...
                if self._stage_being_displayed is None:
                    ...
                    while (
                        self._running_stages_queue
                        and self._running_stages_queue[0].is_done
                    ):
  ...
  """
    ...
    def _AddMessage(self, message, indentation_level=0):
        ...
        return self._console_output.AddMessage(
            message, indentation_level=indentation_level
        )
    ...
    def _SetupOutput(self):
        ...
        for stage in self._stages.values():
            self._stage_messages[stage] = self._AddMessage(
    ...
    def _FailStage(self, stage, exception, message=None):
        ...
        if exception:
            for other_stage in self._stages.values():
                if (
                    other_stage != stage
                    and other_stage.status == StageCompletionStatus.RUNNING
                ):
...
class NoOpStagedProgressTracker(_BaseStagedProgressTracker):
    ...
    def __init__(self, stages, interruptable=False, aborted_message=""):
        super(NoOpStagedProgressTracker, self).__init__(
            ...
            tracker_id="",
            done_message_callback=None,
            console=console_attr.ConsoleAttr(encoding="ascii", suppress_output=True),
        )
        self._aborted_message = aborted_message
        self._done = False
...
class _StubStagedProgressTracker(NoOpStagedProgressTracker):
  ...
  """
    ...
    def __exit__(self, exc_type, exc_val, exc_tb):
        ...
        return super(_StubStagedProgressTracker, self).__exit__(
