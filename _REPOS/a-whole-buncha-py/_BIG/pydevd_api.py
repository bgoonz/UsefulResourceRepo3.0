...
class PyDevdAPI(object):
    ...
    def request_suspend_thread(self, py_db, thread_id="*"):
        ...
        for t in threads:
            if t is None:
                continue
            py_db.set_suspend(
                t, CMD_THREAD_SUSPEND, suspend_other_threads=suspend_all, is_pause=True
            )
    ...
    def add_breakpoint(
        ...
        """
        assert filename.__class__ == str, "Expected str, found: %s" % (
            filename.__class__,
        ...
        pydev_log.debug(
            "Breakpoint (after path/source mapping) in: %s line: %s",
            new_filename,
            new_line,
        )
        ...
        if not filename.startswith("<"):
            ...
            if (
                ...
                and py_db.apply_files_filter(
                    self._DummyFrame(filename), filename, False
                )
            ):
    ...
    def set_dont_trace_start_end_patterns(self, py_db, start_patterns, end_patterns):
        ...
        def custom_dont_trace_external_files(abs_path):
            return abs_path.startswith(start_patterns) or abs_path.endswith(
                end_patterns
            )
