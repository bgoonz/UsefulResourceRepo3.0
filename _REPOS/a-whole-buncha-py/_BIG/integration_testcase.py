...
@unittest.skipUnless(util.RUN_INTEGRATION_TESTS, "Not running integration tests.")
class GsUtilIntegrationTestCase(base.GsUtilTestCase):
    ...
    USER_TEST_ID = "00b4903a97b201e40d2a5a3ddfe044bb1ab79c75b2e817cbe350297eccc81c84"
    ...
    def setUp(self):
        ...
        self.json_api = GcsJsonApi(
            BucketStorageUri, logging.getLogger(), DiscardMessagesQueue(), "gs"
        )
        self.xml_api = BotoTranslation(
            BucketStorageUri,
            logging.getLogger(),
                ...
                for k in bucket_list:
                    try:
                        if isinstance(k, DeleteMarker):
                            bucket_uri.get_bucket().delete_key(
                                k.name, version_id=k.version_id
                            )
                        else:
                            k.delete()
                    except StorageResponseError as e:
                        ...
                        if e.status == 404:
                            ...
                            error = e
                        elif e.status == 403 and (
                            e.error_code == "ObjectUnderActiveHold"
                            or e.error_code == "RetentionPolicyNotMet"
                        ):
    ...
    """
        ...
        retention_policy = self.json_api.GetBucket(
            bucket_uri.bucket_name, fields=["retentionPolicy"]
        ).retentionPolicy
        retention_period = (
            retention_policy.retentionPeriod if retention_policy is not None else 0
        )
        ...
        if retention_period <= 60:
            ...
            raise CommandException(
                (
                    'Retention duration is too large for bucket "{}".'
                    " Use shorter durations for Retention duration in"
                    " tests"
                ).format(bucket_uri)
            )

    def _SetObjectCustomMetadataAttribute(
        self, provider, bucket_name, object_name, attr_name, attr_value
    ):
    ...
    """
        ...
        if provider == "gs":
            self.json_api.PatchObjectMetadata(
                bucket_name, object_name, obj_metadata, provider=provider
            )
        else:
            self.xml_api.PatchObjectMetadata(
                bucket_name, object_name, obj_metadata, provider=provider
            )

    def SetPOSIXMetadata(
        self,
        provider,
        bucket_name,
        object_name,
        ...
        if atime is not None:
            CreateCustomMetadata(
                entries={ATIME_ATTR: atime}, custom_metadata=obj_metadata.metadata
            )
        if mode is not None:
            CreateCustomMetadata(
                entries={MODE_ATTR: mode}, custom_metadata=obj_metadata.metadata
            )
        if mtime is not None:
            CreateCustomMetadata(
                entries={MTIME_ATTR: mtime}, custom_metadata=obj_metadata.metadata
            )
        if uid is not None:
            CreateCustomMetadata(
                entries={UID_ATTR: uid}, custom_metadata=obj_metadata.metadata
            )
        if gid is not None:
            CreateCustomMetadata(
                entries={GID_ATTR: gid}, custom_metadata=obj_metadata.metadata
            )
        if provider == "gs":
            self.json_api.PatchObjectMetadata(
                bucket_name, object_name, obj_metadata, provider=provider
    ...
    """
        ...
        self.RunGsUtil(
            [
                "setmeta",
                "-h",
                "x-%s-meta-%s" % (provider_meta_string, ATIME_ATTR),
                "-h",
                "x-%s-meta-%s" % (provider_meta_string, MTIME_ATTR),
                "-h",
                "x-%s-meta-%s" % (provider_meta_string, UID_ATTR),
                "-h",
                "x-%s-meta-%s" % (provider_meta_string, GID_ATTR),
                "-h",
    ...
    def _ServiceAccountCredentialsPresent(self):
        ...
        ) or config.has_option("GoogleCompute", "service_account")
    ...
    def CreateBucketWithRetentionPolicy(
        self, retention_period_in_seconds, is_locked=None, bucket_name=None
    ):
    ...
    """
        ...
        if bucket_name:
            bucket_name = "".join([bucket_name_prefix, bucket_name, bucket_name_suffix])
            bucket_name = util.MakeBucketNameValid(bucket_name)
        else:
            bucket_name = self.MakeTempName(
                "bucket", prefix=bucket_name_prefix, suffix=bucket_name_suffix
            )
        ...
        if prefer_json_api and provider == "gs":
            ...
            bucket_uri = boto.storage_uri(
                "gs://%s" % json_bucket.name.lower(), suppress_consec_slashes=False
            )
            return bucket_uri

        bucket_uri = boto.storage_uri(
            "%s://%s" % (provider, bucket_name.lower()), suppress_consec_slashes=False
        )
    ...
    """
        ...
        if contents is not None:
            if not isinstance(contents, (six.binary_type, six.text_type)):
                raise TypeError(
                    "contents must be either none or bytes, not {}".format(
                        type(contents)
                    )
                )
        ...
        if (
            ...
            md5 = (
                Base64ToHexHash(json_object.md5Hash),
                json_object.md5Hash.strip("\n\"'"),
            )
            object_uri._update_from_values(None, json_object.generation, True, md5=md5)
        ...
        custom_metadata_present = (
            mode is not None or mtime is not None or uid is not None or gid is not None
        )
        if custom_metadata_present:
            self.SetPOSIXMetadata(
                bucket_uri.scheme,
                bucket_uri.bucket_name,
                object_name,
                atime=None,
    ...
    """
        ...
        self.bucket_uris.append(
            boto.storage_uri("gs://%s" % bucket_name, suppress_consec_slashes=False)
        )
        for i in range(test_objects):
            self.CreateObjectJson(
                bucket_name=bucket_name,
                object_name=self.MakeTempName("obj"),
                contents="test {:d}".format(i).encode("ascii"),
            )
    ...
    def VerifyObjectCustomAttribute(
        self, bucket_name, object_name, attr_name, expected_value, expected_present=True
    ):
    ...
    def VerifyPublicAccessPreventionValue(self, bucket_uri, value):
        stdout = self.RunGsUtil(
            ["publicaccessprevention", "get", suri(bucket_uri)], return_stdout=True
        )
        public_access_prevention_re = re.compile(r":\s+(?P<pap_val>.+)$")
        public_access_prevention_match = re.search(public_access_prevention_re, stdout)
        public_access_prevention_val = public_access_prevention_match.group("pap_val")
        ...
        def _RunTabCompletion():
            ...
            with tempfile.NamedTemporaryFile(delete=False) as tab_complete_result_file:
                if hacky_debugging:
                    ...
                    cmd_str_with_result_redirect = "{cs} 1>{fn} 2>{fn} 8>{fn} 9>{fn}".format(
    ...
    def SetAnonymousBotoCreds(self):
        ...
        with SetBotoConfigForTest(boto_config_for_test, use_existing_config=False):
            ...
            with SetEnvironmentForTest(
                {"DEVSHELL_CLIENT_PORT": None, "AWS_SECRET_ACCESS_KEY": "_"}
            ):
                yield
    ...
    def FlatListBucket(self, bucket_url_string):
        ...
        return self.RunGsUtil(["ls", suri(bucket_url_string, "**")], return_stdout=True)
