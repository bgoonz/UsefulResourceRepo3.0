...
class TestHasTraits(TestCase):
    ...
    def test_traits_metadata_deprecated(self):
        with expected_warnings(
            [r"metadata should be set using the \.tag\(\) method"] * 2
        ):
...
@pytest.mark.parametrize("Trait", (List, Tuple, Set, Dict, Integer, Unicode))
...
@pytest.mark.parametrize(
    "Trait, default_value", ((List, []), (Tuple, ()), (Set, set()))
)
...
class TestLink(TestCase):
    ...
    def test_tranform(self):
        ...
        c = link(
            (a, "value"),
            (b, "value"),
            transform=(lambda x: 2 * x, lambda x: int(x / 2.0)),
        )
...
class TestForwardDeclaredInstanceList(TraitTestBase):
    ...
    _default_value = []
    _good_values = [[ForwardDeclaredBar(), ForwardDeclaredBarSub()], []]
    _bad_values = [
        ForwardDeclaredBar(),
