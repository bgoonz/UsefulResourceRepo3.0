...
class Application(SingletonConfigurable):
    ...
    log_format = Unicode(
        "[%(name)s]%(highlevel)s %(message)s", help="The Logging format template"
    ).tag(config=True)
    ...
    def start_show_config(self):
        ...
        for classname in sorted(config):
            ...
            for traitname in sorted(class_config):
                ...
                print(
                    "  .{} = {}".format(
                        traitname, pprint.pformat(value, **pformat_kwargs)
                    )
                )
    ...
    def _create_loader(self, argv, aliases, flags, classes):
        return KVArgParseConfigLoader(
            argv, aliases, flags, classes=classes, log=self.log
        )
    ...
    def _load_config_files(
        cls, basefilename, path=None, log=None, raise_config_file_errors=False
    ):
