...
class TestKubeConfigLoader(BaseTestCase):
        ...
        "users": [
            ...
            },
            {
                ...
                "user": {
                    "auth-provider": {
                        "name": "gcp",
                        "config": {"access-token": TEST_DATA_BASE64},
                    },
    ...
    def test_oidc_no_refresh(self):
        loader = KubeConfigLoader(
            config_dict=self.TEST_KUBE_CONFIG, active_context="oidc"
        )
        self.assertTrue(loader._load_auth_provider_token())
    ...
    def test_oidc_with_refresh(self, mock_ApiClient, mock_OAuth2Session):
        ...
        loader = KubeConfigLoader(
            config_dict=self.TEST_KUBE_CONFIG, active_context="expired_oidc"
        )
        self.assertTrue(loader._load_auth_provider_token())
    ...
    def test_oidc_with_refresh_nocert(self, mock_ApiClient, mock_OAuth2Session):
        ...
        loader = KubeConfigLoader(
            config_dict=self.TEST_KUBE_CONFIG, active_context="expired_oidc_nocert"
        )
        self.assertTrue(loader._load_auth_provider_token())
        self.assertEqual("Bearer abc123", loader.token)
    ...
    def test_oidc_fails_if_contains_reserved_chars(self):
        ...
        self.assertEqual(
            loader._load_oid_token("oidc_contains_reserved_character"), None
        )
...
class TestKubernetesClientConfiguration(BaseTestCase):
    ...
    def test_get_api_key_with_prefix_returns_token(self):
        ...
        self.assertEqual(
            expected_token, config.get_api_key_with_prefix("authorization")
        )
...
class TestKubeConfigMerger(BaseTestCase):
    TEST_KUBE_CONFIG_PART1 = {
        "current-context": "no_user",
        "contexts": [{"name": "no_user", "context": {"cluster": "default"}}],
