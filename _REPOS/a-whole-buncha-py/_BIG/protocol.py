...
class JsonRPCProtocol(asyncio.Protocol):
    ...
    def _check_ret_type_and_send_response(
        self, method_name, method_type, msg_id, result
    ):
    ...
    def _execute_request(self, msg_id, handler, params):
        ...
        if asyncio.iscoroutinefunction(handler):
            ...
            future.add_done_callback(
                partial(
                    self._execute_request_callback, method_name, method_type, msg_id
                )
            )
        else:
            # Can't be canceled
    ...
    def _handle_request(self, msg_id, method_name, params):
        ...
        try:
            ...
            logger.exception(
                "Failed to handle request %s %s %s", msg_id, method_name, params
            )
    ...
    def _send_data(self, data):
        ...
        try:
            body = data.json(
                by_alias=True, exclude_unset=True, encoder=default_serializer
            )
    ...
    def _send_response(self, msg_id, result=None, error=None):
        ...
        response = JsonRPCResponseMessage(
            id=msg_id, jsonrpc=JsonRPCProtocol.VERSION, result=result, error=error
        )
    ...
    def data_received(self, data: bytes):
        ...
        while len(data):
            ...
            self._procedure_handler(
                json.loads(body.decode(self.CHARSET), object_hook=deserialize_message)
            )
    ...
    def send_request(self, method, params=None, callback=None):
        ...
        request = JsonRPCRequestMessage(
            id=msg_id, jsonrpc=JsonRPCProtocol.VERSION, method=method, params=params
        )
