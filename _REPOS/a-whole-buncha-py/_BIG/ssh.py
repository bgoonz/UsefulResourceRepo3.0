...
class CommandError(core_exceptions.Error):
    ...
    def __init__(self, cmd, message=None, return_code=None):
        ...
        why_failed = " and ".join([f for f in [message_text, return_code_text] if f])

        super(CommandError, self).__init__(
            "[{0}] exited with {1}.".format(self.cmd, why_failed), exit_code=return_code
        )
...
class KnownHosts(object):
  ...
  """
    ...
    DEFAULT_PATH = os.path.realpath(
        files.ExpandHomeDir(os.path.join("~", ".ssh", "google_compute_known_hosts"))
    )
...
def MetadataHasOsloginEnable(metadata):
  ...
  """
    if not (metadata and metadata.items):
        return None
    matching_values = [
        item.value for item in metadata.items if item.key == OSLOGIN_ENABLE_METADATA_KEY
    ]
