...
class UpdateManager(object):
    ...
    DARWIN_X86_64 = platforms.Platform(
        platforms.OperatingSystem.MACOSX, platforms.Architecture.x86_64
    )
    ...
    """
        ...
        if (
            ...
            or properties.VALUES.component_manager.disable_update_check.GetBool()
        ):
    ...
    """
        ...
        self.__text_wrapper = textwrap.TextWrapper(
    ...
    def _EnableFallback(self):
        ...
        return (
            self.__platform_filter
            and self.__platform_filter.operating_system
            == platforms.OperatingSystem.MACOSX
            and self.__platform_filter.architecture == platforms.Architecture.arm
        )
    ...
    def _ShouldDoFastUpdate(self, allow_no_backup=False, fast_mode_impossible=False):
    ...
    """
        ...
        try:
            ...
            log.debug(
                "Could not determine CWD, assuming detached directory not "
                "under SDK root."
            )
    ...
    def _ComputeMappingMessage(
        self, command, commands_map, components_map, components=None
    ):
    ...
    """
        ...
        if components:
            not_components = [
                component for component in components if component not in components_map
            ]
        ...
        if command == update_all:
            mapped_packages = [
                component
                for component in set(components_map.values())
            ...
            mapped_components = [
                ...
                and (component not in not_components)
            ]
            mapped_packages = [
    ...
    """
        ...
        if config.INSTALLATION_CONFIG.disable_updater:
            ...
            mapping_message = self._ComputeMappingMessage(
                command, commands_map, components_map, components
            )
...
version [{1}].  To clear your fixed version setting, run:
        ...
        if repos:
            if self.__warn:
                for repo in repos.split(","):
                    log.warning(
                        "You are using additional component repository: [%s]", repo
                    )
            url = ",".join([url, repos])
        ...
        try:
            ...
            if version:
                log.error(
                    ...
                    "found.  Make sure this is a valid archived Cloud SDK version.".format(
                        version
                    )
                )
            elif self.__fixed_version:
    ...
    def _GetStateAndDiff(self, version=None, command_path="unknown"):
        ...
        latest_snapshot = self._GetLatestSnapshot(
            version=version, command_path=command_path
        )
    ...
    """
        ...
        for component_id, component in six.iteritems(installed_components):
            ...
            if (
                component_def.is_configuration
                or component_def.is_hidden
    ...
    """
        ...
        latest_msg = (
            "The latest available version is: " if not self.__fixed_version else None
        )
        ...
        to_print = (
            diff.AvailableUpdates()
            + diff.Removed()
            + diff.AvailableToInstall()
            + diff.UpToDate()
        )
        return (to_print, current_version, latest_version)
    ...
    def _UpdateWithProgressBar(
        self, components, action, action_func, first=False, last=False
    ):
    ...
    """
        for index, component in enumerate(components):
            ...
            with console_io.ProgressBar(
                ...
                stream=log.status,
                first=first,
                last=last and index == len(components) - 1,
            ) as pb:
    ...
    """
        ...
        if properties.VALUES.component_manager.additional_repositories.Get():
            log.warning(
                "Additional component repositories are currently active.  "
    ...
    """
        ...
        try:
            install_state, diff = self._GetStateAndDiff(
                version=version, command_path="components.update"
            )
        except snapshots.IncompatibleSchemaVersionError as e:
        ...
        self._PrintPendingAction(
            FilterMetaComponents(diff.DetailsForCurrent(to_remove - to_install)),
            "removed",
        )
        self._PrintPendingAction(
            FilterMetaComponents(diff.DetailsForLatest(to_remove & to_install)),
            "updated",
        )
        self._PrintPendingAction(
            FilterMetaComponents(diff.DetailsForLatest(to_install - to_remove)),
            "installed",
        )
        self.__Write(log.status)
        ...
        if disable_backup:
            ...
            with console_io.ProgressBar(
                label="Creating update staging area", stream=log.status, last=False
            ) as pb:
...
""".format(
        ...
        if self.__warn and not encoding.GetEncodedValue(
  ...
  """.format(
    ...
    """
        ...
        if invalid_seeds:
            ...
            if completely_invalid_seeds:
                msgs.append(
                    "The following components are unknown [{}].".format(
                        ", ".join(completely_invalid_seeds)
                    )
                )
            if update_required_seeds:
                msgs.append(
                    "The following components are not available for your "
                    "current SDK version [{}]. Please run `gcloud components "
                    "update` to update your SDK.".format(
                        ", ".join(update_required_seeds)
                    )
                )
    ...
    def _FindToolsOnPath(self, path=None, duplicates=True, other=True):
        ...
        commands = [
            ...
            if os.path.isfile(os.path.join(bin_dir, f)) and not f.startswith(".")
        ]
    ...
    """
        ...
        if not_installed:
            raise InvalidComponentError(
                "The following components are not currently installed [{components}]".format(
                    components=", ".join(not_installed)
                )
            )

        required_components = set(
            c_id
            for c_id, component in six.iteritems(snapshot.components)
        ...
        components_to_remove = sorted(
            snapshot.ComponentsFromIds(to_remove), key=lambda c: c.details.display_name
        )
    ...
    """
        ...
        backup_has_bundled_python = _ContainsBundledPython(
    ...
    """
        ...
        return self._DoFreshInstall(
            schema_version.message, schema_version.no_update, schema_version.url
        )
    ...
    """
        return self._DoFreshInstall(
            e.schema_version.message, e.schema_version.no_update, e.schema_version.url
        )
    ...
    """
        ...
        try:
            with console_io.ProgressBar(
                label="Downloading and extracting updated components", stream=log.status
            ) as pb:
        ...
        encoding.SetEncodedValue(
            env, "CLOUDSDK_REINSTALL_COMPONENTS", ",".join(installed_component_ids)
        )
        installer_path = os.path.join(
            staging_state.sdk_root, "bin", "bootstrapping", "install.py"
        )
        ...
        with console_io.ProgressBar(
            label="Creating backup and activating new installation", stream=log.status
        ) as pb:
            install_state.ReplaceWith(staging_state, pb.SetProgress)
    ...
    """
        ...
        if not msg:
            msg = (
                "This action requires the installation of components: "
        ...
        try:
            ...
            if not self.Install(
                components, throw_if_unattended=True, restart_args=restart_args
            ):
...
""".format(
                        components_list=missing_components_list_str,
            ...
            self.__Write(
                ...
                "    $ {0}".format(
                    " ".join(["gcloud"] + argv_utils.GetDecodedArgv()[1:])
                ),
            )
            raise
    ...
    """
        ...
        if snapshot:
            ...
            if snapshot.sdk_definition.gcloud_rel_path:
                gcloud_path = os.path.join(
                    self.__sdk_root, snapshot.sdk_definition.gcloud_rel_path
                )
        ...
        try:
            with progress_tracker.ProgressTracker(
                message="Performing post processing steps", tick_delay=0.25
            ):
                ...
                try:
                    ...
                    log.debug(
                        "Failed to execute post-processing command", exc_info=True
                    )
...
def CopyPython():
    ...
    return os.path.join(temp_python_install_dir, os.path.basename(sys.executable))
...
(Substitute `%%i` for `%i` if in a .bat script.)""".format(
                    gcloud_cmd_path
                )
            )
            sys.exit(1)
