...
class TestAppAssertionCredentials(unittest.TestCase):
    ...
    class AppIdentityStubImpl(apiproxy_stub.APIProxyStub):
        def __init__(self, key_name=None, sig_bytes=None, svc_acct=None):
    ...
    class ErroringAppIdentityStubImpl(apiproxy_stub.APIProxyStub):
        def __init__(self):
            super(
                TestAppAssertionCredentials.ErroringAppIdentityStubImpl, self
            ).__init__("app_identity_service")
    ...
    def test_raise_correct_type_of_exception(self):
        ...
        apiproxy_stub_map.apiproxy.RegisterStub(
            "app_identity_service", app_identity_stub
        )
        apiproxy_stub_map.apiproxy.RegisterStub(
    ...
    def test_create_scoped(self):
        ...
        self.assertIsInstance(new_credentials, appengine.AppAssertionCredentials)
...
class FlowPropertyTest(unittest.TestCase):
    ...
    def test_flow_get_put(self):
        instance = TestFlowModel(flow=self.flow, key_name="foo")
        instance.put()
    ...
    def test_make_value_from_datastore_none(self):
        self.assertIsNone(appengine.FlowProperty().make_value_from_datastore(None))
...
class CredentialsPropertyTest(unittest.TestCase):
    ...
    def test_credentials_get_put(self):
        instance = TestCredentialsModel(credentials=self.credentials, key_name="foo")
        instance.put()
        retrieved = TestCredentialsModel.get_by_key_name("foo")

        self.assertEqual(self.credentials.to_json(), retrieved.credentials.to_json())

    def test_make_value_from_datastore(self):
        self.assertIsNone(
            appengine.CredentialsProperty().make_value_from_datastore(None)
        )
        self.assertIsNone(appengine.CredentialsProperty().make_value_from_datastore(""))
        self.assertIsNone(
...
class StorageByKeyNameTest(unittest.TestCase):
    ...
    def test__is_ndb(self):
        storage = appengine.StorageByKeyName(object(), "foo", "credentials")
...
class DecoratorTests(unittest.TestCase):
    ...
    def _finish_setup(self, decorator, user_mock):
        ...
        routes = [
            ...
            webapp2.Route(
                r"/bar_path/<year:\d{4}>/<month:\d{2}>",
                handler=TestAwareHandler,
                name="bar",
            ),
        ]
        ...
        self.app = TestApp(
            application,
            extra_environ={"wsgi.url_scheme": "http", "HTTP_HOST": "localhost"},
        )
    ...
    def test_callback_application(self):
        ...
        self.assertEqual(app.router.match_routes[0].handler.__name__, "OAuth2Handler")
    ...
    def test_required(self, new_http):
        ...
        with mock.patch.object(
            appengine, "_parse_state_value", return_value="foo_path", autospec=True
        ) as parse_state_value:
            # Now simulate the callback to /oauth2callback.
            response = self.app.get(
                "/oauth2callback",
                {"code": "foo_access_code", "state": "foo_path:xsrfkey123"},
            )
            ...
            if self.decorator._token_response_param:
                ...
                self.assertEqual(
                    json.loads(DEFAULT_RESP), json.loads(urllib.parse.unquote(response))
                )
        ...
        self.assertEqual(
            "http://localhost/oauth2callback", query_params["redirect_uri"][0]
        )
    ...
    def test_aware(self, new_http):
        ...
        self.assertEqual("foo_scope bar_scope", q["scope"][0])
        self.assertEqual(
            "http://localhost/bar_path/2012/01", q["state"][0].rsplit(":", 1)[0]
        )
        self.assertEqual("code", q["response_type"][0])

        with mock.patch.object(
            appengine, "_parse_state_value", return_value="bar_path", autospec=True
        ) as parse_state_value:
            # Now simulate the callback to /oauth2callback.
            url = self.decorator.authorize_url()
            response = self.app.get(
                "/oauth2callback",
                {"code": "foo_access_code", "state": "bar_path:xsrfkey456"},
            )
    ...
    def test_error_in_step2(self):
        ...
        response = self.app.get("/oauth2callback", {"error": "Bad<Stuff>Happened'"})
        self.assertEqual("200 OK", response.status)
    ...
    def test_kwargs_are_passed_to_underlying_flow(self):
        ...
        self.assertEqual(
            "https://example.org/oauth2callback", decorator.flow.redirect_uri
        )
    ...
    def test_decorator_from_client_secrets(self, new_http):
        ...
        self.assertEqual(
            self.decorator._revoke_uri, "https://oauth2.googleapis.com/revoke"
        )
        self.assertEqual(
            self.decorator._revoke_uri, self.decorator.credentials.revoke_uri
        )
    ...
    def test_decorator_from_client_secrets_toplevel(self):
        ...
        with decorator_patch as decorator_mock:
            ...
            decorator_mock.assert_called_once_with(
                filename, "foo_scope", cache=None, message=None
            )
    ...
    def test_invalid_state(self, new_http):
        ...
        # Execute test after setting up mock.
        with mock.patch.object(
            appengine, "_parse_state_value", return_value=None, autospec=True
        ):
            # Now simulate the callback to /oauth2callback.
            response = self.app.get(
                "/oauth2callback",
...
class DecoratorXsrfSecretTests(unittest.TestCase):
    ...
    def test_build_and_parse_state(self):
        ...
        memcache.delete(
            appengine.XSRF_MEMCACHE_ID, namespace=appengine.OAUTH2CLIENT_NAMESPACE
        )
...
class DecoratorXsrfProtectionTests(unittest.TestCase):
    ...
    def test_build_and_parse_state(self):
        ...
        self.assertEqual(
            "https://example.org", appengine._parse_state_value(state, UserMock())
        )
