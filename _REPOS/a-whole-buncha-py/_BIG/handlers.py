...
class DeprecatedServiceDocumenter(object):
    ...
    def inject_deprecation_notice(self, section, event_name, **kwargs):
        ...
        section.style.ref(
            self._replacement_service_name, self._replacement_service_name
        )
...
def decode_list_object_versions(parsed, context, **kwargs):
    ...
    _decode_list_object(
        top_level_keys=["KeyMarker", "NextKeyMarker", "Prefix", "Delimiter"],
        nested_keys=[
            ("Versions", "Key"),
            ("DeleteMarkers", "Key"),
...
BUILTIN_HANDLERS = [
    ...
    # S3 Control
    #############
    ("before-call.s3-control.*", HeaderToHostHoister("x-amz-account-id").hoist),
    ###########
    # SMS Voice
    ##########
    (
