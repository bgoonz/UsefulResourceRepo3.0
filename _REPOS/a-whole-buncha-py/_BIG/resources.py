...
_METHOD_ID_RE = re.compile(
    r"(?P<collection>{collection})\.get".format(collection=_COLLECTION_SUB_RE)
)
    ...
    def __init__(self, collection, base_urls):
        super(AmbiguousAPIException, self).__init__(
            "collection [{collection}] defined in multiple APIs: {apis}".format(
                collection=collection, apis=repr(base_urls)
            )
        )
...
class ParentCollectionMismatchException(Error):
    ...
    def __init__(self, collection, parent_collection, expected_params, actual_params):
...
class WrongResourceCollectionException(UserError):
    ...
    def __init__(self, expected, got, path):
        super(WrongResourceCollectionException, self).__init__(
            ...
            "path [{path}]".format(expected=expected, got=got, path=path)
        )
        self.got = got
        self.path = path
...
class _ResourceParser(object):
    ...
    """
        ...
        if not match:
            raise InvalidResourceException(
                relative_name,
                "It is not in {0} collection as it does not match path template {1}".format(
                    self.collection_info.full_name, path_template
                ),
            )
    ...
    """
        ...
        ref = Resource(
            self.registry, self.collection_info, subcollection, param_values, base_url
        )
        return ref
    ...
    """
        return [
            prefix
            for idx, prefix in enumerate(path.split("/"))
...
class Resource(object):
    ...
    def __init__(
        self, registry, collection_info, subcollection, param_values, endpoint_url
    ):
    ...
    """
        ...
        effective_params = dict(
            [(k, escape_func(getattr(self, k), safe="")) for k in self._params]
        )

        return urllib.parse.unquote(uritemplate.expand(self._path, effective_params))
    ...
    """
        ...
        all_collections = self._registry.parsers_by_collection[
            self._collection_info.api_name
        ...
        ref = self._registry.Parse(
            None, parent_param_values, collection=parent_collection
        )
        return ref
...
def _GRIsAreEnabled():
    ...
    return (
        properties.VALUES.core.enable_gri.GetBool()
        or properties.VALUES.core.resource_completion_style.Get() == "gri"
    )
...
class GRIPathMismatchException(GRIException):
    ...
    def __init__(self, gri, params, collection=None):
        super(GRIPathMismatchException, self).__init__(
            ...
            "resource type. It must match the format: [{format}]".format(
                gri=gri,
                format=(
...
class Registry(object):
  ...
  """
    ...
    def __init__(
        self, parsers_by_collection=None, parsers_by_url=None, registered_apis=None
    ):
    ...
    """
        ...
        collection_parsers = self.parsers_by_collection.setdefault(
            api_name, {}
    ...
    """
        ...
        parser = (
            self.parsers_by_collection.get(api_name, {})
            .get(api_version, {})
            .get(collection, None)
        )
        if parser is None:
    ...
    """
        if _GRIsAreEnabled():
            ...
            collection = GRI.FromString(
                resource_id, collection=collection, validate=validate
            ).collection
        ...
        base_url = GetApiBaseUrl(
            parser.collection_info.api_name, parser.collection_info.api_version
        )
    ...
    """
        ...
        api_name, api_version, resource_path = resource_util.SplitDefaultEndpointUrl(
            default_enpoint_url
        )
        ...
        return parser.ParseResourceId(
            None, params, base_url=endpoint, subcollection=subcollection
        )

    def ParseRelativeName(
        self, relative_name, collection, url_unescape=False, api_version=None
    ):
        ...
        if match.group(2):
            if collection and collection != "storage.objects":
                raise WrongResourceCollectionException(
                    "storage.objects", collection, url
                )
            ...
            if line.startswith("https://") or line.startswith("http://"):
                try:
                    ...
                    if line.startswith(_GCS_URL):
                        try:
                            bucket_prefix, bucket, object_prefix, objectpath = line[
                                len(_GCS_URL) :
                            ].split("/", 3)
                        except ValueError:
                            raise e
                ...
                if enforce_collection and collection and ref.Collection() != collection:
                    raise WrongResourceCollectionException(
                        expected=collection, got=ref.Collection(), path=ref.SelfLink()
                    )
                return ref
