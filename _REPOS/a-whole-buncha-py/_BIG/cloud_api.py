...
class CloudApi(object):
  ...
  """
    ...
    def SetObjectIamPolicy(
        self, bucket_name, object_name, policy, generation=None, provider=None
    ):
    ...
    def WatchBucket(
        self, bucket_name, address, channel_id, token=None, provider=None, fields=None
    ):
    ...
    def ListHmacKeys(self, project_id, service_account_email, show_deleted_keys=False):
...
class BucketNotFoundException(NotFoundException):
    ...
    def __init__(self, reason, bucket_name, status=None, body=None):
        super(BucketNotFoundException, self).__init__(reason, status=status, body=body)
