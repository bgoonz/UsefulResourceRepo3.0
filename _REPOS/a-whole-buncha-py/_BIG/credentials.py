...
class ProfileProviderBuilder(object):
    ...
    def _create_process_provider(self, profile_name):
        return ProcessProvider(
            profile_name=profile_name, load_config=lambda: self._session.full_config
        )
    ...
    def _create_shared_credential_provider(self, profile_name):
        ...
        return SharedCredentialProvider(
            profile_name=profile_name, creds_filename=credential_file
        )
    ...
    def _set_from_data(self, data):
        ...
        if missing_keys:
            ...
            raise CredentialRetrievalError(
                provider=self.method, error_msg=message % ", ".join(missing_keys)
            )
    ...
    def _resolve_credentials_from_profile(self, profile_name):
        ...
        if self._has_static_credentials(profile) and not self._profile_provider_builder:
            ...
            profile_providers = self._profile_provider_builder.providers(
                profile_name=profile_name, disable_env_vars=True
            )
