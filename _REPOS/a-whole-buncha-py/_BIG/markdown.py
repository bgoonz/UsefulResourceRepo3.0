...
class MarkdownGenerator(six.with_metaclass(abc.ABCMeta, object)):
  ...
  """
    ...
    """
        ...
        self._out(
            "{command} - {index}\n".format(
                command=self._command_name, index=_GetIndexFromCapsule(self._capsule)
            )
        )
    ...
    def GetArgDetails(self, arg, depth=0):
        ...
        if getattr(arg, "detailed_help", None):
            raise ValueError(
                '{}: Use add_argument(help=...) instead of detailed_help="""{}""".'.format(
                    self._command_name, getattr(arg, "detailed_help")
                )
            )
