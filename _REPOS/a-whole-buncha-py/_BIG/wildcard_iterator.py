...
class CloudWildcardIterator(WildcardIterator):
  ...
  """
    ...
    def __init__(
        self, wildcard_url, gsutil_api, all_versions=False, project_id=None, logger=None
    ):
    ...
    """
        ...
        for bucket_listing_ref in self._ExpandBucketWildcards(bucket_fields=["id"]):
            ...
            if self.wildcard_url.IsBucket():
                    ...
                    for obj_or_prefix in self.gsutil_api.ListObjects(
                        ...
                        if (
                            ...
                            yield self._GetPrefixRef(
                                bucket_url_string, obj_or_prefix.data
                            )
                else:
                ...
                if (
                    not ContainsWildcard(self.wildcard_url.url_string)
                        ...
                        for pattern in regex_patterns:
                            if (
                                ...
                                if pattern.match(rstripped_prefix):
                                    if (
                                        suffix_wildcard
                                        and rstripped_prefix != suffix_wildcard
                                    ):
                                        # There's more wildcard left to expand.
                                        url_append_string = "%s%s" % (
                                            bucket_url_string,
                                            rstripped_prefix + "/" + suffix_wildcard,
                                        )
    ...
    """
        ...
        self.logger.debug(
            ...
            PrintableStr(prefix),
            PrintableStr(delimiter),
            PrintableStr(prefix_wildcard),
    ...
    """
        ...
        elif self.wildcard_url.IsBucket() and not ContainsWildcard(
            self.wildcard_url.bucket_name
        ):
    ...
    """
        for blr in self.__iter__(
            bucket_listing_fields=bucket_listing_fields, expand_top_level_buckets=True
        ):
            if blr.IsObject():
                yield blr
            ...
            if self.logger:
                for dirname in dirnames:
                    ...
                    if os.path.islink(full_dir_path):
                        self.logger.info(
                            'Skipping symlink directory "%s"', full_dir_path
                        )
...
def CreateWildcardIterator(
  ...
  """
    ...
    if url.IsFileUrl():
        return FileWildcardIterator(url, ignore_symlinks=ignore_symlinks, logger=logger)
    else:  # Cloud URL
