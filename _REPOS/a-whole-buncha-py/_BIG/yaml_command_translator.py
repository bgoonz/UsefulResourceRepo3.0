...
from googlecloudsdk.command_lib.util.declarative import (
    flags as declarative_config_flags,
)
...
class DeclarativeIamRolesCompleter(completers.ListCommandCompleter):
  ...
  """
    ...
    def Complete(self, prefix, parameter_info):
        ...
        return [
            item for item in items or [] if item is not None and item.startswith(prefix)
        ]
...
class CommandBuilder(object):
    ...
    def ConfigureCommand(self):
        ...
        if (
            not self.display_resource_type
            and resource_arg
    ...
    """
        if self.spec.command_type == yaml_command_schema.CommandType.DESCRIBE:
        ...
        elif self.spec.command_type == yaml_command_schema.CommandType.GET_IAM_POLICY:
            command = self._GenerateGetIamPolicyCommand()
        elif self.spec.command_type == yaml_command_schema.CommandType.SET_IAM_POLICY:
            ...
            raise ValueError(
                "Command [{}] unknown command type [{}].".format(
            ...
            def Run(self_, args):
                ...
                if self.spec.async_:
                    response = self._HandleAsync(
                        ...
                        ref,
                        response,
                        request_string="Delete request issued for: [{{{}}}]".format(
        ...
        class Command(base.Command):
            ...
            def Run(self_, args):
                ...
                response = self._WaitForOperation(
                    ref, resource_ref=None, extract_resource_result=False, args=args
                )
                response = self._HandleResponse(response, args)
                return response
    ...
    """
        ...
        from googlecloudsdk.command_lib.export import (
            util as export_util,
          ...
          """,
            ...
            def Run(self_, args):
                ...
                if args.IsSpecified("destination"):
                    ...
                    return log.status.Print(
                        "Exported [{}] to '{}'.".format(response.name, args.destination)
                    )
                else:
                    export_util.Export(
        ...
        if self.spec.input.confirmation_prompt:
            console_io.PromptContinue(
                message=self._Format(
                    self.spec.input.confirmation_prompt,
                    ref,
        ...
        if self.spec.iam:
            ...
            if "set_iam_policy_request" in overrides:
                set_iam_policy_request = (
                    overrides["set_iam_policy_request"] or set_iam_policy_request
                )
            policy_request_path = (
                self.spec.iam.set_iam_policy_request_path or policy_request_path
            )
    ...
    def _GetIamPolicy(self, args):
        ...
        get_iam_method = registry.GetMethod(
            self.spec.request.collection, "getIamPolicy", self.spec.request.api_version
        )
    ...
    """
        ...
        if add_condition:
            ...
            iam_util.AddBindingToIamPolicy(
                binding_message_type, policy, args.member, args.role
            )
        return policy
    ...
    def _GetExistingResource(self, args):
        get_method = registry.GetMethod(
            self.spec.request.collection, "get", self.spec.request.api_version
        )
    ...
    def _WaitForOperation(
        self, operation_ref, resource_ref, extract_resource_result, args=None
    ):
        poller = AsyncOperationPoller(
            self.spec, resource_ref if extract_resource_result else None, args
        )
    ...
    """
        if resource_ref:
            ...
            d[yaml_command_schema.NAME_FORMAT_KEY] = display_name or resource_ref.Name()
...
class AsyncOperationPoller(waiter.OperationPoller):
    ...
    def IsDone(self, operation):
        ...
        if isinstance(result, apitools_messages.Enum):
            result = result.name
        if (
            result in self.spec.async_.state.success_values
            or result in self.spec.async_.state.error_values
        ):
