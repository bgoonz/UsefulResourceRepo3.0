...
def ArgsForClusterRef(
  ...
  """
    ...
    gce_platform_group = platform_group.add_argument_group(
        help="""\
    ...
    parser.add_argument(
        "--worker-machine-type",
        help="The type of machine to use for workers. Defaults to " "server-specified.",
    )
...
def _AddAcceleratorArgs(parser):
    ...
    parser.add_argument(
        "--master-accelerator",
        type=arg_parsers.ArgDict(spec={"type": str, "count": int}),
...
def GetClusterConfig(
  ...
  """
    ...
    network_ref = args.network and compute_resources.Parse(
        args.network, params={"project": project_id}, collection="compute.networks"
    )
    ...
    secondary_worker_boot_disk_size_gb = api_utils.BytesToGb(
        _FirstNonNone(
            args.secondary_worker_boot_disk_size, args.preemptible_worker_boot_disk_size
        )
    )
    ...
    if not beta:
        if args.identity_config_file or args.secure_multi_tenancy_user_mapping:
            ...
            if args.identity_config_file:
                ...
                user_service_account_mapping = ParseSecureMultiTenancyUserServiceAccountMappingString(
                    args.secure_multi_tenancy_user_mapping
                )
                identity_config = dataproc.messages.IdentityConfig()
                identity_config.userServiceAccountMapping = encoding.DictToAdditionalPropertyMessage(
    ...
    secondary_worker_boot_disk_type = _FirstNonNone(
        args.secondary_worker_boot_disk_type, args.preemptible_worker_boot_disk_type
    )
    num_secondary_worker_local_ssds = _FirstNonNone(
        args.num_secondary_worker_local_ssds, args.num_preemptible_worker_local_ssds
    )
    if (
...
def CreateCluster(
  ...
  """
    ...
    if is_async:
        log.status.write(
            "Creating [{0}] with operation [{1}].".format(cluster_ref, operation.name)
        )
        return
...
def DeleteGeneratedProperties(cluster, dataproc):
  ...
  """
    if (
        not cluster.config
        or not cluster.config.softwareConfig
        or not cluster.config.softwareConfig.properties
    ):
        return
...
def AddKerberosGroup(parser):
    ...
    kerberos_group = parser.add_argument_group(
        mutex=True, help="Specifying these flags will enable Kerberos for the cluster."
    )
    # Not mutually exclusive
