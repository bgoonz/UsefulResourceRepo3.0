...
class _Common(six.with_metaclass(abc.ABCMeta, object)):
    ...
    """
        for track in ReleaseTrack._ALL:  # pylint: disable=protected-access
            if (
                track not in cls._valid_release_tracks
...
class TopicCommand(six.with_metaclass(abc.ABCMeta, Command)):
    ...
    def Run(self, args):
        self.ExecuteCommandDoNotUse(args.command_path[1:] + ["--document=style=topic"])
        return None
...
def ChoiceArgument(
  ...
  """
    ...
    if not isinstance(choices, collections_abc.Iterable) or isinstance(
        choices, six.string_types
    ):
        raise TypeError(
...
def DisableUserProjectQuota():
    ...
    if not properties.VALUES.billing.quota_project.IsExplicitlySet():
        properties.VALUES.billing.quota_project.Set(properties.VALUES.billing.LEGACY)
