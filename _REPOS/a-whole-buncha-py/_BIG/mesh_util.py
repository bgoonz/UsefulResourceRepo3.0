...
_SERVICE_PROXY_INSTALLER_BUCKET_NAME = (
    "gs://gce-service-proxy/service-proxy-agent-installer/" "releases/installer.tgz"
)
_GCE_SERVICE_PROXY_ASM_VERSION_METADATA = "gce-service-proxy-asm-version"
_GCE_SERVICE_PROXY_INSTALLER_BUCKET_METADATA = "gce-service-proxy-installer-bucket"
...
ServiceProxyMetadataArgs = collections.namedtuple(
    "ServiceProxyMetadataArgs",
    [
        "asm_version",
        "project_id",
        "expansionagateway_ip",
        "service_proxy_api_server",
        "identity_provider",
        "service_account",
        "asm_proxy_config",
        "mcp_env_config",
...
def _GetVMIdentityProvider(membership_manifest, workload_namespace):
  ...
  """
    ...
    if not owner_id:
        raise ClusterError(
            "Invalid membership does not have an owner id. Please "
    ...
    if not membership.uniqueId:
        raise exceptions.Error(
            "Invalid membership {} does not have a unique_Id "
...
def _GetWorkloadLabels(workload_manifest):
  ...
  """
    ...
    workload_labels = _GetNestedKeyFromManifest(
        workload_data, "spec", "metadata", "labels"
    )
...
def VerifyWorkloadSetup(workload_manifest):
    ...
    if identity_provider_value != "google":
        raise WorkloadError(
            "Unable to find the GCE IdentityProvider in the "
...
def RetrieveWorkloadRevision(namespace_manifest):
    ...
    workload_revision = _GetNestedKeyFromManifest(
        namespace_data, "metadata", "labels", "istio.io/rev"
    )
    if not workload_revision:
        raise WorkloadError(
            "Workload namespace does not have an Anthos Service "
...
def _RetrieveWorkloadServiceAccount(workload_manifest):
    ...
    service_account = _GetNestedKeyFromManifest(
        workload_data, "spec", "template", "serviceAccount"
    )
    return service_account


def _RetrieveServiceProxyMetadata(
    args,
    is_mcp,
    kube_client,
    project_id,
    ...
    if is_mcp:
        ...
        service_proxy_api_server = "{}:{}".format(
            expansionagateway_ip, _ISTIO_DISCOVERY_PORT
        )
        env_config = None
...
def _ModifyInstanceTemplate(args, is_mcp, metadata_args):
    ...
    asm_labels[
        _ISTIO_CANONICAL_SERVICE_REVISION_LABEL
    ] = metadata_args.canonical_revision
    ...
    if is_mcp:
        service_proxy_agent_recipe["installSteps"] = [
            {
                "scriptRun": {
                    "script": service_proxy_aux_data.startup_script_for_asm_service_proxy_installer
                }
            }
        ]
        ...
        service_proxy_agent_recipe["installSteps"] = [
            {
                "scriptRun": {
                    "script": service_proxy_aux_data.startup_script_for_asm_service_proxy.format(
...
class KubernetesClient(object):
    ...
    """
        for ns in namespaces:
            ...
            if err:
                raise exceptions.Error(
                    "Failed to check if the user can read resources in {} namespace: {}".format(
                        ns, err
                    )
                )
            if "yes" not in out:
    ...
    """
        for ns in namespaces:
            ...
            if err:
                if "NotFound" in err:
                    raise exceptions.Error(
                        "Namespace {} does not exist: {}".format(ns, err)
                    )
                raise exceptions.Error(
    ...
    def GetNamespace(self, namespace):
        ...
        out, err = self._RunKubectl(["get", "namespace", namespace, "-o", "yaml"], None)
        if err:
            raise exceptions.Error(
    ...
    def GetMembershipCR(self):
        ...
        out, err = self._RunKubectl(
            ["get", "memberships.hub.gke.io", "membership", "-o", "yaml"], None
        )
        if err:
            if "NotFound" in err:
                raise ClusterError(
            ...
            raise exceptions.Error("Error retrieving the Membership CR: {}".format(err))
        return out
    ...
    def GetIdentityProviderCR(self):
        ...
        if err:
            ...
            raise exceptions.Error(
                "Error retrieving IdentityProvider google in default namespace: {}".format(
                    err
                )
            )
        return out
    ...
    def RetrieveASMVersion(self, revision):
        ...
        image, err = self._RunKubectl(
            [
                ...
                "deploy",
                "-l",
                "istio.io/rev={},app=istiod".format(revision),
                "-n",
                "istio-system",
                "-o",
        ...
        if not image:
            raise ClusterError(
                "Anthos Service Mesh revision {} does not have an "
    ...
    def RetrieveMeshConfig(self, revision):
        ...
        if err:
            ...
            raise exceptions.Error(
                "Error retrieving the mesh config from the cluster: {}".format(err)
            )
    ...
    def RetrieveMutatingWebhookURL(self, revision):
        ...
        incluster_out, incluster_err = self._RunKubectl(
            [
                ...
                incluster_webhook,
                "-o",
                "jsonpath={.webhooks[0].clientConfig.url}",
            ],
            None,
        )
