...
class Update(base.UpdateCommand):
    ...
    """
        ...
        flags.AddMasterAuthorizedNetworksFlags(parser, enable_group_for_update=group)
    ...
    """
        ...
        if args.IsSpecified("username") or args.IsSpecified("enable_basic_auth"):
            ...
            args.generate_password or args.set_password or args.IsSpecified("password")
        ):
...
to completion."""
            console_io.PromptContinue(
                message=msg_tmpl.format(
                    name=cluster_name, project=cluster_ref.projectId, zone=cluster_zone
                ),
                cancel_on_no=True,
            )
            try:
    ...
    def MaybeLogDataplaneV2ScaleWarning(self, cluster):
        if (
            cluster.networkConfig is not None
            and cluster.networkConfig.datapathProvider is not None
            and cluster.networkConfig.datapathProvider.name == "ADVANCED_DATAPATH"
        ):
