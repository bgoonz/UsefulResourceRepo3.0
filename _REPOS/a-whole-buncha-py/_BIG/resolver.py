...
class NXDOMAIN(dns.exception.DNSException):
    ...
    canonical_name = property(
        canonical_name, doc=("Return the unresolved canonical name.")
    )
    ...
    def __init__(self, qname, rdtype, rdclass, response, raise_on_no_answer=True):
        ...
        for count in xrange(0, 15):
            try:
                ...
                if rdtype != dns.rdatatype.CNAME:
                    try:
                        crrset = response.find_rrset(
                            response.answer, qname, rdclass, dns.rdatatype.CNAME
                        )
        ...
        if rrset is None:
            while 1:
                ...
                # of qname.
                try:
                    srrset = response.find_rrset(
                        response.authority, qname, rdclass, dns.rdatatype.SOA
                    )
    ...
    def read_registry(self):
        ...
        try:
            try:
                ...
                # ME
                tcp_params = _winreg.OpenKey(
                    lm, r"SYSTEM\CurrentControlSet" r"\Services\VxD\MSTCP"
                )
            try:
        ...
        *lifetime*, a ``float``, how long query should run before timing out.
        ...
        for _qname in qnames_to_try:
            ...
            if self.keyname is not None:
                request.use_tsig(
                    self.keyring, self.keyname, algorithm=self.keyalgorithm
                )
            ...
            while response is None:
                ...
                for nameserver in nameservers[:]:
                    ...
                    try:
                        ...
                        # next server
                        #
                        errors.append((nameserver, tcp_attempt, port, ex, response))
                        response = None
                        continue
...
def query(
    ...
    return get_default_resolver().query(
        qname, rdtype, rdclass, tcp, source, raise_on_no_answer, source_port, lifetime
    )
            ...
            if flags & socket.AI_NUMERICHOST == 0:
                try:
                    if family == socket.AF_INET6 or family == socket.AF_UNSPEC:
                        v6 = _resolver.query(
                            host, dns.rdatatype.AAAA, raise_on_no_answer=False
                        )
                    ...
                    if family == socket.AF_INET or family == socket.AF_UNSPEC:
                        v4 = _resolver.query(
                            host, dns.rdatatype.A, raise_on_no_answer=False
                        )
                        host = v4.qname
