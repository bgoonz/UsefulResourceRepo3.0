        ...
        if root_command_args:
            with files.FileWriter(os.devnull) as devnull:
                try:
                    ...
                    subprocess.Popen(
                        root_command_args, stdin=devnull, stdout=devnull, stderr=devnull
                    ).communicate()
                except OSError as e:
    ...
    def SubTree(self, path):
        ...
        while True:
            ...
            if not heading:
                ...
                if content:
                    command[cli_tree.LOOKUP_CAPSULE] = (
                        content[0].split("-", 1)[1].strip()
                    )
        ...
        try:
            with files.FileWriter(os.devnull) as f:
                return encoding.Decode(
                    subprocess.check_output(["man", self.command_name], stderr=f)
                )
    ...
    if command_name == cli_tree.DEFAULT_CLI_NAME:
        ...
        with files.TemporaryDirectory() as tmp:
            ...
            tree = _LoadOrGenerate(
                os.path.join(tmp, command_relative_path), command_name
            )
    else:
  ...
  """
    ...
    for command in sorted(commands):
        if command != cli_tree.DEFAULT_CLI_NAME:
            ...
            if (
                force
                or not completion_tree_mtime
                or completion_tree_mtime < cli_tree_mtime
            ):
...
def LoadAll(
    directory=None, ignore_out_of_date=False, root=None, warn_on_exceptions=True
):
  ...
  """
    ...
    if cli_tree.DEFAULT_CLI_NAME not in root[cli_tree.LOOKUP_COMMANDS]:
        try:
            root[cli_tree.LOOKUP_COMMANDS][cli_tree.DEFAULT_CLI_NAME] = cli_tree.Load()
