...
def html_tableify(item_matrix, select=None, header=None, footer=None):
    ...
    if select:
        row, col = select
        tds_items[row][col] = (
            '<td class="inverted">' + item_matrix[row][col] + "  </td>"
        )
...
class CompletionHtml(QtWidgets.QWidget):
    ...
    def __init__(self, console_widget):
        ...
        """
        assert isinstance(
            console_widget._control, (QtWidgets.QTextEdit, QtWidgets.QPlainTextEdit)
        )
        super().__init__()
    ...
    def _select_index(self, row, col):
        ...
        # case 6
            ...
            raise NotImplementedError(
                "you'r trying to go where no completion\
                           have gone before : %d:%d (%d:%d)"
                % (row, col, nr, nc)
            )
    ...
    def show_items(self, cursor, items, prefix_length=0):
        ...
        items_m, ci = text.compute_item_matrix(
            items, empty=" ", displaywidth=displaywidth
        )
    ...
    def _update_list(self, hilight=True):
        ...
        items_m = self._justified_items[
            self._sliding_interval.start : self._sliding_interval.stop + 1
        ]
