...
def generate_html_error_report(
    ...
    tester.evaluate(
        data, "Error Analysis Run", error_log, filter_language=filter_language
    )
    ...
    with open(outfile, "w") as f:
        ...
        for query_result in tqdm(error_log, total=len(error_log)):
            with io.StringIO() as sb:
                ...
                sb.write(
                    f"<strong>Target Snippet</strong>\n{to_highlighted_html(target_code, language=language)}\n"
                )
                sb.write(
                    f"Target snippet was ranked at position <strong>{query_result.target_rank}</strong>.\n"
                )
                ...
                for pos, sample_idx in enumerate(query_result.top_ranked_idxs):
                    ...
                    sb.write(
                        f'{data[sample_idx]["repo"]} {data[sample_idx]["path"]}:{data[sample_idx]["lineno"]}\n'
                    )
...
def run(arguments):
    ...
    tester = MrrSearchTester(
        ...
        hypers_override=hypers_override,
    )

    # Load dataset
    if arguments["--standard-dataset"] or arguments["--method2code-dataset"]:
        data = model_test.get_dataset_from(
            test_data_dirs, use_func_names=arguments["--method2code-dataset"]
        )
    else:
        raise Exception(f"No dataset option seems to have been passed in.")

    generate_html_error_report(
        tester=tester,
        data=data,
