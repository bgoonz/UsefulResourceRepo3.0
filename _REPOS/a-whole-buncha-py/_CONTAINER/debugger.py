...
class Bdb(StdBdb, object):
    ...
    def get_stack_data(self, frame, traceback, event_type):
        ...
        for frame_ind, (frame, lineno) in enumerate(stack_frames):
            ...
            if not self.is_notebook_frame(frame) or self.is_other_cell_frame(frame):
            ...
            user_locals = filter_dict(
                frame.f_locals, ignore_vars + list(self.ipy_shell.user_ns_hidden.keys())
            )
        ...
        if not skip_this_stack and not stack_data.is_empty():
            self.trace_history.append(stack_data, heap_data, self.stdout.getvalue())
