...
crypto_generichash_PERSONALBYTES = lib.crypto_generichash_blake2b_personalbytes()
...
def _checkparams(digest_size, key, salt, person):
    """Check hash paramters"""
        ...
        isinstance(key, bytes), "Key must be a bytes sequence", raising=exc.TypeError
    )

    ensure(
        isinstance(salt, bytes), "Salt must be a bytes sequence", raising=exc.TypeError
    )
