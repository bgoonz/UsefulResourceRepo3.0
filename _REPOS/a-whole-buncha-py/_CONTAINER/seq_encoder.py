...
class SeqEncoder(Encoder):
    ...
    def __init__(
        ...
        if hyperparameters["%s_use_bpe" % label]:
            assert not hyperparameters[
                "%s_use_subtokens" % label
            ], "Subtokens cannot be used along with BPE."
    ...
    def _make_placeholders(self):
        ...
        self.placeholders["tokens"] = tf.placeholder(
            tf.int32, shape=[None, self.get_hyper("max_num_tokens")], name="tokens"
        )
    ...
    @classmethod
    def _to_subtoken_stream(
        cls, input_stream: Iterable[str], mark_subtoken_end: bool
    ) -> Iterable[str]:
    ...
    def load_data_from_sample(
        ...
        if (
            not is_test
            and hyperparameters["fraction_using_func_name"] > 0.0
            and function_name
            and len(function_name) >= hyperparameters["min_len_func_name_for_query"]
        ):
            if encoder_label == "query":
                # Set the query tokens to the function name, broken up into its sub-tokens:
                data_holder[
                    QueryType.FUNCTION_NAME.value
                ] = split_identifier_into_parts(function_name)
        ...
        if (
            result_holder[f"{encoder_label}_tokens_mask_{QueryType.DOCSTRING.value}"]
            is None
            or int(
                np.sum(
                    result_holder[
    ...
    def extend_minibatch_by_sample(
        ...
        current_sample["tokens_lengths"] = sample[
            f"{self.label}_tokens_length_{query_type}"
        ]
        ...
        if (
            is_train
            and self.label == "query"
            and self.hyperparameters["query_random_token_frequency"] > 0.0
        ):
            ...
            insert_indices = np.array(
                [random.uniform(0.0, 1.0) for _ in range(length_without_padding)]
            )  # don't allow insertions in the padding
            insert_indices = (
                insert_indices < self.hyperparameters["query_random_token_frequency"]
            ...
            if len(insert_indices) > 0:
                ...
                tokens_to_add = [
                    random.randrange(0, len(self.metadata["common_tokens"]))
                    for _ in range(len(insert_indices))
                ]  # select one of the most common tokens for each location
                tokens_to_add = [
                    self.metadata["common_tokens"][token][0] for token in tokens_to_add
                ]  # get the word corresponding to the token we're adding
                tokens_to_add = [
                    self.metadata["token_vocab"].get_id_or_unk(token)
                ...
                for idx in range(
                    ...
                    if (
                        to_insert < len(insert_indices)
                        and idx == insert_indices[to_insert]
                    ):
                ...
                current_sample["tokens_mask"][
                    length_without_padding : length_without_padding + len(tokens_to_add)
                ] = 1.0
