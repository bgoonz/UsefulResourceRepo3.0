...
class crypto_sign_ed25519ph_state(object):
    ...
    def __init__(self):
        self.state = ffi.new("unsigned char[]", crypto_sign_ed25519ph_STATEBYTES)
...
def crypto_sign_ed25519ph_final_create(edph, sk):
    ...
    """
        ...
        ("secret key must be {0} " "bytes long").format(crypto_sign_SECRETKEYBYTES),
        raising=exc.TypeError,
    )
    signature = ffi.new("unsigned char[]", crypto_sign_BYTES)
    rc = lib.crypto_sign_ed25519ph_final_create(edph.state, signature, ffi.NULL, sk)
    ensure(rc == 0, "Unexpected library error", raising=exc.RuntimeError)
