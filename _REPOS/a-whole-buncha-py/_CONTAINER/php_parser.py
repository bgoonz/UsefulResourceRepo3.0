...
class PhpParser(LanguageParser):
    ...
    def get_declarations(
        declaration_node, blob: str, node_type: str
    ) -> List[Dict[str, Any]]:
        declarations = []
                    ...
                    declarations.append(
                        {
                            "type": node_type,
                            "identifier": "{}.{}".format(
                                declaration_name, metadata["identifier"]
                            ),
    ...
    def get_definition(tree, blob: str) -> List[Dict[str, Any]]:
        ...
        for trait_declaration in trait_declarations:
            definitions.extend(
                PhpParser.get_declarations(
                    trait_declaration, blob, trait_declaration.type
                )
            )
        for class_declaration in class_declarations:
            definitions.extend(
                PhpParser.get_declarations(
                    class_declaration, blob, class_declaration.type
                )
            )
        return definitions
