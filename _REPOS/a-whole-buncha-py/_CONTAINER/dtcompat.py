...
class DocTestRunner:
    ...
    __LINECACHE_FILENAME_RE = re.compile(
        r"<doctest " r"(?P<name>[\w\.]+)" r"\[(?P<examplenum>\d+)\]>$"
    )
...
class Tester:
    def __init__(
        self, mod=None, globs=None, verbose=None, isprivate=None, optionflags=0
    ):

        warnings.warn(
            "class Tester is deprecated; " "use class doctest.DocTestRunner instead",
            DeprecationWarning,
            stacklevel=2,
        )
...
def debug_script(src, pm=False, globs=None):
    ...
    try:
        ...
        if pm:
            try:
                exec(
                    compile(open(srcfilename).read(), srcfilename, "exec"), globs, globs
                )
            except:
