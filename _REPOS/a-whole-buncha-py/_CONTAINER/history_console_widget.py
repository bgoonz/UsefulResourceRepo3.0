...
class HistoryConsoleWidget(ConsoleWidget):
    ...
    def _down_pressed(self, shift_modifier):
        ...
        if self._get_cursor().blockNumber() == end_cursor.blockNumber():
            ...
            # Perform the search.
            replaced = self.history_next(
                self._history_prefix, as_prefix=not shift_modifier
            )
    ...
    def history_previous(self, substring="", as_prefix=True):
        ...
        while index > 0:
            ...
            if history == self.input_buffer:
                continue
            if (as_prefix and history.startswith(substring)) or (
                not as_prefix and substring in history
            ):
                replace = True
                break
    ...
    def _history_locked(self):
        ...
        return (
            self.history_lock
            and (self._get_edited_history(self._history_index) != self.input_buffer)
            and (
    ...
    def _store_edits(self):
        ...
        if (
            self._history_index == len(self._history)
            or self._history[self._history_index] != current
        ):
