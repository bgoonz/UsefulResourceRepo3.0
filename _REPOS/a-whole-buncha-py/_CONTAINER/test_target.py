...
class TestTarget(BaseTestCase):
    ...
    async def test_default_page(self):
        ...
        self.assertEqual(
            await page.evaluate('["Hello", "world"].join(" ")'), "Hello world"
        )
    ...
    async def test_report_new_page(self):
        ...
        self.context.once(
            "targetcreated", lambda target: otherPagePromise.set_result(target)
        )
        await self.page.evaluate(
            "url => window.open(url)", "http://127.0.0.1:{}".format(self.port)
        )
        otherPage = await (await otherPagePromise).page()

        self.assertIn("127.0.0.1", otherPage.url)
        self.assertEqual(
            await otherPage.evaluate('["Hello", "world"].join(" ")'), "Hello world"
        )
        ...
        self.context.once(
            "targetdestroyed", lambda t: asyncio.ensure_future(get_close_page(t))
        )
    ...
    async def test_report_service_worker(self):
        ...
        self.context.once(
            "targetdestroyed", lambda t: destroyedTargetPromise.set_result(t)
        )
        await self.page.evaluate(
    ...
    async def test_opener(self):
        ...
        targetPromise = asyncio.get_event_loop().create_future()
        self.context.once(
            "targetcreated", lambda target: targetPromise.set_result(target)
        )
