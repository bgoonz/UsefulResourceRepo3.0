...
class Installer(object):
    ...
    def find_version_to_install(self, name):
        ...
        try:
            ...
            raise ValueError("no installable version found for {0!r}".format(name))
        return best_match
...
class Pyenv(Installer):
    ...
    def install(self, version):
        ...
        """
        c = self._run("install", "-s", str(version), timeout=PIPENV_INSTALL_TIMEOUT)
        return c
...
class Asdf(Installer):
    ...
    def install(self, version):
        ...
        """
        c = self._run("install", "python", str(version), timeout=PIPENV_INSTALL_TIMEOUT)
        return c
