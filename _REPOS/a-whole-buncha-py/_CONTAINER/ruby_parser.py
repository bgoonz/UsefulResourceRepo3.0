...
class RubyParser(LanguageParser):
    ...
    @staticmethod
    def get_methods(
        module_or_class_node, blob: str, module_name: str, node_type: str
    ) -> List[Dict[str, Any]]:
        definitions = []
        comment_buffer = []
        ...
        for child in module_or_class_node.children:
            if child.type == "comment":
                ...
                docstring = "\n".join(
                    [
                        match_from_span(comment, blob).strip().strip("#")
                ...
                definitions.append(
                    {
                        "type": "class",
                        "identifier": "{}.{}.{}".format(
                            module_name, module_or_class_name, metadata["identifier"]
                        ),
    ...
    def get_definition(tree, blob: str) -> List[Dict[str, Any]]:
        ...
        if "ERROR" not in set([child.type for child in tree.root_node.children]):
            modules = [
                child for child in tree.root_node.children if child.type == "module"
            ]
            for module in modules:
                if module.children:
                    ...
                    sub_modules = [
                        ...
                        if child.type == "module" and child.children
                    ]
                    classes = [
                        child for child in module.children if child.type == "class"
                    ]
                    for sub_module_node in sub_modules:
                        definitions.extend(
                            RubyParser.get_methods(
                                sub_module_node, blob, module_name, sub_module_node.type
                            )
                        )
                    for class_node in classes:
                        definitions.extend(
                            RubyParser.get_methods(
                                class_node, blob, module_name, class_node.type
                            )
                        )
        return definitions
    ...
    def get_function_metadata(function_node, blob: str) -> Dict[str, str]:
        metadata = {"identifier": "", "parameters": ""}
