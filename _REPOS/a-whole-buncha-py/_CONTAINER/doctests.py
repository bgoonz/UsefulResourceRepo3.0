...
class Doctest(Plugin):
    ...
    def matches(self, name):
        ...
        return (
            ...
            or (
                self.conf.include
                and [_f for _f in [inc.search(name) for inc in self.conf.include] if _f]
            )
        ) and (
            not self.conf.exclude
            or not [_f for _f in [exc.search(name) for exc in self.conf.exclude] if _f]
        )
    ...
    def wantFile(self, file):
        ...
        if (
            ...
            and (
                not self.conf.exclude
                or not [
                    _f for _f in [exc.search(file) for exc in self.conf.exclude] if _f
                ]
            )
        ):
            return True
