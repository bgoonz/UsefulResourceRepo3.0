...
def patch_jupyter_dirs():
    ...
    for name in ("notebook", "runtime"):
        d = jupyter_dirs["server"][name] = os.path.join(test_dir, "server", name)
    ...
    for mod in modules_to_patch:
        applicable_patches = {
            attrname: newval
            for attrname, newval in path_patches.items()
            if hasattr(mod, attrname)
        }
        jupyter_patches.append(patch.multiple(mod, **applicable_patches))
