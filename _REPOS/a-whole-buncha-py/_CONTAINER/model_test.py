...
class MrrSearchTester:
    ...
    QueryResult = NamedTuple(
        "QueryResult",
        [("target_idx", int), ("target_rank", int), ("top_ranked_idxs", List[int])],
    )
    ...
    def evaluate(
        ...
        if filter_language is None:
            ...
            idxs = np.array(
                [i for i in range(len(data)) if data[i]["language"] == filter_language]
            )
        if len(idxs) == 0:
        ...
        def self_or_random_representation(
            representation: Optional[np.ndarray]
        ) -> np.ndarray:
        ...
        examples_sample[
            np.random.choice(
                np.arange(full_batch_len),
                replace=False,
                size=min(full_batch_len, max_samples),
            )
        ] = True
        examples_table = []
        ...
        for batch_idx, (batch_data, batch_sample) in enumerate(
            zip(batched_data, batched_sample)
        ):
            ...
            assert (
                len(code_representations)
                == len(query_representations)
                == self.__test_batch_size
            )

            # Construct numpy batch
            num_uncomputed_representations = sum(
                1
                for i in range(self.__test_batch_size)
                if code_representations[i] is None or query_representations[i] is None
            )
            if num_uncomputed_representations > 0:
                print(
                    f"Ignoring {num_uncomputed_representations} samples whose representation could not be computed"
                )
            ...
            batch_code_representations = np.array(
                [
                    self_or_random_representation(code_representations[i])
                    for i in range(self.__test_batch_size)
                ],
                dtype=np.float32,
            )
            batch_query_representations = np.array(
                [
                    self_or_random_representation(query_representations[i])
                    for i in range(self.__test_batch_size)
                ],
                dtype=np.float32,
            )

            ranks, distances = compute_ranks(
                batch_code_representations,
            ...
            if wandb.run:
                ...
                for example, sample, rank in zip(batch_data, batch_sample, ranks):
                    ...
                    examples_table.append(
                        [rank, language, example["func_name"], markdown_code]
                    )

            sum_mrr += np.mean(1.0 / ranks)

            if error_log is not None:
                batch_sample_idxs = idxs[
                    batch_idx
                    * self.__test_batch_size : (batch_idx + 1)
                    * self.__test_batch_size
                ]
                for i in range(len(ranks)):
                    if ranks[i] >= error_log_rank_threshold:
                        result = MrrSearchTester.QueryResult(
                            ...
                            target_rank=ranks[i],
                            top_ranked_idxs=batch_sample_idxs[
                                np.argsort(distances[i])[:3]
                            ],
                        )
        ...
        if wandb.run and examples_table:
            wandb.log(
                {
                    "Examples-%s"
                    % examples_table_name: wandb.Table(
...
def expand_data_path(data_path: str, azure_info_path: Optional[str]) -> List[RichPath]:
    ...
    return [
        RichPath.create(data_dir, azure_info_path)
...
def log_row_count_diff(
    ...
    print(
        f"{label}: parsed {filtered_row_count:,} out of {original_row_count:,} rows. ({pcnt_parsed*100:.1f}%)"
    )
    if wandb.run:
...
def compute_evaluation_metrics(
    ...
    evaluation_sets = list(
        (l, True) for l in dataset_languages
    ...
    for language_name, filter_language in evaluation_sets:
        if (
            filter_language
            and language_name not in tester.model.per_code_language_metadata
        ):
            continue
        mrr = tester.evaluate(
            test_data,
            f"Test-{language_name}",
        ...
        mrr = tester.evaluate(
            get_dataset_from(
                test_data_dirs, use_func_names=True, max_files_per_dir=max_files_per_dir
            ),
        ...
        tester.evaluate(
            get_dataset_from(valid_data_dirs, max_files_per_dir=max_files_per_dir),
