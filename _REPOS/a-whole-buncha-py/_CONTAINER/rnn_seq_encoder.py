...
def _make_deep_rnn_cell(
    ...
    """
    if num_layers == 1:
        return __make_rnn_cell(
            cell_type, hidden_size, dropout_keep_rate, recurrent_dropout_keep_rate
        )
    else:
        cells = [
            __make_rnn_cell(
...
class RNNEncoder(SeqEncoder):
    ...
    def _encode_with_rnn(
        self, inputs: tf.Tensor, input_lengths: tf.Tensor
    ) -> Tuple[tf.Tensor, tf.Tensor]:
        cell_type = self.get_hyper("rnn_cell_type").lower()
        rnn_cell_fwd = _make_deep_rnn_cell(
            num_layers=self.get_hyper("rnn_num_layers"),
            cell_type=cell_type,
            hidden_size=self.get_hyper("rnn_hidden_dim"),
            dropout_keep_rate=self.placeholders["rnn_dropout_keep_rate"],
            recurrent_dropout_keep_rate=self.placeholders[
                "rnn_recurrent_dropout_keep_rate"
            ],
        )
        if not self.get_hyper("rnn_is_bidirectional"):
            ...
            if cell_type == "lstm":
                ...
                final_state = tf.concat(
                    [
                        tf.concat(layer_final_states, axis=-1)  # concat across layers
                        for layer_final_states in final_states
                    ],
                    axis=-1,
                )  # concat fwd & bwd
    ...
    def make_model(self, is_train: bool = False) -> tf.Tensor:
        with tf.variable_scope("rnn_encoder"):
            ...
            self.placeholders["tokens_lengths"] = tf.placeholder(
                tf.int32, shape=[None], name="tokens_lengths"
            )

            self.placeholders["rnn_dropout_keep_rate"] = tf.placeholder(
                tf.float32, shape=[], name="rnn_dropout_keep_rate"
            )

            self.placeholders["rnn_recurrent_dropout_keep_rate"] = tf.placeholder(
                tf.float32, shape=[], name="rnn_recurrent_dropout_keep_rate"
            )
            ...
            if output_pool_mode == "rnn_final":
                ...
                token_mask = tf.expand_dims(
                    tf.range(tf.shape(seq_tokens)[1]), axis=0
                )  # 1 x T
                token_mask = tf.tile(
                    token_mask, multiples=(tf.shape(seq_tokens_lengths)[0], 1)
                )  # B x T
                token_mask = tf.cast(
