...
InsertNote.prototype._prepareDialog = function () {
  ...
  if (!this.html) {
    Xinha._getback(
      Xinha.getPluginDir("InsertNote") + "/dialog.html",
      function (getback) {
        self.html = getback;
        self._prepareDialog();
      }
...
InsertNote.prototype.show = function () {
  ...
  for (var i = 0; i < orderedIds.length; i++) {
    ...
    if (displayName.length > 50) {
      // Truncate preview text to 50 chars
...
InsertNote.prototype.cursorInFootnote = function (ignoreMarkers) {
  ...
  for (var i = 0; i < ancestors.length; i++) {
    if (ancestors[i].id == this.NOTE_LIST_ID) return true;
...
InsertNote.prototype.repairNotes = function () {
  ...
  for (var i = 0; i < markers.length; i++) {
    ...
    if (marker)
      ...
      isEmpty = marker.innerHTML.match(
        /^(<span[^>]*>)?(<sup>)?(<a[^>]*>)?[\s]*(<\/a>)?(<\/sup>)?(<\/span>)?$/m
      );
    if (isEmpty) marker.parentNode.removeChild(marker);
  ...
  for (var id in this.notes) {
    ...
    if (note) {
      if (markers.length == 0) {
        // Remove note if it's not referenced anywhere
        // remove the note
  ...
  if (newNoteList) {
    ...
    if (noteList) noteList.parentNode.removeChild(noteList);
  }
};
...
InsertNote.prototype._getNextId = function (prefix) {
  ...
  while (this.editor._doc.getElementById(id)) id = Xinha.uniq(prefix);
  return id;
};
...
InsertNote.prototype._getOrderedNoteIds = function () {
  ...
  for (var id in this.notes) orderedIds.push(id);
...
InsertNote.prototype._getNumFromNoteId = function (noteId) {
  if (!this.noteNums[noteId]) return 0;
...
InsertNote.prototype._letterNum = function (num) {
  ...
  var s = "";
  for (var i = 0; i < len; i++) s += letters.substr(num % letters.length, 1);
  return s;
};
...
InsertNote.prototype.onKeyPress = function (event) {
  ...
  if (event.keyCode == 8 || event.keyCode == 46) {
    var self = this;
    if (!this.repairScheduled && !this.cursorInFootnote(true)) {
      // ignore if in a footnote
