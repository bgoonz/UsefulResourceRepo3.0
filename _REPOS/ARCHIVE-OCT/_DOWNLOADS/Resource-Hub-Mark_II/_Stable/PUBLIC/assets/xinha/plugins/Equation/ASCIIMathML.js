...
function init() {
  ...
  if (document.getElementById == null) {
    alert(
      "This webpage requires a recent browser such as Mozilla Firefox/Netscape 7+ or Internet Explorer 6+ with MathPlayer and Adobe SVGviewer"
    );
    return null;
  }
...
var AMcal = [
  0xef35, 0x212c, 0xef36, 0xef37, 0x2130, 0x2131, 0xef38, 0x210b, 0x2110,
  0xef39, 0xef3a, 0x2112, 0x2133, 0xef3b, 0xef3c, 0xef3d, 0xef3e, 0x211b,
  0xef3f, 0xef40, 0xef41, 0xef42, 0xef43, 0xef44, 0xef45, 0xef46,
];
var AMfrk = [
  0xef5d, 0xef5e, 0x212d, 0xef5f, 0xef60, 0xef61, 0xef62, 0x210c, 0x2111,
  0xef63, 0xef64, 0xef65, 0xef66, 0xef67, 0xef68, 0xef69, 0xef6a, 0x211c,
  0xef6b, 0xef6c, 0xef6d, 0xef6e, 0xef6f, 0xef70, 0xef71, 0x2128,
];
var AMbbb = [
  0xef8c, 0xef8d, 0x2102, 0xef8e, 0xef8f, 0xef90, 0xef91, 0x210d, 0xef92,
  0xef93, 0xef94, 0xef95, 0xef96, 0x2115, 0xef97, 0x2119, 0x211a, 0x211d,
  0xef98, 0xef99, 0xef9a, 0xef9b, 0xef9c, 0xef9d, 0xef9e, 0x2124,
];
...
function define(oldstr, newstr) {
  if (oldstr.substr(0, 1) == "\\")
    LMsymbols = LMsymbols.concat([
      { input: oldstr, tag: "mo", output: newstr, ttype: DEFINITION },
    ]);
  else
...
function parseMath(str, latex) {
  ...
  AMnestingDepth = 0;
  frag = latex
    ? LMparseExpr(str.replace(/^\s+/g, ""), false, false)[0]
...
function LMparseSexpr(str) {
  ...
  if (symbol == null || symbol.ttype == RIGHTBRACKET) return [null, str, null];
  if (symbol.ttype == DEFINITION) {
  ...
  switch (symbol.ttype) {
    ...
    case CONST:
      ...
      if (isIE) {
        ...
        else if (symbol.input == "''") output = "\u2033";
        else if (symbol.input == "'''") output = "\u2033\u2032";
        else if (symbol.input == "''''") output = "\u2033\u2033";
        else if (symbol.input == "\\square") output = "\u25A1";
        // same as \Box
    ...
    case STRETCHY: // added by DRW
      if (isIE && symbol.input == "\\backslash") symbol.output = "\\"; // doesn't expand, but then nor does "\u2216"
      node = createMmlNode(symbol.tag, document.createTextNode(symbol.output));
      if (
        symbol.input == "|" ||
        symbol.input == "\\vert" ||
      ...
      if (symbol.rtag != null) return [node, str, symbol.rtag];
      else return [node, str, symbol.tag];
    case BIG: // added by DRW
      var atval = symbol.atval;
      if (isIE) atval = symbol.ieval;
      symbol = LMgetSymbol(str);
    ...
    case MATRIX: //read (expr+)
      if (symbol.input == "\\begin{array}") {
        ...
        symbol = LMgetSymbol(str);
        str = LMremoveCharsAndBlanks(str, 0);
        if (symbol == null) mask = "l";
        else {
          str = LMremoveCharsAndBlanks(str, symbol.input.length);
          if (symbol.input != "{") mask = "l";
          else
            do {
              symbol = LMgetSymbol(str);
              if (symbol != null) {
                str = LMremoveCharsAndBlanks(str, symbol.input.length);
                if (symbol.input != "}") mask = mask + symbol.input;
              }
            } while (
              symbol != null &&
              symbol.input != "" &&
        ...
        if (isIE) return [node, result[1], null];
        ...
        result = LMparseExpr("{" + str, true, true);
        node = createMmlNode("mtable", result[0]);
        if (isIE) node.setAttribute("columnspacing", "0.25em");
        // best in practice?
        else node.setAttribute("columnspacing", "0.167em"); // correct (but ignored?)
    ...
    case UNARY:
      ...
      if (symbol.input == "\\sqrt") {
        ...
        var output = symbol.output;
        if (isIE) {
          if (symbol.input == "\\hat") output = "\u0302";
          else if (symbol.input == "\\widehat") output = "\u005E";
          else if (symbol.input == "\\bar") output = "\u00AF";
          else if (symbol.input == "\\grave") output = "\u0300";
          else if (symbol.input == "\\tilde") output = "\u0303";
        }
        ...
        else node1.setAttribute("accent", "true");
...
function LMparseIexpr(str) {
  ...
  if (symbol.ttype == INFIX) {
    ...
    if (symbol.input == "_" || symbol.input == "^") {
      ...
      node.appendChild(result[0]);
      if (symbol.input == "\\choose") node = createMmlNode("mfenced", node);
    }
  }
  return [node, str, tag];
}
...
function LMparseExpr(str, rightbracket, matrix) {
  ...
  if (symbol.ttype == RIGHTBRACKET) {
    ...
    if (symbol != null) str = LMremoveCharsAndBlanks(str, symbol.input.length); // ready to return
    var len = newFrag.childNodes.length;
    if (
      matrix &&
      len > 0 &&
...
var tcnt = 0,
  dcnt = 0; //theorem and definition counters
...
function ASCIIandgraphformatting(st) {
  ...
  st = st.replace(/QED/g, "&nbsp; &nbsp; &#x25A1;");
  st = st.replace(/(\\?end{?a?math}?)/gi, "<span></span>$1");
  st = st.replace(/(\bamath\b|\\begin{a?math})/gi, "<span></span>$1");
  st = st.replace(
    /([>\n])(Theorem|Lemma|Proposition|Corollary|Definition|Example|Remark|Problem|Exercise|Conjecture|Solution)(:|\W\W?(\w|\s|-|\.)*?\W?:)/g,
    "$1<b>$2$3</b>"
  );
  st = st.replace(
    /<embed\s+class\s?=\s?"?ASCIIsvg"?/gi,
  ...
  st = st.replace(
    ...
    function (s, t) {
      return (
        '<table><tr><td><div class="ASCIIsvg"><embed class="ASCIIsvg" src="' +
        dsvglocation +
        'd.svg" wmode="transparent" script=\'' +
        t.replace(/<\/?(br|p|pre)\s?\/?>/gi, "\n") +
        "'/></div></td></tr></table>"
      );
    }
  );
...
var cpi = "\u03C0",
  ctheta = "\u03B8"; // character for pi, theta
var log = function (x) {
  return ln(x) / ln(10);
};
var pi = Math.PI,
...
function chop(x, n) {
  // Truncate decimal number to n places after decimal point
  if (n == null) n = 0;
  return Math.floor(x * Math.pow(10, n)) / Math.pow(10, n);
}

function ran(a, b, n) {
  // Generate random number in [a,b] with n digits after .
  if (n == null) n = 0;
...
function showobj(obj) {
  ...
  for (i in obj)
    st +=
      obj.getAttribute(i) == null ? "" : " " + i + ":" + obj.getAttribute(i);
  return st;
}

function findPos(obj) {
  // top-left corner of obj on HTML page in pixel
...
function checkSVG() {
  if (navigator.appName.slice(0, 8) == "Netscape")
    ...
    // works only for 9.50b1
    noSVG = null;
  else noSVG = true;
  //noSVG = true; //uncomment to check
  if (noSVG && notifyIfNoSVG) {
    var msg =
      "To view the ASCIIsvg images use Internet Explorer + Adobe SVGviewer or Mozilla Firefox 2.0 or later.";
    if (alertIfNoSVG) alert(msg);
    else return msg;
  }
}

function setText(st, id) {
  // add text to an existing node with given id
...
function getX(evt) {
  // return mouse x-coord in user coordinate system
  ...
  return (
    (evt.clientX +
      ...
      pos[0] -
      svgroot.getAttribute("ox")) /
    parseFloat(svgroot.getAttribute("xunitlength"))
  );
}

function getY(evt) {
  // return mouse y-coord in user coordinate system
  var svgroot = evt.target.parentNode;
  pos = findPos(svgroot.parentNode);
  //alert(showobj(svgroot)+svgroot.getAttribute("mytop"))
  return (
    (svgroot.getAttribute("height") -
      svgroot.getAttribute("oy") -
      (evt.clientY + (isIE ? 0 : window.pageYOffset) - pos[1])) /
    parseFloat(svgroot.getAttribute("yunitlength"))
  );
}

function translateandeval(src) {
  //modify user input to JavaScript syntax
  var errstr;
...
function drawPictures() {
  // main routine; called after webpage has loaded
...
function setdefaults() {
  //called before each graph is evaluated
...
function switchTo(id) {
  // used by dynamic code to select appropriate graph
...
function changepicturesize(evt, factor) {
  ...
  src = src.replace(
    /width\s*=\s*\d+/,
    "width=" + factor * (pic.getAttribute("width") - 0)
  );
  src = src.replace(
    /height\s*=\s*\d+/,
    "height=" + factor * (pic.getAttribute("height") - 0)
  );
...
function mClick(evt) {
  if (sinceFirstClick != 0) {
    if (sinceFirstClick <= 40) {
      ...
      } else if (evt.altKey) zoom(evt, 2);
      //changepicturesize(evt,.5);
...
function line(p, q, id, endpts) {
  // segment connecting points p,q (coordinates in units)
  var node;
...
function path(plist, id, c, endpts) {
  ...
  if (typeof plist == "string") st = plist;
    ...
    st = "M";
    st +=
      plist[0][0] * xunitlength +
      origin[0] +
      "," +
      (height - plist[0][1] * yunitlength - origin[1]) +
      " " +
      c;
    for (i = 1; i < plist.length; i++)
      st +=
        plist[i][0] * xunitlength +
        origin[0] +
        "," +
  ...
  if (endpts != null) {
    ...
    if (endpts.indexOf("->") != -1)
      arrowhead(plist[plist.length - 2], plist[plist.length - 1]);
...
function loop(p, d, id) {
  ...
  if (marker == "arrow" || marker == "arrowdot")
    arrowhead(
      [
        p[0] + Math.cos(1.4) * d[0] - Math.sin(1.4) * d[1],
        p[1] + Math.sin(1.4) * d[0] + Math.cos(1.4) * d[1],
      ],
      p
    );
}

function arc(start, end, radius, id, largearc) {
  // coordinates in units
  var node, v;
  ...
  node.setAttribute(
    "d",
    "M" +
      (start[0] * xunitlength + origin[0]) +
      "," +
      (height - start[1] * yunitlength - origin[1]) +
      " A" +
      radius * xunitlength +
      "," +
      radius * yunitlength +
...
function sector(center, start, end, id) {
  // center,start,end should be isoceles
  var rx = start[0] - center[0],
    ry = start[1] - center[1];
...
function rect(p, q, id, rx, ry) {
  // opposite corners in units, rounded by radii
  var node;
  if (id != null) node = doc.getElementById(id);
  if (node == null) {
...
function text(p, st, pos, id, fontsty) {
  ...
  if (/(`|\$)/.test(str)) {
    ...
    if (dnode != null) {
      ...
      if (/`/.test(str)) AMprocessNode(node);
      else LMprocessNode(node);
...
function mtext(p, st, pos, fontsty, fontsz) {
  // method for updating text on an svg
...
function ASdot(center, radius, s, f) {
  // coordinates in units, radius in pixel
  if (s == null) s = stroke;
  if (f == null) f = fill;
...
function dot(center, typ, label, pos, id) {
  ...
  if (typ == "+" || typ == "-" || typ == "|") {
    ...
    node.setAttribute(
      "fill",
      typ == "open" ? "white" : typ == "closed" ? stroke : markerfill
    );
  }
  if (label != null)
    text(
...
function arrowhead(p, q) {
  // draw arrowhead at q (in units) add size param
  var up;
  var v = [
    p[0] * xunitlength + origin[0],
    height - p[1] * yunitlength - origin[1],
...
function mathjs(st) {
  ...
  while ((i = st.indexOf("^")) != -1) {
    ...
    if ((ch >= "0" && ch <= "9") || ch == "-") {
      ...
      k++;
      while (
        (k < st.length && (ch = st.charAt(k)) >= "a" && ch <= "z") ||
        (ch >= "A" && ch <= "Z")
      )
        k++;
    } else {
...
function displayCoord(evt) {
  if (showcoordinates) {
    ...
    cnode.mtext = mtext;
    cnode.mtext(
      [svgroot.getAttribute("width") - -7, svgroot.getAttribute("height") - 7],
      "(" + getX(evt).toFixed(2) + ", " + getY(evt).toFixed(2) + ")",
      "left",
      "",
      "11"
    );
...
function removeCoord(evt) {
  ...
  cnode.mtext = mtext;
  cnode.mtext(
    [svgroot.getAttribute("width") - 0, svgroot.getAttribute("height") - 0],
    "",
    "aboveleft",
    ""
  );
...
function calculate(inputId, outputId) {
  ...
  if (!isNaN(res) && res != "Infinity")
    str =
      ...
      str +
      " =` " +
      (Math.abs(res - Math.round(res * 1000000) / 1000000) < 1e-15
        ? Math.round(res * 1000000) / 1000000
        : res) +
      err;
