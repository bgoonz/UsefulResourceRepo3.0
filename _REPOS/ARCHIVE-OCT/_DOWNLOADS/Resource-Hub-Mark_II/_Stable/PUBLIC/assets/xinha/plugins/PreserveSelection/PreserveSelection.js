      ...
      if (target == "textarea") {
        ...
        if (range.startContainer.nodeType == 3) {
          ...
          if (range.startOffset) {
            var startTextNode = range.startContainer.insertBefore(
              this._doc.createTextNode(ccStart),
              range.startContainer.childNodes[range.startOffset]
            );
          } else {
            var startTextNode = range.startContainer.appendChild(
              this._doc.createTextNode(ccStart)
            );
          }
        } else {
        ...
        if (collapsed == false) {
          if (range.endContainer.nodeType == 3) {
            ...
            if (range.endOffset) {
              var endTextNode = range.endContainer.insertBefore(
                this._doc.createTextNode(ccEnd),
                range.endContainer.childNodes[range.endOffset]
              );
            } else {
              var endTextNode = range.endContainer.appendChild(
                this._doc.createTextNode(ccEnd)
              );
            }
          } else {
    ...
    if (target == "textarea") {
      ...
      try {
        ...
        doc.body.innerHTML = doc.body.innerHTML.replace(
          new RegExp(ccStart),
          '<span id="XinhaEditingStart"></span>'
        );
        doc.body.innerHTML = doc.body.innerHTML.replace(
          new RegExp(ccEnd),
        ...
        if (
          !endEl &&
          startEl.tagName &&
          startEl.tagName.toLowerCase().match(/table|img|input|textarea|select/)
        ) {
