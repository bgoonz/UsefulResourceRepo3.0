...
var jsGET = {
    ...
    if (this.vars.last_hash_loaded !== new_hash) {
      ...
      if (
        ...
        for (i = 0; i < hashVars.length; i++) {
          ...
          this.vars.current[this.decode(hashVar[0])] =
            typeof hashVar[1] !== "undefined" ? this.decode(hashVar[1]) : "";
        }
      }
  ...
  get: function (key) {
    this.load();
    return this.vars.current.hasOwnProperty(key)
      ? this.vars.current[key]
      : null;
  },
  set: function (set) {
    var key;
    ...
    for (key in this.vars.current) {
      if (this.vars.current.hasOwnProperty(key)) {
        if (set.hasOwnProperty(key)) {
          ...
          hashString +=
            sep + this.encode(key) + "=" + this.encode(this.vars.current[key]);
        }
        sep = "&";
      }
    }
  ...
  setChangedVars: function () {
    ...
    for (key in oldVars) {
      if (
        oldVars.hasOwnProperty(
          key
        ) /* && !this.vars.changed.hasOwnProperty(key) */
      ) {
  ...
  addListener: function (listener, callAlways, bind, freq) {
    // use the returned interval ID for removeListener
  ...
  removeListener: function (listenerID) {
    // use the interval ID returned by addListener
