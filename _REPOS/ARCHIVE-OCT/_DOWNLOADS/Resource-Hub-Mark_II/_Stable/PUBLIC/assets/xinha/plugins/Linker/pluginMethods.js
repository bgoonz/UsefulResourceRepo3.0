  ...
  if (a && a.tagName.toLowerCase() == "a") {
    ...
    if (m) {
      ...
      if (
        ...
        String(a.getAttribute("onclick")).length
      ) {
        var m = a
  ...
  var doOK = function () {
    ...
    if (values.type == "url") {
      if (values.href) {
        ...
        if (values.target == "popup") {
          ...
          atr.onclick =
            "if(window.parent && window.parent.Xinha){return false}window.open(this.href, '" +
            values.p_name.replace(/[^a-z0-9_]/i, "_") +
            "', '" +
            values.p_options.join(",") +
            "');return false;";
        }
      }
    ...
    if (a && a.tagName.toLowerCase() == "a") {
      if (!atr.href) {
        if (
          confirm(
            linker._dialog._lc("Are you sure you wish to remove this link?")
          )
        ) {
          var p = a.parentNode;
      ...
      for (var i = 0; i < anchors.length; i++) {
        ...
        if (anchor.href == tmp) {
          ...
          if (!a) a = anchor;
          for (var j in atr) {
            if (String(atr[j]).length > 0) anchor.setAttribute(j, atr[j]);
          }
        }
      }
    }
...
Linker.Dialog.prototype._prepareDialog = function () {
  ...
  if (!linker.lConfig.dialog && Xinha.Dialog)
    linker.lConfig.dialog = Xinha.Dialog;

  var dialog = (this.dialog = new linker.lConfig.dialog(
    linker.editor,
    Linker.html,
    "Linker",
    ...
    if (typeof files[i] == "string") {
      this.dTree.add(
        ...
        "javascript:document.getElementsByName('" +
          this.dialog.id.href +
          "')[0].value=decodeURIComponent('" +
          encodeURIComponent(files[i]) +
          "');document.getElementsByName('" +
          this.dialog.id.type +
          "')[0].click();document.getElementsByName('" +
          this.dialog.id.href +
          "')[0].focus();void(0);",
        files[i]
      );
    } else if (
      typeof files[i] == "object" &&
      files[i] &&
      typeof files[i].length === "number"
    ) {
      // there seems to be a strange bug in IE that requires this complicated check, see #1197
...
Linker.Dialog.prototype.show = function (inputs, ok, cancel) {
  ...
  if (disabledTargets.length == 3) {
    // only one target option is visible
  ...
  if (
    inputs.href == "http://www.example.com/" &&
