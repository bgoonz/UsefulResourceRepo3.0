...
Xinha.prototype.cleanHTML = function (sHtml) {
  ...
  if (Xinha.is_ie && c[13].test(sHtml)) {
    sHtml = sHtml.replace(
      c[13],
      "$1" + Xinha._escapeDollars(this.stripBaseURL(RegExp.$3)) + '"'
    );
  }
  ...
  if (this.config.only7BitPrintablesInURLs) {
    ...
    if (c[13].test(sHtml)) {
      try {
        //Mozilla returns an incorrectly encoded value with innerHTML
        sHtml = sHtml.replace(
          c[13],
          "$1" +
            Xinha._escapeDollars(
              decodeURIComponent(RegExp.$3).replace(
                /([^!-~]+|%[0-9]+)/g,
                function (chr) {
                  return escape(chr);
                }
        ...
        // once the URL is escape()ed, you can't decodeURIComponent() it anymore
        sHtml = sHtml.replace(
          c[13],
          "$1" +
            Xinha._escapeDollars(
              RegExp.$3.replace(/([^!-~]+|%[0-9]+)/g, function (chr) {
                return escape(chr);
              }) + '"'
            )
        );
...
Xinha.getHTML = function (root, outputRoot, editor) {
  ...
  if (root.nodeType == 11) {
    ...
    html += temp.innerHTML.replace(c[23], function (strn) {
    ...
    html += innerhtml.replace(c[23], function (strn) {
      //skip content inside script tags
      strn = strn.replace(c[22], function (tag) {
        if (/^<[!\?]/.test(tag)) return tag;
        //skip comments and php tags
          ...
          !(
            ...
            editor.config.htmlRemoveTags.test(tag.replace(/<([^\s>\/]+)/, "$1"))
          )
        )
