  ...
  FileManager.Gallery = new Class({
    ...
    initialize: function (options) {
      ...
      this.wrapper = new Element("div", {
        ...
        this.input,
        imgdiv,
        new Element("button", { text: this.language.gallery.save }).addEvent(
          "click",
          function (e) {
      ...
      this.captionDialogOffsets = {
        x: Math.round(
          wrapper_pos.left -
            imgdiv_pos.left -
            (imgdiv_pos.width - this.captionImgContainerSize.x) / 2
        ),
        y: Math.round(
          wrapper_pos.top -
            imgdiv_pos.top -
            (imgdiv_pos.height - this.captionImgContainerSize.y) / 2
        ),
      ...
      this.captionOverlay = new Overlay({
        class:
          "filemanager-overlay filemanager-overlay-dialog filemanager-overlay-caption",
        events: {
          click: function () {
            this.removeClone();
    ...
    show_gallery: function () {
      ...
      var img_cnt = Math.floor(
        (size.x - g_rem - scrollbarCorr - radiiCorr) / imgWidth
      );
    ...
    show_caption_editor: function (img_el, li_wrapper, file) {
      ...
      // -> add overlay
      $$("filemanager-overlay-caption").addEvent(
        "click",
        function (e) {
      ...
      nl = Math.round(
        (vp.x - this.captionDialogOffsets.w) / 2 - this.captionDialogOffsets.x
      );
      nt = Math.round(
        (vp.y - this.captionDialogOffsets.h) / 2 - this.captionDialogOffsets.y
      );
      ...
      this.clone
        .store("file", file)
        .store("parent", li_wrapper)
        .addClass("filemanager-clone")
        .set({
          styles: {
            position: "relative",
            width: w,
    ...
    onDragComplete: function (el, droppable, caption) {
      ...
      var self = this;
      var destroyIcon = Asset.image(
        this.assetBasePath + "Images/destroy.png"
      ).set({
      ...
      var li = new Element("li")
        ...
        .adopt(imgcontainer, destroyIcon)
        .inject(this.gallery);
        ...
        var req = new FileManager.Request(
          {
            ...
            onFailure: function (xmlHttpRequest) {}.bind(this),
          },
          this
        );
    ...
    removeClone: function (e, target) {
      ...
      if (
        ...
        (e &&
          e.relatedTarget &&
          ([this.clone, this.wrapper].contains(e.relatedTarget) ||
            (e &&
              this.wrapper.contains(e.relatedTarget) &&
              e.relatedTarget != this.wrapper)))
      )
        return;
    ...
    switchButton: function () {
      if (typeof this.gallery != "undefined") {
        ...
        this.menu
          .getElement("button.filemanager-serialize")
          .set("disabled", !chk)
          [(chk ? "remove" : "add") + "Class"]("disabled");
      }
    },
    ...
    populate: function (data, path_is_urlencoded /* default = TRUE */) {
      ...
      if (
        typeof path_is_urlencoded == "undefined" ||
        path_is_urlencoded === null
      )
        path_is_urlencoded = true;
      // set default to TRUE
      else path_is_urlencoded = !!path_is_urlencoded;
      ...
      Object.each(
        ...
        function (v, i) {
          this.diag.log(
            "GALLERY.populate push: index = ",
            i,
            ", value = ",
            v,
            ", enc: ",
            1 * path_is_urlencoded
          );
    ...
    startPopulatingTheGallery: function () {
      if (!this.root) {
        this.diag.log(
          "### FATAL error in startPopulatingTheGallery(): no valid .root path!"
        );
        return;
      }
      ...
      if (count) {
        ...
        for (i = 0; i < count; ++i) {
          ...
          if (path) {
            ...
            this.diag.log(
              "### gallery populate: invalid input (not in legal URL space): ",
              item,
              ", root: ",
              this.root
            );
    ...
    serialize_on_click: function (e) {
      ...
      Object.each(
        this.files,
        function (file) {
          var path = this.options.deliverPathAsLegalURL
            ? file.file.path
            : this.escapeRFC3986(
                this.normalize("/" + this.root + file.file.path)
              ); // the absolute URL for the given file, rawURLencoded
