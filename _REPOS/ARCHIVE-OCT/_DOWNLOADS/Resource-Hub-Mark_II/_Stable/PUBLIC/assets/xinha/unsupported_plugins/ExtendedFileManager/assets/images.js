...
function renameFile(oldPath) {
  ...
  if (Xinha.ie_version > 6) {
    popupPrompt(
      i18n("Please enter new name for this file..."),
      oldName,
      rename,
      i18n("Rename")
    );
...
function renameDir(oldName) {
  ...
  if (Xinha.ie_version > 6) {
    popupPrompt(
      i18n("Please enter new name for this folder..."),
      oldName,
      rename,
      i18n("Rename")
    );
...
function update_selected() {
  ...
  if (_current_selected) {
    _current_selected.className = _current_selected.className.replace(
      /(^| )active( |$)/,
      "$1$2"
    );
...
function updateDiskMesg(newMessage) {
  ...
  if (diskmesg) {
    if (diskmesg.firstChild) diskmesg.removeChild(diskmesg.firstChild);
...
function confirmDeleteFile(file) {
  if (confirm(i18n('Delete file "$file=' + file + '$"?'))) return true;
...
function confirmDeleteDir(dir, count) {
  ...
  if (confirm(i18n('Delete folder "$dir=' + dir + '$"?'))) return true;

  return false;
}

function showPreview(f_url) {
  window.parent.document.getElementById("f_preview").src = f_url
    ? window.parent._backend_url + "__function=thumbs&img=" + f_url
    : window.parent.opener.Xinha.getPluginDir("ExtendedFileManager") +
      "/img/1x1_transparent.gif";
}
