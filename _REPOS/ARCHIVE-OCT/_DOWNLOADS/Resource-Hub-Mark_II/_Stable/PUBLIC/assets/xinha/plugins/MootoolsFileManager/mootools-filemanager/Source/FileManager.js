...
var FileManager = new Class({
  ...
  initialize: function (options) {
    ...
    if (this.options.language !== "en") {
      this.language = Object.merge(
        this.language,
        FileManager.Language[this.options.language]
      );
    }
    ...
    this.container = new Element("div", {
      class:
        ...
        (Browser.opera ? " filemanager-engine-presto" : "") +
        (Browser.ie ? " filemanager-engine-trident" : "") +
        (Browser.ie8 ? "4" : "") +
        (Browser.ie9 ? "5" : ""),
      styles: {
    ...
    this.filemanager = new Element("div", {
      class: "filemanager",
      styles: Object.append({}, this.options.styles, {
    ...
    this.previewLoader = new Element("div", {
      class: "loader",
      opacity: 0,
      tween: { duration: "short" },
    });
    this.browserLoader = new Element("div", {
      class: "loader",
      opacity: 0,
      tween: { duration: "short" },
    });
    ...
    this.selectablePath = new Element("input", {
      type: "text",
      class: "filemanager-dir",
      readonly: "readonly",
    });
    this.pathTitle = new Element("a", {
      href: "#",
      class: "filemanager-dir-title",
      text: this.language.dir,
    }).addEvent(
      "click",
      function (e) {
        ...
        if (this.header.getElement("span.filemanager-dir") !== null) {
          this.selectablePath.setStyle(
            "width",
            this.header.getSize().x - this.pathTitle.getSize().x - 55
          );
    ...
    this.browsercontainer = new Element("div", {
      class: "filemanager-browsercontainer",
    }).inject(this.filemanager);
    this.browserheader = new Element("div", {
      class: "filemanager-browserheader",
    }).inject(this.browsercontainer);
    this.browserheader.adopt(this.browserLoader);
    this.browserScroll = new Element("div", {
      class: "filemanager-browserscroll",
    })
      .inject(this.browsercontainer)
      .addEvents({
        mouseover: function (e) {
          ...
          var row = null;
          if (e.target) {
            row = e.target.hasClass("fi")
              ? e.target
              : e.target.getParent("span.fi");
            if (row) {
              row.addClass("hover");
            }
          }
    ...
    if (this.options.showDirGallery) {
      ...
      this.browserMenu_thumbList = new Element("a", {
        ...
        function () {
          ...
          this.diag.log(
            "show_dir_Thumb_gallery button click: current directory!",
            this.CurrentDir,
            ", startdir: ",
    ...
    this.browser_paging.adopt([
      this.browser_paging_first,
      this.browser_paging_prev,
      this.browser_paging_info,
      this.browser_paging_next,
      this.browser_paging_last,
    ]);

    // Partikule : Added the browserMenu_thumbList to the browserheader
    this.browserheader.adopt([
      this.browserMenu_thumbList,
      this.browserMenu_thumb,
      this.browserMenu_list,
      this.browser_dragndrop_info,
      this.browser_paging,
    ]);
    // /Partikule

    this.browser = new Element("ul", { class: "filemanager-browser" }).inject(
      this.browserScroll
    );
    ...
    }).addEvent("click:relay(img.preview)", function () {
    ...
    // 3. Less is more :-)
    this.info
      .adopt([this.info_head, this.preview_area])
      .inject(this.filemanager);
    // /Partikule
    ...
    this.tips = new Tips({
      ...
      onShow: function () {
        this.tip
          .setStyle("z-index", self.options.zIndex + 501)
  ...
  allow_DnD: function (j, pagesize) {
    if (!this.options.move_or_copy) return false;

    if (!j || !j.dirs || !j.files || !pagesize) return true;
  ...
  fitSizes: function () {
    if (this.options.standalone) {
      ...
      var parent =
        this.options.parentContainer !== null
          ? document.id(this.options.parentContainer)
  ...
  relayClick: function (e, el) {
    ...
    if (el.retrieve("el_ref")) el = el.retrieve("el_ref");
  ...
  relayDblClick: function (e, el) {
    ...
    if (this.CurrentFile.mime !== "text/directory") this.open_on_click(null);
  },
  ...
  toggleList: function (e) {
    ...
    this.fill(
      null,
      this.get_view_fill_startindex(),
      this.listPaginationLastSize
    );
  },
  ...
  hashHistory: function (vars) {
    ...
    if (vars.changed["fmPath"] === "") vars.changed["fmPath"] = "/";
                ...
                function (current) {
                  ...
                  if (current.get("title") == value) {
                    ...
                    this.diag.log(
                      ...
                      current,
                      "file = ",
                      current.getParent("span.fi").retrieve("file")
                    );
  ...
  show: function (e, loaddir, preselect) {
    ...
    if (typeof jsGET !== "undefined") {
      ...
      if (jsGET.get("fmListType") !== null) {
        ...
        if (this.listType === "thumb")
          this.browserMenu_thumb.store("set", true).setStyle("opacity", 1);
        else this.browserMenu_list.store("set", true).setStyle("opacity", 1);
      }
      jsGET.set({
        fmID: this.ID,
        fmPath: loaddir,
    ...
    if (
      (Browser.Engine && (Browser.Engine.trident || Browser.Engine.webkit)) ||
      Browser.ie ||
      Browser.chrome ||
      Browser.safari
    )
      document.addEvent("keydown", this.bound.keyboardInput);
    else document.addEvent("keypress", this.bound.keyboardInput);
  ...
  hide: function (e) {
    ...
    window
      .removeEvent("scroll", this.bound.scroll)
      .removeEvent("resize", this.bound.scroll);
    ...
    else document.removeEvent("keypress", this.bound.keyboardInput);
  ...
  open_on_click: function (e) {
    ...
    this.fireEvent("complete", [
      this.options.deliverPathAsLegalURL
        ? file.path
  ...
  download: function (file) {
      ...
      events: {
        load: function () {
          ...
          Function.attempt(
            ...
            function () {
              response =
                '{status: 0, error: "Download: download assumed okay: can\'t find response."}';
            }
          );
    ...
    this.downloadForm = new Element("form", {
      target: "_downloadIframe",
      method: "post",
      enctype: "multipart/form-data",
    });
    this.menu.adopt(this.downloadForm);

    var tx_cfg = this.options.mkServerRequestURL(
      this,
      "download",
      Object.merge(
        {},
    ...
    Object.each(
      Object.merge({}, this.options.propagateData, tx_cfg.data),
      function (v, k) {
        this.downloadForm.adopt(
          new Element("input").set({ type: "hidden", name: k, value: v })
        );
      }.bind(this)
    );
  ...
  create_on_click: function (e) {
    ...
    var click_ok_f = function (e) {
      ...
      if (e.key === "enter") {
        e.stopPropagation();
        e.target
          .getParent("div.filemanager-dialog")
          .getElement("button.filemanager-dialog-confirm")
          .fireEvent("click");
      }
    }.bind(this);
  ...
  load: function (dir, preselect) {
    ...
    this.diag.log(
      "### 'view' request: onRequest invoked @ load(): ",
      dir,
      ", preselect: ",
      preselect
    );
    ...
    this.Request = new FileManager.Request(
      {
        ...
        onSuccess: function (j) {
          ...
          if (
            this.listType == "thumb" &&
            j.files.length + j.dirs.length >
              this.options.listMaxSuggestedDirSizeForThumbnails
          ) {
            this.listType = "list";
            this.browserMenu_thumb.setStyle("opacity", 0.5);
            this.browserMenu_list.setStyle("opacity", 1);
        ...
        onFailure: function (xmlHttpRequest) {
          ...
          this.diag.log(
            "### 'view' request: onFailure invoked",
            xmlHttpRequest
          );
          this.browserLoader.fade(0);
        }.bind(this),
    ...
    this.Request = new FileManager.Request(
      {
        ...
        onSuccess: function (j) {
          ...
          if (this.view_fill_json) {
            this.delete_from_dircache(
              file
            ); /* do NOT use j.name, as that one can be 'cleaned up' as part of the 'move' operation! */
            ...
            if (
              this.view_fill_json.dirs.length +
                this.view_fill_json.files.length >
              this.listPaginationLastSize
            ) {
        ...
        this.Request = new FileManager.Request(
          {
            ...
            onSuccess: function (j) {
              ...
              file.element
                .getElement("span.filemanager-filename")
                .set("text", j.name)
                .set("title", j.name);
              file.element.addClass("selected");
              file.name = j.name;
              this.diag.log(
                "move : onSuccess going to fillInfo: file = ",
                Object.clone(file),
                ", json: ",
                j
              );
  ...
  browserSelection: function (direction) {
    ...
    if (direction === "go-bottom") {
      ...
      if (csel !== null) csel.removeClass("selected");
      ...
      this.browser.getElement("span.fi.hover") === null &&
      this.browser.getElement("span.fi.selected") === null
    ) {
    ...
    switch (direction) {
      ...
      case "pagedown":
        if (stepsize == 1) {
          if (
            current.getPosition(this.browserScroll).y +
              current.getSize().y * 2 <
            this.browserScroll.getSize().y
          ) {
            stepsize =
              Math.floor(
                (this.browserScroll.getSize().y -
            ...
            stepsize = Math.floor(
              this.browserScroll.getSize().y / current.getSize().y
            );
          }
        }
      /* fallthrough */
      case "down":
        ...
        if (next === null) {
          if (this.paging_goto_next(null, "go-bottom")) break;
        } else {
      ...
      case "pageup":
        if (stepsize == 1) {
          ...
          stepsize = Math.floor(
            current.getPosition(this.browserScroll).y / current.getSize().y
          );
          if (stepsize < 1) stepsize = 1;
        }
      /* fallthrough */
      case "up":
        ...
        if (previous === null) {
          if (this.paging_goto_prev(null, "go-top")) break;
        } else {
          for (; stepsize > 0; stepsize--) {
    ...
    this.diag.log(
      "key handler: current X/Y = ",
      current.getPosition(this.browserScroll),
      ", H/W/SCROLL = ",
      this.browserScroll.getSize(),
      ", 1U/SIZE = ",
      current.getSize()
    );
    ...
    if (
      direction !== "up" &&
      current.getPosition(this.browserScroll).y + current.getSize().y * 2 >=
        this.browserScroll.getSize().y
    ) {
      ...
      browserScrollFx = new Fx.Scroll(this.browserScroll, {
        duration: dy < 150 ? 150 : dy > 1000 ? 1000 : dy.toInt(),
      });
      ...
      var sy =
        this.browserScroll.getScroll().y +
        current.getPosition(this.browserScroll).y -
        this.browserScroll.getSize().y +
        current.getSize().y * 2;
      ...
      browserScrollFx.start(
        current.getPosition(this.browserScroll).x,
        sy >= 0 ? sy : 0
      );
    }
  },
  ...
  revert_drag_n_drop: function (el) {
    ...
    try {
      el.setStyle("z-index", "auto");
    } catch (e) {
      el.setStyle("z-index", "");
    } // IE<8 Complains about 'auto' for z-index
  ...
  paging_goto_prev: function (e, kbd_dir) {
    ...
    return this.paging_goto_helper(
      startindex - this.listPaginationLastSize,
      this.listPaginationLastSize,
      kbd_dir
    );
  },
  ...
  paging_goto_last: function (e, kbd_dir) {
    ...
    if (
      ...
      startindex >
        this.view_fill_json.dirs.length +
          this.view_fill_json.files.length -
          this.options.listPaginationSize
    )
      return false;
  ...
  fill: function (j, startindex, pagesize, kbd_dir, preselect) {
    ...
    current_path.split("/").each(
      function (folderName) {
        ...
        var path = "/" + pre.join("/") + "/";
        this.diag.log(
          "on fill: display directory path chunks: JSON root = ",
          j.root,
          ", path: ",
          path,
          ", folder: ",
    ...
    if (pagesize) {
      ...
      if (pagesize < j_item_count) {
        ...
        if (curpagno > 1) {
          this.browser_paging_first.set(
            "title",
            this.language.goto_page + " 1"
          );
          this.browser_paging_first.fade(1);
          this.browser_paging_prev.set(
            "title",
            this.language.goto_page + " " + (curpagno - 1)
          );
        ...
        if (curpagno < pagecnt) {
          this.browser_paging_last.set(
            "title",
            this.language.goto_page + " " + pagecnt
          );
          this.browser_paging_last.fade(1);
          this.browser_paging_next.set(
            "title",
            this.language.goto_page + " " + (curpagno + 1)
          );
    ...
    this.view_fill_timer = this.fill_chunkwise_1.delay(1, this, [
      startindex,
      endindex,
      endindex - startindex,
      pagesize,
      support_DnD_for_this_dir,
      starttime,
      els,
      kbd_dir,
      preselect,
  ...
  list_row_maker: function (thumbnail_url, file) {
    return (file.element = new Element("span", {
      class: "fi " + this.listType,
      href: "#",
    })
      .adopt(
        new Element("span", {
          ...
          styles: {
            "background-image":
              ...
              (thumbnail_url
                ? thumbnail_url
                : this.assetBasePath + "Images/loader.gif") +
              ")",
          },
  ...
  dir_gallery_set_actual_img: function (file, dg_el) {
    ...
    Asset.image(img_url, {
      ...
      onLoad: function () {
        var img_el = this;
        var img_div = dg_el
          .getElement("div.dir-gal-thumb-bg")
      ...
      onError: function () {
        ...
        var iconpath =
          self.assetBasePath + "Images/Icons/Large/default-error.png";
        dg_el
          .getElement("div.dir-gal-thumb-bg")
          .setStyle("background-image", "url(" + iconpath + ")");
      },
      onAbort: function () {
  ...
  fill_chunkwise_1: function (
    ...
    for (idx = startindex; idx < endindex && idx < j.dirs.length; idx++) {
      ...
      if (idx % 10 === 0) {
         ...
         */
        this.adaptive_update_pagination_size(
          idx,
          endindex,
          render_count,
          pagesize,
      ...
      el = this.list_row_maker(
        this.listType === "thumb" ? file.icon48 : file.icon,
        file
      );
      ...
      editButtons.each(function (v) {
        //icons.push(
        Asset.image(this.assetBasePath + "Images/" + v + ".png", {
          title: this.language[v],
        })
          .addClass("browser-icon")
          .setStyle("opacity", 0)
          .addEvent(
            "mouseup",
            function (e, target) {
      ...
      el.inject(
        new Element("li", { class: this.listType }).inject(this.browser)
      ).store("parent", el.getParent());
        ...
        if (
          ...
          el = function (file, dg_el) {
            ...
            var iconpath =
              this.assetBasePath +
              "Images/Icons/" +
              (this.listType === "thumb" ? "Large/" : "") +
              "default-error.png";
            var list_row = this.list_row_maker(
              this.listType === "thumb" ? file.icon48 : file.icon,
              file
            );
                ...
                onSuccess: function (j) {
                  if (!j || !j.status || !j.thumb48) {
                    ...
                    dg_el
                      ...
                      .setStyle(
                        "background-image",
                        "url(" + (j.icon48 ? j.icon48 : iconpath) + ")"
                      );
                  } else {
                    list_row
                      .getElement("span.fm-thumb-bg")
                      .setStyle(
                        "background-image",
                        "url(" +
                          (this.listType === "thumb" ? j.thumb48 : j.icon) +
                          ")"
                      );
                    if (j.thumb48) {
        ...
        if (!this.fillInfoOnFillFired) {
          if (preselect) {
            if (preselect === file.name) {
              ...
              new Fx.Scroll(this.browserScroll, {
                duration: "short",
                offset: { x: 0, y: -(this.browserScroll.getSize().y / 4) },
              }).toElement(file.element);
              file.element.addClass("selected");
    ...
    this.view_fill_timer = this.fill_chunkwise_2.delay(1, this, [
      render_count,
      pagesize,
      support_DnD_for_this_dir,
      starttime,
  ...
  fill_chunkwise_2: function (
    ...
    this.dir_filelist.getChildren("ul li div.fi").each(
      function (thumb) {
        els[0].push(thumb);
      }.bind(this)
    );
    ...
    if (support_DnD_for_this_dir) {
      ...
      $$(els[0]).makeDraggable({
        ...
        onStart: function (el, e) {
          ...
          if (dpos.y > 0) dpos.y = -Math.round(elsize.y / 2);
          else if (dpos.y < -elsize.y) dpos.y = -Math.round(elsize.y / 2);
    ...
    // go to the next stage, right after these messages... ;-)
    this.view_fill_timer = this.fill_chunkwise_3.delay(1, this, [
      render_count,
      pagesize,
      support_DnD_for_this_dir,
      starttime,
  ...
  adaptive_update_pagination_size: function (
    currentindex,
    endindex,
    render_count,
    pagesize,
    ...
    if (avgwait) {
      ...
      if (done_so_far > 0.05) {
        ...
        if (delta < 0.5) newpsize = 0.5 * pagesize;
        else if (delta > 1.2) newpsize = 1.2 * pagesize;
        newpsize = newpsize.toInt();

        // and never let it drop below rediculous values:
        if (newpsize < 20) newpsize = 20;
  ...
  fillInfo: function (file) {
    ...
    if (typeof jsGET !== "undefined" && this.storeHistory) {
      if (file.mime !== "text/directory") jsGET.set({ fmFile: file.name });
      else jsGET.remove(["fmFile"]);
    }
    ...
    if (this.drop_pending == 0) {
      ...
      this.diag.log(
        "fillInfo: request detail for file: ",
        Object.clone(file),
        ", dir: ",
        dir
      );
      if (file.mime === "text/directory") {
        if (file.readme && file.readme.length) {
          this.preview.set("html", file.readme);
          this.preview_area.setStyle("display", "block").fade("in");
          return;
        }
      ...
      var tx_cfg = this.options.mkServerRequestURL(this, "detail", {
        ...
        file:
          file.mime === "text/directory" && file.path === "/" ? "/" : file.name,
        filter: this.options.filter,
      ...
      this.Request = new FileManager.Request(
        {
          ...
          onSuccess: function (j) {
            ...
            if (typeof this.options.previewHandlers[j.mime] === "function") {
              ...
              prev
                .set(
                  "html",
                  j.content
                    ? j.content.substitute(this.language, /\\?\$\{([^{}]+)\}/g)
                    : ""
                )
                .getElement("img.preview");
            }
            ...
            if (prev && !j.thumb250 && j.thumbs_deferred) {
              var iconpath =
                this.assetBasePath + "Images/Icons/Large/default-error.png";
  ...
  audioPreview: function (previewArea, fileDetails) {
    ...
    previewArea = new Element("div", {
      class: "filemanager-preview-content",
    }).inject(previewArea);
    ...
    new Element("object", {
      ...
      .adopt(
        new Element("param", {
          name: "flashvars",
          value: "mp3=" + fileDetails.url + "&volume=50&showtime=1",
        })
      )
      .inject(previewArea);
  ...
  switchButton4Current: function () {
    ...
    els.each(function (el) {
      if (el) {
        el.set("disabled", !chk)[(chk ? "remove" : "add") + "Class"](
          "disabled"
        );
      }
    });
  ...
  reset_fill: function () {
    ...
    this.browser_dragndrop_info.set(
      "title",
      this.language.drag_n_drop_disabled
    );
  ...
  cvtXHRerror2msg: function (xmlHttpRequest) {
    ...
    var response =
      ...
      this.language["backend.unidentified_error"];
...
FileManager.Request = new Class({
  ...
  initialize: function (options, filebrowser) {
    ...
    this.options.data = Object.merge(
      {},
      filebrowser.options.propagateData,
      this.options.data
    );
...
(function () {
  ...
  if (typeof __MFM_ASSETS_DIR__ === "undefined") {
    var __DIR__ = (function () {
      ...
      return (
        script.substring(0, script.lastIndexOf("/")).replace(host, "") + "/"
      );
    })();
  ...
  if (
    typeof __MFM_USE_BACK_BUTTON_NAVIGATION__ != "undefined" &&
    __MFM_USE_BACK_BUTTON_NAVIGATION__
  ) {
  ...
  FileManager.Dialog = new Class({
    ...
    initialize: function (text, options) {
      ...
      }).adopt([typeOf(text) === "string" ? this.str2el(text) : text]);

      this.el = new Element("div", {
        class:
          "filemanager-dialog" +
          (Browser.ie ? " filemanager-dialog-engine-trident" : "") +
          (Browser.ie ? " filemanager-dialog-engine-trident" : "") +
          (Browser.ie8 ? "4" : "") +
          (Browser.ie9 ? "5" : ""),
        opacity: 0,
      ...
      Array.each(
        ...
        function (v) {
          new Element("button", {
            class: "filemanager-dialog-" + v,
            text: this.options.language[v],
          })
            .addEvent(
              "click",
              function (e) {
    ...
    show: function () {
      ...
      this.restrictSize();
      var autofocus_el = this.options.autofocus_on
        ? this.el.getElement(this.options.autofocus_on)
        : this.el.getElement("button.filemanager-dialog-confirm") ||
          this.el.getElement("button");
      if (autofocus_el) {
      ...
      this.el
        ...
        .chain(
          function () {
            ...
            if (autofocus_el) {
              if (0) {
                // the delay suggested as a fix there is part of the fade()...
                (function (el) {
                  el.focus();
      ...
      window.FileManager.prototype.diag.log(
        "add key up(ESC)/resize/scroll on show 1500"
      );
      document.addEvents({
        scroll: this.bound.scroll,
        resize: this.bound.scroll,
    ...
    destroy: function () {
      ...
      document
        .removeEvent("scroll", this.bound.scroll)
        .removeEvent("resize", this.bound.scroll)
        .removeEvent("keyup", this.bound.keyesc);
  ...
  this.Overlay = new Class({
    ...
    resize: function () {
      if (!this.el) {
        ...
        document.id(this.el).setStyles({
          // IE7 thinks this.el is no longer a mootools object, ^shrug^
