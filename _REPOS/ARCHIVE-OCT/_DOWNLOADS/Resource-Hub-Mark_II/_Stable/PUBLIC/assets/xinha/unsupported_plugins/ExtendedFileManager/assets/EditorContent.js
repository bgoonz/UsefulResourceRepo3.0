...
function init_resize() {
  ...
  if (canvas == null) canvas = MM_findObj("imgCanvas");
  ...
  if (ant == null) ant = MM_findObj("ant");
...
initEditor = function () {
  ...
  if (markerImg.src.indexOf("img/t_white.gif") > 0) toggleMarker();
...
function toggleMarker() {
  ...
  if (ant.className == "selection") ant.className = "selectionWhite";
  else ant.className = "selection";

  if (jg_doc.getColor() == "#000000") jg_doc.setColor("#FFFFFF");
  else jg_doc.setColor("#000000");
...
function move(e) {
  if (dragapproved) {
    var w = ns6
      ? temp1 + e.pageX - x
      : temp1 + event.clientX + document.body.scrollLeft - x;
    var h = ns6
      ? temp2 + e.pageY - y
      : temp2 + event.clientY + document.body.scrollTop - y;
    ...
    switch (resizeHandle) {
      ...
      case "nw-resize":
        if (oa_h - h >= 0 && oa_w - w >= 0) {
          ...
          ant.style.width = oa_w - w;
          ant.style.top = oa_y + h;
          if (constrained.checked) ant.style.height = rapp_inv * (oa_w - w);
          else ant.style.height = oa_h - h;
        }
        break;
      case "ne-resize":
        if (oa_h - h >= 0 && oa_w + w >= 0) {
...
function drags(e) {
  ...
  if (firedobj.className == "handleBox") {
    ...
    if (status == null) status = MM_findObj("status");
...
function showStatus() {
  ...
  var zoom =
    100 / parseInt(window.parent.document.getElementById("zoom").value, 10);
  if (mode == "measure") {
    mx1 = x - pic_x;
    my1 = y - pic_y;
    ...
    if (mw < 0 && mh < 0) ma = ma + 180;

    if (mw < 0 && mh > 0) ma = ma - 180;

    ma = parseInt(ma * 100) / 100;

    if (m_sx != null && !isNaN(mx1)) m_sx.value = mx1 * zoom + "px";
    if (m_sy != null && !isNaN(my1)) m_sy.value = my1 * zoom + "px";
    if (m_w != null && !isNaN(mw)) m_w.value = mw * zoom + "px";
    if (m_h != null && !isNaN(mh)) m_h.value = mh * zoom + "px";

    if (m_d != null && !isNaN(md)) m_d.value = md * zoom + "px";
    if (m_a != null && !isNaN(ma)) m_a.value = ma + "";

    if (r_ra != null && !isNaN(ma)) r_ra.value = ma;
  ...
  if (mode == "crop") {
    if (t_cx != null) t_cx.value = cx * zoom;
    if (t_cy != null) t_cy.value = cy * zoom;
    if (t_cw != null) t_cw.value = cw * zoom;
    if (t_ch != null) t_ch.value = ch * zoom;
  } else if (mode == "scale") {
    var sw = aw,
      sh = ah;
    ...
    if (s_sw != null) s_sw.value = sw * zoom;
    if (s_sh != null) s_sh.value = sh * zoom;
  }
}
...
function dragStopped() {
  ...
  if (mode == "crop") {
    ...
    var h1 = ah;
    var y1 = ay;
    if (ah + ay >= pic_height + pic_y) h1 = pic_height + pic_y - ay;
    else if (ay <= pic_y) {
      h1 = ay + ah - pic_y;
      y1 = pic_y;
    }
    jg_doc.fillRectPattern(pic_x, y1, ax - pic_x, h1, pattern);
    jg_doc.fillRectPattern(
      ax + aw,
      y1,
      pic_x + pic_width - ax - aw,
      h1,
      pattern
    );
    jg_doc.fillRectPattern(
