...
function SuperClean(editor, args) {
  ...
  editor.config.registerButton(
    "superclean",
    this._lc("Clean up HTML"),
    [_editor_url + "iconsets/Tango/ed_buttons_main.png", 6, 4],
    true,
...
Xinha.Config.prototype.SuperClean = {
  ...
  filters: {
    ...
    word_clean: {
      label: Xinha._lc("Clean bad HTML from Microsoft Word.", "SuperClean"),
      checked: true,
      fullonly: true,
    },
    word: {
    ...
    remove_sizes: {
      label: Xinha._lc("Remove custom font sizes.", "SuperClean"),
      checked: true,
    },
    remove_colors: {
      label: Xinha._lc("Remove custom text colors.", "SuperClean"),
      checked: true,
    },
    remove_emphasis: {
      label: Xinha._lc("Remove emphasis and annotations.", "SuperClean"),
      checked: true,
    },
    remove_sup_sub: {
      label: Xinha._lc("Remove superscripts and subscripts.", "SuperClean"),
      checked: false,
    },
    remove_alignment: {
      label: Xinha._lc("Remove alignment (left/right/justify).", "SuperClean"),
      checked: true,
    },
    remove_all_css_classes: {
      label: Xinha._lc("Remove all classes (CSS).", "SuperClean"),
      checked: false,
    },
    remove_all_css_styles: {
      label: Xinha._lc("Remove all styles (CSS).", "SuperClean"),
      checked: true,
    },
    remove_lang: {
      label: Xinha._lc("Remove lang attributes.", "SuperClean"),
      checked: true,
    },
    remove_fancy_quotes: {
      label: Xinha._lc(
    ...
    paragraph: {
      label: Xinha._lc("Remove Paragraphs", "SuperClean"),
      checked: false,
    },
    remove_all_tags: {
      label: Xinha._lc("Remove All HTML Tags", "SuperClean"),
      checked: false,
    },
  },
...
SuperClean.prototype.onGenerateOnce = function () {
  if (this.editor.config.tidy_handler) {
    ...
    this.editor.config.SuperClean.tidy_handler =
      this.editor.config.tidy_handler;
    this.editor.config.tidy_handler = null;
  }
  if (
    !this.editor.config.SuperClean.tidy_handler &&
    this.editor.config.filters.tidy
  ) {
...
SuperClean.loadAssets = function () {
  ...
  self.loading = true;
  Xinha._getback(
    Xinha.getPluginDir("SuperClean") + "/pluginMethods.js",
    function (getback) {
      eval(getback);
  ...
  Xinha._getback(
    Xinha.getPluginDir("SuperClean") + "/dialog.html",
    function (getback) {
      self.html = getback;
    }
  );
...
SuperClean.prototype.loadFilters = function () {
  ...
  for (var filter in this.editor.config.SuperClean.filters) {
    if (
      /^(remove_colors|remove_sizes|remove_faces|remove_lang|word_clean|remove_fancy_quotes|tidy|remove_emphasis|remove_sup_sub|remove_alignment|remove_all_css_classes|remove_all_css_styles|remove_all_tags)$/.test(
        filter
      )
    )
    ...
    if (!SuperClean.filterFunctions[filter]) {
      ...
      if (typeof filtDetail.filterFunction != "undefined") {
        ...
        Xinha._getback(
          Xinha.getPluginDir("SuperClean") + "/filters/" + filter + ".js",
          function (func) {
