...
Xinha.Config.prototype.FormOperations = {
  ...
  default_form_html: Xinha._geturlcontent(
    Xinha.getPluginDir("FormOperations") + "/default_form.html"
  ),
};
...
function FormOperations(editor) {
  ...
  editor.config.pageStyleSheets.push(
    Xinha.getPluginDir("FormOperations") + "/iframe.css"
  );
...
FormOperations.prototype.onGenerate = function () {
  ...
  if (Xinha.is_gecko) {
    ...
    Xinha._addEvents(doc, ["mousemove"], function (event) {
...
FormOperations.prototype._preparePanel = function () {
  ...
  if (this.html == false) {
    Xinha._getback(
      Xinha.getPluginDir("FormOperations") + "/panel.html",
      function (txt) {
        fo.html = txt;
        fo._preparePanel();
      }
  ...
  if (typeof Xinha.PanelDialog == "undefined") {
    Xinha._loadback(
      _editor_url + "modules/Dialogs/panel-dialog.js",
      function () {
        fo._preparePanel();
      }
    );
  ...
  this.panel = new Xinha.PanelDialog(
    this.editor,
    "bottom",
    this.html,
    "FormOperations"
...
FormOperations.prototype.onUpdateToolbar = function () {
  ...
  if (activeElement != null) {
    ...
    if (tag === "form") {
      ...
      if (
        this.editor.config.FormOperations.allow_edit_form &&
        typeof activeElement.form != "undefined" &&
        activeElement.form
      ) {
...
FormOperations.prototype.showText = function (input) {
  ...
  var vals = {
    ...
    text_width: input.style.width
      ? parseFloat(input.style.width.replace(/[^0-9.]/, ""))
      : "",
    text_width_units: input.style.width
      ? input.style.width.replace(/[0-9.]/, "").toLowerCase()
      : "ex",
  ...
  this.panel.getElementById("text_type").onchange = function () {
    if (!Xinha.is_ie) {
      ...
      if (!/type=/.test(i.outerHTML)) {
        tmpContainer.innerHTML = i.outerHTML.replace(
          /<INPUT/i,
          '<input type="' + this.options[this.selectedIndex].value + '"'
        );
      } else {
        tmpContainer.innerHTML = i.outerHTML.replace(
          /type="?[a-z]+"?/i,
  ...
  this.panel.getElementById("text_width").onkeyup = this.panel.getElementById(
    "text_width_units"
  ).onchange = function () {
...
FormOperations.prototype.showTextarea = function (input) {
  ...
  var vals = {
    ...
    textarea_width: input.style.width
      ? parseFloat(input.style.width.replace(/[^0-9.]/, ""))
      : "",
    textarea_width_units: input.style.width
      ? input.style.width.replace(/[0-9.]/, "").toLowerCase()
      : "ex",
    textarea_height: input.style.height
      ? parseFloat(input.style.height.replace(/[^0-9.]/, ""))
      : "",
    textarea_height_units: input.style.height
      ? input.style.height.replace(/[0-9.]/, "").toLowerCase()
      : "ex",
  };
  ...
  this.panel.getElementById("textarea_width").onkeyup =
    this.panel.getElementById("textarea_width_units").onchange = function () {
      if (!w.value || isNaN(parseFloat(w.value))) {
        i.style.width = "";
      }
      i.style.width = parseFloat(w.value) + wu.options[wu.selectedIndex].value;
  ...
  this.panel.getElementById("textarea_height").onkeyup =
    this.panel.getElementById("textarea_height_units").onchange = function () {
...
FormOperations.prototype.showSelect = function (input) {
  ...
  var vals = {
    ...
    select_width: input.style.width
      ? parseFloat(input.style.width.replace(/[^0-9.]/, ""))
      : "",
    select_width_units: input.style.width
      ? input.style.width.replace(/[0-9.]/, "").toLowerCase()
      : "ex",
    select_height: input.style.height
      ? parseFloat(input.style.height.replace(/[^0-9.]/, ""))
      : input.size && input.size > 0
      ? input.size
      : 1,
    select_height_units: input.style.height
      ? input.style.height.replace(/[0-9.]/, "").toLowerCase()
      : "items",
  };
  ...
  this.panel.getElementById("select_width").onkeyup = this.panel.getElementById(
    "select_width_units"
  ).onchange = function () {
    if (!w.value || isNaN(parseFloat(w.value))) {
      i.style.width = "";
  ...
  this.panel.getElementById("select_height").onkeyup =
    this.panel.getElementById("select_height_units").onchange = function () {
      if (!h.value || isNaN(parseFloat(h.value))) {
        i.style.height = "";
        return;
      }
      ...
      if (hu.selectedIndex == 0) {
        ...
        i.style.height =
          parseFloat(h.value) + hu.options[hu.selectedIndex].value;
      }
    };
  ...
  this.panel.getElementById("add_option").onclick = function () {
    var txt = prompt(
      Xinha._lc("Enter the name for new option.", "FormOperations")
    );
    if (txt == null) return;
...
FormOperations.prototype.formatName = function (input, name) {
  ...
  switch (this.editor.config.FormOperations.multiple_field_format) {
    ...
    default: {
      throw (
        "Unknown multiple field format " +
        this.editor.config.FormOperations.multiple_field_format
      );
    }
  }
