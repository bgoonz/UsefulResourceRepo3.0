...
Gecko._pluginInfo = {
  ...
  developer_url:
    "$HeadURL: http://svn.xinha.org/trunk/modules/Gecko/Gecko.js $".replace(
      /^[^:]*:\s*(.*)\s*\$$/,
      "$1"
    ),
  sponsor: "",
...
Gecko.prototype.outwardHtml = function (html) {
  ...
  html = html.replace(
    /<script[\s]*src[\s]*=[\s]*['"]chrome:\/\/.*?["']>[\s]*<\/script>/gi,
    ""
  );
...
Xinha.prototype.insertNodeAtSelection = function (toBeInserted) {
  if (toBeInserted.ownerDocument != this._doc) {
    // as of FF3, Gecko is strict regarding the ownerDocument of an element
    try {
