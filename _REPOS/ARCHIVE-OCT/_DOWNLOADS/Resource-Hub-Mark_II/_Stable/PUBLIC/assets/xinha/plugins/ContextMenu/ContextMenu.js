...
ContextMenu.prototype.onGenerate = function () {
  ...
  Xinha._addEvents(doc, ["contextmenu"], function (event) {
    return self.popupMenu(
      Xinha.is_ie ? self.editor._iframe.contentWindow.event : event
    );
  });
...
ContextMenu.prototype.getContextMenu = function (target) {
  ...
  if (!Xinha.is_gecko) {
    if (selection) {
      menu.push(
        [
          Xinha._lc("Cut", "ContextMenu"),
          function () {
            editor.execCommand("cut");
          },
          null,
          config.btnList["cut"][1],
        ],
        [
          Xinha._lc("Copy", "ContextMenu"),
          function () {
            editor.execCommand("copy");
          },
          null,
          config.btnList["copy"][1],
        ]
      );
      menu.push([
        Xinha._lc("Paste", "ContextMenu"),
        function () {
          editor.execCommand("paste");
        },
        null,
        config.btnList["paste"][1],
      ]);
    }
  }
              ...
              if (
                confirm(
                  Xinha._lc(
                    ...
                    "\n" +
                    Xinha._lc("Link points to:", "ContextMenu") +
                    " " +
                    link.href
                )
              ) {
  ...
  if (selection && !link)
    menu.push(null, [
      Xinha._lc("Make lin_k...", "ContextMenu"),
      function () {
        editor.config.btnList["createlink"][3](editor);
      },
  ...
  if (
    editor.config.ContextMenu.customHooks[currentTarget.tagName.toLowerCase()]
  ) {
    var items =
      editor.config.ContextMenu.customHooks[
  ...
  if (!/html|body/i.test(currentTarget.tagName))
    menu.push(
      null,
      [
        Xinha._lc("Remove the $elem Element...", "ContextMenu", {
          elem: "&lt;" + currentTarget.tagName + "&gt;",
        }),
        function () {
          if (
            confirm(
...
ContextMenu.prototype.popupMenu = function (ev) {
  ...
  function keyPress(ev) {
    ...
    var key = String.fromCharCode(
      Xinha.is_ie ? ev.keyCode : ev.charCode
    ).toLowerCase();
  ...
  self.closeMenu = function () {
    ...
    if (Xinha.is_ie) self.iePopup.hide();
  };
  ...
  if (!Xinha.is_ie) {
    ...
    doc.open();
    doc.write(
      "<html><head><style type='text/css'>@import url(" +
        Xinha.getPluginDir("ContextMenu") +
        "/menu.css); html, body { padding: 0px; margin: 0px; overflow: hidden; border: 0px; }</style></head><body unselectable='yes'></body></html>"
    );
    doc.close();
  }
  div = doc.createElement("div");
  if (Xinha.is_ie) div.unselectable = "on";
  ...
  for (var i = 0; i < options.length; ++i) {
    ...
    if (Xinha.is_ie) item.unselectable = "on";
    else
    ...
    if (!option) {
      ...
      if (Xinha.is_ie) td1.unselectable = "on";
      item.appendChild(td1);
      td1.className = "icon";
      if (item.__msh.icon) {
      ...
      if (Xinha.is_ie) td2.unselectable = "on";
      ...
      item.oncontextmenu = function (ev) {
        this.__msh.activate();
        if (!Xinha.is_ie) Xinha._stopEvent(ev);
        return false;
      };
  ...
  if (keys.length > 0) Xinha._addEvent(this.editordoc, "keypress", keyPress);
