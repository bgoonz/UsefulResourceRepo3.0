...
function WebKitResize(editor) {
  ...
  if (
    Xinha.is_webkit ||
    this.editor.config.WebKitResize.enableTDForGeckoAlso ||
    this.editor.config.WebKitResize.enableTABLEForGeckoAlso
  ) {
...
WebKitResize.prototype.onGenerateOnce = function () {
  ...
  if (
    !(typeof jQuery != "undefined" && jQuery.fn && jQuery.fn.webkitimageresize)
  ) {
    var self = this;
  ...
  try {
    ...
    if (
      Xinha.is_webkit ||
      (Xinha.is_gecko && this.editor.config.WebKitResize.enableTDForGeckoAlso)
    ) {
