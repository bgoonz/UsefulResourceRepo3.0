...
Events.Pseudos = function (pseudos, addEvent, removeEvent) {
  ...
  var storageOf = function (object) {
    return {
      store: object.store
        ...
        : function (key, value) {
            (object.$monitorEvents || (object.$monitorEvents = {}))[key] =
              value;
          },
  ...
  var splitType = function (type) {
    ...
    while (l--)
      if (pseudos[parsedPseudos[l].key]) {
        splits.push({
          event: parsed.tag,
  ...
  return {
    addEvent: function (type, fn, internal) {
      ...
      split.each(function (item) {
        var stackFn = stack;
        stack = function () {
          (eventOptions.listener || pseudos[item.pseudo].listener).call(
            self,
            item,
            stackFn,
            arguments,
    ...
    removeEvent: function (type, fn) {
      ...
      events.each(function (monitor, i) {
        if (!fn || monitor.event == fn)
          removeEvent.apply(this, [eventType, monitor.monitor].concat(args));
        delete events[i];
      }, this);
  ...
  if (this.Element && Element.implement && this.Fx) {
    Element.implement({
      chains: function (effects) {
        Array.from(effects || ["tween", "morph", "reveal"]).each(function (
          effect
        ) {
...
(function () {
  ...
  var check = function (split, target, event) {
    var elementEvent = Element.Events[split.event],
      condition;
    if (elementEvent) condition = elementEvent.condition;
    return (
      Slick.match(target, split.value) &&
      (!condition || condition.call(target, event))
    );
  };
  ...
  var bubbleUp = function (split, event, fn) {
    for (
      ...
      if (target && check(split, target, event))
    ...
    return {
      ...
      onRemove: function (element) {
        element.retrieve($delegationKey + "forms", []).each(function (el) {
          el.retrieve($delegationKey + "listeners", []).each(function (
            listener
          ) {
            el.removeEvent(eventName, listener);
          });
          el.eliminate($delegationKey + eventName + "listeners").eliminate(
            $delegationKey + eventName + "originalFn"
          );
        });
      },
      ...
      listener: function (split, fn, args, monitor, options) {
        ...
        if (!formEvents.contains(fn)) {
          ...
          form
            .store($delegationKey + eventName + "originalFn", formEvents)
    ...
    pin: function (enable, forceScroll) {
      ...
      if (enable !== false) {
        pinnedPosition = this.getPosition(
          supportsPositionFixed ? document.body : this.getOffsetParent()
        );
...
Form.Validator = new Class({
  ...
  test: function (className, field, warn) {
    ...
    var isValid =
      this.hasValidator(field, "ignoreValidation") ||
  ...
  [
    "validate-reqchk-byname",
    {
      ...
      test: function (element, props) {
        ...
        var oneCheckedItem = $$(document.getElementsByName(grpName)).some(
          function (item, index) {
            return item.checked;
          }
        );
    ...
    "validate-match",
    {
      errorMsg: function (element, props) {
        return Form.Validator.getMsg("match").substitute({
          matchName:
            props.matchName || document.id(props.matchInput).get("name"),
        });
      },
      test: function (element, props) {
  ...
  calculateLimit: function () {
    ...
    if (element.getStyle("position") == "relative") {
      ...
      if (container != offsetParent) {
        ...
        top +=
          (Browser.ie6 || Browser.ie7 ? 0 : containerMargin.top) +
          offsetParentPadding.top;
      }
    } else {
      left -= elementMargin.left;
      top -= elementMargin.top;
...
var Slider = new Class({
  ...
  toStep: function (position) {
    ...
    return this.options.steps
      ? Math.round((step -= step % this.stepSize))
      : step;
  },
...
var Asset = {
  javascript: function (source, properties) {
    ...
    var load = loadEvent
      ? function () {
          // make sure we only call the event once
...
var HtmlTable = new Class({
  ...
  push: function (row, rowProperties, target, tag, where) {
    ...
    return {
      tr: new Element("tr", rowProperties)
        .inject(target || this.body, where)
        .adopt(tds),
      tds: tds,
    };
  },
...
(function () {
  ...
  Locale.define("ru-RU", "Date", {
    ...
    monthsAgo: function (delta) {
      return (
        "{delta} " + pluralize(delta, "месяц", "месяца", "месецев") + " назад"
      );
    },
    yearAgo: "год назад",
    yearsAgo: function (delta) {
    ...
    monthsUntil: function (delta) {
      return (
        "через {delta} " + pluralize(delta, "месяц", "месяца", "месецев") + ""
      );
    },
    yearUntil: "через",
    yearsUntil: function (delta) {
