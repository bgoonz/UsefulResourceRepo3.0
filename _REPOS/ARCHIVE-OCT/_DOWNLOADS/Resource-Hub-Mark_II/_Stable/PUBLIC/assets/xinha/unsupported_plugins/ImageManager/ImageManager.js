...
Xinha.Config.prototype.ImageManager = {
  backend:
    Xinha.getPluginDir("ImageManager") + "/backend.php?__plugin=ImageManager&",
  backend_data: null,
...
Xinha.prototype._insertImage = function (image) {
  ...
  if (image) {
    outparam = {
      ...
      f_hspace:
        image.hspace && image.hspace != "-1" ? parseInt(image.hspace) : "",
      f_vspace:
        image.vspace && image.vspace != "-1" ? parseInt(image.vspace) : "",
    };
  ...
  if (editor.config.ImageManager.backend_config != null) {
    ...
    manager +=
      ...
      encodeURIComponent(
        editor.config.ImageManager.backend_config_secret_key_location
      );
  }
  ...
  if (editor.config.ImageManager.backend_data != null) {
    for (var i in editor.config.ImageManager.backend_data) {
      manager +=
        ...
        i +
        "=" +
        encodeURIComponent(editor.config.ImageManager.backend_data[i]);
    }
  }
