...
Opera._pluginInfo = {
  ...
  developer_url:
    "$HeadURL: http://svn.xinha.org/trunk/modules/Opera/Opera.js $".replace(
      /^[^:]*:\s*(.*)\s*\$$/,
      "$1"
    ),
...
Xinha.prototype.setCC = function (target) {
  ...
  try {
    if (target == "textarea") {
      ...
      ta.value = ta.value.replace(
        cc,
        '<span id="XinhaOperaCaretMarker">MARK</span>'
      );
    } else {
      var sel = this.getSelection();
...
Xinha.prototype.findCC = function (target) {
  if (target == "textarea") {
    var ta = this._textArea;
    var pos = ta.value.search(
      /(<span\s+id="XinhaOperaCaretMarker"\s*\/?>((\s|(MARK))*<\/span>)?)/
    );
    if (pos == -1) return;
    var cc = RegExp.$1;
...
Xinha.prototype.initIframe = function () {
  if (!this._iframeLoadDone) {
    if (
      ...
      this._iframe.contentWindow.xinhaReadyToRoll
    ) {
