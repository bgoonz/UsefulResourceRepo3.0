...
InsertSmiley.prototype.onGenerateOnce = function () {
  ...
  if (this.editor.config.InsertSmiley.smileyURL) {
    var smileys = [];
    var smileylist = Xinha._geturlcontent(
      Xinha.getPluginDir("InsertSmiley") + "/smileys.txt"
    );
    var smileyURL = this.editor.config.InsertSmiley.smileyURL;

    smileylist = smileylist.match(/^.+$/gm);
    for (var i = 0; i < smileylist.length; i++) {
      smileys[smileys.length] = {
        title: smileylist[i],
        src: smileyURL + encodeURIComponent(smileylist[i]),
      };
    }
...
InsertSmiley.prototype.loadAssets = function () {
  ...
  if (typeof this.editor.config.InsertSmiley.smileys != "string") {
    ...
    var smileys_def = this.editor.config.InsertSmiley.smileys
    ...
    Xinha._getback(smileys_def, function (sm) {
      self.smileys = eval(sm);
      self.smileys_base = smileys_def.replace(/\/[^\/]+(\?.*)?$/, "/");
    });
  }

  Xinha._getback(
    Xinha.getPluginDir("InsertSmiley") + "/dialog.html",
    function (html) {
      self.makeDialog(html);
    }
  );
};
...
InsertSmiley.prototype.makeDialog = function (html) {
  ...
  for (var i = 0; i < this.smileys.length; i++) {
    src = this.smileys[i].src.match(/^(([a-zA-Z]+:\/\/)|\/)/)
      ? this.smileys[i].src
      : this.smileys_base + this.smileys[i].src;
    ...
    Xinha.addDom0Event(a, "click", function (ev) {
      self.editor.insertHTML(
        '<img src="' + this.smiley_src + '" alt="' + this.title + '" />'
      );
      Xinha._stopEvent(window.event ? event : ev);
      return false;
    });
