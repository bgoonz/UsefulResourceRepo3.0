...
InternetExplorer._pluginInfo = {
  ...
  developer_url:
    "$HeadURL: http://svn.xinha.org/trunk/modules/InternetExplorer/InternetExplorer.js $".replace(
      /^[^:]*:\s*(.*)\s*\$$/,
      "$1"
    ),
  sponsor: "",
      ...
      case "6":
        {
          this.editor.execCommand(
            "formatblock",
            false,
            "<h" + this.editor.getKey(ev).toLowerCase() + ">"
          );
          Xinha._stopEvent(ev);
          return true;
        }
...
InternetExplorer.prototype.inwardHtml = function (html) {
  ...
  // those here
  html = html.replace(
    /<span[^>]+id="__InsertSpan_Workaround_[a-z]+".*?>([\s\S]*?)<\/span>/i,
    "$1"
  );
...
Xinha.prototype.restoreSelection = function (savedSelection) {
  ...
  if (
    savedSelection.parentElement &&
    findDoc(savedParentElement) == findDoc(rangeParentElement)
  ) {
...
Xinha.prototype.selectNodeContents = function (node, collapseToStart) {
  ...
  if (
    ...
    if (3 == node.nodeType) {
      ...
      for (
        ;
        borderElement && 1 != borderElement.nodeType;
        borderElement = borderElement.previousSibling
      ) {
        if (3 == borderElement.nodeType) {
          // IE doesn't count '\r' as a character, so we have to adjust the offset.
          texthackOffset +=
            borderElement.nodeValue.length -
            borderElement.nodeValue.split("\r").length -
            1;
        }
      }
      ...
      for (
        ;
        borderElement && 1 != borderElement.nodeType;
        borderElement = borderElement.nextSibling
      ) {
        if (3 == borderElement.nodeType) {
          // IE doesn't count '\r' as a character, so we have to adjust the offset.
          texthackOffset +=
