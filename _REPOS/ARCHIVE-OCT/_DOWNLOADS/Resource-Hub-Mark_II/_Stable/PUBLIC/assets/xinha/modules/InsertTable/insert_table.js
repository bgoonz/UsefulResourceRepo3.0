...
InsertTable._pluginInfo = {
  ...
  developer_url:
    "$HeadURL: http://svn.xinha.org/trunk/modules/InsertTable/insert_table.js $".replace(
      /^[^:]*:\s*(.*)\s*\$$/,
      "$1"
    ),
  sponsor: "",
...
InsertTable.loadAssets = function () {
  ...
  self.loading = true;
  Xinha._getback(
    _editor_url + "modules/InsertTable/dialog.html",
    function (getback) {
      self.html = getback;
  ...
  Xinha._getback(
    _editor_url + "modules/InsertTable/pluginMethods.js",
    function (getback) {
      eval(getback);
...
InsertTable.prototype.prepareDialog = function () {
  ...
  this.borderColorPicker = new Xinha.colorPicker.InputBinding(
    dialog.getElementById("border_color")
  );

  this.dialog.onresize = function () {
    this.getElementById("layout_fieldset").style.width =
      this.width / 2 + 50 + "px";
    this.getElementById("spacing_fieldset").style.width =
      this.width / 2 - 120 + "px";
  };
