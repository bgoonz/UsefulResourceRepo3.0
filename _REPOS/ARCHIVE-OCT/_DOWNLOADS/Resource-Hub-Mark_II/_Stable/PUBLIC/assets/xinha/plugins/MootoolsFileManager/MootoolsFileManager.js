...
Xinha.Config.prototype.MootoolsFileManager = {
  backend:
    ...
    "/backend.php?__plugin=MootoolsFileManager&",
  backend_data: {},
};
...
if (typeof FileManager == "undefined") {
  ...
  MootoolsFileManager.AssetLoader
    ...
    .loadScript(
      "mootools-filemanager/Language/Language.en.js",
      "MootoolsFileManager"
    )
    .whenReady(function () {
  ...
  if (__MFM_USE_FLASH__) {
    MootoolsFileManager.AssetLoader.loadScript(
      "mootools-filemanager/Source/Uploader/Swiff.Uploader.js",
      "MootoolsFileManager"
    )
      .loadScript(
        "mootools-filemanager/Source/Uploader/Fx.ProgressBar.js",
        "MootoolsFileManager"
      )
      .loadScript(
        "mootools-filemanager/Source/Uploader.js",
        "MootoolsFileManager"
      );
  } else {
    MootoolsFileManager.AssetLoader.loadScript(
      "mootools-filemanager/Source/NoFlash.Uploader.js",
      "MootoolsFileManager"
    );
  }
}
MootoolsFileManager.AssetLoader.loadStyle(
  "MootoolsFileManager.css",
  "MootoolsFileManager"
);
...
function MootoolsFileManager(editor) {
  ...
  var phpcfg = Xinha._posturlcontent(
    editor.config.MootoolsFileManager.backend + "__function=read-config",
    editor.config.MootoolsFileManager.backend_data
  );
...
MootoolsFileManager.prototype.hookUpButtons = function () {
  ...
  if (phpcfg.files_dir) {
    MootoolsFileManager.AssetLoader.loadScript(
      "MootoolsFileManager.FileManager.js",
      "MootoolsFileManager"
    );
    ...
    this.editor.config.registerButton({
      ...
      action: function (editor) {
        MootoolsFileManager.AssetLoader.whenReady(function () {
  ...
  if (phpcfg.images_dir) {
    MootoolsFileManager.AssetLoader.loadScript(
      "MootoolsFileManager.ImageManager.js",
      "MootoolsFileManager"
    );
...
MootoolsFileManager.prototype.ScaleImage = function (
  origdim,
  newdim,
  flexside
) {
  ...
  switch (flexside) {
    case "width":
      ratio = origdim.width / origdim.height;
      break;
    case "height":
      ratio = origdim.height / origdim.width;
      break;
  }
...
MootoolsFileManager.prototype.convertToHex = function (color) {
  ...
  if (typeof color == "string" && / /.test.color) {
    ...
    for (var i = 0; i < colors.length; i++) {
      ...
      if (i + 1 < colors.length) colorstring += " ";
    }
    return colorstring;
  }
