...
function changeDir(newDir) {
  ...
  location.href =
    _backend_url + "__function=images&dir=" + encodeURIComponent(newDir);
}

function newFolder(dir, newDir) {
  location.href =
    _backend_url +
    "__function=images&dir=" +
    encodeURIComponent(dir) +
    "&newDir=" +
    encodeURIComponent(newDir);
}
...
function selectImage(filename, alt, width, height) {
  ...
  topDoc.getElementById("f_preview").src =
    window.parent._backend_url +
    "__function=thumbs&img=" +
...
function confirmDeleteFile(file) {
  if (confirm(i18n("Delete file?"))) return true;
...
function confirmDeleteDir(dir, count) {
  ...
  if (confirm(i18n("Delete folder?"))) return true;
