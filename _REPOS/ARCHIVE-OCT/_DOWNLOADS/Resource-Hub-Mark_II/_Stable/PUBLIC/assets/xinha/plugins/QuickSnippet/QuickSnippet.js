...
function QuickSnippet(editor) {
  ...
  if (editor.config.QuickSnippet.snippetfile) {
    ...
    Xinha._getback(
      editor.config.QuickSnippet.default_snippetfile,
      function (t, r) {
        self.addSnippets(JSON.parse(t));
      }
    );
  ...
  editor.config.addToolbarElement(
    ["QuickSnippet", "separator"],
    "formatblock",
    -1
  );
}
...
QuickSnippet.prototype.doInsert = function () {
  ...
  if (selected.parent) {
    ...
    var parent = jQuery(this.editor.getParentElement())
      .closest(selected.parent)
      .first();
    ...
    if (!parent.length) {
      ...
      parent.children().each(function (i, v) {
        if (
          jQuery(v)
            .find(self.editor.getParentElement())
            .addBack(self.editor.getParentElement()).length
        ) {
          found = true;
...
QuickSnippet.prototype.updateSnippets = function () {
      ...
      if (!focusElement.closest(snippet.parent).length) {
        ...
        if (!optionElements.closest("option[value=" + i + "]").length) {
          var newOption = jQuery(
            '<option value="' + i + '">' + snippet.name + "</option>"
          ).first();
          newOption.insertAfter(lastOption);
          lastOption = newOption;
          somethingChanged++;
        } else {
          lastOption = optionElements
            .closest("option[value=" + i + "]")
            .first();
        }
      }
    }
  ...
  if (somethingChanged) {
    ...
    if (
      typeof jQuery(select).select2 != "undefined" &&
      jQuery(select).data("select2").isOpen()
    ) {
...
QuickSnippet.prototype._prepareDialog = function () {
  ...
  if (!this.html) {
    Xinha._getback(
      Xinha.getPluginDir("QuickSnippet") + "/dialog.html",
      function (getback) {
        self.html = getback;
        self._prepareDialog();
      }
  ...
  this.dialog = new Xinha.Dialog(editor, this.html, "QuickSnippet", {
    width: 400,
  });
