...
FileManager.implement({
  ...
  onDialogCloseWhenUpload: function () {
    if (this.swf && this.swf.box)
      this.swf.box.setStyle("visibility", "visible");
  },
  ...
  startUpload: function () {
    ...
    this.upload = {
      ...
      uploader: new Element("div", {
        opacity: 0,
        class: "filemanager-uploader-area",
      }).adopt(
    ...
    if (this.options.resizeImages) {
      ...
      check();
      this.upload.label = new Element("label")
        .adopt(
          resizer,
          new Element("span", { text: this.language.resizeImages })
        )
        .addEvent("click", check)
        .inject(this.menu);
    }
    ...
    var File = new Class({
      ...
      initialize: function (base, data) {
        ...
        var tx_cfg = self.options.mkServerRequestURL(self, "upload", {
          ...
          resize:
            self.options.resizeImages && resizer.hasClass("checkboxChecked")
              ? 1
              : 0,
        });
      ...
      render: function () {
        if (this.invalid) {
          ...
          if (this.validationError === "sizeLimitMin")
            sub.size_min = Swiff.Uploader.formatUnit(
              this.base.options.fileSizeMin,
              "b"
            );
          else if (this.validationError === "sizeLimitMax")
            sub.size_max = Swiff.Uploader.formatUnit(
              this.base.options.fileSizeMax,
              "b"
            );
        ...
        this.ui = {};
        this.ui.icon = new Asset.image(
          self.assetBasePath + "Images/Icons/" + this.extension + ".png",
          {
            class: "icon",
            onerror: function () {
              new Asset.image(
                self.assetBasePath + "Images/Icons/default.png"
              ).replaces(this);
            },
          }
        );
        ...
        this.ui.title = new Element("span", {
          class: "file-title",
          text: laname,
          title: this.name,
        });
        this.ui.size = new Element("span", {
          class: "file-size",
          text: Swiff.Uploader.formatUnit(this.size, "b"),
        });

        var file = this;
        this.ui.cancel = new Asset.image(
          self.assetBasePath + "Images/cancel.png",
          { class: "file-cancel", title: self.language.cancel }
        ...
        var progress = new Element("img", {
          class: "file-progress",
          src: self.assetBasePath + "Images/bar.gif",
        });
      ...
      onComplete: function (file_obj) {
        ...
        this.ui.element
          ...
          .highlight(!failure ? "#e6efc2" : "#f0c2c2");
        (function () {
          this.ui.element
    ...
    this.getFileTypes = function () {
      ...
      if (this.options.filter == "image")
        fileTypes = {
          "Images (*.jpg, *.gif, *.png)": "*.jpg; *.jpeg; *.bmp; *.gif; *.png",
        };
      if (this.options.filter == "video")
        fileTypes = {
          "Videos (*.avi, *.flv, *.mov, *.mpeg, *.mpg, *.wmv, *.mp4)":
            "*.avi; *.flv; *.fli; *.movie; *.mpe; *.qt; *.viv; *.mkv; *.vivo; *.mov; *.mpeg; *.mpg; *.wmv; *.mp4",
        };
      if (this.options.filter == "audio")
        fileTypes = {
          "Audio (*.aif, *.mid, *.mp3, *.mpga, *.rm, *.wav)":
            "*.aif; *.aifc; *.aiff; *.aif; *.au; *.mka; *.kar; *.mid; *.midi; *.mp2; *.mp3; *.mpga; *.ra; *.ram; *.rm; *.rpm; *.snd; *.wav; *.tsi",
        };
      if (this.options.filter == "text")
        fileTypes = {
          "Text (*.txt, *.rtf, *.rtx, *.html, *.htm, *.css, *.as, *.xml, *.tpl)":
            "*.txt; *.rtf; *.rtx; *.html; *.htm; *.css; *.as; *.xml; *.tpl",
        };
      if (this.options.filter == "application")
        fileTypes = {
          "Application (*.bin, *.doc, *.exe, *.iso, *.js, *.odt, *.pdf, *.php, *.ppt, *.swf, *.rar, *.zip)":
            "*.ai; *.bin; *.ccad; *.class; *.cpt; *.dir; *.dms; *.drw; *.doc; *.dvi; *.dwg; *.eps; *.exe; *.gtar; *.gz; *.js; *.latex; *.lnk; *.lnk; *.oda; *.odt; *.ods; *.odp; *.odg; *.odc; *.odf; *.odb; *.odi; *.odm; *.ott; *.ots; *.otp; *.otg; *.pdf; *.php; *.pot; *.pps; *.ppt; *.ppz; *.pre; *.ps; *.rar; *.set; *.sh; *.skd; *.skm; *.smi; *.smil; *.spl; *.src; *.stl; *.swf; *.tar; *.tex; *.texi; *.texinfo; *.tsp; *.unv; *.vcd; *.vda; *.xlc; *.xll; *.xlm; *.xls; *.xlw; *.zip",
        };
    ...
    this.swf = new Swiff.Uploader({
      ...
      onComplete: function (info) {
        ...
        (function () {
          ...
          .bind(this)
          .delay(this.upload.error_count > 0 ? 5500 : 1));
      }.bind(this),
      onFileComplete: function (f) {
        self.diag.log(
          "FlashUploader: onFileComplete",
          arguments,
          ", fileList: ",
          self.swf.fileList
        );
      ...
      onFail: function (error) {
        self.diag.log(
          "FlashUploader: onFail",
          arguments,
          ", swf: ",
          self.swf,
          ", fileList: ",
          typeof self.swf !== "undefined" ? self.swf : "---"
        );
        if (error !== "empty") {
          $$(self.upload.button, self.upload.label).dispose();
          self.showError(
            self.language.flash[error] || self.language.flash.flash
          );
        }
      },
      ...
      onDisabledBrowse: function () {
        self.diag.log(
          "FlashUploader: onDisabledBrowse",
          arguments,
          ", fileList: ",
          self.swf.fileList
        );
