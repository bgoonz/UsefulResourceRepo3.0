...
function ExtendedFileManager(editor) {
  ...
  var manager =
    editor.config.ExtendedFileManager.backend + "__function=manager";
  if (cfg.ExtendedFileManager.backend_config != null) {
    ...
    manager +=
      ...
      encodeURIComponent(
        cfg.ExtendedFileManager.backend_config_secret_key_location
      );
  }
  ...
  if (cfg.ExtendedFileManager.backend_data != null) {
    for (var i in cfg.ExtendedFileManager.backend_data) {
      manager +=
        ...
        i +
        "=" +
        encodeURIComponent(cfg.ExtendedFileManager.backend_data[i]);
    }
  }
...
Xinha.Config.prototype.ExtendedFileManager = {
  use_linker: true,
  backend:
    Xinha.getPluginDir("ExtendedFileManager") +
...
if (typeof ImageManager == "undefined") {
  Xinha.prototype._insertImage = function (image) {
    ...
    if (image) {
      ...
      outparam.param.f_backgroundColor = convertToHex(
        outparam.param.f_backgroundColor
      );
      outparam.param.f_borderColor = convertToHex(outparam.param.f_borderColor);
    }
    Dialog(
      this.config.ExtendedFileManager.manager,
      function (param) {
        if (!param) {
        ...
        if (!img) {
          ...
          if (!param.f_url) {
            // delete the image if empty url passed
...
Xinha.prototype._linkFile = function (link) {
  ...
  if (!link) {
    ...
    var compare = 0;
    if (Xinha.is_ie) {
      if (sel.type == "Control") compare = range.length;
      else compare = range.compareEndPoints("StartToEnd", range);
    } else {
  ...
  Dialog(
    this.config.ExtendedFileManager.manager + "&mode=link",
    function (param) {
