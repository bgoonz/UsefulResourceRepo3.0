...
function SaveSubmit(editor) {
  ...
  cfg.registerIcon(
    "savesubmitchanged",
    Xinha.getPluginDir("SaveSubmit") + "/img/ed_save_red.gif"
  );
  cfg.registerIcon(
    "savesubmitunchanged",
    Xinha.getPluginDir("SaveSubmit") + "/img/ed_save_green.gif"
  );
...
SaveSubmit.prototype.setChanged = function () {
  this.editor._toolbarObjects.savesubmit.swapImage(
    this.editor.config.iconList.savesubmitchanged
  );
...
SaveSubmit.prototype.setUnChanged = function () {
  this.changed = false;
  this.editor._toolbarObjects.savesubmit.swapImage(
    this.editor.config.iconList.savesubmitunchanged
  );
};
...
SaveSubmit.prototype.setMessage = function (string) {
  ...
  if (!document.getElementById("message_sub_" + textarea.id)) {
    return;
  }
...
SaveSubmit.prototype.removeMessage = function () {
  var textarea = this.textarea;
  if (!document.getElementById("message_" + textarea.id)) {
    return;
  }
...
SaveSubmit.prototype.buildMessage = function () {
  ...
  loading_message.style.top =
    Xinha.findPosY(htmlarea) + parseInt(htmlarea.offsetHeight) / 2 - 50 + "px";
