...
describe("MDCTopAppBarFoundation", () => {
  ...
  it("top app bar : moveTopAppBar_ update required transition from fully shown to 1px scrolled", () => {
    const { foundation, mockAdapter } = setupTest();
    (foundation as any).currentAppBarOffsetTop_ = -1; // Indicates 1px scrolled up
  ...
  it("top app bar : moveTopAppBar_ update required transition from 1px shown to fullyHidden ", () => {
    const { foundation, mockAdapter } = setupTest();
    (foundation as any).currentAppBarOffsetTop_ = -64; // Indicates 64px scrolled
    (foundation as any).checkForUpdate_ = () => true;
    (foundation as any).moveTopAppBar_();
    expect(mockAdapter.setStyle).toHaveBeenCalledWith(
      "top",
      "-" + numbers.MAX_TOP_APP_BAR_HEIGHT + "px"
    );
  });
  ...
  it("top app bar : moveTopAppBar_ update is not required results in no top app bar style change", () => {
    ...
    (foundation as any).moveTopAppBar_();
    expect(mockAdapter.setStyle).not.toHaveBeenCalledWith(
      "top",
      jasmine.anything()
    );
  });
  ...
  it(
    "top app bar : handleTargetScroll does not update currentAppBarOffsetTop_ if " +
  ...
  it(
    "top app bar : handleTargetScroll scroll greater than top app bar height results in " +
  ...
  it(
    "top app bar : handleTargetScroll scrolling does not generate a " +
