...
describe("mdc-typography.scss", () => {
  ...
  it("should allow global variable overrides with @import", () => {
    const css = fs
      .readFileSync(path.join(__dirname, "global-variables.test.css"), "utf8")
      .trim();
    ...
    expect(headline1Css.includes("font-family: Arial")).toBe(
      true,
      "$mdc-typography-font-family should override"
    );
    expect(headline1Css.includes("font-size: 1rem")).toBe(
      true,
      "$mdc-typography-styles-headline1-font-size should override"
    );
  });
});
