...
let NodeDebugAdapter = /** @class */ (() => {
  class NodeDebugAdapter extends vscode_chrome_debug_core_1.ChromeDebugAdapter {
    ...
    launch(args) {
      ...
      return __awaiter(this, void 0, void 0, function* () {
        ...
        const resolvedProgramPath = yield this.resolveProgramPath(
          programPath,
          args.sourceMaps
        );
        let program;
        ...
        if (
          ...
          const termArgs = {
            kind:
              args.console === "integratedTerminal" ? "integrated" : "external",
          ...
          yield this.launchInInternalConsole(
    ...
    doAttach(
      ...
      return __awaiter(this, void 0, void 0, function* () {
        ...
        this._session.sendEvent(
          new vscode_debugadapter_1.CapabilitiesEvent({
      ...
      return new Promise((resolve, reject) => {
        ...
        nodeProcess.stdout.on("data", (data) => {
          if (
            ...
            this._session.sendEvent(
              new vscode_debugadapter_1.OutputEvent(msg, "stdout")
            );
          }
        });
        resolve();
    ...
    captureStderr(nodeProcess, noDebugMode) {
      nodeProcess.stderr.on("data", (data) => {
        ...
        if (
          ...
          this._session.sendEvent(
            new vscode_debugadapter_1.OutputEvent(msg, "stderr")
          );
        }
    ...
    configurationDone() {
      ...
      return __awaiter(this, void 0, void 0, function* () {
        ...
        this.events.emit(
          vscode_chrome_debug_core_1.ChromeDebugSession
            .FinishedStartingUpEventName,
    ...
    killNodeProcess() {
      if (this.nodeProcessId && !this.normalAttachMode) {
        if (this.nodeProcessId === 1) {
          vscode_chrome_debug_core_1.logger.log(
            "Not killing launched process. It has PID=1"
          );
        } else {
          vscode_chrome_debug_core_1.logger.log(
            "Killing process with id: " + this.nodeProcessId
          );
      ...
      return __awaiter(this, void 0, void 0, function* () {
        ...
        const restartArgs =
          this._restartMode && !this._clientRequestedSessionEnd
            ? { port: this._port }
            : undefined;
        return _super.terminateSession.call(
          this,
          reason,
          undefined,
          restartArgs
    ...
    resolveProgramPath(programPath, sourceMaps) {
      return __awaiter(this, void 0, void 0, function* () {
        vscode_chrome_debug_core_1.logger.verbose(
          `Launch: Resolving programPath: ${programPath}`
        );
        if (!programPath) {
          return programPath;
        }
    ...
    beginWaitingForDebuggerPaused() {
      ...
      let count = this.normalAttachMode
        ...
        : 100;
      vscode_chrome_debug_core_1.logger.log(
        Date.now() / 1000 + ": Waiting for initial debugger pause"
      );
    ...
    getNodeProcessDetailsIfNeeded() {
      return __awaiter(this, void 0, void 0, function* () {
        ...
        const response = yield this.chrome.Runtime.evaluate({
          expression: "[process.pid, process.version, process.arch]",
          returnByValue: true,
          contextId: 1,
        }).catch((error) =>
          vscode_chrome_debug_core_1.logger.error(
            "Error evaluating `process.pid`: " + error.message
          )
        );
        if (!response) {
          return;
        ...
        if (response.exceptionDetails) {
          const description =
            vscode_chrome_debug_core_1.chromeUtils.errorMessageFromExceptionDetails(
          ...
          vscode_chrome_debug_core_1.logger.log(
            `Target node version: ${version} ${arch}`
          );
          /* __GDPR__
                        "nodeVersion" : {
    ...
    getReadonlyOrigin(aPath) {
      return path.isAbsolute(aPath) ||
        aPath.startsWith(
          vscode_chrome_debug_core_1.ChromeDebugAdapter.EVAL_NAME_PREFIX
        )
        ? localize(7, null)
        : localize(8, null);
    }
  ...
  NodeDebugAdapter.DEBUG_BRK_DEP_MSG =
...
function getSourceMapPathOverrides(cwd, sourceMapPathOverrides) {
  return sourceMapPathOverrides
    ? resolveCwdPattern(cwd, sourceMapPathOverrides, /*warnOnMissing=*/ true)
    : resolveCwdPattern(
        cwd,
...
function fixNodeInternalsSkipFilePattern(pattern) {
  ...
  if (internalsMatch) {
    return `^(?!\/)(?![a-zA-Z]:)(?!file:///)${vscode_chrome_debug_core_1.utils.pathGlobToBlackboxedRegex(
      internalsMatch[1]
    )}`;
  } else {
    return null;
...
function detectSupportedDebugArgsForLaunch(config, runtimeExecutable, env) {
  if (
    ...
    vscode_chrome_debug_core_1.logger.log(
      "Using --inspect-brk because a runtimeExecutable is set"
    );
    ...
    try {
      ...
      vscode_chrome_debug_core_1.logger.error(
        "Node version detection failed: " + (e && e.message)
      );
    }
