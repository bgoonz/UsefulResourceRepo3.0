...
(function () {
  ...
  function savePage() {
    localStorage.setItem(
      "page",
      escape(document.getElementById("save").innerHTML)
    );
  }
  ...
  function getPageContents() {
    ...
    var elsToRemove = baseEl.querySelectorAll(
      "script, iframe, #document-controls, #github-link"
    );
  ...
  function addPage() {
    ...
    var sheetHtml = `<section class="sheet">
            <aside></aside>
            <section></section>
        </section>`;
  ...
  function addDocumentControls() {
    ...
    var docControlsStr = `<!-- Document control buttons-->
    ...
    for (var i = 0, e = buttons.length; i < e; i++) {
      if (buttons[i].dataset.action in actions) {
        buttons[i].addEventListener(
          "click",
          actions[buttons[i].dataset.action]
        );
      }
    }
  ...
  function makeEditable() {
    if (USE_CONTENTEDITABLE) {
      var editableNodes = document.querySelectorAll(
        "p, span, ul.editable, ol.editable, ul:not(.editable) li, ol:not(.editable) li, time, h1, h2, h3, h4, h5, h6, address"
      );
  ...
  function updateMetaDate() {
    document
      ...
      .setAttribute("content", getDateFormatted());
  }
  ...
  function updateMetaSubject() {
    ...
    document
      .querySelector('meta[name="subject"]')
      .setAttribute("content", summaryText);
  }
  ...
  function updateMetaAuthor() {
    ...
    document
      ...
      .setAttribute("content", authorName);
  }
  ...
  function updateMetaKeywords() {
    ...
    document
      ...
      .setAttribute("content", skills.join(","));
  }
