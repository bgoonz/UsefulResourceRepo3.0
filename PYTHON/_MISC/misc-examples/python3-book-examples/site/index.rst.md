...
::: {.module synopsis="Site-wide configuration"} site :::
...
::: {.literalinclude caption="" start-after="#end_pymotw_header"} site_import_path.py :::
...
::: {.literalinclude caption="" start-after="#end_pymotw_header"} site_user_base.py :::
...
::: {.literalinclude caption="" start-after="#end_pymotw_header"} site_enable_user_site.py :::
...
As paths are added to the import path, they are also scanned for _path configuration files_. A path configuration file is a plain text file with the extension `.pth`. Each line in the file can take one of four forms.
...
::: {.literalinclude caption="" start-after="#end_pymotw_header"} with_modules/mymodule.py :::

This script shows how `addsitedir()` extends the import path so the interpreter can find the desired module.

::: {.literalinclude caption="" start-after="#end_pymotw_header"} site_addsitedir.py :::
...
::: {.module synopsis="Site-specific configuration"} sitecustomize :::
...
::: {.literalinclude caption="" start-after="#end_pymotw_header"} with_sitecustomize/sitecustomize.py :::

A simple script can be used to show that `sitecustomize.py` is imported before Python starts running your own code.

::: {.literalinclude caption="" start-after="#end_pymotw_header"} with_sitecustomize/site_sitecustomize.py :::
...
::: {.module synopsis="User-specific configuration"} usercustomize :::
...
::: {.literalinclude caption="" start-after="#end_pymotw_header"} with_usercustomize/usercustomize.py :::
...
::: {.literalinclude caption="" start-after="#end_pymotw_header"} with_usercustomize/site_usercustomize.py :::
...
- `sys-imports`{.interpreted-text role="ref"} \-- Description of how the import path defined in `sys` works.
- [setuptools](https://setuptools.readthedocs.io/en/latest/index.html) \--Packaging library and installation tool `easy_install`.
- [Running code at Python startup](http://nedbatchelder.com/blog/201001/running_code_at_python_startup.html) \-- Post from Ned Batchelder discussing ways to cause the Python interpreter to run custom initialization code before starting the main program execution. :::
