...
describe('Module-level features:', function () {
  ...
  describe('Default log level of INFO', function () {
    ...
    it('should log an INFO level message', function () {
      expect(logger.info('BAR', 'foo()', mockLogFunction)).to.equal(
        '1111111111:INFO: foo(): BAR'
      );
    });
    // WARN
    it('should log a WARN level message', function () {
      expect(logger.warn('BAR', 'foo()', mockLogFunction)).to.equal(
        '1111111111:WARN: foo(): BAR'
      );
    });
    // ERROR
    it('should log an ERROR level message', function () {
      expect(logger.error('BAR', 'foo()', mockLogFunction)).to.equal(
        '1111111111:ERROR: foo(): BAR'
      );
    });
    // SEVERE
    it('should log an SEVERE level message', function () {
      expect(logger.severe('BAR', 'foo()', mockLogFunction)).to.equal(
        '1111111111:SEVERE: foo(): BAR'
      );
    });
    // FATAL
    it('should log a FATAL level message', function () {
      expect(logger.fatal('BAR', 'foo()', mockLogFunction)).to.equal(
        '1111111111:FATAL: foo(): BAR'
      );
    });
  });
});
...
describe('Log Levels and the API:', function () {
  describe('when current log Level=TRACE', function () {
    ...
    it('should output TRACE level message', function () {
      expect(logger.trace('whatever', 'whatever()', mockLogFunction)).to.equal(
        '1111111111:TRACE: whatever(): whatever'
      );
    });
    // DEBUG
    it('should output DEBUG level message', function () {
      expect(logger.debug('whatever', 'whatever()', mockLogFunction)).to.equal(
        '1111111111:DEBUG: whatever(): whatever'
      );
    });
    // INFO
    it('should output INFO level message', function () {
      expect(logger.info('whatever', 'whatever()', mockLogFunction)).to.equal(
        '1111111111:INFO: whatever(): whatever'
      );
    });
    // WARN
    it('should output WARN level message', function () {
      expect(logger.warn('whatever', 'whatever()', mockLogFunction)).to.equal(
        '1111111111:WARN: whatever(): whatever'
      );
    });
    // ERROR
    it('should output ERROR level message', function () {
      expect(logger.error('whatever', 'whatever()', mockLogFunction)).to.equal(
        '1111111111:ERROR: whatever(): whatever'
      );
    });
    // SEVERE
    it('should log an SEVERE level message', function () {
    ...
    it('should output FATAL level message', function () {
      expect(logger.fatal('whatever', 'whatever()', mockLogFunction)).to.equal(
        '1111111111:FATAL: whatever(): whatever'
      );
    });
  });
  describe('when current log Level=DEBUG', function () {
    ...
    it('should not output TRACE level message', function () {
      expect(logger.trace('whatever', 'whatever()', mockLogFunction)).to.be
        .null;
    });
    it('should output DEBUG level message', function () {
      expect(logger.debug('whatever', 'whatever()', mockLogFunction)).to.equal(
  ...
  describe('when current log Level=INFO', function () {
    ...
    it('should not output DEBUG level message', function () {
      expect(logger.debug('whatever', 'whatever()', mockLogFunction)).to.be
        .null;
    });
    it('should output INFO level message', function () {
      expect(logger.info('whatever', 'whatever()', mockLogFunction)).to.equal(
  ...
  describe('when current log Level=SEVERE', function () {
    ...
    it('should output ERROR level message', function () {
      expect(logger.error('whatever', 'whatever()', mockLogFunction)).to.be
        .null;
    });
    it('should log an SEVERE level message', function () {
      expect(logger.severe('BAR', 'foo()', mockLogFunction)).to.equal(
  ...
  describe('when current log Level=OFF', function () {
    ...
    it('should not output FATAL level message', function () {
      expect(logger.fatal('whatever', 'whatever()', mockLogFunction)).to.be
        .null;
    });
  });
});
