  ...
  async.parallel(
    ...
    function (err, results) {
      ...
      let whenCreated = dateformat(
        new Date(results.list.whenCreated),
        'mm-dd-yyyy hh:MM:ss'
      );
      let whenModified = results.list.whenModified
        ? dateformat(new Date(results.list.whenModified), 'mm-dd-yyyy hh:MM:ss')
...
function renderUpdatePage(req, res, next) {
  ...
  request('GET', '/rest/lists/' + listId, null, (err, data) => {
    if (err) {
      ...
      let jsonData = JSON.parse(data);
      let whenCreated = dateformat(
        new Date(jsonData.whenCreated),
        'mm-dd-yyyy hh:MM:ss'
      );
      let whenModified = jsonData.whenModified
        ? dateformat(new Date(jsonData.whenModified), 'mm-dd-yyyy hh:MM:ss')
        : null; //eslint-disable-line
      // Now render the page
