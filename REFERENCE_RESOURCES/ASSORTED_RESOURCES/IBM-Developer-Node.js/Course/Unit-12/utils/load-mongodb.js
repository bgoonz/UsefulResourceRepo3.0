...
function handleBrandDocument(fields) {
  ...
  brands.insertOne(brand, (err, result) => {
    if (err) {
      logger.error(
        'Error occurred while creating brand record: ' + JSON.stringify(brand),
        'handleBrandDocument()'
...
function handleItemDocument(fields) {
  ...
  brands.find(brand).toArray(function (err, brandDocs) {
    if (err) {
      logger.error(
        'Error occurred while finding brand record: ' + JSON.stringify(brand),
        'handleItemDocument()'
      ); // eslint-disable-line max-len
      throw err;
    }
    if (brandDocs.length == 1) {
      ...
      let errorMessage =
        'Expected one document from brands query using description: ' +
        brandDescription +
        ' but instead got back ' +
        brandDocs.length +
        ' documents. Cannot continue!';
...
async function initializeDb() {
  ...
  await db
    .dropCollection('shoppingLists')
    .then(() => {
      logger.info(
        'Successfully dropped shoppingLists collection',
        'initializeDb()'
      );
    })
    .catch((err) => {
      ...
      logger.warn(
        'Error while initializing database: ' + err.message,
        'initializeDb()'
      );
    });
...
utils.dbConnect().then((database) => {
  ...
  initializeDb().then(() => {
    ...
    loadData(appSettings.brand_file_name, handleBrandDocument)
      .then(() => {
