...
before(function (done) {
  ...
  logger.warn(
    '* WARNING: Before running this functional test suite, make sure the DB is empty! *'
  );
  logger.warn(
    '*          If you do not, the tests will most likely fail! You have been warned. *'
  );
  logger.warn(
    '**********************************************************************************'
  );
  ...
  utils
    ...
    .then((database) => {
      ...
      brands.insertOne(brand, (err, result) => {
        if (err) {
          logger.error(
            'Error occurred while creating brand record: ' +
        ...
        if (result.insertedCount != 1) {
          let errorMessage =
            'Expected to insert one record, instead inserted ' +
...
after(() => {
  ...
  db.collection('shoppingLists')
    .deleteMany({ description: { $regex: '.*test.*', $options: 'i' } })
    .then(() => {
      logger.info('Shopping lists added for functional test, removed.');
    });
  db.collection('items')
    .deleteMany({ itemDescription: { $regex: '.*test.*', $options: 'i' } })
    .then(() => {
...
describe(`GET /rest/lists/:listId/items`, function testListsFindByIdWithAllItems() {
  it('should return a collection of lists whose length is > 0 and whose first item belongs to its shopping list', (done) => {
    // eslint-disable-line max-len
    request(
      'POST',
      `/rest/lists`,
      `{ "description" : "Test Shopping List for testListsFindByIdWithAllItems()" }`,
      (err, data) => {
        // eslint-disable-line max-len
        if (err) {
          logger.error(
            `TEST FAILED with message: ${err.message} `,
            'testListsFindByIdWithAllItems()'
          );
          return;
        }
        if (data) {
                  ...
                  (err, data) => {
                    ...
                    if (data) {
                      try {
                        logger.debug(
                          'Raw response data: ' + data,
                          'testListsFindByIdWithAllItems(2)'
                        );
                        // This request returns JSON data
                        let jsonData = JSON.parse(data);
                      ...
                      let message =
                        'testListsFindByIdWithAllItems(): TEST FAILED. No data to be processed. Try again.'; // eslint-disable-line max-len
...
describe(
  'DELETE /rest/lists/:listId/items/' + ftItemId.toHexString(),
