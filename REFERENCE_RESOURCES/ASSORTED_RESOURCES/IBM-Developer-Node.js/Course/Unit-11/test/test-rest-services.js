...
describe('GET /rest/items?id=8', function testItemFindById() {
  it('should return the item with id = 8 and upc 051600080015 (and lots more)', (done) => {
    ...
    const expected_item_description =
    ...
    request('GET', `/rest/items?id=${expected_item_id}`, null, (err, data) => {
      ...
      if (data) {
        try {
          ...
          expect(jsonData.brand_description).to.equal(
            expected_brand_description
          );
...
describe('GET /rest/items?upc=052603054294', function testItemFindByUpc() {
  it('should return the matching item', (done) => {
    ...
    const expected_item_description =
      'Pacific Natural Foods Broth Free Range Chicken Organic - 4 Ct';
  ...
  it('should return a collection of lists whose length is > 0 and whose first item belongs to its shopping list', (done) => {
    ...
    request('GET', `/rest/lists/${listId}/items`, '{}', (err, data) => {
      ...
      if (data) {
        ...
        let message =
          'testListsFindByIdWithAllItems(): TEST FAILED. No data to be processed. Try again.';
...
describe('PUT /rest/lists/1/items/10', function testListsUpdateItem() {
  it('should update one and only one row', (done) => {
    ...
    request(
      'PUT',
      `/rest/lists/${listId}/items/${itemId}`,
      `{ "quantity" : 5 }`,
      (err, data) => {
        if (err) {
          logger.error(
        ...
        if (data) {
          ...
          let message =
            'testListsUpdateItem(): TEST FAILED. No data to be processed. Try again.';
    ...
    request(
      ...
      (err, data) => {
        ...
        if (data) {
          ...
          let message =
            'testListsRemoveItem(): TEST FAILED. No data to be processed. Try again.';
