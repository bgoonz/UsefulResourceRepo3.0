  ...
  return new Promise((resolve, reject) => {
    ...
    db.run(sql, description, id, function callback(err) {
      if (err) {
        ...
        resolve({
          data: `{ "rowsAffected" : ${this.changes} }`,
          statusCode: 200,
...
function addItem(listId, itemId, quantity) {
  return new Promise((resolve, reject) => {
    ...
    const sql =
      'INSERT INTO shopping_list_item (item_id, shopping_list_id, quantity) VALUES(?, ?, ?)';
    // Run the SQL (note: must use named callback to get properties of the resulting Statement)
...
function updateItem(listId, itemId, quantity, pickedUp) {
  return new Promise((resolve, reject) => {
    ...
    const sql =
      'UPDATE shopping_list_item SET quantity = ?, picked_up = ? WHERE shopping_list_id = ? AND item_id = ?'; // eslint-disable-line max-len
...
function removeItem(listId, itemId) {
  return new Promise((resolve, reject) => {
    const sql =
      'DELETE FROM shopping_list_item WHERE shopping_list_id = ? AND item_id = ?';
