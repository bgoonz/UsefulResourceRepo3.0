...
function renderViewPage(req, res, next) {
  ...
  async.parallel(
    ...
    function (err, results) {
      ...
      let whenCreated = dateformat(
        new Date(results.list.when_created),
        'mm-dd-yyyy hh:MM:ss'
      );
      let whenModified = results.list.whenModified
        ? dateformat(
            new Date(results.list.when_modified),
            'mm-dd-yyyy hh:MM:ss'
          )
        : null;
      // Now render the page
...
function renderUpdatePage(req, res, next) {
  ...
  request('GET', '/rest/lists/' + listId, null, (err, data) => {
    if (err) {
      ...
      let jsonData = JSON.parse(data);
      let whenCreated = dateformat(
        new Date(jsonData.when_created),
        'mm-dd-yyyy hh:MM:ss'
      );
      let whenModified = jsonData.whenModified
        ? dateformat(new Date(jsonData.when_modified), 'mm-dd-yyyy hh:MM:ss')
        : null;
      // Now render the page
...
const addListItems = [
  (req, res, next) => {
    ...
    if (Array.isArray(itemIds)) {
      ...
      itemsToAdd.push(Number.parseInt(itemIds));
    }
    ...
    if (itemsToAdd.length > 0) {
      ...
      logger.warn(
        'Search produced no results, or nothing selected.',
        'addListItems[]'
      );
      res.redirect('/lists/' + listId);
    }
...
const removeItems = [
  (req, res, next) => {
    ...
    if (Array.isArray(itemIds)) {
      ...
      itemsToDelete.push(Number.parseInt(itemIds));
    }
    //itemIds.push(req.body.selected);
    if (itemsToDelete.length > 0) {
      ...
      logger.warn(
        `Nothing selected, returning to /lists/${listId}.`,
        'removeItems[]'
      );
      res.redirect('/lists/' + listId);
    }
