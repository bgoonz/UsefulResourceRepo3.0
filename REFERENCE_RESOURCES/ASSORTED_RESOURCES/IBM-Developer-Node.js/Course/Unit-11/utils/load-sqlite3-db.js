  ...
  return new Promise((resolve, reject) => {
    return new Promise((resolve, reject) => {
      ...
      .then((shoppingListSql) => {
        ...
        logger.info(
          'Creating shopping_list table, done.',
          'createDbFixtures()'
        );
      ...
      .then((shoppingListItemSql) => {
        logger.info(
          'Creating shopping_list_item table...',
          'createDbFixtures()'
        );
        db.run(shoppingListItemSql);
        logger.info(
          'Creating shopping_list_item table, done.',
          'createDbFixtures()'
        );
...
function handleBrandRowForSqlDb(fields) {
  ...
  // Insert the row
  db.run(
    'INSERT INTO brand (description, manufacturer, location, website) VALUES (?, ?, ?, ?)',
    description,
    manufacturer,
    address,
    website,
    (err) => {
      if (err) {
        logger.error(
          'Error occurred while inserting record: description = ' +
            description +
            ', manufacturer = ' +
            manufacturer +
            ', address = ' +
...
function handleItemRowForSqlDb(fields) {
  ...
  // Insert the row
  db.run(
    'INSERT INTO item (upc, description, brand_id) VALUES (?, ?, (SELECT id FROM brand WHERE description = ?))',
    upc,
    itemDescription,
    brandDescription,
    (err) => {
      if (err) {
        logger.error(
          'Error occurred while inserting this record: upc = ' +
            upc +
            ', brand = ' +
            brandDescription +
            ', item = ' +
...
returnPromise
  .then(() => {
    logger.info(
      'Loading data for brand...',
      'mainline:createDbFixtures(resolved Promise)'
    );
    loadData(appSettings.brand_file_name, handleBrandRowForSqlDb).then(() => {
      logger.info(
        'Loading brand data, done.',
        'mainline:createDbFixtures(resolved Promise)'
      );
      logger.info(
  ...
  .catch((err) => {
    logger.error(
      'Better luck next time: ' + err.message,
      'mainline():createDbFixtures(rejected Promise)'
    );
  });
