...
let chunk2 =
  'ar|1|South Beach Beverage Co., LLC|Purchase, NY 10577|www.nofear.com\n9081|Golden Class';
let chunk3 =
...
let cornerCaseChunk2 =
  '9079|Slow-mag|1|Purdue Products L.P.||\n9080|No Fear|1|';
let cornerCaseChunk3 =
  'South Beach Beverage Co., LLC|Purchase, NY 10577|www.nofear.com\n9081|Golden Classic';
...
function testChunkTransformerOptimistic() {
  ...
  assert.deepEqual(transformedData.fieldsArray[0], [
    '9077',
    'No Jet Lag',
    '1',
    '',
  ...
  assert.deepEqual(transformedData.fieldsArray[1], [
    '9078',
    'Citra Drain',
    '1',
    '',
  ...
  assert.deepEqual(transformedData.fieldsArray[2], [
    '9079',
    'Slow-mag',
    '1',
    'Purdue Products L.P.',
  ...
  transformedData = utils.transformChunkIntoLines(
    transformedData.leftOvers + chunk2
  );
  ...
  assert.deepEqual(transformedData.fieldsArray[1], [
    ...
    '1',
    'Post Foods, LLC',
    '1 Upper Pond Road Parsippany, NJ 07054 USA',
    'postfoods.com',
  ]);
...
function testChunkTransformerCornerCase() {
  ...
  transformedData = utils.transformChunkIntoLines(
    transformedData.leftOvers + cornerCaseChunk2
  );
  assert.equal(transformedData.fieldsArray.length, 1);
  assert.deepEqual(transformedData.fieldsArray[0], [
    '9079',
  ...
  transformedData = utils.transformChunkIntoLines(
    transformedData.leftOvers + cornerCaseChunk3
  );
  assert.equal(transformedData.fieldsArray.length, 1);
  assert.deepEqual(transformedData.fieldsArray[0], [
    '9080',
...
function testUrlParserOptimistic() {
  ...
  testUrlComponents(utils.parseUrl(url1), 'http:', 'example.com', null, [
    'foo',
  ]);
  testUrlComponents(utils.parseUrl(url2), 'http:', 'example.com', '8080', [
    'foo',
  ]);
  testUrlComponents(utils.parseUrl(url3), 'http:', 'example.com', '8080', [
    'foo',
    '123',
  ]);
  testUrlComponents(utils.parseUrl(url4), 'http:', 'example.com', '8080', [
    'foo',
    '123',
    'bar',
  ]);
  testUrlComponents(utils.parseUrl(url5), 'http:', 'example.com', '8080', [
    'foo',
    '123',
    'bar',
    '567',
  ]);
  testUrlComponents(utils.parseUrl(url6), 'http:', 'example.com', '8080', [
    'foo',
    '123',
    'bar',
    '567',
