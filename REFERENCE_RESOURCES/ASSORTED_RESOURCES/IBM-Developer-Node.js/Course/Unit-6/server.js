...
http
  .createServer((request, response) => {
    ...
    if (parsedUrl.pathname.startsWith('/items')) {
      logger.debug(
        `Greetings from pathname: ${parsedUrl.pathname}`,
        'http.createServer:on(request)'
      );
      // Route /items URL
      routes
        .routeItemsRequest(request, parsedUrl)
        .then((results) => {
          utils.writeServerJsonResponse(
            response,
            results.data,
            results.statusCode
          );
      ...
      // Route /lists URL
      routes
        .routeListsRequest(request, parsedUrl)
        .then((results) => {
          utils.writeServerJsonResponse(
            response,
            results.data,
