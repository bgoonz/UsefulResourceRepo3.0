...
function handleListsCreate(request, resolve, reject) {
  utils
    ...
    .then((requestBody) => {
      logger.debug(
        `Calling listsDao.create() with request: ${requestBody}`,
        'handleListsCreate()'
      );
      let requestBodyJson = JSON.parse(requestBody);
      listsDao
        .create(requestBodyJson.description)
        .then((result) => {
          resolve(result);
        })
        .catch((err) => {
 ...
 */
function handleListsFindByIdWithAllItems(
  request,
  resolve,
  reject,
  id,
  ...
  if (secondaryResource == 'items') {
    logger.debug(
      ...
      'handleListsFindByIdWithAllItems()'
    );
    listsDao
      .findByIdWithAllItems(id)
      .then((result) => {
        resolve(result);
      })
      .catch((err) => {
...
function handleListsAddItem(request, resolve, reject, id, secondaryResource) {
  ...
  if (secondaryResource == 'items') {
    ...
    utils
      ...
      .then((requestBody) => {
        logger.debug(
          `Calling listsDao.addItem() with request: ${requestBody}`,
          'handleListsUpdate()'
        );
        let requestBodyJson = JSON.parse(requestBody);
        listsDao
          .addItem(id, requestBodyJson.itemId, requestBodyJson.quantity)
          .then((result) => {
            resolve(result);
          })
          .catch((err) => {
 ...
 * listsDao.updateItem()
 */
function handleListsWithItemsUpdate(
  request,
  resolve,
  reject,
  id,
 ...
 */
function handleListsWithItemsDelete(
  request,
  resolve,
  reject,
  id,
...
module.exports.handleListsFindByIdWithAllItems =
  handleListsFindByIdWithAllItems;
