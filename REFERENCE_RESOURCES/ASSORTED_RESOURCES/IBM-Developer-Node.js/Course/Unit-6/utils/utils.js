  ...
  return new Promise((resolve, reject) => {
    ...
    request
      ...
      .on('end', () => {
        ...
        logger.debug(
          `Request body processed. Length: ${body.length}.`,
          'processRequestBody()'
        );
        resolve(body);
      })
      .on('error', (err) => {
        logger.error(
          `Error occurred while processing request body, message: ${err.message}`,
          'processRequestBody()'
        );
        reject(err.message);
      });
...
function db_init() {
  let db = new sqlite3.Database(appSettings.db_file_name, (err) => {
    if (err) {
      logger.error(
        `Error occurred while opening database file: ${appSettings.db_file_name}: ${err.message}`,
        'init()'
      );
    } else {
      ...
      process.on('exit', (code) => {
        logger.info(
          `CLOSING Database ${appSettings.db_file_name}, exit code: ${code}`
        );
        db.close((err) => {
          logger.error(
            `Error closing DB with message: ${err.message}: and code ${code}`
          );
        });
      });
    }
