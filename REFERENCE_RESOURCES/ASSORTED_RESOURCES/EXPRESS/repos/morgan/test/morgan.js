    ...
    describe(':date', function () {
      it('should get current date in "web" format by default', function (done) {
        var cb = after(2, function (err, res, line) {
          if (err) return done(err);
          assert.ok(
            /^\w{3}, \d{2} \w{3} \d{4} \d{2}:\d{2}:\d{2} GMT$/.test(line)
          );
          done();
        });
      ...
      it('should get current date in "clf" format', function (done) {
        var cb = after(2, function (err, res, line) {
          if (err) return done(err);
          assert.ok(
            /^\d{2}\/\w{3}\/\d{4}:\d{2}:\d{2}:\d{2} \+0000$/.test(line)
          );
          done();
        });
  ...
  describe('formats', function () {
    ...
    describe('a string', function () {
      ...
      it('should accept special characters', function (done) {
        ...
        request(
          createServer('LOCAL\\:remote-user ":method :url" :status', {
            stream: stream,
          })
        )
          .get('/')
      ...
      it('should match expectations', function (done) {
        var cb = after(2, function (err, res, line) {
          ...
          assert.strictEqual(
            masked,
            res.text +
              ' - tj [_timestamp_] "GET / HTTP/1.1" 200 - "http://localhost/" "my-ua"'
          );
          done();
        });
    ...
    describe('common', function () {
      it('should match expectations', function (done) {
        var cb = after(2, function (err, res, line) {
          ...
          assert.strictEqual(
            masked,
            res.text + ' - tj [_timestamp_] "GET / HTTP/1.1" 200 -'
          );
          done();
        });
