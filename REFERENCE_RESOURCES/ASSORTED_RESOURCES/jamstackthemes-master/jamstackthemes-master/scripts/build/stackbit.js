...
const generateStackbit = (frontmatter) => {
  ...
  if (
    ssgArray.some((ssg) => allowedSsg.includes(urlSlug(ssg))) &&
    cmsArray.some((cms) => allowedCms.includes(urlSlug(cms)))
  ) {
    if (ssgArray.length > 1) {
      stackbitData.createUrl = `https://app.stackbit.com/create?theme=${frontmatter.github}`;
    } else {
      stackbitData.createUrl = `https://app.stackbit.com/create?theme=${
        frontmatter.github
      }&ssg=${urlSlug(ssgArray)}`;
    }
  }
  ...
  manualEnabled.forEach((url) => {
    if (url === frontmatter.github) {
      stackbitData.createUrl = `https://app.stackbit.com/create?theme=${
        frontmatter.github
      }&ssg=${urlSlug(frontmatter.ssg)}&cms=${urlSlug(frontmatter.cms)}`;
    }
  });
  if (stackbitData.createUrl) {
    return stackbitData;
...
const generateStackbitData = (markdownData) => {
  ...
  const stackbitData = markdownData
    .map((theme) => {
