...
const updateFrontmatter = (file, update = {}) => {
  ...
  const fileData = fs.existsSync(absFilepath)
...
const loadThemeFrontMatter = (absFilename) => {
  ...
  if (!frontmatter.github) {
    ...
    if (file === '.DS_Store') {
      ...
      throw new Error(
        `${absFilename} invalid file. This file was deleted, try again...`
      );
    }
...
const generateMarkdownData = async (markdownFiles, options = {}) => {
  ...
  const markdownData = markdownFiles
    .map((absFilename) => loadThemeFrontMatter(absFilename))
    .filter((frontmatter) => {
      if (options.file) {
  ...
  if (options.file) {
    ...
    console.log(
      `Processing ${markdownData.length}/${markdownFiles.length} themes...`
    );
  }
