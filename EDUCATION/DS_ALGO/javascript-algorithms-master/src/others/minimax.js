((exports) => {
   ...
   */
  function minimaxBuilder(getPossibleNextStatesFn, isGameOverFn, getScoreFn) {
    /**
    ...
    const minimax = (state, maximize, depth, alpha, beta) => {
      ...
      if (maximize) {
        ...
        for (const next of possibleMoveResults) {
          const result = minimax(next.state, false, depth - 1, alpha, beta);
        ...
        for (const next of possibleMoveResults) {
          const result = minimax(next.state, true, depth - 1, alpha, beta);
