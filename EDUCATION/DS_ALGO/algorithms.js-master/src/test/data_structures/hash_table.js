...
describe('Hash Table', () => {
  ...
  it('initializes the table with the given capacity', () => {
    let h = new HashTable();
  ...
  it('allows putting and getting elements from the table', () => {
  ...
  it('replaces items if the same key is reused', () => {
    const h = new HashTable(16);
    const a = { a: 'foo', b: 'bar' };
    h.put('foo', a);
    assert.strictEqual(h.get('foo'), a);
  ...
  it('increases capacity when needed', () => {
  ...
  it('performs a function to all keys with forEach', () => {
    const h = new HashTable();
    h.put(1, 10);
    h.put(2, 20);
    h.put(3, 30);
