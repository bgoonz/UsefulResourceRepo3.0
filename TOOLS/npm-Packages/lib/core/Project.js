          ...
          if (that._options.saveExact || that._config.saveExact) {
            if (decEndpoint.name !== decEndpoint.source) {
              jsonEndpoint[decEndpoint.name] =
                decEndpoint.source + "#" + decEndpoint.pkgMeta.version;
            } else {
      ...
      .spread(function (json, tree, flattened) {
        ...
        names.forEach(function (name) {
          ...
          promise = promise.then(function () {
            ...
            message =
              dependantsNames.join(", ") + " depends on " + decEndpoint.name;
            data = {
        ...
        return validLink(dir).spread(function (valid, err) {
          ...
          return readJson(dir, {
            ...
            if (deprecated) {
              that._logger.warn(
                "deprecated",
                "Package " + name + " is using the deprecated " + deprecated
              );
            }
...
Project.prototype._restoreNode = function (
  node,
  flattened,
  jsonKey,
  processed
