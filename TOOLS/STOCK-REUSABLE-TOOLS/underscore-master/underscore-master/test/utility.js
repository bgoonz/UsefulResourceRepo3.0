(function () {
  ...
  QUnit.test("toPath", function (assert) {
    ...
    assert.deepEqual(
      _.toPath(key),
      path,
      "bare strings are wrapped in a single-element array"
    );
  ...
  QUnit.test(
    ...
    function (assert) {
      ...
      assert.strictEqual(
        _.result({ a: 1 }, "b", 2),
        2,
        "uses the fallback value when property is missing"
      );
      assert.strictEqual(
        _.result({ a: 1 }, ["b", "c"], 2),
        2,
        "uses the fallback value when any property is missing"
      );
      assert.strictEqual(
        _.result({ a: void 0 }, ["a"], 1),
        1,
        "uses the fallback when value is undefined"
      );
      assert.strictEqual(
        _.result({ a: false }, ["a"], "foo"),
        false,
      ...
      assert.strictEqual(
        _.result({ a: func }, "a"),
        "f",
        "can get a direct method"
      );
      assert.strictEqual(
        _.result({ a: { b: func } }, ["a", "b"]),
        "f",
        "can get a nested method"
      );
      assert.strictEqual(
        _.result(),
        void 0,
        "returns undefined if obj is not passed"
      );
      assert.strictEqual(
        _.result(void 1, "a", 2),
        2,
        "returns default if obj is not passed"
      );
      assert.strictEqual(
        _.result(void 1, "a", func),
        "f",
        "executes default if obj is not passed"
      );
      assert.strictEqual(
        _.result({}, void 0, 2),
        2,
        "returns default if prop is not passed"
      );
      assert.strictEqual(
        _.result({}, void 0, func),
        "f",
        "executes default if prop is not passed"
      );
      ...
      assert.strictEqual(
        _.result(obj, "a"),
        obj,
        "uses the parent object as context"
      );
      assert.strictEqual(
        _.result(obj, "e", context),
        obj,
        "uses the object as context when executing the fallback"
      );
      assert.strictEqual(
        _.result(obj, ["a", "x"], context),
        obj,
        "uses the object as context when executing the fallback"
      );
      assert.strictEqual(
        _.result(obj, ["b", "c"]),
        childObj,
        "uses the parent as context when accessing deep methods"
      );

      assert.strictEqual(
        _.result({}, [], "a"),
        "a",
        "returns the default when prop is empty"
      );
      assert.strictEqual(
        _.result(obj, [], context),
        obj,
        "uses the object as context when path is empty"
      );
      ...
      assert.strictEqual(
        _.result(nested, ["d", "e"]),
        obj,
        "can unpack nested function calls"
      );
      assert.strictEqual(
        _.result(nested, ["d", "f"]).e(),
        obj,
        "uses parent as context for nested function calls"
      );
      assert.strictEqual(
        _.result(nested, ["d", "x"], context).e(),
        obj,
        "uses last valid child as context for fallback"
      );
      ...
      if (typeof Symbol !== "undefined") {
        ...
        assert.strictEqual(
          _.result(symbolObject, [y, x]),
          "bar",
          "can use symbols as keys for deep matching"
        );
      }
    }
  );
  ...
  QUnit.test(
    "#2911 - _.template must not trigger CVE-2021-23337.",
    function (assert) {
