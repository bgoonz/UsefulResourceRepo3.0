(function (exports, global) {
  ...
  AST_Toplevel.DEFMETHOD("figure_out_scope", function (options) {
    ...
    var tw = new TreeWalker(function (node, descend) {
      ...
      if (node instanceof AST_SymbolLambda) {
        ...
        def.constant =
          node instanceof AST_SymbolConst || last_var_had_const_pragma;
  ...
  AST_Var.DEFMETHOD("has_const_pragma", function () {
    ...
    var lastComment =
      comments_before && comments_before[comments_before.length - 1];
  ...
  (function () {
      ...
      function collapse_single_use_vars(statements, compressor) {
        ...
        for (var stat_index = statements.length; --stat_index >= 0; ) {
          ...
          [
            ...
            node &&
              node.body &&
              collapse_single_use_vars(node.body, compressor);
          });
          ...
          for (var var_defs_index = var_defs.length; --var_defs_index >= 0; ) {
            ...
            if (var_decl.value.is_constant(compressor)) {
              var ctt = new TreeTransformer(function (node) {
                if (node === ref) return replace_var(node, ctt.parent(), true);
              });
              stat.transform(ctt);
              continue;
            }
            ...
            if (ref.scope !== self) {
              side_effects_encountered |=
                var_decl.value.has_side_effects(compressor);
              continue;
            }
            var tw = new TreeWalker(function (node) {
              if (
                node instanceof AST_SymbolRef &&
                is_lvalue(node, tw.parent())
              ) {
        ...
        if (var_defs_removed)
          for (var i = statements.length; --i >= 0; ) {
            if (
              statements.length > 1 &&
              statements[i] instanceof AST_EmptyStatement
            )
          ...
          if (ll.length > 1) {
            if (ll[1]) {
              ...
              return maintain_this_binding(
                compressor.parent(),
                self,
                self.right.evaluate(compressor)[0]
              );
            } else {
              compressor.warn(
    ...
    OPT(AST_Conditional, function (self, compressor) {
      ...
      if (
        consequent.is_constant(compressor) &&
        ...
        if (self.condition.has_side_effects(compressor)) {
          return AST_Seq.from_array([
            self.condition,
            make_node_from_constant(compressor, consequent_value, self),
          ]);
        } else {
