$(function () {
  ...
  QUnit.test("should expose defaults var for settings", function (assert) {
    assert.expect(1);
    assert.ok(
      $.fn.bootstrapModal.Constructor.DEFAULTS,
  ...
  QUnit.test(
    "should add padding-right of scrollbar width to .navbar-fixed-top and .navbar-fixed-bottom before open",
    function (assert) {
      assert.expect(2);
      ...
      var $fixedTop = $('<nav class="navbar navbar-fixed-top" />').appendTo(
        $body
      );
      var $fixedBottom = $(
      ...
      $('<div id="modal-test"/>')
        ...
        .on("shown.bs.modal", function () {
          ...
          assert.strictEqual(
            scrollbarWidth,
            fixedTopPaddingRight,
            ".navbar-fixed-top has padding-right (" +
              fixedTopPaddingRight +
              ") equal to scrollbar width (" +
              scrollbarWidth +
              ")"
          );
          ...
          assert.strictEqual(
            scrollbarWidth,
            fixedBottomPaddingRight,
            ".navbar-fixed-bottom has padding-right (" +
              fixedBottomPaddingRight +
              ") equal to scrollbar width (" +
              scrollbarWidth +
              ")"
  ...
  QUnit.test(
    "should restore inline padding-right for .navbar-fixed-top and .navbar-fixed-bottom after close",
    function (assert) {
      assert.expect(2);
      var done = assert.async();
      var $body = $(document.body);
      ...
      var $fixedTop = $(
        '<nav class="navbar navbar-fixed-top" style="padding-right: 10px;" />'
      ).appendTo($body);
      var $fixedBottom = $(
        '<nav class="navbar navbar-fixed-bottom" style="padding-right: 5%;" />'
      ).appendTo($body);
      ...
      $('<div id="modal-test"/>')
        .on("hidden.bs.modal", function () {
          assert.strictEqual(
            $fixedTop.css("padding-right"),
            "10px",
            ".navbar-fixed-top has inline padding-right restored"
          );

          assert.strictEqual(
            $fixedBottom[0].style.paddingRight,
            "5%",
