...
var import_dropbox = function (req, res) {
  ...
  Dropbox.searchForMdFiles(
    req.session.dropbox.oauthtoken,
    ...
    function (err, data) {
      ...
      if (err.status === 401)
        return res
          .status(401)
          .send(
            'You are not authenticated with Dropbox. Please unlink and link again.'
          )
      if (err.status === 400)
        return res
          .status(400)
          .send('Bad request to Dropbox. Please unlink and link again.')
      if (err.status > 399)
        return res.status(status).send('Something went wrong. Please refresh.')
    }
  )
}
