    ...
    Q.nfcall(mkdirp, dstDir)
        ...
        return Q.nfcall(fs.stat, src).fail(function (error) {
          if (error.code === "ENOENT") {
            throw createError(
              ...
              {
                details:
                  src + " does not exist or points to a non-existent file",
              }
            );
          }
      ...
      .then(function (stat) {
        ...
        return Q.nfcall(fs.symlink, src, dst, type).fail(function (err) {
          ...
          return Q.nfcall(fs.symlink, src, dst, "junction").fail(function (
            err
          ) {
            throw createError(
              "Unable to create link to " + path.basename(src),
              err.code,
              {
                details:
                  err.message.trim() +
