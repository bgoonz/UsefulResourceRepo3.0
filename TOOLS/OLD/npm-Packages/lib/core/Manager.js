      ...
      mout.object.forOwn(
        ...
        function (pkg) {
          ...
          mout.object.forOwn(
            ...
            function (dependency, name) {
              var dissected =
                ...
                (this._resolved[name] ? this._resolved[name][0] : dependency);
          ...
          pkg.dependants = pkg.dependants.map(function (dependant) {
            ...
            var dissected =
              this._dissected[name] ||
              (this._resolved[name] ? this._resolved[name][0] : dependant);
      ...
      function () {
        ...
        mout.object.forOwn(
          ...
          function (resolution, name) {
            ...
            this._logger.warn(
              "extra-resolution",
              "Unnecessary resolution: " + name + "#" + resolution,
              {
                name: name,
...
Manager.prototype._electSuitable = function (name, semvers, nonSemvers) {
  ...
  if (resolution && !unresolvable) {
    ...
    if (suitable === -1) {
      suitable = mout.array.findIndex(picks, function (pick) {
        return (
          pick.target === resolution || pick.pkgMeta._release === resolution
        );
      });
    }
    ...
    if (suitable === -1) {
      this._logger.warn(
        "resolution",
        "Unsuitable resolution declared for " + name + ": " + resolution,
        {
          name: name,
          picks: dataPicks,
          resolution: resolution,
