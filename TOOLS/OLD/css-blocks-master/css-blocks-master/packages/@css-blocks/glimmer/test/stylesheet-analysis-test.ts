...
describe("Stylesheet analysis", function () {
        ...
        let expected: ElementsAnalysis = {
          a: {
            ...
            sourceLocation: {
              start: {
                line: 1,
                filename: "template:/styled-app/components/my-app",
              },
              end: {
                line: 1,
                filename: "template:/styled-app/components/my-app",
  ...
  it("analyzes styles from a referenced block", function () {
    ...
    return analyzer
      .analyze("with-multiple-blocks")
      .then((analyzer: GlimmerAnalyzer) => {
        let analysis = analyzer.getAnalysis(0).serialize();
        assert.equal(
          analysis.template.identifier,
          "template:/styled-app/components/with-multiple-blocks"
        );
        assert.deepEqual(analysis.blocks, {
          "": "glimmer:stylesheet:/styled-app/components/with-multiple-blocks",
          h: fixture(
            "styled-app/src/ui/components/with-multiple-blocks/header.css"
          ),
        });
  ...
  it("analyzes styles from a referenced block with dynamic state", function () {
    ...
    let analyzer = new GlimmerAnalyzer(projectDir);
    return analyzer
      .analyze("with-dynamic-states")
      .then((analyzer: GlimmerAnalyzer) => {
        let analysis = analyzer.getAnalysis(0).serialize();
        assert.equal(
          analysis.template.identifier,
          "template:/styled-app/components/with-dynamic-states"
        );
        assert.deepEqual(analysis.blocks, {
          "": "glimmer:stylesheet:/styled-app/components/with-dynamic-states",
          h: fixture(
            "styled-app/src/ui/components/with-dynamic-states/header.css"
          ),
        });
        assert.deepEqual(analysis.stylesFound, [
          ".world",
        ...
        assert.deepEqual(analysis.elements, {
          a: {
            ...
            sourceLocation: {
              start: {
                line: 1,
                filename: "template:/styled-app/components/with-dynamic-states",
              },
              end: {
                line: 1,
                filename: "template:/styled-app/components/with-dynamic-states",
  ...
  it("analyzes styles from a referenced block with dynamic classes", function () {
    ...
    let analyzer = new GlimmerAnalyzer(projectDir);
    return analyzer
      .analyze("with-dynamic-classes")
      .then((analyzer) => {
        let analysis = analyzer.getAnalysis(0).serialize();
        assert.equal(
          analysis.template.identifier,
          "template:/styled-app/components/with-dynamic-classes"
        );
        assert.deepEqual(analysis.blocks, {
          "": "glimmer:stylesheet:/styled-app/components/with-dynamic-classes",
          h: fixture(
            "styled-app/src/ui/components/with-dynamic-classes/header.css"
          ),
          t: fixture(
            "styled-app/src/ui/components/with-dynamic-classes/typography.css"
          ),
        });
        assert.deepEqual(analysis.stylesFound, [
          ".world",
        ...
        assert.deepEqual(analysis.elements, {
          ...
          d: {
            ...
            staticStyles: [],
            dynamicClasses: [
              { condition: true, whenTrue: [0], whenFalse: [3] },
            ],
            dynamicAttributes: [],
            sourceLocation: {
              start: {
          ...
          e: {
            ...
            staticStyles: [],
            dynamicClasses: [
              { condition: true, whenTrue: [3], whenFalse: [0] },
            ],
            dynamicAttributes: [],
            sourceLocation: {
              start: {
