...
function getParsedSelectors(
  ...
  try {
    ...
    throw new errors.InvalidBlockSyntax(e.message, sourceLocation(file, rule));
  }
  return res;
}

export async function constructBlock(
  root: postcss.Root,
  block: Block,
  file: string
): Promise<Block> {
  ...
  root.walkRules((rule) => {
    ...
    if (!shouldBeParsedAsBlockSelector(rule)) {
      return;
    }
      ...
      while (sel) {
        ...
        for (let node of sel.nodes) {
          if (isRootNode(node)) {
            ...
            foundStyles.push(
              blockClass!.ensureAttributeValue(toAttrToken(node))
            );
          }
        }

        // If we haven't found any terminating states, we're targeting the discovered Block class.
        if (blockClass && !foundStyles.length) {
          foundStyles.push(blockClass);
        }
...
function assertValidSelector(
  ...
  while (currentCompoundSel.next) {
    ...
    if (
      isClassLevelObject(currentObject) &&
      nextLevelIsRoot &&
      SIBLING_COMBINATORS.has(combinator)
    ) {
      throw new errors.InvalidBlockSyntax(
        `A class is never a sibling of a ${blockTypeName(
          nextObject.blockType
        )}: ${rule.selector}`,
    ...
    if (foundClassLevel && nextLevelIsRoot) {
      throw new errors.InvalidBlockSyntax(
        `Illegal scoping of a ${blockTypeName(currentObject.blockType)}: ${
          rule.selector
        }`,
        loc(file, rule, currentCompoundSel.next.combinator)
      );
    ...
    if (
      foundRootLevel &&
      nextLevelIsRoot &&
      foundCombinators.some((c) => HIERARCHICAL_COMBINATORS.has(c))
    ) {
      // unless it's only referencing the same object.
      if (
        !foundObjects.every(
          (f) => f.node.toString() === nextObject.node.toString()
        )
      ) {
        throw new errors.InvalidBlockSyntax(
          `Illegal scoping of a ${blockTypeName(currentObject.blockType)}: ${
    ...
    if (
      isRootLevelObject(currentObject) &&
      nextLevelIsClass &&
      SIBLING_COMBINATORS.has(combinator)
    ) {
      throw new errors.InvalidBlockSyntax(
        `A ${blockTypeName(
          nextObject.blockType
        )} cannot be a sibling with a ${blockTypeName(
          currentObject.blockType
        )}: ${rule.selector}`,
        loc(file, rule, currentCompoundSel.next.combinator)
      );
    }
    ...
    if (isClassLevelObject(nextObject)) {
      let conflictObj = foundObjects.find(
        (obj) =>
          isClassLevelObject(obj) &&
          obj.node.toString() !== nextObject.node.toString()
      );
      if (conflictObj) {
        // slightly better error verbiage for objects of the same type.
        if (conflictObj.blockType === nextObject.blockType) {
          throw new errors.InvalidBlockSyntax(
            `Distinct ${blockTypeName(conflictObj.blockType, {
              plural: true,
            })} cannot be combined: ${rule.selector}`,
            loc(file, rule, nextObject.node)
          );
        } else {
          throw new errors.InvalidBlockSyntax(
            `Cannot combine a ${blockTypeName(
              conflictObj.blockType
            )} with a ${blockTypeName(nextObject.blockType)}}: ${
              rule.selector
            }`,
            loc(file, rule, nextObject.node)
          );
...
function assertBlockObject(
  ...
  let nonStateAttribute = sel.nodes.find(
    (n) => selectorParser.isAttribute(n) && !isAttributeNode(n)
  );
  ...
  let result = sel.nodes.reduce<NodeAndType | null>((found, n) => {
    ...
    if (isRootNode(n)) {
      if (found === null) {
        ...
        if (
          found.blockType === BlockType.class ||
          found.blockType === BlockType.classAttribute
        ) {
          throw new errors.InvalidBlockSyntax(
    ...
    // and throw the appropriate error if conflicting selectors are found.
      ...
      if (!found) {
        ...
        found.blockType === BlockType.block ||
        found.blockType === BlockType.root ||
        found.blockType === BlockType.attribute
      ) {
  ...
  if (!result) {
    throw new errors.InvalidBlockSyntax(
      `Missing block object in selector component '${sel.nodes.join("")}': ${
        rule.selector
      }`,
      loc(file, rule, sel.nodes[0])
    );
  ...
  if (isExternalBlock(result)) {
    ...
    if (!external) {
      throw new errors.InvalidBlockSyntax(``, loc(file, rule, sel.nodes[0]));
    }
    let globalStates = external.rootClass
      .allAttributeValues()
      .filter((a) => a.isGlobal);
    ...
    throw new errors.InvalidBlockSyntax(
      `Missing global state selector on external Block '${
        result.node.value
      }'. Did you mean one of: ${globalStates
        .map((s) => s.asSource())
        .join(" ")}`,
      loc(file, rule, sel.nodes[0])
    );
