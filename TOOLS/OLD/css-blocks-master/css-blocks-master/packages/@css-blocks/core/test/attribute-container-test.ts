...
export class AttributeContainerTest extends BEMProcessor {
  ...
  @test "finds boolean attributes"() {
    ...
    return factory
      .getBlock(importer.identifier(null, filename, config))
      .then((block) => {
  ...
  @test "finds attribute groups"() {
    ...
    return factory
      .getBlock(importer.identifier(null, filename, config))
      .then((block) => {
        let sizeGroup: Array<AttrValue> =
          block.rootClass.getAttributeValues("[state|size]");
        assert.equal(sizeGroup.length, 2);
        assert.includeMembers(
          sizeGroup.map((s) => s.value),
          ["large", "small"]
        );
        let attrGroup: Array<AttrValue> = block.rootClass.getAttributeValues(
          "[state|size]",
          "large"
        );
        assert.equal(attrGroup.length, 1);
        assert.includeMembers(
          attrGroup.map((s) => s.value),
          ["large"]
        );
        let missingGroup: Array<AttrValue> =
          block.rootClass.getAttributeValues("[state|asdf]");
        assert.equal(missingGroup.length, 0);
        let noAttr: Array<AttrValue> = block.rootClass.getAttributeValues(
          "[state|size]",
          "tiny"
        );
        assert.equal(noAttr.length, 0);
        typedAssert.isNotNull(block.getClass("foo")).and((classObj) => {
          let modeGroup: Array<AttrValue> =
            classObj.getAttributeValues("[state|mode]");
          assert.equal(modeGroup.length, 3);
          assert.includeMembers(
            modeGroup.map((s) => s.value),
            ["collapsed", "minimized", "expanded"]
          );
        });
      });
  }
