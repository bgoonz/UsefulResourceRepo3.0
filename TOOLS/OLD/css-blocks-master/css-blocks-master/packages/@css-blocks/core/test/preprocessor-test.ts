...
export class PreprocessorTest {
  @test
  "raises an error if a file that isn't css is missing a preprocessor."() {
  ...
  @test "converts a file to css"() {
    let preprocessors: Preprocessors = {
      other: (
        ...
        _sourceMap?: RawSourceMap | string
      ) => {
  ...
  @test
  "processes css as a second pass when a css preprocessor is available."() {
    let preprocessors: Preprocessors = {
      other: (
        _fullPath: string,
        content: string,
  ...
  @skip
  @test
  "handles input sourcemaps in final output sourcemap"() {}
}
