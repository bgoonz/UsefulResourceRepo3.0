...
export class Test {
  @test "All blocks are discovered in multi-file app from entrypoint"() {
    let base = path.resolve(
      __dirname,
      "../../../test/fixtures/basic-multifile"
    );
    return parseFile("index.tsx", { baseDir: base }).then(
      (analyzer: Analyzer) => {
  ...
  @test "Duplicate blocks are only parsed once"() {
    let base = path.resolve(
      __dirname,
      "../../../test/fixtures/duplicate-blocks-multifile"
    );
    return parseFile("index.tsx", { baseDir: base }).then(
      (analyzer: Analyzer) => {
        assert.equal(analyzer.analysisCount(), 2);
  ...
  @test "Conflicting local import names don't interfere with each other"() {
    let base = path.resolve(
      __dirname,
      "../../../test/fixtures/conflicting-local-multifile"
    );
    return parseFile("index.tsx", { baseDir: base }).then(
      (analyzer: Analyzer) => {
        assert.equal(analyzer.analysisCount(), 2);
