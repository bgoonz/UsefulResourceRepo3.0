...
export class Test {
  ...
  @test "Unrecognized classes throw"() {
    mock({
      "bar.block.css":
        ":scope { color: red; } .foo { color: blue; } .bar { float: left; }",
    });
    ...
    return parse(`
      ...
      .catch((err: Error) => {
        assert.equal(
          err.message,
          '[css-blocks] MalformedBlockPath: No class named "baz" found on block "bar". Did you mean one of: :scope, .foo, .bar (4:47)'
        );
      });
  }
  ...
  @test "Throw when referencing non-existent sub-state"() {
    ...
    return parse(`
      ...
      .catch((err: Error) => {
        assert.equal(
          err.message,
          '[css-blocks] MalformedBlockPath: State ".foo[state|baz]" has no value "biz" on Block "bar".\n  Did you mean: .foo[state|baz]? (4:47)'
        );
      });
  }
}
