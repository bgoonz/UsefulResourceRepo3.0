...
describe("Router", function () {
	describe("#pathMatcher", function () {
		it("with string", function (done) {
			var fn = Router.pathMatcher(
				":protocol(https?://)?:www(www.)?youtube.com/(watch/:id)?"
			);
			assert.equal(typeof fn, "function");
			assert.ok(
				compareObjects(fn("youtube.com/"), {
					url: "youtube.com/",
					protocol: undefined,
					www: undefined,
					id: undefined,
			...
			assert.ok(
				compareObjects(fn("https://youtube.com/"), {
			...
			assert.ok(
				compareObjects(fn("https://www.youtube.com/"), {
			...
			assert.ok(
				compareObjects(fn("https://www.youtube.com/watch/mNhMogx3YmU"), {
			...
			assert.ok(
				compareObjects(
					Router.pathMatcher("*")("https://www.youtube.com/watch/mNhMogx3YmU"),
					{
						url: "https://www.youtube.com/watch/mNhMogx3YmU",
					}
				)
	...
	describe("on", function () {
		...
		it("with function", function (done) {
			r.on(Router.pathMatcher(LH + "/watch/:id"))
				.createStatic()
				.onStatusCode(200, function () {
					done();
				});
	...
	it("use", function (done) {
		var r = new Router(),
			stInstance;
		stInstance = scraper.StaticScraper.create().scrape(
			function ($) {
				return $(".title a")
					.map(function () {
						return $(this).text();
		...
		r.on(LH + "/hacker-news-clone").use(stInstance);
		r.route(LH + "/hacker-news-clone", function (found) {
			assert.ok(found);
		});
	});
