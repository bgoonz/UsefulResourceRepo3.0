(function () {
      ...
      _.each(
        ...
        function () {
          assert.strictEqual(
            typeof this,
            "object",
            "context is a wrapped primitive"
          );
          assert.strictEqual(
            this.valueOf(),
            context,
            "the unwrapped context is the specified primitive"
          );
          assert.equal(
            this,
            context,
            "context can be coerced to the specified primitive"
          );
        },
        context
      );
    ...
    function (assert) {
      ...
      _.each(collection.concat(array), function (method) {
        ...
        assert.strictEqual(
          count,
          3,
          method + " is resistant to length changes"
        );
      });
      ...
      _.each(collection.concat(object), function (method) {
        ...
        assert.strictEqual(
          count,
          2,
          method + " is resistant to property changes"
        );
      });
    }
  );
  ...
  QUnit.test("includes with NaN", function (assert) {
    ...
    assert.strictEqual(
      ...
      false,
      "Expected [1, 2, Infinity] to NOT contain NaN"
    );
  });
  ...
  QUnit.test("invoke", function (assert) {
    ...
    assert.deepEqual(
      _.invoke(arr, ["a", "b"]),
      ["foo"],
      "supports deep method access via an array syntax"
    );
    assert.deepEqual(
      _.invoke(arr, ["a", "c"]),
      [item.a],
      "executes deep methods on their direct parent"
    );
    assert.deepEqual(
      _.invoke(arr, ["a", "d", "z"]),
      [void 0],
      "does not try to access attributes of non-objects"
    );
    assert.deepEqual(
      _.invoke(arr, ["a", "d"]),
      [null],
      "handles deep null values"
    );
    assert.deepEqual(
      _.invoke(arr, ["e"]),
      ["foo"],
      "handles path arrays of length one"
    );
    assert.deepEqual(
      _.invoke(arr, ["f"]),
      [item],
      "correct uses parent context with shallow array syntax"
    );
    assert.deepEqual(
      _.invoke(arr, ["g", "h"]),
      [void 0],
      "does not execute intermediate functions"
    );
    ...
    assert.deepEqual(
      _.invoke(arr, "a"),
      ["foo", "bar"],
      "can handle different methods on subsequent objects"
    );
  });
  ...
  QUnit.test("groupBy", function (assert) {
    ...
    assert.deepEqual(
      _.groupBy(collection, ["stats", "power"]),
      expected,
      "can group by deep properties"
    );
    ...
    assert.deepEqual(
      _.groupBy(foos, "foo"),
      { "1,2": foos },
      "will coerce arrays to string keys"
    );
  });
