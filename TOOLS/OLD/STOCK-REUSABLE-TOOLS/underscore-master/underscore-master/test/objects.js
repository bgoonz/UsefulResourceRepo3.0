(function () {
  ...
  QUnit.test("extend", function (assert) {
    ...
    assert.strictEqual(
      _.extend({ x: "x" }, { a: "b" }).x,
      "x",
      "properties not in source don't get overridden"
    );
  ...
  QUnit.test("pick", function (assert) {
    ...
    assert.deepEqual(
      result,
      { a: 1, b: 2 },
      "can restrict properties to those named in mixed deep args"
    );
    result = _.pick(["a", "b"], 1);
  ...
  QUnit.test("omit", function (assert) {
    ...
    assert.deepEqual(
      result,
      { a: 1 },
      "can omit properties named in a nested array"
    );
    result = _.omit(["a", "b"], 0);
  ...
  QUnit.test("isEqual", function (assert) {
    ...
    if (typeof Symbol !== "undefined") {
      ...
      assert.strictEqual(
        _.isEqual(symbol, sameStringSymbol),
        false,
    ...
    if (typeof ArrayBuffer !== "undefined") {
      ...
      assert.ok(
        _.isEqual(u8.buffer, u8b.buffer),
        "Identical ArrayBuffers are equal"
      );
      assert.ok(
        _.isEqual(u8.buffer, i8.buffer),
        "Identical ArrayBuffers of different typed arrays are equal"
      );

      assert.notOk(
        _.isEqual({ a: 1, buffer: u8.buffer }, { a: 2, buffer: u8b.buffer }),
        "Unequal objects with similar buffer properties are not equals"
      );

      assert.notOk(
        _.isEqual(u8, i8),
        "Different types of typed arrays with the same byte data are not equal"
      );
      assert.notOk(
        _.isEqual(u8, u16),
        "Typed arrays with different types and different byte length are not equal"
      );
      assert.notOk(
        _.isEqual(u8, u16one),
        "Typed arrays with different types, same byte length but different byte data are not equal"
      );
      assert.notOk(
        _.isEqual(u8.buffer, u16.buffer),
        "Different ArrayBuffers with different length are not equal"
      );
      assert.notOk(
        _.isEqual(u8.buffer, u16one.buffer),
        "Different ArrayBuffers with different byte data are not equal"
      );
      ...
      assert.notOk(
        _.isEqual(view1, view2),
        "same buffer with different offset is not equal"
      );

      // Some older browsers support typed arrays but not DataView.
      if (typeof DataView !== "undefined") {
        assert.ok(
          _.isEqual(new DataView(u8.buffer), new DataView(u8b.buffer)),
          "Identical DataViews are equal"
        );
        assert.ok(
          _.isEqual(new DataView(u8.buffer), new DataView(i8.buffer)),
          "Identical DataViews of different typed arrays are equal"
        );
        assert.notOk(
          _.isEqual(new DataView(u8.buffer), new DataView(u16.buffer)),
          "Different DataViews with different length are not equal"
        );
        assert.notOk(
          _.isEqual(new DataView(u8.buffer), new DataView(u16one.buffer)),
          "Different DataViews with different byte data are not equal"
        );
      }
    }
  });
  ...
  if (typeof ArrayBuffer != "undefined") {
    ...
    QUnit.test("isTypedArray", function (assert) {
      ...
      _.each(typedArrayTypes, function (ctor) {
        assert.ok(
          _.isTypedArray(new ctor(buffer)),
          ctor.name + " is a typed array"
        );
      });
    });
  }
  ...
  QUnit.test("has", function (assert) {
    ...
    assert.ok(
      !_.has(obj, "baz"),
      "returns false if the object doesn't have the property."
    );
    assert.ok(_.has(obj, "func"), "works for functions too.");
    obj.hasOwnProperty = null;
    assert.ok(
      _.has(obj, "foo"),
      "works even when the hasOwnProperty method is deleted."
    );
    function Child() {}
    Child.prototype = obj;
    var child = new Child();
    assert.ok(
      !_.has(child, "foo"),
      "does not check the prototype chain for a property."
    );
    ...
    assert.ok(
      _.has({ a: { b: "foo" } }, ["a", "b"]),
      "can check for nested properties."
    );
    assert.ok(
      !_.has({ a: child }, ["a", "foo"]),
      "does not check the prototype of nested props."
    );
  });
  ...
  QUnit.test("get", function (assert) {
    ...
    assert.strictEqual(
      _.get("foo", null),
      void 0,
      "should return undefined for null object"
    );
    assert.strictEqual(_.get({ x: null }, "x"), null, "can fetch null values");
    assert.strictEqual(
      _.get(null, "length"),
      void 0,
      "does not crash on property access of non-objects"
    );
    assert.strictEqual(
      _.get(stooge, "size", 10),
      10,
      "allows a fallback value for undefined properties"
    );
    assert.strictEqual(
      _.get(stooge, "name", 10),
      "moe",
  ...
  QUnit.test("property", function (assert) {
    ...
    assert.strictEqual(
      _.property("x")({ x: null }),
      null,
      "can fetch null values"
    );
    assert.strictEqual(
      _.property("length")(null),
      void 0,
      "does not crash on property access of non-objects"
    );
    ...
    assert.strictEqual(
      _.property("a")({ a: 1 }),
      1,
      "can get a direct property"
    );
    assert.strictEqual(
      _.property(["a", "b"])({ a: { b: 2 } }),
      2,
      "can get a nested property"
    );
    assert.strictEqual(
      _.property(["a"])({ a: false }),
      false,
      "can fetch falsy values"
    );
    assert.strictEqual(
      _.property(["x", "y"])({ x: { y: null } }),
      null,
      "can fetch null values deeply"
    );
    assert.strictEqual(
      _.property(["x", "y"])({ x: null }),
      void 0,
      "does not crash on property access of nested non-objects"
    );
    assert.strictEqual(
  ...
  QUnit.test("propertyOf", function (assert) {
    ...
    var deepPropertyOf = _.propertyOf({
      curly: { number: 2 },
      joe: { number: null },
    });
    assert.strictEqual(
      deepPropertyOf(["curly", "number"]),
      2,
      "can fetch nested properties of obj"
    );
    assert.strictEqual(
      deepPropertyOf(["joe", "number"]),
      null,
      "can fetch nested null properties of obj"
    );
  });
  ...
  QUnit.test("isMatch", function (assert) {
    ...
    assert.strictEqual(
      _.isMatch(moe, { hair: true }),
      true,
      "Returns a boolean"
    );
    assert.strictEqual(
      _.isMatch(curly, { hair: true }),
      false,
      "Returns a boolean"
    );
    ...
    assert.deepEqual(
      ...
      "doesnt falsy match constructor on undefined/null"
    );
  });
  ...
  QUnit.test("matcher", function (assert) {
    ...
    assert.strictEqual(
      _.matcher({ hair: true })(moe),
      true,
      "Returns a boolean"
    );
    assert.strictEqual(
      _.matcher({ hair: true })(curly),
      false,
      "Returns a boolean"
    );
