(function () {
  ...
  QUnit.test("flatten", function (assert) {
    ...
    assert.deepEqual(
      _.flatten(list, 0),
      list,
      "can flatten arrays to depth of 0"
    );
    assert.deepEqual(
      _.flatten(list, -1),
      list,
      "can flatten arrays to depth of -1"
    );
    ...
    assert.strictEqual(
      _.flatten([new Array(10)]).length,
      10,
      "can flatten array with nulls of size n"
    );
    assert.strictEqual(
  ...
  QUnit.test("uniq", function (assert) {
    ...
    list = [-2, -1, 0, 1, 2];
    var notInjective = function (x) {
      return x * x;
    };
    assert.deepEqual(
      _.uniq(list, true, notInjective),
      [-2, -1, 0],
      "can find values of sorted array which map to unique values through a non one-to-one function by switching to slower algorithm even when isSorted=true"
    );
    ...
    assert.deepEqual(
      ...
      [{ 0: 1 }, { 0: 2 }],
      "can use falsy pluck like iterator"
    );
  ...
  QUnit.test("indexOf", function (assert) {
    ...
    assert.strictEqual(
      ...
      -1,
      "sorted indexOf doesn't use binary search"
    );
    assert.ok(
    ...
    assert.strictEqual(
      index,
      -1,
      "empty array with truthy `isSorted` returns -1"
    );
  });
  ...
  QUnit.test("lastIndexOf", function (assert) {
    ...
    assert.deepEqual(
      actual,
      expected,
      "should treat falsy `fromIndex` values, except `0` and `NaN`, as `array.length`"
    );
    assert.strictEqual(
