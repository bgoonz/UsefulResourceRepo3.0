...
assert(
  isNumber(meaningOfLife),
  false,
  "meaningOfLife is not a number datatype"
);
...
assert(
  isEmail("thedonald@makeamericagreatagain.com"),
  true,
  "isEmail detects an email"
);
assert(isEmail("3333@"), false, "isEmail does not give a false positive");
assert(
  isEmail("johnny.b.good"),
  false,
...
assert(
  numberCount,
  expectedNumberCount,
  "countIf can count the numbers in an array"
);
assert(
  stringCount,
  expectedStringCount,
  "countIf can count the strings in an array"
);
...
assert(
  emails.length,
  44,
  "filter and isEmail returns the correct number of emails"
);
...
assert(
  matchesArrayOfArrays,
  true,
  "the generated array of array of strings matches the expected array"
);
