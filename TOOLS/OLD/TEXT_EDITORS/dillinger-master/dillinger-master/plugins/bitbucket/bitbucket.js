...
var bitbucketConfigFile = path.resolve(
    __dirname,
    '../../configs/bitbucket/',
...
if (fs.existsSync(bitbucketConfigFile)) {
  ...
  isConfigEnabled = true
  console.log(
    'Bitbucket config found in environment. Plugin enabled. (Key: "' +
...
exports.Bitbucket = (function () {
  ...
  return {
    ...
    getUsername: function (req, res, cb) {
      var uri =
        bitbucketApi + 'user?access_token=' + req.session.bitbucket.oauth
          ...
          res.json({
            ...
            pagination: {
              ...
              last: {
                page:
                  d.size % d.pagelen
                    ? Math.floor(d.size / d.pagelen) + 1
                    : Math.floor(d.size / d.pagelen)
              },
              next: d.next,
              prev: d.previous
            }
        ...
        if (e) {
          ...
          function filterFiles() {
            ...
            files.forEach(function (file, index, files) {
              files[index].url = (uri.split('?')[0] + file.path).replace(
                '/src/',
                '/raw/'
              )
          ...
          if (directories.length === 0) {
            ...
            directories.forEach(function (dir, index, dirs) {
              ...
              if (dirs.length === index + 1) {
                Promise.all(everyFile).then(filterFiles())
              }
            })
          }
        } // end else if
