...
if (fs.existsSync(configFile)) {
  ...
  isConfigEnabled = true
  console.log(
    'OneDrive config found in environment. Plugin enabled. (Key: "' +
      config.client_id +
      '")'
  )
} else {
  ...
  console.warn(
    'OneDrive config not found at ' + configFile + '. Plugin disabled.'
  )
}
...
var OneDrive = {
  ...
  generateAuthUrl: function () {
    return encodeURI(
      util.format(
        'https://login.live.com/oauth20_authorize.srf?client_id=%s&' +
          'scope=%s&response_type=code&redirect_uri=%s',
        config.client_id,
        scopes.join(' '),
        config.redirect_uri
      )
