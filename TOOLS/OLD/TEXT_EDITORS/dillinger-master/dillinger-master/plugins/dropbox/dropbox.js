...
var dropbox_config_file = path.resolve(
    __dirname,
    '../../configs/dropbox/',
    'dropbox-config.json'
  ),
...
if (fs.existsSync(dropbox_config_file)) {
  ...
  console.warn(
    'Dropbox config not found at ' + dropbox_config_file + '. Plugin disabled.'
  )
}

exports.Dropbox = (function () {
  var dbx = new DropboxSDK({
    accessToken: dropbox_config.app_key,
  ...
  return {
    ...
    getAuthUrl: function (req, res, cb) {
      return cb(
        dbx.getAuthenticationUrl(dropbox_config.callback_url, null, 'code')
      )
    },
    // Get an OAuth2 access token from an OAuth2 Code.
    getRemoteAccessToken: function (code, cb) {
      dbx
        .getAccessTokenFromCode(dropbox_config.callback_url, code)
    ...
    saveFileToDropbox: function (req, res) {
      ...
      var pathToMdFile =
        req.body.pathToMdFile ||
        '/Dillinger/' + md.generateRandomMdFilename('md')
    ...
    saveImageToDropbox: function (req, res) {
      ...
      dbx
        ...
        .then(function () {
          return dbx
            .sharingCreateSharedLink({ path: pathToImage })
            .then(function (reply) {
