...
var Github = (function () {
  ...
  function _listOrgs(orgs) {
    var list =
      ...
      '<li data-org-name="' +
      githubUser +
      '" data-user="true"><a class="org" href="#">' +
      githubUser +
      '</a></li>'
    // Sort alpha
  ...
  function _listRepos(repos) {
    var list =
        '<li class="github_org"><a href="#">Back to organizations...</a></li>',
      pagination =
        '<ul class="repos pager"><li class="previous' +
        (!Github.currentPage || Github.currentPage === 1 ? ' disabled' : '') +
        '"><a href="#">&larr; Prev</a></li><li class="next"><a href="#">Next &rarr;</a></li></ul>'
  ...
  function _listBranches(repo, branches) {
    var list =
      '<li class="github_repo"><a href="#">Back to repositories...</a></li>'
  ...
  function _listTreeFiles(repo, branch, sha, treefiles) {
    var mdFiles = _extractMdFiles(repo, treefiles),
      list =
        '<li class="refresh_branches" data-repo-name="' +
        repo +
        '"><a class="repo" href="#">Back to branches in ' +
        repo +
        '...</a></li>'
      ...
      mdFiles.forEach(function (item) {
        ...
        list += Github.isRepoPrivate
          ? '<li data-tree-file-sha="' +
            ...
            '" data-branch="' +
            branch +
            '" class="private_repo"><a class="tree_file" href="#">' +
            item.path +
            '</a></li>'
          : '<li data-tree-file-sha="' +
            item.sha +
    ...
    $('.modal-header h3').text(
      Github.currentOwner + ' > ' + repo + ' > ' + Github.currentBranch
    )
  ...
  return {
      ...
      function _doneHandler(a, b, res) {
        ...
        if (res.status < 400) {
          ...
          Notifier.showMessage(
            Notifier.messages.docSavedGithub + ' as ' + data.content.path
          )
        } else {
    ...
    bindNav: function () {
      $('#import_github').on('click', function () {
        Github.fetchOrgs()
        return false
      })

      $('#save_github').on('click', function () {
        Github.save()
        saveFile()
        return false
      })
