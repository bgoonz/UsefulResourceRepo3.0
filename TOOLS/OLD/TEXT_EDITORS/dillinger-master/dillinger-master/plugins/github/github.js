...
var githubConfigFile = path.resolve(
    __dirname,
    '../../configs/github/',
    'github-config.json'
  ),
...
if (fs.existsSync(githubConfigFile)) {
  ...
  isConfigEnabled = true
  console.log(
    'Github config found in environment. Plugin enabled. (Key: "' +
  ...
  isConfigEnabled = true
  console.log(
    'Github config found in environment. Plugin enabled using a personal access_token.'
  )
} else {
  githubConfig = {
    client_id: 'YOUR_ID',
  ...
  console.warn(
    'Github config not found at ' + githubConfigFile + '. Plugin disabled.'
  )
}
    ...
    fetchFile: function (req, res) {
      ...
      request(options, function (e, r, d) {
        if (e) {
          ...
          if (isPrivateRepo) {
            d = JSON.parse(d)
            jsonResp.data.content = new Buffer(d.content, 'base64').toString(
              'utf-8'
            )
          }
      ...
      if (!data.uri) {
        ...
        request(options, function (e, r, d) {
          ...
          try {
            ...
            return res
              .status(400)
              .json({ error: 'Unable to save file: ' + (e || data.message) })
          }
