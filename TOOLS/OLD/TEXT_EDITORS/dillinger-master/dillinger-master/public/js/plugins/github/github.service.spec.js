...
describe('githubService', function () {
  ...
  it('should fetch a file from github and set it on the service.fetched.file variable', function () {
    ...
    $httpBackend.expectPOST('import/github/file').respond({
      data: {
    ...
    service
      .fetchFile(markDownFile.url, markDownFile.path)
  ...
  it('should save the current file to github and return success message', function () {
    ...
    $httpBackend
      .expectPOST('save/github')
      .respond({ content: { path: markDownDocument.path } })
    ...
    expect(diNotifyElementsText).toContain(
      'Successfully saved to ' + markDownDocument.path
    )
  })
})
