...
module.exports = function (grunt) {
	...
	grunt.initConfig({
		exec: {
			coverage: {
				command:
					"istanbul cover ./node_modules/mocha/bin/_mocha -x src/PhantomWrapper.js -- -t " +
					MOCHA_TIMEOUT_MS +
					" --root src/ test/",
			},
			coveralls: {
				command:
					"istanbul cover ./node_modules/mocha/bin/_mocha -x src/PhantomWrapper.js --report lcovonly -- -t " +
					MOCHA_TIMEOUT_MS +
					" -x src/PhantomWrapper.js --root src/ test/ && cat ./coverage/lcov.info | ./node_modules/coveralls/bin/coveralls.js",
			},
			"check-coverage": {
				command:
					"istanbul check-coverage --lines " +
					COVERAGE_THRESHOLD +
					" --statements " +
					COVERAGE_THRESHOLD +
					" --functions " +
