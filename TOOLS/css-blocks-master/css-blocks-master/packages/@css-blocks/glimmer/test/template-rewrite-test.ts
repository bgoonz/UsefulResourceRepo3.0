...
async function optimize(analyzer: GlimmerAnalyzer): Promise<CSSAndMapping> {
  ...
  for (let block of blocks) {
    ...
    optimizer.addSource({
      content: compiled.toResult({
        to: blockOpts.importer.filesystemPath(block.identifier, blockOpts)!,
      }),
    });
  }
  let optimized = await optimizer.optimize("result.css");
  let styleMapping = new StyleMapping<"GlimmerTemplates.ResolvedFile">(
...
function rewrite(
  ...
  let plugin = (env: ASTPluginEnvironment) =>
    new GlimmerRewriter(
      env.syntax,
      result.styleMapping,
      analyzer.getAnalysis(0),
      {}
    );
  let options = {
    meta: {},
...
describe("Template Rewriting", function () {
  ...
  it("rewrites styles from dynamic classes", async function () {
    ...
    let analyzer = new GlimmerAnalyzer(project);
    let templatePath = fixture(
      "styled-app/src/ui/components/with-dynamic-classes/template.hbs"
    );
  ...
  it("rewrites styles from dynamic attributes from readme", async function () {
    ...
    let analyzer = new GlimmerAnalyzer(project);
    let templatePath = fixture(
      "readme-app/src/ui/components/page-layout/template.hbs"
    );
