...
export type TemplateElement = ElementAnalysis<
  BooleanExpression,
  StringExpression,
  TernaryExpression
>;
...
export class ElementAnalyzer {
  ...
  analyze(node: AST.ElementNode, atRootElement: boolean): AnalysisElement {
    let element = this.analysis.startElement<null, null, null>(
      nodeLocation(node),
      node.tag
    );
  ...
  analyzeForRewrite(
    ...
    let element = new ElementAnalysis<
      BooleanExpression,
      StringExpression,
      TernaryExpression
    >(nodeLocation(node), node.tag);
    this._analyze(node, atRootElement, { element, storeConditionals: true });
    return element;
  }

  // tslint:disable-next-line:prefer-whatever-to-any
  private debugAnalysis(
    node: AST.ElementNode,
    atRootElement: boolean,
    element: ElementAnalysis<any, any, any>
  ) {
    if (!debug.enabled) return;
    let startTag = `<${node.tag} ${node.attributes
      .map((a) => print(a))
      .join(" ")}>`;
    debug(
      `Element ${startTag} is ${
        atRootElement ? "the root " : "a sub"
      }element at ${this.debugTemplateLocation(node)}`
    );
    debug(
      `↳ Analyzed as: ${element.forOptimizer(this.cssBlocksOpts)[0].toString()}`
    );
  }

  private debugTemplateLocation(node: AST.ElementNode) {
    let templatePath = this.cssBlocksOpts.importer.debugIdentifier(
  ...
  private debugBlockPath() {
    return this.cssBlocksOpts.importer.debugIdentifier(
  ...
  private _analyze(
    ...
    let classAttr: AST.AttrNode | undefined = node.attributes.find(
      (n) => n.name === "class"
    );
  ...
  private lookupClass(name: string, node: AST.Node): BlockClass {
    ...
    if (found) {
      ...
      if (/\./.test(name)) {
        throw cssBlockError(
          `No class or block named ${name} is referenced from ${this.debugBlockPath()}`,
          node,
          this.template
        );
      } else {
        throw cssBlockError(
          `No class or block named ${name}`,
          node,
          this.template
        );
      }
      ...
      if (isTextNode(statement)) {
        ...
        if (helperType) {
          ...
          if (isStringLiteral(mainBranch)) {
            ...
            throw cssBlockError(
              `{{${helperType}}} expects a string literal as its second argument.`,
              mainBranch,
              this.template
            );
          }
          ...
          if (elseBranch) {
            if (isStringLiteral(elseBranch)) {
              ...
              throw cssBlockError(
                `{{${helperType}}} expects a string literal as its third argument.`,
                elseBranch,
                this.template
              );
            }
          ...
          if (analysis.storeConditionals) {
            analysis.element.addDynamicClasses(
              dynamicClasses(condition, whenTrue, whenFalse)
            );
          } else {
            analysis.element.addDynamicClasses(
              dynamicClasses(null, whenTrue, whenFalse)
            );
          }
        } else {
          throw cssBlockError(
            `Only {{style-if}} or {{style-unless}} helpers are allowed in class attributes.`,
            node,
            this.template
          );
        }
  ...
  private processState(
    ...
    let stateBlock = blockName
      ? this.block.getReferencedBlock(blockName)
      : this.block;
    if (stateBlock === null) {
      throw cssBlockError(
        `No block named ${blockName} referenced from ${this.debugBlockPath()}`,
        node,
        this.template
      );
    }
    ...
    if (containers.length === 0) {
      throw cssBlockError(
        `No block or class from ${
          blockName || "the default block"
        } is assigned to the element so a state from that block cannot be used.`,
        node,
        this.template
      );
    }
    ...
    for (let container of containers) {
      ...
      if (stateGroup && staticSubStateName) {
        ...
        if (state) {
          ...
          throw cssBlockError(
            `No sub-state found named ${staticSubStateName} in state ${stateName} for ${container.asSource()} in ${
              blockName || "the default block"
            }.`,
            node,
            this.template
        ...
        if (stateGroup.hasResolvedValues()) {
          if (dynamicSubState) {
            if (analysis.storeConditionals) {
              analysis.element.addDynamicGroup(
                container,
                stateGroup,
                dynamicSubState
              );
            ...
            throw cssBlockError(
              `No sub-state specified for ${stateName} for ${container.asSource()} in ${
                blockName || "the default block"
              }.`,
              node,
              this.template
          ...
          if (dynamicSubState) {
            if (dynamicSubState.type === "ConcatStatement") {
              throw cssBlockError(
                `The dynamic statement for a boolean state must be set to a mustache statement with no additional text surrounding it.`,
                dynamicSubState,
                this.template
              );
            }
            ...
            if (analysis.storeConditionals) {
              analysis.element.addDynamicAttr(
                container,
                state!,
                dynamicSubState
              );
        ...
        if (staticSubStateName) {
          throw cssBlockError(
            `No state found named ${stateName} with a sub-state of ${staticSubStateName} for ${container.asSource()} in ${
              blockName || "the default block"
            }.`,
            node,
            this.template
          ...
          throw cssBlockError(
            `No state(s) found named ${stateName} for ${container.asSource()} in ${
              blockName || "the default block"
            }.`,
            node,
            this.template
...
function isMustacheStatement(
  value: AST.TextNode | AST.MustacheStatement | AST.ConcatStatement
): value is AST.MustacheStatement {
  return value.type === "MustacheStatement";
}

function isStyleIfHelper(
  node: AST.MustacheStatement
): "style-if" | "style-unless" | undefined {
...
function dynamicClasses(
  condition: null,
  whenTrue: BranchStyles,
  whenFalse: BranchStyles
): DynamicClasses<null>;
function dynamicClasses(
...
function dynamicClasses(
  ...
  whenTrue: BranchStyles,
  whenFalse: BranchStyles
): DynamicClasses<TernaryExpression | null> {
