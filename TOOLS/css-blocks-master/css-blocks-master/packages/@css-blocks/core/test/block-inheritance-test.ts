...
export class BlockInheritance extends BEMProcessor {
  ...
  @skip
  @test
  "can unset an inherited property"() {
    let { imports, config } = setupImporting();
  ...
  @test "inheritance conflicts automatically resolve to the base class"() {
    let { imports, config } = setupImporting();
    imports.registerSource("base.css", `:scope { width: 100%; }`);
  ...
  @test "README inheritance example"() {
    ...
    return this.process(
      "basic-form.block.css",
      imports.sources["basic-form.block.css"].contents,
      config
    )
      .then((result) => {
        assert.deepEqual(
  ...
  @test
  "inheritance conflicts automatically resolve pseudoelements to the base class"() {
    let { imports, config } = setupImporting();
    imports.registerSource("base.css", `.foo::after { width: 100%; }`);
  ...
  @test
  "multiple rulesets for the same target object pseudoelement get resolved"() {
    let { imports, config } = setupImporting();
    imports.registerSource(
      "base.css",
      `.nav { margin: 10px; }
  ...
  @skip
  @test
  "longhand inheritance conflicts automatically resolve to the base class"() {
    let { imports, config } = setupImporting();
    imports.registerSource("base.css", `:scope { border: 1px solid black; }`);
