...
const isBlock = (token?: Partial<Token>): token is BlockToken =>
  !!token && token.type === "block";
const isClass = (token?: Partial<Token>): token is ClassToken =>
  !!token && token.type === "class";
const isAttribute = (token?: Partial<Token>): token is AttrToken =>
  !!token && token.type === "attribute";
const isQuoted = (token?: Partial<AttrToken>): boolean =>
  !!token && !!token.quoted;
const isIdent = (ident?: string): boolean =>
  !ident || CLASS_NAME_IDENT.test(ident);
const hasName = (token?: Partial<Token>): boolean => !!token && !!token.name;
const isValidNamespace = (token?: Partial<AttrToken>): boolean =>
  !!token &&
  (token.namespace === undefined || token.namespace === STATE_NAMESPACE);
...
export const ERRORS = {
  ...
  expectsSepInsteadRec: (c: string) =>
    `Expected separator tokens "[" or ".", instead found \`${c}\``,
  illegalCharNotInAttribute: (c: string) =>
    `Only attribute selectors may contain the \`${c}\` character.`,
  illegalCharInAttribute: (c: string) =>
    `Attribute selectors may not contain the \`${c}\` character.`,
  multipleOfType: (t: string) =>
    `Can not have more than one ${t} selector in the same Block path`,
};
...
function stringify(tokens: Token[]): string {
  ...
  for (let token of tokens) {
    if (isBlock(token)) {
      ...
      out += token.name === ROOT_CLASS ? token.name : `.${token.name}`;
    } else if (isAttribute(token)) {
      let namespace = token.namespace ? `${token.namespace}|` : "";
      let value =
        token.value && token.value !== ATTR_PRESENT ? `="${token.value}"` : "";
...
export class BlockPath {
  ...
  private addToken(token: Partial<Token>, isUserProvided: boolean): void {
    ...
    if (!isBlock(token) && !hasName(token)) {
      this.throw(ERRORS.noname);
    }
    if (isAttribute(token) && !isValidNamespace(token)) {
      this.throw(ERRORS.namespace);
    }

    // Ensure we only have a single token of each type per block path.
    if (isBlock(token)) {
      this._block = this._block
        ? this.throw(ERRORS.multipleOfType(token.type))
        : token;
    }
    if (isClass(token)) {
      ...
      if (!this._block) {
        this.addToken({ type: "block", name: "" }, false);
      }
    }
    if (isAttribute(token)) {
      ...
      if (!this._class) {
        this.addToken({ type: "class", name: ROOT_CLASS }, false);
      }
    }

    // Add the token.
    if (isUserProvided) {
      this._tokens.push(token as Token);
    }
  ...
  private tokenize(): void {
    ...
    while ((char = walker.next())) {
      switch (true) {
        case char === PSEUDO_BEGIN:
          if (!isBlock(token)) {
            return this.throw(
              ERRORS.invalidIdent(PSEUDO_BEGIN),
              working.length
            );
          }
          token.name = working;
        ...
        case char === CLASS_BEGIN:
          if (isAttribute(token)) {
            this.throw(ERRORS.illegalCharInAttribute(char));
          }
          if (!isIdent(working)) {
            return this.throw(ERRORS.invalidIdent(working), working.length);
          }
        ...
        case char === NAMESPACE_END:
          if (!isAttribute(token)) {
            return this.throw(ERRORS.illegalCharNotInAttribute(char));
          }
          if (!isIdent(working)) {
            return this.throw(ERRORS.invalidIdent(working), working.length);
          }
        ...
        case char === VALUE_START:
          if (!isAttribute(token)) {
            this.throw(ERRORS.illegalCharNotInAttribute(char));
          }
          if (!working) {
            this.throw(ERRORS.noname);
          }
        ...
        case char === SINGLE_QUOTE || char === DOUBLE_QUOTE:
          ...
          token.quoted = true;
          if (walker.peek() !== char) {
            this.throw(ERRORS.mismatchedQuote);
          }
        ...
        case char === ATTR_END:
          ...
          if (next && !SEPARATORS.has(next)) {
            this.throw(ERRORS.expectsSepInsteadRec(next));
          }
          token =
            next === ATTR_BEGIN ? { type: "attribute" } : { type: "class" };
          break;
    ...
    if (isAttribute(token)) {
      this.throw(ERRORS.unclosedAttribute);
    }
   ...
   */
  childPath() {
    return BlockPath.from(
      this.parts.slice(this._block && this._block.name ? 1 : 2)
    );
  }
