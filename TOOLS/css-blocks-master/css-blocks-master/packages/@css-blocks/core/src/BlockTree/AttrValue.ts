...
export class AttrValue extends Style<AttrValue, Block, Attribute, never> {
  ...
  protected get ChildConstructor(): never {
  ...
  get isPresenceRule(): boolean {
    return this.value === ATTR_PRESENT;
  }
  ...
  asSourceAttributes(): Attr[] {
    if (!this._sourceAttributes) {
      ...
      let value = this.isPresenceRule
        ? attrValues.absent()
        : attrValues.constant(this.value);
      if (this.parent.namespace) {
        this._sourceAttributes.push(
          new AttributeNS(this.parent.namespace, this.parent.name, value)
        );
      } else {
  ...
  asSource(scope?: Block | boolean): string {
    let namespace = this.attribute.namespace
      ? `${this.attribute.namespace}|`
      : "";
    let value =
      this.value && this.value !== ATTR_PRESENT ? `=${this.value}` : "";
    return (
      this.attribute.blockClass.asSource(scope) +
      `[${namespace}${this.parent.name}${value}]`
    );
  }
  ...
  public cssClass(config: ResolvedConfiguration): string {
    switch (config.outputMode) {
      case OutputMode.BEM:
        return `${this.parent.cssClass(config)}${
          this.isPresenceRule ? "" : `-${this.value}`
        }`;
      default:
