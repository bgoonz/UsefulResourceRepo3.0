...
export type PostProcessorOption =
  | PostcssProcessorOption
  | GenericProcessorOption
  | (PostcssProcessorOption & GenericProcessorOption);

function isPostcssProcessor(
  processor: PostProcessorOption
): processor is PostcssProcessorOption {
  return !!(<PostcssProcessorOption>processor).postcss;
}

function isGenericProcessor(
  processor: PostProcessorOption
): processor is GenericProcessorOption {
...
export class CssAssets {
  ...
  constructor(options: Partial<CssAssetsOptions>) {
    let defaultOpts: CssAssetsOptions = {
      cssFiles: {},
      concat: {},
      emitSourceMaps: true,
  ...
  apply(compiler: WebpackCompiler) {
    ...
    compiler.plugin("emit", (compilation, cb) => {
      ...
      async.forEach(
        ...
        (assetPath, outputCallback) => {
          ...
          let chunk: WebpackAny | undefined =
            chunkName && chunks.find((c) => c.name === chunkName);
          if (chunkName && !chunk) {
          ...
          if (Array.isArray(sourcePath)) {
            const sourcePaths = sourcePath.map((sourcePath) =>
              path.resolve(compiler.options.context!, sourcePath)
            );
    ...
    compiler.plugin("emit", (compilation, cb) => {
      ...
      for (let concatFile of concatFiles) {
        ...
        let inputFiles = Array.isArray(concatenation)
          ? concatenation
          : concatenation.sources;
        let concatenationOptions = Array.isArray(concatenation)
        ...
        if (missingFiles.length === 0) {
          for (let inputFile of inputFiles) {
            ...
            let chunksWithInputAsset = compilation.chunks.filter(
              (chunk: WebpackAny) =>
                (<Array<string>>chunk.files).indexOf(inputFile) >= 0
            );
          ...
          if (concatenationOptions.postProcess) {
            postProcessResults.push(
              postProcess(
                concatenationOptions.postProcess,
                concatSource,
                concatFile
              ).then((source) => {
    ...
    compiler.plugin("emit", (compilation, cb) => {
      ...
      let assetPaths = Object.keys(compilation.assets).filter((p) =>
        /\.css$/.test(p)
      );
...
function assetAsSource(contents: string, filename: string): Source {
  ...
  if (/sourceMappingURL/.test(contents)) {
    sourcemap =
      convertSourceMap.fromSource(contents) ||
...
function assetFilesAsSource(
  filenames: string[],
  callback: (err: Error | undefined, source?: ConcatSource) => void
) {
...
function assetFileAsSource(
  sourcePath: string,
  callback: (err: Error | undefined, source?: Source) => void
) {
...
function makePostcssProcessor(plugins: PostcssProcessor): GenericProcessor {
