...
export class Test {
  ...
  @test "Objstr lookup understands scope"() {
    ...
    `).catch((err: Error) => {
      assert.equal(
        err.message,
        "[css-blocks] AnalysisError: The call to style function 'objstr' does not resolve to an import statement of a known style helper. (7:21)"
      );
    });
  }
  ...
  @test "handles unknown function call in class attribute"() {
    ...
    `).catch((err: Error) => {
      assert.equal(
        err.message,
        "[css-blocks] AnalysisError: Function called within class attribute value 'wtf' must be either an 'objstr' call, or a state reference (8:19)"
      );
    });
  }
}
